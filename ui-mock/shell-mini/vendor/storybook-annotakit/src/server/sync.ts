...
export function createAutoSync(opts: {
  ...
  const refHasOurReadme = async (ref: string): Promise<boolean> => {
    ...
    // PATCH (nle-core-spec vendored v0.5.0, 2026-09-05): the original
    // `ls-tree <tree> -- README` is PATHSPEC-relative to gitAsync's
    // `-C <projectRoot>` — when the Storybook project sits in a
    // SUBDIRECTORY of the git repo (ui-mock/shell-mini inside
    // nle-core-spec), the pathspec matches nothing → every remote branch
    // reads as "foreign" → sync never parents on it → all pushes after
    // the first are rejected non-fast-forward forever. The colon-path
    // form (<ref>:README) resolves the blob sha cwd-independently.
