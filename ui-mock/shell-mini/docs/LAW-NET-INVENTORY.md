...
> **R30 K3-progress note (the crawl's bulk begun):** the re-expression maps are FILED — four maps in the app repo (`nle-test-app/docs/k3-map-{store,geometry,libchrome,timeline}.md`, 160 rows / 495 tests audited across the whole corpus; the dispositions: RE-EXPRESS/SUPERSEDED/N-A-DIFFERENT/OVERLAP-LANDED/DOM-GATED with every target-surface citation grep-verified) — and the first two authoring waves LANDED (the app suite 256 → **377/377** across 12 files): the store/policy tranche (engineService dispatch-outcome pins 31, the history laws 15, the GluedShell mounted-UI additions, the wire reject/drag-session/mixed-kind laws), the geometry tranche (timeline-geometry 38: zoom clamps, the frame quantizer, neighborBounds/wouldOverlap projection pins, trim geometry + the 1-frame floor + HA-1-1, snap 10px nearest-inclusive both-edges, the ruler label laws, contentEnd, the seek clamp), the C16 auto-repeat guards BOTH trees (the port's use-keybindings + the package's useShortcuts — the held-key machine-gun bug found + fixed + mutation-verified), the SLIP family, the patch-path refusals, and the 3-verb dead-history-entry package defect fixed (`nle-ui` @ 5779295, 701/701). The adversarial review: 13/13 mutations caught (zero tautologies); the Part-A hole (Timeline.test.tsx's 52 families) now mapped by k3-map-timeline.md — its RE-EXPRESS-PORT rows + the D26.5 testid mapping table (21 clean / 4 partial / 18 gaps) are the DOM-structural tranche's work order. The standing counts (495/12 files/157 describes → 163 census units) are UNCHANGED — the mini's corpus is sealed; the K3 progress is app-side.

> **R31 K3-progress note (the DOM-structural tranche COMPLETE):** all 13 RE-EXPRESS-PORT rows of the timeline map LANDED (the app suite 377 → **418/418** across 21 files, +36 tests in 9 new files: dom-structural 8 + scrub-session 4 + clip-lifecycle 3 + trim-gesture 3 + edge-autoscroll 2 + zoom-anchor 2 + dnd-fallback 3 + mid-drag-yield 5 + e2e-crawl 6) — the D26.5 testid mapping row RATIFIED with the four partial rulings landed as code (the resize-handle `data-element-id` composed-selector grammar, the zoom-slider `aria-valuetext`, the drop-indicator marker+index ruling, the audiolane registered divergence); the 18 GAP units FILED in OT's queue (`31f2764`); **D51 the mid-drag keyboard yield** (this inventory's M2 interaction-lock law — the mini's useKeys.ts:60 `dragActive` gate — implemented as the app's engine-world twin, adversarially ratified, the two-keymap residual registered as the D52 candidate); **the K4 e2e legs begun** (the import→cut→play→export chain through the real UI — the capstone commits exactly six wire verbs, each ok, each origin ui). The mini's standing counts UNCHANGED (sealed). The row corrections the waves found are registered in the map's R31 addendum (row 38's Enter law inverted app-side — Enter=gotoStart; row 5's arithmetic slip; row 24's registry-only drop path).

> **R32 w1-progress note (the walk's entry rungs):** the K4 exit verdict LANDED (the crawl's automated completion proof COMPLETE — the interpretation ruling + the move-drag leg + the VLM net, all adversarially ratified); **D52 the two-keymap yield LANDED** (the D51 residual closed: the shell's 9 doc-mutating rows gain the silent yield through the OPTIONAL TimelineRouter.isInteractionActive twin — ONE source TWO listeners, the same OR-composed getter feeding D51's port gate and D52's shell twin; the per-element session families joined with it via the doc-committing session registry — keyframe-drag + volume-line, with scrub/marquee/box-select EXCLUDED by ruling); **the C0 MiniShell composition LANDED in nle-ui** (701→737, 78 stories — the mini's shell anatomy productized); **the D25.3a mini-theme wave LANDED in OT** (theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins — the grammar's TOKEN half live on the real /view surface; 649/649 = 632 classic + 17 M60); the w1-entry's human-rounds infrastructure LANDED in the app (the round-trip protocol + the annotakit review loop + the side-by-side fixture pair + the gold captures). The mini's standing counts UNCHANGED (sealed). The next rungs: the human rounds themselves + w2.
...
nets) → 357 (R24-1: the audio-clip surrounding-box regression nets —
the user's blend report) → 377 (R24-2 W0: the mini-plus foundation —
the fields-primitives law file + the gate family + the deep-clone/
no-alias machinery family) → 396 (R24-2 W1: the plus-surface law
file — the additive gate law, the property-editing routing, the dB
pair, the effects stack, the F1 nested-undo discriminator) → 415 (R24-2 W2: the transition tool + layer + the split
...
| trim clamps — neighbor + MIN_DUR | 1 | **RE-FILED R30 (the GAP-verify answer, verified against the vendored OT @ 55c81c0):** OT's trim floor is **1 FRAME, fps-derived** (`Math.round(TICKS_PER_SECOND·fps.den/fps.num)` — `ops/group-resize.ts:162-164`; 5000 ticks @24fps), NOT the mini's 0.5s — the constant does not exist OT-side; re-filed as HOLDS-with-a-different-constant, pinned app-side (`nle-test-app/src/timeline-geometry.test.ts`, the 1-frame floor + the HA-1-1 collapse family) | row 6 | K3 LANDED |
| splitPoint — window + quantize | 2 | **HOLDS-on-OT** (split semantics parity) | row 7 | OT 459 |
| splitPoint — MIN_DUR=0.5 constant | 2 | **RE-FILED R30 (the GAP-verify answer):** NO split min-duration exists OT-side — `splitElementsOnTracks` splits anywhere strictly inside; sub-frame splits are WIRE-LEGAL (`group-resize.ts:194-210`, the HA-1-1 comment); the mini's both-halves-≥0.5s + sub-1s-reject is MOCK POLICY — the app pins the INVERSE legality (arbitrary interior splits incl. sub-frame halves; `nle-test-app/src/engineService.test.ts`, the split family) | row 7 | K3 LANDED (N-A) |
...
All 52 families / 122 tests are **GAP-app-C1** (the view port reproduces
...
- Pure view families (no citation): render · selection · trim zones ·
  ...
  trim-mode edge shade · the R24 audio-clip surrounding-box family ·
...
(e.g. 122 component tests over ~52 view laws) was always the corpus shape;
