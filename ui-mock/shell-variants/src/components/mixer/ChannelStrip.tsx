/* ChannelStrip — one mixer strip per audio track (spec 20 §4.2 G-layer
   projection): badge+name+role chip, stereo meter, dB fader, pan knob,
   M/S/L (same toggles/commands as track headers), 2 insert slots, aux sends
   (A1/A2 + pre/post), output bus select, and the Duck-under row on
   BGM/Music-role strips (the spec 20 §12.2 sidechain-UX answer, mock).
   Aux return strips + MASTER strip live here too (MixerRow composes them). */
...
export function ChannelStrip({ track, sceneId, compact, focused, onStripClick }: {
  ...
  return (
    ...
    >
      ...
      {!compact && (
        <StripMeter trackId={track.id} db={strip.fader} duckAmount={role === 'bgm' || role === 'music' ? (duck?.amount ?? 0) : 0} label={track.name} />
      )}

      {/* fader + pan side by side */}
      <div className="flex items-end gap-1.5">
        <Fader db={strip.fader} onChange={(db) => setMixerTrack(track.id, { fader: db })} height={compact ? 64 : 84} ariaLabel={`${track.name} fader`} />
        {!compact && <PanKnob pan={strip.pan} onChange={(pan) => setMixerTrack(track.id, { pan })} ariaLabel={`${track.name} pan`} />}
      </div>
...
export function AuxStrip({ bus, compact }: { bus: 'a1' | 'a2'; compact: boolean }) {
  ...
  return (
    <div className="flex shrink-0 flex-col items-center gap-1 border-r border-hairline bg-inset px-1.5 pb-1.5 pt-1" style={{ width: compact ? 84 : 104 }}
...
export function MasterStrip({ compact }: { compact: boolean }) {
  ...
  return (
    <div className="flex shrink-0 flex-col items-center gap-1 px-1.5 pb-1.5 pt-1" style={{ width: compact ? 96 : 116 }}
