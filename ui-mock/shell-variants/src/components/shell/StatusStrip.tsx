/* StatusStrip — spec 18 §3.1/§6.3: 12px strip, save-status chip (autosave
   lifecycle), selection info, duration, zoom readout. */
...
export function StatusStrip() {
  ...
  return (
    <div className="flex shrink-0 items-center gap-3 border-t border-hairline bg-shell px-3 text-[10px] text-tfaint" style={{ height: 12, minHeight: 12 }}>
      ...
      <span className="mono">{selection.length > 0 ? `${selection.length} clip${selection.length > 1 ? 's' : ''} selected` : 'no selection'}</span>
