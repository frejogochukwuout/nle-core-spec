#!/usr/bin/env python3
"""battery_r29.py — the Round-29 battery (the battery_r28 successor: THE EXECUTION-ROUND re-base).
...
check("LIVE: the app's submodules are engine e3f55bd + nle-ui 6754979 + WDC ec8fd5c + the OT mirror lock 55c81c0 (the R17 divergence RESOLVED: the app's engine vendor e3f55bd == the engine HEAD — re-converged, not drift)", lambda: (
    _git("nle-test-app", "submodule", "status", "vendor/nle-engine").lstrip(" +-").startswith("e3f55bd") and
    _git("nle-test-app", "submodule", "status", "vendor/nle-ui").lstrip(" +-").startswith("6754979") and
...
print(f"\nbattery_r29: {len(results) - len(fails)}/{len(results)} PASS")
