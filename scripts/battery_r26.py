#!/usr/bin/env python3
"""battery_r26.py — the Round-26 battery (the battery_r25 successor: THE EXECUTION round). The three R25
cross-repo filings were EXECUTED at their owners, each adversarially reviewed (PASS, all folds landed):
the engine E1/E2 linked-companion fixes (8188d4e+412693d; 471/471; the 13-test pin file; the E1-a..d/E2-a
r1 residue register), the WDC waveform-contract promotion (035afe8+387f327; 773/773; the named surface +
the 14-pin test; the extraction law held), the app census-CI mechanism (9d310f8+3e4f0cd; the register +
the vendored components-tree reference + the checker + the mutation teeth + the .agents/ bootstrap + the
WDC consumer re-pin 85b81b0→387f327). The pin checks assert the R26 world: engine 0a49286/471, OT HEAD
fdb771c (code pin c15a629 UNCHANGED)/536, WDC 387f327/773, nle-ui 3026099/674 (unmoved), app 3e4f0cd/206.
Keeps ALL battery_r25 check classes re-based + the R26 execution-state classes (the filings' landing
evidence, LIVE git-verified at the module repos, the census artifacts, the E1/E2 pin file)."""
import re, os, sys
...
FLEET_HEAD = {"engine": "0a49286", "OT": "fdb771c", "WDC": "387f327", "nle-ui": "3026099", "app": "3e4f0cd"}
...
def _mini_scraped():
    ...
    return (passed == 377 and total == 377 and files == 9), f"scraped {passed}/{total} tests / {files} files (declared 377/9 — the R23 seal 355 + the R24-mini nets: 2 clip-box + 20 mini-plus W0)"
...
def _inventory_math():
    ...
    ok = (units == 135 and tests == 377
          and total_row and int(total_row.group(1)) == 135 and int(total_row.group(2)) == 377
          and authored and int(authored.group(1)) == 377 - holds and int(authored.group(2)) == 135 - 13)
    return ok, f"units={units} tests={tests} holds={holds} authored={authored.groups() if authored else None}"
check("LAW-NET-INVENTORY arithmetic parses + sums (135 units / 377 / authored=377−33)", _inventory_math, "census math")
...
check("mini count current (377) + variants 1521+ live", lambda: (
    "377" in specs[18] and "1521" in s00, "mocks"))
        ...
        with open(f, encoding="utf-8") as fh:
            for line in fh:
                ...
                if s.startswith("it(") or s.startswith("it ("):
                    blocks += 1
    ok = ("1521+" in reg and "377" in reg and len(files) == 64 and blocks == 1733)
    return ok, f"{blocks} it-blocks / {len(files)} files (register: 1,733/64, canon 1521+)"
...
# === Q. THE R26 PIN RE-BASE (R25 class 6 re-based) ===========================
check("00's fleet rows cite the R26 pins: engine 0a49286 / OT fdb771c / WDC 387f327 / nle-ui 3026099 / app 3e4f0cd", lambda: (
    all(v in s00 for v in ["0a49286", "fdb771c", "387f327", "3026099", "3e4f0cd"]), "R26 pins"))
check("00's consumer pins re-read live (engine→OT c15a629 + WDC 494f6ff; app→engine 5036387 + byte-lock c15a629 + nle-ui 3026099 + WDC 387f327)", lambda: (
    all(k in s00 for k in ["c15a629", "494f6ff", "5036387", "3026099", "byte-exact minus `testing/`"]) and "387f327" in s00, "consumer pins R26"))
check("00's status is v11.0 (the R26 execution round)", lambda: ("**Status:** v11.0 (Round 26" in s00, "v11.0"))
...
# === T. THE R26 EXECUTION STATE (the three filings, LIVE-verified) ===========
import subprocess
def _git(repo, *args):
    try:
        ...
        return ""
check("LIVE: the engine repo HEAD is 0a49286 (the E1/E2 fix + folds + PLAN tick)", lambda: (
...
check("LIVE: the engine's E1/E2 pin file exists (timeline-linked-source-edit.test.ts, 13 it-blocks)", lambda: (
    ...
    and read(os.path.join("..", "nle-engine", "tests", "vitest", "engine", "timeline-linked-source-edit.test.ts")).count("  it(") == 13, "pin file"))
...
check("LIVE: the WDC contract module + the pinning test exist (14 it-blocks)", lambda: (
    ...
    and read(os.path.join("..", "web-daw-core", "src", "test", "waveform-contract.test.ts")).count("  it(") == 14, "contract+test"))
...
check("LIVE: the app's WDC consumer pin is 387f327 (the submodule re-pin — in-sync: space prefix)", lambda: (
    _git("nle-test-app", "submodule", "status", "vendor/web-daw-core").lstrip(" +-").startswith("387f327"), _git("nle-test-app", "submodule", "status", "vendor/web-daw-core")[:13] or "unreadable"))

# === U. THE R26 CORPUS RECORDS (the spec-side landing evidence) ==============
...
check("00's §2A.7 census-law note: the mechanism is REAL (the register + the checker + the teeth)", lambda: (
    "the mechanism is now REAL" in s00 and "census-mutation-gate" in s00, "mechanism real"))
check("the plan's S-app row stamps step (0) + (1b) DONE R26", lambda: (
    "DONE R26 @ `9d310f8`" in plan and "EXECUTED R26 @ `9d310f8`+`3e4f0cd`" in plan, "S-app stamps"))
check("the plan's S-engine row stamps E1+E2 FIXED R26 + the S-wdc row stamps the promotion EXECUTED", lambda: (
    "E1 + E2 FIXED R26 @ `8188d4e`+`412693d`" in plan and "EXECUTED R26 @ `035afe8`+`387f327`" in plan, "S-rows stamped"))
check("00's D36.1/36.2/36.3 carry the R26 EXECUTED notes", lambda: (
    s00.count("(R26 EXECUTED") >= 3, "D36 executed notes"))
...
check("00 carries D30-D36 (+31A) + the v10.0 R25 status", lambda: (
    ...
    and ("**Status:** v11.0 (Round 26" in s00 or "**Status:** v10.0 (Round 25" in s00), "D30-36"))
...
if not fails:
    print("ALL GREEN — the R26 world is coherent (the mode matrix + the grammar register + the keyboard family + the reference register + the R26 execution canon + the census + the plan + the three filings' LIVE landing evidence).")
