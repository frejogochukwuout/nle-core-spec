...
def _mini_scraped():
    ...
    return (passed == 396 and total == 396 and files == 10), f"scraped {passed}/{total} tests / {files} files (declared 396/10 — the R23 seal 355 + the R24-mini nets: 2 clip-box + 20 W0 + 19 W1)"
...
def _inventory_math():
    ...
    ok = (units == 141 and tests == 396
          and total_row and int(total_row.group(1)) == 141 and int(total_row.group(2)) == 396
          and authored and int(authored.group(1)) == 396 - holds and int(authored.group(2)) == 141 - 13)
    return ok, f"units={units} tests={tests} holds={holds} authored={authored.groups() if authored else None}"
check("LAW-NET-INVENTORY arithmetic parses + sums (141 units / 396 / authored=396−33)", _inventory_math, "census math")
...
check("mini count current (396) + variants 1521+ live", lambda: (
    "396" in specs[18] and "1521" in s00, "mocks"))
        ...
        with open(f, encoding="utf-8") as fh:
            for line in fh:
                ...
                if s.startswith("it(") or s.startswith("it ("):
                    blocks += 1
    ok = ("1521+" in reg and "396" in reg and len(files) == 64 and blocks == 1733)
