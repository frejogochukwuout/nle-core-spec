...
def _mini_scraped():
    ...
    return (passed == 415 and total == 415 and files == 10), f"scraped {passed}/{total} tests / {files} files (declared 415/10 — the R23 seal 355 + the R24-mini nets: 2 clip-box + 20 W0 + 19 W1 + 19 W2)"
...
def _inventory_math():
    ...
    ok = (units == 145 and tests == 415
          and total_row and int(total_row.group(1)) == 145 and int(total_row.group(2)) == 415
          and authored and int(authored.group(1)) == 415 - holds and int(authored.group(2)) == 145 - 13)
    return ok, f"units={units} tests={tests} holds={holds} authored={authored.groups() if authored else None}"
check("LAW-NET-INVENTORY arithmetic parses + sums (145 units / 415 / authored=415−33)", _inventory_math, "census math")
...
check("mini count current (415) + variants 1521+ live", lambda: (
    "415" in specs[18] and "1521" in s00, "mocks"))
        ...
        with open(f, encoding="utf-8") as fh:
            for line in fh:
                ...
                if s.startswith("it(") or s.startswith("it ("):
                    blocks += 1
    ok = ("1521+" in reg and "415" in reg and len(files) == 64 and blocks == 1762)
