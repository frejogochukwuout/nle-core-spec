...
check("R31: D51 the mid-drag keyboard yield — the gate in use-keybindings.ts + the composed getter in TimelineView + the 10-leg pin file", lambda: (
    ...
    "isInteractionActive: () =>" in read("../nle-test-app/src/timeline-port/TimelineView.tsx") and
...
check("LIVE: the OT repo HEAD is 31f2764 (the R31 queue filing docs-only: the carrier-reduction work order + the D25.3b worklist + the data-test freeze ask; the code pin 970948a stays)", lambda: (
    _git("opencut-timeline", "rev-parse", "--short", "HEAD") == "31f2764", _git("opencut-timeline", "rev-parse", "--short", "HEAD") or "HEAD unreadable"))
...
check("LIVE: the app's submodules are engine e3f55bd + nle-ui dc3644b + WDC ec8fd5c + the OT mirror lock 55c81c0 (the R17 divergence RESOLVED: the app's engine vendor e3f55bd == the engine HEAD — re-converged, not drift)", lambda: (
    _git("nle-test-app", "submodule", "status", "vendor/nle-engine").lstrip(" +-").startswith("e3f55bd") and
    _git("nle-test-app", "submodule", "status", "vendor/nle-ui").lstrip(" +-").startswith("dc3644b") and
...
print(f"\nbattery_r31: {len(results) - len(fails)}/{len(results)} PASS")
