...
check("LIVE: the OT repo HEAD is 39003d3 (the R32 D25.3a mini-theme wave LANDED: theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins + the gold captures; 649/649 = 632 classic + 17 M60; the code pin MOVED off 970948a)", lambda: (
...
check("LIVE: the app repo HEAD is ce7cfc3 (426/426 the 22-file roof; the K4 exit verdict + the move-drag LEG 2b + the doc-committing session registry + the D52 shell-twin family + the w1-entry deliverables; the census register RE-DECLARED 43 = 32 zero-action + 9 carriers + 2 hosts @ the 39003d3 re-pin)", lambda: (
...
check("LIVE: the app's submodules are engine e3f55bd + nle-ui 1e0c30b + WDC ec8fd5c + the OT mirror lock 39003d3 (the R17 divergence RESOLVED: the app's engine vendor e3f55bd == the engine HEAD — re-converged, not drift; the R32 re-pin: nle-ui to the D52 twin + the OT mirror to the D25.3a wave)", lambda: (
...
check("R32: the app census register RE-DECLARED (43 = 32 zero-action (6 byte + 26 mechanical) + 9 carriers + 2 hosts @ the 39003d3 re-pin)", lambda: (
