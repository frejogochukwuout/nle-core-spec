#!/usr/bin/env python3
"""battery_r28.py — the Round-28 battery (the battery_r27 successor: THE SEAL round).
...
commit + the e7cdede OV-12 infra fix; 57,085 src/lib LOC) · OT HEAD 55c81c0 (code pin
970948a, src-diff empty; 632/632) · WDC 83b8850 (777/777, docs-only over ec8fd5c) · nle-ui
32abd58 (690/690) · app 85cff80 (252/252, the 8-suite roof; census 42 = 36 zero-action +
5 carriers + 1 host @ the OT mirror 6e2b91a) · the wire census 31 = 28 routed + 3 exceptions
...
thirteen. Keeps every still-valid r27 class re-keyed to the R28 figures."""
import re, os, sys
...
FLEET_HEAD = {"engine": "074a2f6", "OT": "55c81c0", "WDC": "83b8850", "nle-ui": "32abd58", "app": "85cff80"}
check("00 fleet rows pin all five repos (HEAD class, R28 — the CR-F1/F4 fold; the engine range NOT docs-only)", lambda: (all(v in s00 for v in FLEET_HEAD.values()), "HEAD pins"))
check("00 carries the OT code pin 970948a + the 632 count", lambda: ("970948a" in s00 and "632" in s00, "code pin"))
check("00 carries the live consumer pins (engine→OT 6e2b91a + WDC ec8fd5c; app→engine 74bef08 the code anchor + OT mirror 6e2b91a + nle-ui 83ff8a8 + WDC ec8fd5c)", lambda: (
    all(k in s00 for k in ["6e2b91a", "ec8fd5c", "83ff8a8", "74bef08"]) and s00.count("6e2b91a") >= 3, "consumer pins"))
check("consumer lockstep: the engine's OT vendor pin == the app's OT mirror == 6e2b91a; the app vendors the engine CODE ANCHOR 74bef08 (the R17 divergence, BY DESIGN)", lambda: (
    s00.count("6e2b91a") >= 3 and "970948a" in s00 and "74bef08" in s00 and "one step behind" in s00, "lockstep"))
...
def _loc_era():
    """Every current-state LOC read agrees with its pin era: 57,064 paired with the R27 pins
    (f9ac806/50b91f5/'the R27 census'/'R27 fleet re-verify') is a dated record; 57,064 paired
    with the R28 pins (074a2f6/74bef08/749/749) is an inconsistency — the T5-pre re-base's
    ...
    for n, t in [(19, specs[19])]:
        for m in re.finditer(r"57,064", t):
            ...
            if any(k in ctx for k in ["074a2f6", "74bef08", "749/749", "R28"]):
                ln = t[:m.start()].count("\n") + 1
                bad.append(f"{n:02d}:{ln}")
                break
    return (not bad), ("; ".join(bad) or "clean") + " — the R27-LOC/R28-pin pairs (the dated R27-only reads are exempt)"
check("R28-A: zero R27-LOC reads paired with R28 pins (19's current-state rows read 57,064 at 074a2f6/749 — the re-base residue)", _loc_era, "LOC era")
...
def _matrix_gate():
    """The matrix preamble/§0 cites the R28 pins; §0's D30.3 footnote states the zero-orphan gate.
    The R27 re-anchor parenthetical at :14 is a dated record; the R28 pins live at the BASE rows —
    the app pin's matrix-gate home is the parenthetical, re-key expected at 85cff80 (06's §0
    carries 074a2f6/970948a/zero-orphan-rows live at the BASE rows)."""
    s0 = specs[6][:specs[6].find("## 1. Purpose")] if "## 1. Purpose" in specs[6] else specs[6][:8000]
    missing = [k for k in ["074a2f6", "970948a", "85cff80", "zero orphan rows"] if k not in s0]
    return (not missing), (",".join(missing) or "all cited") + (" — the R28 app pin 85cff80 has NO 06-side mention (the matrix preamble's re-anchor parenthetical still carries the R27 pins f9ac806/c020b2a; the T5-pre re-base re-keyed 06's BASE rows only)" if "85cff80" in missing else "")
check("06's matrix cites the R28 pins + the D30.3 zero-orphan-rows ladder gate", _matrix_gate, "matrix gate")
def _companion():
...
def _git(repo, *args):
    try:
        ...
        return ""
check("LIVE: the engine repo HEAD is 074a2f6 (code anchor 74bef08; 749/749 — ONE code commit + the e7cdede infra fix, NOT docs-only)", lambda: (
    _git("nle-engine", "rev-parse", "--short", "HEAD") == "074a2f6", _git("nle-engine", "rev-parse", "--short", "HEAD") or "HEAD unreadable"))
...
check("LIVE: the WDC repo HEAD is 83b8850 (docs-only over code anchor ec8fd5c; 777/777 unchanged)", lambda: (
    _git("web-daw-core", "rev-parse", "--short", "HEAD") == "83b8850", _git("web-daw-core", "rev-parse", "--short", "HEAD") or "HEAD unreadable"))
...
check("LIVE: the app repo HEAD is 85cff80 (252/252; the 8-suite roof; the census 42=36+5+1)", lambda: (
    _git("nle-test-app", "rev-parse", "--short", "HEAD") == "85cff80", _git("nle-test-app", "rev-parse", "--short", "HEAD") or "HEAD unreadable"))
...
check("LIVE: the app's submodules are engine 74bef08 + nle-ui 83ff8a8 + WDC ec8fd5c + the OT mirror lock 6e2b91a (the recorded divergence: the app's engine vendor 74bef08 != the engine HEAD 074a2f6 BY DESIGN, not drift)", lambda: (
    _git("nle-test-app", "submodule", "status", "vendor/nle-engine").lstrip(" +-").startswith("74bef08") and
...
def _r16_class():
    dead = ("1796" not in open("scripts/battery_r28.py").read()[:8000])
...
print(f"\nbattery_r28: {len(results) - len(fails)}/{len(results)} PASS")
for name, ok, detail in results:
    if not ok:
        print(f"  FAIL  {name}  {detail}")
if not fails:
    print("ALL GREEN — the R28 world is coherent: the seal round's D42-D50 rulings folded (W4), the test law moved in the same round (T3/S-spec(6)), the register at 0 open design rows, the THIRTEEN R28 check classes green (the step-0 facet-completeness + the plan-scrape carve + the census + the signoffs), the pin world re-based (074a2f6/83b8850/85cff80), the D-index D37-D41 + ARCH-R28 §4.1's ratification record.")
