...
- opencut-timeline (OT) @ HEAD **`008e7f3`** — **675/675** (R33, engine seal-round register P3-31 — the re-key: the s18 seal round [the lib-tree wave — linkage.ts NEW, the D42 pairwise model; edit-domains.ts NEW; api.ts +254, timeline-core.ts +144 — all additive, the components tree byte-unchanged, the wire census UNCHANGED 31 = 28 + 3 — live at the mirrors `17a19f8`] + the M58R flake fix; at HEAD, the m58 V-1 review-note settle — the range leg's unreachable `waitForSelectedIndicators(2)` wait closed, 6/6 re-verified; the lineage: R32 HEAD `39003d3` — the code pin MOVED, the D25.3a mini-theme wave LANDED: theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 pins; **649/649 = 632 classic + 17 M60**, the report json at HEAD the count authority) — was `3e18722` — **632/632** (72 milestone entries — the total only per the R27 counting convention; the report json committed at the code pin is the count authority), tsc 0; the OT-standalone "docs-only over `970948a`" claim is RETIRED (R33, engine seal-round register P3-31 — M60 moved code at `39003d3`, the s18 lib-tree wave moved it again at `17a19f8`; the engine's own vendor — now @ `17a19f8`, was `55c81c0` — is the checking pin for the §3.3A/census claims, all verified there): the SceneTracks model this spec's `SceneTracksJSON` mirrors (D12: one `TimelineCore` per scene — the single-scene editing SSOT). The W11 "complete-UI" round is at the code pin — every op wired via the wire-dispatch seam (`WIRE_COMMAND_TYPES`, **31 verbs = 28 routed + 3 exceptions**, M49C 28/3 @ `970948a` — the exported-registry consumption law per 00 §2A.10; D29's instrument set). The doc-model round-trip surfaces at the code pin (re-verified R27 at `970948a`): `toJSON(): TScene` (committed scene only, deep copy — a live preview never leaks into saved state) + `static fromJSON(scene, fps)` — **the scene-load boundary (§3.3A, D-HB2)**: the typed structural gate FIRST (rejected-whole), then the normalization set (strict-boolean `locked` — a truthy string never locks, S2-F7; missing `trimStart`/`trimEnd` default 0 — W9 E13; animations normalized at ingestion — SC-5/SE-4; the id counter seeded from element+track+**keyframe** ids — review-6 A3). Element-level persistence fields at the code pin: `params` (volume dB + muted — D28-A2/D-S5), `transitionOut` (S3, the engine-SSOT data home), `retime` (W2), `animations` (W5 — keyframe persistence). The S-round + T-round + F1 hardening (setTracksLocked D-T3, setTracksMuted **S3** with M46, the F1 whole-codebase review) all landed; the app consumes the vendored mirror — the declared historical lock-copy @ `c15a629` (UPSTREAM.lock: byte-exact upstream minus `testing/`, live-verified R24; retired at the R27 re-key: the mirror now @ `17a19f8` (re-keyed R33, the s18 lib-tree wave; was `55c81c0` [re-keyed R29, was `6e2b91a`]) + the owner's HEAD `008e7f3` — the two-truth form). **(R31: HEAD `31f2764` — 632 UNCHANGED, docs-only over the code pin `970948a`: the R31 queue filing = the carrier-reduction work order + the D25.3b structural-gap worklist + the data-test freeze ask)** **(R31: HEAD `31f2764` — 632/632 UNCHANGED, docs-only: the R31 queue filing = the carrier-reduction work order + the D25.3b worklist + the data-test freeze ask)**
- nle-test-app @ `7664603` — **426/426** (the 22-file roof; R33, engine seal-round register P3-31 — the lockstep re-pin wave: engine vendor `e3f55bd`→`574d8d3` + BOTH OT mirrors `39003d3`→`17a19f8` + the census register amended (31 = 28 + 3 UNCHANGED, classes 32 zero-action + 9 carriers + 2 hosts); gates: vitest 426/426, tsc 0, boundary OK, census register-equality OK, mutation 8/8, build OK; the lineage: `876f2b8` 256/256 — the R8 D29 real-wiring round + the R9/R25-R27 waves, the 8-suite roof: GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + wire-coverage 9 + engineSeam 7 + waveformPeaks 9 → R30 `8f12cf9` 377/377 → R31 `f58147c` 418/418 → R32 `ce7cfc3` 426/426 → the current `7664603`), tsc 0, boundary green: the app's scene wiring — the ARCH-R15 §2.4 multi-scene-app-level ruling executing app-side. (Consumer pins at this HEAD: nle-ui @ `1e0c30b` (743/743 — the R32 C0 MiniShell composition + the D52 twin, UNCHANGED at the R33 re-key; lineage: `6754979` the AW1-2 absorb LANDED with the freshness re-pin; R30: `5779295` — 701/701; R31: `dc3644b` — 701 UNCHANGED, docs-only: the D29 DECISIONS first-party record + the C0 filing), the OT lock-copy @ `17a19f8` (byte-exact minus `testing/` per UPSTREAM.lock, live-verified — the s18 re-pin LANDED @ `7664603`; was `39003d3` [R32, the M60 wave], `55c81c0` [the s17 re-pin @ `876f2b8`], `6e2b91a` — the `970948a` re-pin the filed queue `c020b2a`), engine @ `574d8d3` (was `e3f55bd`), WDC @ `ec8fd5c` (UNCHANGED). R30: the app @ `8f12cf9` — 377/377, the 12-file roof; R31: `f58147c` — 418/418; R32: `ce7cfc3` — 426/426.) **The sceneBridge's state (re-verified R27)** (`src/sceneBridge.ts`, 582 LOC + a 41-test suite): a BIDIRECTIONAL bridge — `sceneToTScene` (mock `SceneJSON` → OT `TScene` + element/track sidecars) and `tSceneToScene` (engine → doc, re-merging sidecars), with the identity law (every track/element/scene id survives unchanged), the source-window law (`trimStart + duration + trimEnd == sourceDuration`), the field law D2e (structure engine-owned: startTime/duration/trims/track muted+hidden+**locked**/bookmarks/element name/transitionOut+retime+params; sidecar-owned: speed/opacity/fades/effects/linkedTo/solo/waveform), the marker⇄bookmark hex map, the W2 speed⇄retime mapping with the RR1-B-1 identity fix (rate projects even at 1; load maps speed 1 → no-retime), and the S3-C6 element-mute round-trip (`el.muted` ⇄ `params.muted`). **The R8-b1 scene-switch rebuild fix (`c2be330`):** the EngineMount load effect's `core`-dep removed + the prior-rate read via `useCommittedRef` — the infinite-rebuild loop (setCore self-retrigger, live since R5, exposed by the W2.5 per-scene grade pin) FIXED; a dedicated real-scene-switch regression pin + the per-scene grade isolation round-trip are live. The consumed mock doc model is nle-ui's spec-09-shaped `SceneJSON`/`TrackJSON`/`ElementJSON`/`Marker` (package @ `fc4cc35`, vendored at the same HEAD — the 2-scene sample project; REGISTERED-DELTA subset: flat kind-tagged `tracks[]` + `dirty` flag where the spec persists the nested `SceneTracksJSON`; display-shaped `MediaRecord` extras — see the GAP register's media row).
- **The app-side persistence surface — the R8 W2.3 landing (BASE, promoted R24; re-verified R27 at `c020b2a`):** `src/persistenceService.ts` (410 LOC + a 20-test suite) — engine-free by law (the nle-ui store + localStorage only, synchronous writes; the doc slice = `scenes`, the G slice = `mixer`/`masterVolume`/`masterMuted`, view prefs + the W2.5 `color` grade sidecar ride the snapshot). The 2s debounce is keyed on the IDENTITY of `scenes`/`mixer`/`masterVolume`/`masterMuted`/`activeSceneId` (+ `color` since W2.5) — NEVER `docVersion` (the engine rebuild signal; mirror echo writes with same refs never dirty). The lifecycle `edit→dirty→debounce→saving→saved` drives the package's `saveState` seam (nle-ui W2.3 `9b0a36c`, now @ `6754979`; R30: `5779295`; R31: `dc3644b` (docs-only); R32: `1e0c30b` (743/743 — the C0 MiniShell composition + the D52 twin); R34: `cb04919` (747/747 — the DeliverPage blob-URL supersede contract + the Viewer's §4.2 empty-state pair); R35: `45d42fb` (754/754 — the C-1a edit-styles batch: the mediaInsert seam widening + the 4 pool insert rows + the 5 shortcutMap rows)). ⌘S = the real-dirty gate → immediate flush; beforeunload = the synchronous last-chance flush when dirty/saving; boot `hydrateFromStorage()` at App.tsx MODULE SCOPE (pre-render, no `docVersion` bump — beats EngineMount's first build); the baseline law = the current state IS the saved state; `loadSnapshotFile` (docVersion bump + scene.dirty cleared + re-baseline; never `loadSampleProject`) + `saveSnapshotFile` (JSON→Blob→`a[download]` — manual export ≠ autosave write). The BOTH-DOORS hostile-input validator (F3, audit H1-D P1): `isValidSceneShape`/`isValidSnapshot` (`persistenceService.ts:98-157`) validate the inner scene/track/element shape AND the W2.5 grade sidecar (finite-numbered per-scene grades, `:144-156`) at BOTH doors — the Load file path (`:386`) AND boot `hydrateFromStorage` (`:327`, via the shared `readStorage` gate `:159-166`) — a hostile file or hostile localStorage never hydrates (the probe-proven crash classes — dangling `activeSceneId`, non-array `markers`/`tracks` — closed at both doors). *(R33, engine seal-round register P3-32 — added; verified @ the current app pin `7664603`: 410 LOC, 20 `it(` sites, the debounce/⌘S/beforeunload/boot cites all exact.)* Landed `51592cf` (R8 W2.3, 132/132) + the W2.5 grade-sidecar extension `41f5bc9` (a grade write dirties like any authored change; pre-W2.5 snapshots hydrate neutral). The Save/Load FILE-AFFORDANCE wiring (the toolbar surface for these APIs) is the D30 W-E/R7 remainder — see the GAP row.
- nle-engine @ `2a0ecf4` (code anchor `28aa387` — HEAD is the HANDOFF docs commit) — **785/785** (749 classic + 33 property + 3 P-F2 — the seal-round count-gate re-pin; REFERENCE, not canon — §10.3/§19 §7; the engine's own vendored OT is re-pinned to `17a19f8` (the s18 lib-tree wave; was `55c81c0` — the `970948a` absorption — at `e3f55bd`) — making the W11 wire surface importable at its test seam — zero engine mediation by design): the engine-side project loading now has its own fast-venue suites — `tests/vitest/engine/persistence.test.ts` (P1-P7: `migrateProject` version-gate laws, `hydrateTimeline` never-throws, `serializeTimeline` defensive-clean warnings, `normalizeProjectData` idempotence, P5 non-finite clip-numerics drop-verbs :262, P6 HW1-1 transform sub-fields :302, P7 HW1-3 sourceFps repair :423 — the `NLE_SCHEMA_VERSION`/`ProjectWarning` pins live HERE, :31-32/:77/:90/:105) + `load-validation.test.ts` (V1-V7: the `NleRenderWarning` finders — source overrun / source range / transition / transform-parent validation, the import block :39-50) + the property tier's `tests/vitest/property/persistence-roundtrip.property.test.ts` (the serialize⇄hydrate round-trip + the hydrate-normalized-both-sides law, inside the seal round's 33 property sites). *(R33, engine seal-round register P3-30 — re-keyed @ `2a0ecf4`: the count 749→785, the suite census P1-P4→P1-P7, the `NLE_SCHEMA_VERSION`/`ProjectWarning` pins re-attributed to the persistence suite (they were mis-filed under load-validation), vendor OT `55c81c0`→`17a19f8`; all line cites live-verified.)* The engine's FreeCut-shaped single-timeline `Project` stays the C8-adapter convergence task (engine D1 resolved AGAINST this spec — the corrective register owns it, spec 19 §7); N2b keyframed volume (gain automation driving real audio, `37cdd28`) landed — this spec's `KeyframeTrackJSON` `'volume'` property is consumed end-to-end engine-side.
...
- **ProjectJSON persistence — the w3 REMAINDER (owner: nle-test-app, phase w3).** The R8 W2.3 localStorage surface is BASE (the row above) — the R23 claim "no OPFS/IndexedDB/localStorage surface exists anywhere app-side" is RETIRED (contradicted by `51592cf`, resolved R24). What REMAINS for w3: the §4–§6 OPFS layer (atomic write, the migration ladder, locks — localStorage is the current synchronous engine; zero OPFS/IndexedDB code app-side, re-grep-verified at the current pin `7664603`) and the persistence-format formalization (the doc slice is "spec-09-ish" per its own comment — the REGISTERED-DELTA mock subset travels: flat `tracks[]` + `dirty`; the nested `SceneTracksJSON` + schema-versioned wrapper are the w3 target). **(R33, engine seal-round register P2-20) The D30 W-E/R7 file-affordance wiring is DISCHARGED at the current app pin — the toolbar affordance LANDED** (the app's R9 W-E `b40216e`, "the Save/Load affordances CLOSE the W2.3 debt"; the register's `b5fe3b3` cite was the doc-committing-sessions commit — corrected here to `b40216e`): toolbar-save → `saveSnapshotFile`, toolbar-load → the hidden file input → the hostile-input-validated `loadSnapshotFile` (+ success/failure toasts, hostile JSON rejected), live at `7664603` as `src/timeline-port/EngineMount.tsx:29/:455-507` + `src/timeline-port/TimelineToolbar.tsx:263-273` (the `toolbar-save`/`toolbar-load` buttons); the pre-landing "zero `onSaveScene`/`onLoadScene` hits at `c885ece`" state is historical lineage only; the mock ⌘S theater survives only in the package's `'idle'` default world. (acceptance: project round-trip vs per-scene OT `toJSON`/`fromJSON` + the scene suite green — the plan §2 w3 gate, [P].)
...
- No-gap ruling, stated so it stops being asked: per-scene OT stays SINGLE-SCENE by design (D12) — scene-level concerns live app-side per §3.1A; nothing to build in OT. (Confirmed live at `c15a629` (R24; re-verified R27 @ `970948a`; re-verified R33 @ `344123a`; re-verified R34 @ `8f96ab2` — `src/lib/timeline/` byte-unchanged past the R33 code pin, the serialization surface unmoved): the core is one-scene; `toJSON()/fromJSON()` remain the whole serialization surface — W11 wired the op surface, not the doc model.)

**ACCEPTANCE & TEST PLAN:** §9 (intent list) + the executable `## Testing` section (schema/round-trip/migration/autosave tiers + the 3 kimdogyeom gating regressions); spec 17 §3 matrix rows "Project save / load (JSON round-trip)" / "Project schema migration (v1 → v2)" / "OPFS persistence (media cache, project autosave)" + the §13A facet rows; battery posture: the OT + app + engine suites at the pins above stay green (the regression role — the engine's persistence/load-validation suites carry the engine-side half; the app's persistenceService 20-pin suite (R33, engine seal-round register P3-30 — re-counted @ `7664603`: 20 `it(` sites, was "17-pin") + the R8-b1 real-scene-switch pin carry the app-side persistence/scene half).
...
interface ElementJSON {
  ...
  linkedTo?: string;             // (Round 15 amendment, B1) A/V link — id of the linked
                                 ...
                                 // (R28/D42) the field's HOME is OT's own
                                 // BaseTimelineElement — see §3.1A B1's D42 note
  ...
  volume: number;                // (Round 15 amendment, B2; the volume-domain law,
                                 // R27 fold — D38.2) persisted unit is LINEAR gain
                                 // (1 = unity; the R15 "0..1" reading is corrected).
                                 // The authoring/display domain is dB in [−60,+20]
                                 // — ONE home: opencut's core/audio-params (see
                                 // §3.1A B2). ABSENT ≡ unity (1 linear / 0 dB)
  pan?: number;                  // (Round 15 amendment, B2) −100..100 (full L..full R)
  preservePitch?: boolean;       // (Round 15 amendment, B2; the R9-b ruling, R27
                                 // fold — D38.1) retime keeps pitch. ABSENT ≡ TRUE
                                 // (the NLE default); explicit false = pitch-
                                 // affected varispeed. A PROJECTION of the engine
                                 // retime's maintainPitch — see §3.1A B2
  muted: boolean;
...
**B1 — link + sync-lock fields.** `ElementJSON.linkedTo?: string` backs 05 §12.3 linked selection and 16 §3.4 `toggleAVLink` (the `linkGroupId` alternative was REJECTED — one linked companion is the v1 shape, matching the mock). **(Round 25 amendment note, D32.5/ARCH-R25 — the model-shape reconciliation; the field definition is unchanged):** the persistence form is PAIRWISE (`linkedTo` — this field; the rejection above was the persisted-shape question); the runtime expansion is GROUPS — the engine's `linkedGroupId`, expanded from the pairwise links at load (pairwise at rest → groups at runtime), never persisted here; a split relinks BOTH halves, each half carrying the link (06 §5.0's split-link law — cross-cite). A DUPLICATE severs the copy's `linkedTo` — the copy enters unlinked, the original keeps its pair (the F4/R14 twin of the split law; mock `useUiStore.ts:938-945`; OT-side severing is by construction — new ids carry no sidecar meta). **(Round 28 amendment note, D42/ARCH-R28 — the field's home + the runtime closure; the field shape is unchanged):** the linkage model lives IN opencut-timeline — `linkedTo?: string` on OT's own `BaseTimelineElement` (this `ElementJSON` field is its persistence form; D32.5's pairwise-at-rest consummated OT-side at the port). The runtime group is DERIVED PER OP by a pure closure helper in OT's `ops/` — the app's av-link law ported: symmetric, live-id-only, order-stable, cycle-free, **locked-partner-skip** — and the groups→pairs mapping happens at the port per 00-master Decision 12 clause 3 (linked-groups take SceneTracks shapes per specs 06/07; OT's ops layer is the one algorithm home); the engine's `linkedGroupId` stays VENUE-INTERNAL, never persisted here. The app's dispatch-level expansion and the `linkedTo` sidecar (sceneBridge's `ElementMeta`) RETIRE into the field at the port — the carried E1/E2/F6 pins assert link values through undo/redo/serialization, impossible without a field in the SSOT. One correction to the D32.5 note above: with the field OT's own, DUPLICATE severing is NO LONGER by construction — OT's `duplicateElements` must delete the field explicitly (the `{...element}` spread would carry it to the copy; the F4 sever law, 06 §5.0). **(R33, engine seal-round register P2-19 — the B1/D42 status marker, re-keyed past the register's own filing): LANDED — Stage-0 EXECUTED in OT.** OT @ `008e7f3` (seal18; the mechanism set landed `dcb4951`, the L1.5 Stage-0 wave) carries the full D42 model: `linkedTo?: string` on `BaseTimelineElement` (`src/lib/timeline/types/index.ts:202`), the pure closure `ops/linkage.ts` (symmetric / transitive / live-id-only / order-stable / cycle-free / locked-partner-skip + `findInboundLinks`), the move fan-out, the split re-pair mint, the DUPLICATE SEVER (`duplicateElements` deletes the field explicitly — `timeline-core.ts:1431`, this note's own spread-correction EXECUTED), the delete survivor-prune, the `updateElements` linkedTo patch surface (`timeline-core.ts:431-458`), and the `toJSON`/`fromJSON` round-trip — all pinned by the M63 family (`src/lib/timeline/testing/milestones-linkage.ts`; 11 pins inside OT's 675/675 at the seal18 FINAL `ee55a87` (the report json — the dcb4951 commit prose said 10 discriminating + the split control makes 11)); the app's OT mirrors were re-pinned to `17a19f8` at the app's `7664603` — the vendored trees carry it. What stays OPEN is the r1 consumer port only: the app's sceneBridge `ElementMeta.linkedTo` sidecar (`src/sceneBridge.ts:80`) + dispatch-level expansion RETIRE into the field, and the engine bridge's `av-link.ts` closure (the five laws, `av-link.ts:26-37` + the locked-partner-skip `:143-153` @ `2a0ecf4`) converges onto OT's ops as its one algorithm home. `TrackJSON.syncLock?: boolean` backs 06 §6's sync-lock (default true, FreeCut's `syncLock !== false`).

**B2 — audio element fields (R27 amendment: the R9-b preservePitch ruling + the volume-domain law — the model-coherence pair, registered D38.1/D38.2).** `ElementJSON.pan?: number` (−100..100) and `preservePitch?: boolean` join `volume`.

*preservePitch (D38.1 — the R9-b model ruling; the R26-filed ask, resolved in code: nle-ui `af32e69` + app `94d5a2e`/`2de34ac`/`d8d1b2b`):* **ABSENT ≡ TRUE** — the NLE default (speed changes keep pitch); explicit `false` is pitch-affected varispeed (the chipmunk law). The doc field is a **PROJECTION of the engine's `retime.maintainPitch`** (the D-T5 pattern): authored through the engine patch surface, carried back by the mirror — the back-projection writes ONLY explicit `false` (true/absent project no doc field). A dormant `false` at rate 1 does not survive reload: that drop is the **APP's load-bridge identity law** (sceneBridge skips authoring retimes at speed 1 — `sceneBridge.ts:229-262`), NOT engine law: the engine stores a committed `{rate: 1}` retime verbatim and `fromJSON` never normalizes retimes (§3.3A). Writing the default onto an absent field is identity (the absent-≡-default class — no history entry). The toggle is the retime's COMPANION, not an independent element property; 06 §5.12 carries the op-side law; 18 §4.4 the Video-tab editor.
...
**NORMALIZED (kept):** id-counter seeding (element + track + **keyframe** ids), missing trims → 0, strict-boolean lock coercion, unknown extra fields ride opaque, animation canonicalization — but **never retimes** (a committed `{rate: 1}` retime survives verbatim — the preservePitch B2/D38.1 attribution).

**THE ROUND-TRIP LAW:** `toJSON()` output must ALWAYS pass this validation — the validator may never reject anything our own scenes contain (pinned: OT M53 hostile-class table + M49R load-surface pins @ `344123a` — re-keyed R33, was `970948a`). The raw constructor remains the documented escape hatch (test-fixture / synthetic-state venues).

Code: OT `ops/timeline-core.ts:2561-2655` @ `344123a` (R33 re-key — was `:2443-2537` @ `970948a`; the M60 mini-theme + s18 lib-tree waves grew the file above the fromJSON boundary; the validator its own `validateSceneForLoad` `:2733-3012`, called at `:2589`). The PROJECT-layer boundary composes with this: project JSON validates + migrates at the ProjectManager (§5.1 `migrateProject` → `ProjectSchema.parse`), then each scene re-validates at `fromJSON` on engine mount. (Mirror one line in spec 17's round-trip matrix row — cross-spec.)
...
>
> **(R33, engine seal-round register P2-18)** — the §10.3 anchors re-verified against the engine @ `2a0ecf4` (785/785; the register's own cites were pinned @ `e3f55bd`): the two lifecycle-zod rows are RETIRED-REPLACED (the schemas were DELETED — `api.ts:1808-1809` documents the removal, P1.13; the old `:1737`/`:1745` anchors now land inside the zod CLIP-op schemas, `:1734-1748` — they no longer carry `schemaVersion`/`fps`), and every drifted anchor refreshed in place (the E2:156 row is the one anchor that did NOT drift).

| Spec section | Engine file:line (or gaps/audit) | Verified quote | Status | Note |
|---|---|---|---|---|
| §3.1 ProjectJSON schema | `src/lib/nle/core/types.ts:1486` (R33 re-cite; was :1451) | `export interface Project {` | CORRECTIVE | FreeCut-shaped single-timeline container; spec's metadata/settings/scenes wins (engine D1 resolved AGAINST the spec — the C8 adapter is the convergence task, spec 19 §7) |
| §3.3 Zod validation | `src/lib/nle/headless/api.ts:2179` (R33 re-cite; was :2319) | `export async function normalizeProjectForHeadless(raw: unknown): Promise<Project> {` | CORRECTIVE | Presence-style normalization (now delegated, richer than the 3-field stub); full ProjectSchema wins |
| §3.3 version literal | `src/lib/nle/core/types.ts:1497` (R33 REPLACE — the old `api.ts:1737` cite was the DELETED lifecycle zod schema) | `schemaVersion?: number;` | CORRECTIVE | Optional number vs z.literal(N); spec's Correction #10 wins. The lifecycle zod family (`lifecycleTimelineSchema`/`lifecycleProjectSchema`…) was REMOVED — `api.ts:1808-1809` documents the deletion (P1.13); the corrective re-anchors to the ONE `Project`'s own optional field |
| §5.1 migration framework | `src/lib/nle/persistence/index.ts:361/:424` + `:78` (NLE_SCHEMA_VERSION)/`:91` (ProjectWarning) (R33 re-cite; was `:65-67/:162-183` Round-8, was `gaps/audit/MASTER.md:65`) | migrate gate + normalize + warnings channel (1,451 LOC, Wave 4B — the module grew past the R8 note's 1,174) | ALIGNED-semantics / CORRECTIVE-shape | The migrate/normalize/warnings SEMANTICS match spec §5; the serialized JSON shape is engine-native v2 — the C8 adapter re-targets it to spec 09 ProjectJSON (spec 19 §7 D1) |
| §4.1/§4.4 OPFS | `gaps/audit/MASTER.md:79` (R33 re-cite; was :85, the pre-4B quote) | (Wave 4B landed the module; D5 storage plane still OPEN) | PARTIAL | Storage-agnostic module exists; spec's OPFS layout + host MediaStore is the D5 answer when the engine attaches storage |
| §4 save path integrity | `src/lib/nle/headless/api.ts:1065` (R33 re-cite; was :2818) | `async editProject(input: NleEditInput): Promise<NleEditResult> {` | RESOLVED (was CORRECTIVE) | **The fake round-trip is RETIRED (Wave 4B)** — editProject now serializes the post-edit timeline (the `serializeTimeline` call at api.ts:1112, the R33 re-cite of scout R8-B's :1070-1130 block); the counter-example stays documented in spec 19 §6 |
| storage seam | `src/lib/nle/headless/api.ts:2499` (R33 re-cite; was :2578) | `serializeProject?: () => Project;` | ALIGNED | Optional serialize hook reserved (unwired) |
| wire shape (D1) | `src/lib/nle/core/types.ts:1486` (R33 REPLACE — the old `api.ts:1745` cite was the DELETED lifecycle wire schema) | `export interface Project {` | RESOLVED (was CORRECTIVE) | **"Two conflicting Project shapes in one module" is OBSOLETE** — the lifecycle wire family (the second shape) was REMOVED (`api.ts:1808-1809`, P1.13 second half); ONE `Project` remains; spec 15's ProjectJSON still answers D1 at the C8 adapter |
| §6 autosave ↔ undo | `src/lib/nle/timeline/timeline.ts:1665` (R33 re-cite; was :1772) | `function snapshotsEqual(a: TimelineData, b: TimelineData): boolean {` | RESOLVED (was CORRECTIVE) | **P0.6 FIXED (Wave 4A)** — equality now covers keyframes + compositions + backgroundColor (jsonDeepEqual :1691-1697, R33 re-cite of :1798-1803); m20 20.7 regression-tested; counter-example retained in spec 19 §6 |
| §5 load normalization | `src/lib/nle/persistence/index.ts:321-340` + `:424` (R33 re-cite; was `:352-387` Round-8, was MASTER.md:85) | hydrate: sanitize + orphan-prune + warnings (4B) | ALIGNED-semantics | The weak `restore()` path is retired; normalization now inline — shape still engine-native (C8) |
...
| §7.1 import probe | `src/lib/nle/headless/api.ts:2732` (R33 re-cite; was :2775) | `// Wave 1 stub: the real impl uses OPFS + mediabunny to decode + probe.` | CORRECTIVE | Engine probe is a stub; spec §7.1's flow is the growth path |
