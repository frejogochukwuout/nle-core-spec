...
- No-gap ruling, stated so it stops being asked: per-scene OT stays SINGLE-SCENE by design (D12) — scene-level concerns live app-side per §3.1A; nothing to build in OT. (Confirmed live at `c15a629` (R24; re-verified R27 @ `970948a`; re-verified R33 @ `344123a`): the core is one-scene; `toJSON()/fromJSON()` remain the whole serialization surface — W11 wired the op surface, not the doc model.)
