...
>
> **[2026-09-16 — the R28 re-rider note.]** The four re-stamp conditions' R28 state: **(1)** the R27 wave was followed by the R28 seal round's corpus folds — the W4 law fold (D42-D50 + R15-R21, 14 files @ `980920b` — 12 numbered specs + PLAN + REGISTER; the commit subject's own "13 files" was the miscount) AND the T3 test-law fold (the 12/17 amendments + the model-fold prereq sweep + the plan's test-track carve, S-spec(6)) — LANDED; **(2)** battery_r27 149/149 green at its round; battery_r28 — pending (W5/W6; carries the R28 check classes incl. the step-0 facet-completeness class); **(3)** the register's OPEN design decisions — **MET: 0 open design rows** (D42-D50 + R15-R21 ruled at W2, adversarially ratified 0-REJECT at W3 — ARCH-R28 §4.1 — corpus-folded at W4; the register's rows 5-8 CLOSED-D48/D49/D50; the implementation halves keep their phase tags); **(4)** K4's crawl exit — pending. The decisions corpus is now D8-**D50**; the K1 net at the R28 pins, **re-keyed R29 (the same-day parallel execution fleet move)**: engine 749 / OT 632 / WDC 777 / nle-ui **691** / app **256** (the register + 12 §0 carry the canon; battery_r28 re-baselines). **The certification clause extends at the full re-stamp:** test-law completeness — every facet the corpus declares (D42-D50 included) carries a row in 17 §3.1/§13A.7, machine-checked by battery_r28's facet class — so the corpus is implementation-ready **in architecture AND in verification law**: the design law and the test law moved in the same round.
