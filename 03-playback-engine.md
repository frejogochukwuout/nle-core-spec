...
- **The app's JKL/transport-mirror convergence (D30, IN FLIGHT — the R24 delta's GAP half)**: the port adopts upstream's M28R stateless ladder (R5 — REPLACES the tap-accel shuttle; the S1 pin's comment rewrite, assertions surviving per the 9-A1 trace; new pins K-pause-only + Space-inherits-rate; ⇧L/⇧J fixed-2× stay port-local, the S-round law), the wire-dispatch port (W-C: `use-wire-dispatch.ts` NEW + the EngineMount wire mint + `engineService.attachWire` + the coverage carry — the transport verbs cross the wire), the loop two-way echo-guarded bridge (R4: tick-domain echo keys both sides, identity-stable mirror writes, null-region keeps the last loop values and flips `loopEnabled` only), the Z4 `[data-transport]` scrub guard + the R6 `r`-key yield-set (one nle-ui package commit, same-round lockstep) — **K3** (app; the D30 W-C/W-D/W-E/W-F waves — the coverage-gate instrument per ARCH-R24 D29.1(b); verified PENDING at `c885ece`: the tap-accel ladder still live, no `data-transport` guard, no wire symbols in src/) (acceptance: the wave pins green — wire-routing observability, rate segments, the loop round-trip BOTH directions, the coverage gate's accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions).
...
- The **K3 audio pins for playback** (the C3 audio-crawl row): offline-render + call-spy pins over the engine/WDC audio paths (the waveform/mute laws; the Node-venue law — no audio device in CI, meters are a live-push law per WDC's offline guard) — **K3** (app authors; **hard prerequisite: S-wdc's waveform seam contract** — the only K3 sub-half with an external prerequisite; the contract is STILL unstated WDC-side — the D29.4 promotion action moves it from WDC's parking-lot bullet into the HANDOFF next-session scope) (acceptance: the pins green under the Node venue; the mute law = engine law, one owner).
...
**ACCEPTANCE & TEST PLAN:** §12 (the seed intent list, preserved verbatim) + the unnumbered `## Testing` section at file end (Tier 1/2/3 + property — frame accuracy, AV drift, scrub latency, varispeed pitch); spec 17 §13A facet rows (the §3.1 "Playback (frame-accurate)", "Varispeed (pitch preservation)", "Scrubbing latency" matrix rows); battery_r24's posture checks (BASE rows re-verified at the live pins — the regression role; the K2 combined-family census registers the engine-in-the-middle + WDC-integration families in spec 12/17).
...
### 13E. Code References — nle-engine (the historical register, re-verified at `5036387`)

> The private **nle-engine** repo (github.com/bearachprema/nle-engine) is a clean-room FreeCut port that has since been ACCEPTED as the BASE playback core (§0: `5036387`, 458/458 vitest, tsc 0 — the R24 re-baseline's posture; the R22/R23 pins and the Round-7-era "37,958 LOC, 124 tests, in-between reference" framing are superseded, kept here as history). Where engine code conflicts with this spec, **the spec wins** (the standing law). Full reconciliation: `19-code-references.md`. Line numbers below re-verified against the live tree this round — clock.ts 764 LOC; player.ts 3,656; video-sync.ts 1,234, all UNCHANGED at the live pin (the playback core is source-stable across the R24 re-pin; the +18 test delta lives in the bridge/composition seam, spec 04's territory).

| Spec section | Engine file:line (re-verified @ `5036387`) | Verified quote | Status | Note |
|---|---|---|---|---|
...
| §3.5 StaticClock | — | COULD-NOT-VERIFY (no StaticClock class — re-verified at `5036387`: still absent) | SPEC-ONLY | No render-entry clock |
