...
> nle-engine ports all six plan functions and drift constants verbatim (`playback/video-sync.ts`, BASE at `e3f55bd` — line numbers in §13E re-verified this round; the file is unchanged since `b8c6f88`), but applies them to `HTMLVideoElement` via a DOM registry — the plan logic is the reference; the DOM application surface is the delta §5.1 corrects (decode-rate actuation, not element playbackRate). See `19-code-references.md`.
...
| §3.5 StaticClock | — | COULD-NOT-VERIFY (no StaticClock class — re-verified at `f9ac806`: still absent) | SPEC-ONLY | No render-entry clock |
