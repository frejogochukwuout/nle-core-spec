...
| §7.2 three-tier scrub | `src/lib/nle/playback/player.ts:280` | `When the user scrubs the timeline (beginScrub → frame seek → endScrub),` | ENGINE-GAP (narrowed) | Wave 4A landed the scrub cache + prewarm (single-tier LRU + `_scrubPrewarmPromise`); the FreeCut 3-tier GPU/RAM cache remains engine-side unbuilt |
