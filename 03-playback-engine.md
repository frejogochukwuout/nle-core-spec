...
**Status:** v-next (Round 23 — the per-file fleet re-audit: every BASE pin/count/feature re-verified against the live repos at the 2026-09-07 pins; the R22 BASE (engine playback + OT's S1 transport policy + the app's ticker/JKL/loop mirror) EXTENDED with the round's landings — WDC's W2 varispeed/retime domain, the app's W3 JKL audio half, N2b keyframed volume, RR1-A the fade-clamp SPAN law; the GAP re-tagged onto the D24 ladder (C2-behavior→**K3**, C2-frame→**w1**, C3→**K3**, W-media→**w2**, audio depth→**r2**) per `IMPLEMENTATION-PLAN.md` §2; spec 14's plan content retired to the redirect stub); Refined spec — sub-agent scout SCOUT-03 has verified all claims against FreeCut + OpenCut-classic source (the historical verification record — §0 carries the live pin world)
...
- The **K3 audio pins for playback** (the C3 audio-crawl row): offline-render + call-spy pins over the engine/WDC audio paths (the waveform/mute laws; the Node-venue law — no audio device in CI, meters are a live-push law per WDC's offline guard) — **K3** (app authors; **hard prerequisite: S-wdc's waveform seam contract** — the only K3 sub-half with an external prerequisite) (acceptance: the pins green under the Node venue; the mute law = engine law, one owner).
...
- The app's **engine consumer re-pin** (the transport/audio laws above land app-side only after the pin bump; the app's `4ef0147` trails `b8c6f88` — one commit, per the fleet's git-level re-count) — **K1-continuous** (S-app; the pin-lockset law: module lands first, app pin-bumps within one business day) (acceptance: the app's suite green at the re-pinned HEAD; W2/N2b/RR1-A consumers verified).

**ACCEPTANCE & TEST PLAN:** §12 (the seed intent list, preserved verbatim) + the unnumbered `## Testing` section at file end (Tier 1/2/3 + property — frame accuracy, AV drift, scrub latency, varispeed pitch); spec 17 §13A facet rows (the §3.1 "Playback (frame-accurate)", "Varispeed (pitch preservation)", "Scrubbing latency" matrix rows); battery_r23's posture checks (BASE rows re-verified at the live pins — the regression role; the K2 combined-family census registers the engine-in-the-middle + WDC-integration families in spec 12/17).
...
> nle-engine ports all six plan functions and drift constants verbatim (`playback/video-sync.ts`, BASE at `b8c6f88` — line numbers in §13E re-verified this round), but applies them to `HTMLVideoElement` via a DOM registry — the plan logic is the reference; the DOM application surface is the delta §5.1 corrects (decode-rate actuation, not element playbackRate). See `19-code-references.md`.
...
### 13E. Code References — nle-engine (the historical register, re-verified at `b8c6f88`)

> The private **nle-engine** repo (github.com/bearachprema/nle-engine) is a clean-room FreeCut port that has since been ACCEPTED as the BASE playback core (§0: `b8c6f88`, 440/440 vitest, tsc 0 — the R22 re-baseline's posture; the Round-7-era "37,958 LOC, 124 tests, in-between reference" framing is superseded, kept here as history). Where engine code conflicts with this spec, **the spec wins** (the standing law). Full reconciliation: `19-code-references.md`. Line numbers below re-verified against the live tree this round — the files grew (clock.ts 764 LOC; player.ts 3,656; video-sync.ts 1,234 at the live pin).

| Spec section | Engine file:line (re-verified @ `b8c6f88`) | Verified quote | Status | Note |
|---|---|---|---|---|
...
| §3.5 StaticClock | — | COULD-NOT-VERIFY (no StaticClock class — re-verified at `b8c6f88`: still absent) | SPEC-ONLY | No render-entry clock |
