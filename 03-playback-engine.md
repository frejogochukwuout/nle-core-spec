...
**Status:** v-next (Round 24 — the per-file fleet re-audit: every BASE pin/count/feature re-verified against the live repos at the 2026-09-08 pins — engine `5036387`/458, OT `ded43c4` (code tip `c15a629`)/536, WDC `85b81b0`/759, nle-ui `fc4cc35`/674, app `c885ece`/174, the consumer pins zero-lag; the R23 BASE EXTENDED with the round's landings — OT's W11 complete-UI round (the wire-dispatch seam + **W11-c transport completion, the S1 half at the UI layer**: the M28R stateless rate-based JKL ladder + the transport verbs routed), the engine re-pin (458 — the W2.5/R8-REV composition-seam pins; the playback core source-stable), WDC's S4 seal (the JKL audio half CLOSED; R4 ⇧K the residue), the app's R8/R9 waves (117→174; the D30 W-C/W-D JKL adoption in flight); the JKL/transport rows re-based — BASE = OT's M28R ladder + the routed transport verbs, GAP = the app's D30 adoption; the GAP phase tags re-verified on the D24 ladder per `IMPLEMENTATION-PLAN.md` §2 — the dual-vocabulary window closes at this fleet's end, D24 set only); Refined spec — sub-agent scout SCOUT-03 has verified all claims against FreeCut + OpenCut-classic source (the historical verification record — §0 carries the live pin world)
...
## 0. FORWARD INVENTORY (R24 posture — re-audited against the live fleet; the BASE is accepted, not re-explained)

**BASE (accepted, pinned 2026-09-08; every row re-verified by the R24 fleet audit against live code):**
- nle-engine @ `60232ea` (code anchor `60232ea` — the P-F4 src fix, the pin = the last src-touching commit; the R33 reads: HEAD `2a0ecf4` / anchor `28aa387` — the P-F2 corollary, with `afa3f92` the property tier: the first src/ movement since R28) — **789/789 vitest** (live recount: 29 test files — bridge-seams **151**, nle-bridge **99**, planner 52, timeline-math 42, transform-resolver 40, load-validation **33**, video-sync 31, … **plus the W2 property tier** `tests/vitest/property/` 4 files at **33 sites** (undo-soak 11 + timeline-math 10 + persistence-roundtrip 6 + transform-resolver 6) + the 3 P-F2 pins + the 4 P-F4 pins — **789 = 749 + 33 + 3 + 4**; the runtime count gate `scripts/gate-vitest-count.mjs` the pin [PINNED_TOTAL 789; locked-track 12 static→49 runtime, nan-guard 31→85]; **R33 re-key**: was `e3f55bd`/`74bef08` at R29 — 749/749, 25 test files; **R34 re-key**: was `2a0ecf4`/`28aa387`/785 at R33 — the P-F4 catch (the degenerate head-trim source-window inversion, the nightly exploration venue's first dispatch) + the count-gate re-pin 785→789 same-commit + the nightly + the 5-job CI both GREEN; timeline.ts 9,078 LOC (9,051 at R33); vendors OT @ `17a19f8` + WDC @ `94f6460` UNMOVED through R34; the live-run venue is GHA CI), tsc 0 — the playback core: `core/clock.ts` (the FreeCut Clock port, grown — R1-B7 the Clock.fps re-anchor while playing + the R2-4 fake-timer RAF test law; **R33 re-attribution (register P1-1): the R1-B8/R2-5 scrub-pause family does NOT live here — clock.ts has zero scrub matches; it lives in `playback/player.ts:793-804`, unpinned in-suite**), `playback/player.ts` (transport + the FX-2/C2 transport-generation counter that kills in-flight `play()` tails; per-type frame build — video/image/text/composition per the N1 family + Wave 4C/4D-B; scrub cache + prewarm + the endScrub delay per Wave 3F, the awaitable prewarm promise (`_scrubPrewarmPromise`/`awaitScrubPrewarm`) + event forwarding per Wave 4A — re-attributed (R33, engine seal-round register P3-11)), `playback/video-sync.ts` (all six sync plans + the drift constants verbatim), the av-link dispatch (`bridge/av-link.ts`, N4), the real audio crossfade paths (audio-scene transition windows + the R3-3-REFINED mid-entry crossfade in-ramp clamp / author-head-fade drop). **New since the R22 pin:** RR1-A — the fade-clamp **SPAN law** (`clampAuthorFadeToSpan(fadeSec, spanSec)` in `audio/audio-scene.ts` — `spanSec` is the OUTPUT span: the clamped bridge duration on the export path, the remaining timeline span on the player's mid-entry path; BOTH venues call the one helper — player↔export parity by construction, closing the preview-only-unclamped-fade break); consumer-compat `toSorted` (ES2023 — planner.ts/texture-pool.ts); **N2b keyframed volume LANDED** (`37cdd28` — the flattener folds `animations["volume"]` to gainAutomation breakpoints, lane-REPLACES the static clip gain, merge-blocking both directions; the adapter's dedicated automation gain node with the dB-domain exponential-ramp walk + the R1-B2 native core path `foldVolumeKeyframesToAutomation`/`ScheduleClipOptions.gainAutomation`; 18 bridge pins + 2 offline-render sample-level parity pins); the `timeline-edit-ops` combined tests (the K2 cross-module seed: vendored-OT ops × engine composition, incl. the R2-4 Clock.fps re-anchor pin). **New since the R23 pin:** the vendor re-pins — OT @ **`c15a629`** (a4e971d→c15a629 by `5036387`, the W11 complete-UI round: `HeadlessTimelineApi` attach + the dispatch recorder + `WIRE_COMMAND_TYPES` + `applyBatch data.results` importable at the engine's vitest-alias test seam with ZERO engine mediation by design — the engine consumes the scene STRUCTURALLY only, type-only imports + one runtime fn), WDC @ **`494f6ff`** (f446512→494f6ff by `5e5bbe4`, docs-only for consumers; the W2+whole-CR pin chain 5570321→…→de09c93→f446512→494f6ff; one docs-only commit behind `85b81b0`); the **W2.5 effects sidecar + R8-REV #5 blur clamp** (app-stream rounds `8a0b7fe`/`4b2dfd1`: `CompositionElementParams.effects` + `buildElementFilterString` in `bridge/composition-frame.ts` — the honest CSS-filter preview subset, bridge-level outside the frozen barrel; +17/+1 pins = the whole 440→458 delta, spec 04's territory); **the playback core stable through the R27 re-pin (R24's SOURCE-STABLE claim retired by the RA round: clock.ts 764 / video-sync.ts 1,234 unchanged; player.ts **3,695** (+39: the RA round's HW1-4 registry auto-ids + epoch-keyed still-texture cache, `e9f2d91` HW1-10 grouping loop — 03's §13E cites sit outside the moved region, all 13 re-verified exact)**; the src tree has since grown through the RC/M2 waves — see the vendor sentence below).
- opencut-timeline (OT) @ HEAD **`8f96ab2`** (R34 — the M59/M64 carrier round: the CR families CR-2/3/4/5/8/9 LANDED [TimelineContextMenu CR-2 + `EMPTY_CONTEXT_ITEMS`, TimelineView's CR-3 `scrollAnchorId` + CR-4 `cancelRegistry` + the M59 carrier props, use-timeline-actions CR-5 `performElementDelete` + the confirm-delete carrier, use-keybindings CR-8 `surfaceOwnedKeys` + the exported `SURFACE_OWNED_KEYS`, the CR-9 DragEventLike JSDoc] + the M64 fps param; **714/714 @ 83 report entries** — the report json at the pin the count authority; the code pin `8f96ab2`, was `344123a`; `src/lib/timeline/{ops,headless,bookmarks,testing}` byte-unchanged — the wire census 31 = 28 routed + 3 exceptions UNCHANGED; re-keyed R34, was `008e7f3`/675/675 @ 77 entries. R33 — the seal18 close: the Stage-0 mechanism set [D42 linkage + D43 edit-domains + D44 ERROR_CODES] + the real-defect wave + the M61/M62/M63 families + the M58R flake fix; **675/675 = 632 classic [incl. the 3-test M58R family] + 17 M60 + 11 M61 + 11 M63 + 4 M62, 77 report entries**; the code pin `344123a`; the R32 layer: `39003d3` — the code pin MOVED — the D25.3a mini-theme wave: theme.ts mode maps + the TimelineAudioWaveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins; 649/649 = 632 classic + 17 M60, the report json at HEAD the count authority) — was `3e18722`/**code pin `970948a`** (docs/runner-artifacts only, src diff empty) — **632/632 (72 milestone entries — the total only, the split convention changed shape; the report json committed at `970948a` is the count authority), tsc 0** — **S1 the transport policy LANDED in core** (`ops/timeline-core.ts`): `setPlaybackRate` (finite NONZERO; negative = reverse; **position-continuous** — the origin is NOT rebased, S1-F5, so JKL presses never skip a frame; notify-ONCE; NOT undoable per the seek law), `setLoopRegion`/`getLoopRegion` (validated 0≤start<end≤committedDuration; reconciled on every committed state change S1-F4; **wrap fires on CROSSING only** S1-F3, both directions, in-region-bounded), `advancePlayhead(dt)` (consumer-supplied wall dt — the engine stays timer-free and deterministic; **anti-drift: the math runs on the UNSNAPPED origin**; boundary stops evaluate against the COMMITTED duration S1-F2; **the atomic boundary pause** — one notify, listeners never observe playing=true at the end; the S1-F7 mirror-cadence law — notify only when the published frame-snapped playhead changed), the headless `timeline.setPlaybackRate`/`setLoopRegion` command surface (+ the `getLoopRegion` getter echoed in setLoopRegion's result). Plus S2 track lock (core `toggleTrackLocked` + the components' actions-layer pre-filter — locked refs drop BEFORE the op), S3 `setTracksMuted`, M45 (T-round wave-F fix pins) + M46 (S3 seam pins, +3), the F1 whole-codebase hardening, the W10F fix waves + M44-M48. **The W11 complete-UI round (the R24 delta):** the wire-dispatch seam (`useWireDispatch` → `HeadlessTimelineApi`; **31 wire verbs = 28 UI-routed + 3 documented exceptions** (`WIRE_COMMAND_TYPES` @ api.ts:243-275 + `WIRE_UI_EXCEPTIONS` @ :328-335 — the exceptions now an EXPORTED tsc-asserted Record: selectElements/advancePlayhead/trim; **D-ARCH-6**: `timeline.insertBatch`/`removeKeyframes`/`retimeKeyframes` landed one-history-entry-each, the singular remove/retime RETIRED), M49C the coverage gate machine-checking the routed set (28/28, origin-attributed — the gate consumes the exported constant live), tsc-lockstep both directions — **the transport verbs routed: `timeline.{play, pause, seek, setLoopRegion, setPlaybackRate}`**) and **W11-c the transport completion — the S1 half at the UI layer**: the **stateless rate-based JKL ladder** (DECISIONS #25's conscious divergence from classic's j/k/l jump rows; **M28R rewritten to pin it** — rate ∈ {0,±1,±2,±4}; a same-direction press DOUBLES, capped ±4; an opposite-direction press or a press from paused resets to ∓1; **K pauses ONLY** — Space resumes at the persisted rate, the engine's rate-persistence law; **dispatch order setPlaybackRate BEFORE play** — the S1 rate-conditioned restart reads the rate at play(); `use-timeline-actions.ts:55-62/:178-197`, the real-mouse pins at `scripts/m28-actions-real-mouse.mjs:271-338`) + the rate control + loop region + overlays (M49T, 5 pins incl. the wrap laws); the op-parity audit at HEAD (11/11 NLE edit verbs vs opencut-classic). `src/components/timeline/` = **the canonical React UI tree (D25)**: `use-playback-ticker.ts` (rAF → `advancePlayhead`), `use-timeline-playhead.ts` (the F-B6 playback-follow fix — the imperative playhead left updates on every engine notify; the R15-era "playhead FREEZES during engine playback" punch item is CLOSED). **(R31: HEAD `31f2764` — 632 UNCHANGED, docs-only over the code pin `970948a`: the R31 queue filing = the carrier-reduction work order + the D25.3b structural-gap worklist + the data-test freeze ask)**
- web-daw-core (WDC) @ **`ec8fd5c`** — **777/777 vitest**, tsc 0 (the S-series upstream wave — the EngineDiagnostic bus + heartbeat monitor, the rebuilt dsp-effects-worklet bundle (the DspDiagnosedProcessor mixin), the stall bypass-to-passthrough ladder, the rt-safety-gate transitive-heritage fix — the diagnostics law text at 02 §7.4; WDC's own HANDOFF never absorbed the wrap — their next session's problem) — **W2 varispeed/soundtouch LANDED (pre-R23, consumed, not re-done — re-verified: `retimeChannels` + `RETIME_RATE_MIN/MAX` = 1/32/32 at `varispeed.ts:36-44`)** (`lib/daw/soundtouch.ts` the full WSOLA port + `lib/daw/varispeed.ts`): `retimeChannels` — pure WSOLA retime, `tempo = rate` ONLY, output length EXACTLY `round(inFrames / rate)` (the F1 flush law); identity laws (rate 1 / non-finite / ≤0 / >2ch → input returned reference-equal — the legacy pitch-affected path stays the caller's fallback); **the retime RATE DOMAIN [1/32, 32]** (CR-A1: outside it the WSOLA math degenerates); the general tempo/pitch/rate worklet; `maintainPitch` the bridge's WSOLA pre-retime path. Plus the whole-CR hardening: the meter-tap offline guard (meters are a LIVE-push law — offline `onMeter` no-ops), the drift-gate now FAILS-on-drift. The integration family (`audio-integration`, `dsp-bounce-parity`, `dsp-effects-integration`, `nle-audio-core-derisk`, offline render + detection, `varispeed*`) = the K2 combined-net seed (759 held — zero code delta since R23). **The S4 seal, documented in the wrap:** the **JKL audio half CLOSED** (W3 — the composed-varispeed law family; see the app row: composed varispeed = element × transport, wall = timeline/r, fades ÷ r, one `lastScheduledTr` idempotence key, J = pragmatic silent-reverse; the design v1.1 lives APP-side at `nle-test-app/docs/design-jkl-audio-follow.md`); residue **R4 only** (⇧K is a dead key in the engine world — a P3 document item). The **async pre-retime STILL QUEUED** (the adapter's sync WSOLA compute on the play edge is O(span) — 3-min stereo @ rate 0.5 ≈ seconds of jank; the fix specced at `docs/design-w2-varispeed.md` §4 (async pre-retime + legacy fallback while pending); WDC's HANDOFF #3 = the engine's G2 — the ENGINE-OWNED design round comes FIRST).
- nle-test-app @ `d28847c` — **426/426 vitest, the 22-file roof (R33, engine seal-round register P2-4; R34: the lockstep re-pin `7664603`→`d28847c` — vendor engine→`60232ea` + both OT mirrors→`05d88d9` + nle-ui→`cb04919` + WDC→`94f6460`; the count + the roof UNCHANGED, the census register re-declared 43 = 33 zero-action + 8 carriers + 2 hosts)** — the recount at the pin: GluedShell 124 + audioService 42 + sceneBridge 41 + timeline-geometry 38 + engineService 31 + wire-coverage 26 + persistenceService 20 + deliverService 20 + history-laws 15 + e2e-crawl 12 + waveformPeaks 9 + dom-structural 8 + engineSeam 7 + doc-committing-sessions 7 + mid-drag-yield 5 + scrub-session 4 + keybindings-repeat 4 + trim-gesture 3 + dnd-fallback 3 + clip-lifecycle 3 + zoom-anchor 2 + edge-autoscroll 2 (the R30 re-key's census was stale at its own recorded pin — its eight listed suites summed to 252 and its "wire-coverage 9" was already wrong at `876f2b8`; the lineage: the R8 D29 real-wiring round + the R9/R25-R27 waves took 117→174→206→252, the R30 K3 corpus →377, R31 →418, R32-W1 →419, R32-W3-a →422→426; the pin then lockstep-re-pinned to `7664603` with vendor/nle-engine → `574d8d3` + both OT mirrors → `17a19f8`), tsc 0 — the app's ticker/JKL/loop mirror over the engine clock, now with **W3 (the JKL AUDIO half) LANDED** — the six pinned laws: (1) **both rate-change entry paths reschedule with ONE idempotence key** (the core-notify edge runs BEFORE the jump heuristic — a rate drop 4→1 leaves one old-rate frame delta above the fresh-rate bar and would fire the seek branch spuriously; `lastScheduledTr` is the key, so the S3-C4 store-echo edge sees its own value consumed — exactly one stop+reschedule per rate change, whichever edge fires first); (2) **the composed varispeedRate = segRate × tr** (spans/anchors/fades ÷ tr); (3) **the #24 offset law is ELEMENT-domain** (`offsetSec = trim + into × segRate` — the transport rate moves how fast you traverse, not WHERE in the clip you are); (4) **silent-reverse J** (tr ≤ 0: segments stop, NOTHING schedules — WebAudio cannot play backward; the honest degradation, pinned); (5) **timeline-authored fades** (ramps sweep WALL seconds — fade ÷ tr, the R3-3 clamp computed in the timeline domain first); (6) **the routed setShuttle round-trip** (the store's `setShuttle` → the package router's `setPlaybackRate`: rate 0 = pause, nonzero = setPlaybackRate + play; the mirror carries playing/playRate back — the impl never writes them). Plus the port's S1 JKL keymap (**the M28R stateless ladder LANDED (D30 W-D, `f681d34`)**: `use-keybindings.ts:37-46` + `use-timeline-actions.ts:223-240` — the tap-accel window RETIRED, K = pause-only, Space resumes at the persisted rate, setPlaybackRate BEFORE play; ⇧L/⇧J fixed-2× stay port-local, the S-round law; the shell's mock-world JKL, router-gated, untouched), the EngineMount transport laws (the loop-region one-way push — store authors, engine owns policy; the playRate seed survives scene switches; the echo-guarded two-way loop bridge LANDED (W-E — `EngineMount.tsx:332-409`, tick-domain echo keys both sides; cite re-anchored (R33, engine seal-round register P3-12) — push :332-355 (`core.setLoopRegion(region)` :354), reverse bridge :388-409 (tick-domain key compare :398, `patch.loop` :403); the old `:238-294` range is now the D-ARCH-6 `timeline.insertBatch` pool switch); D30's JKL/transport-mirror GAP row RETIRED to BASE here (R33, engine seal-round register P2-2): the r1 residue it held open — the OT mirror re-pin — LANDED @ `876f2b8` (the s17 re-pin, the consumer note below; the mirrors since advanced to `17a19f8` at this pin), and the acceptance gate is green at the current pin — the live-registry wire-coverage gate, 31 wire verbs = 28 routed + 3 exceptions, 26 tests (`wire-coverage.test.tsx:29` the live registry imports, `:577-582` THE GATE)), N2b's 2 glue pins (`scheduleAll` threads the automation lane; `segmentSignature` gates keyframe edits while playing), ProgramCanvas (R7/N1 — the engine-wired program monitor), the WDC audio host (the singleton-law graph).
- Consumer-pin note (R24's ZERO-lag claim superseded — R27's D-ARCH-6 re-pin queue, **R29: LANDED, re-converged**; the post-R31 app-lane rider absorbed at the `7664603` lockstep re-pin — re-keyed (R33, engine seal-round register P3-14)): the app's engine vendor pin is **`574d8d3`** (the engine's seal-round re-pin — its vendor OT `39003d3`→`17a19f8`, the s18 seal round; the app lockstep-re-pinned at `7664603`; the earlier chain: the R28 reading's `74bef08` ≠ the engine HEAD BY DESIGN, not drift → `e3f55bd`, the R29 re-convergence); the app's OT-mirror lock-copy sits @ **`17a19f8`** (BOTH mirrors, `mirroredAt` 2026-09-19 — the s18 seal round: the lib-tree wave [`linkage.ts` + `edit-domains.ts` NEW, `api.ts`/`timeline-core.ts`/`split.ts`/`types` additive] + the M58R flake fix; the wire census UNCHANGED — **31 verbs = 28 routed + 3 exceptions, the registry claim survives the move**; byte-exact minus `testing/`; the earlier chain: `6e2b91a` → `55c81c0` (s17, LANDED @ `876f2b8` — the ported gate went RED at the re-pin until the gesture-seam switches landed, `use-timeline-actions.ts:452` + `use-keyframe-drag.ts:96`) → `39003d3` (the M60 wave, 2026-09-19) → `17a19f8`); the app also pins nle-ui @ **`1e0c30b`** (743/743, the D52 part-1 twin; the earlier chain `83ff8a8` → `6754979` (the AW1-2 absorb) → `dc3644b` (R31) → `1e0c30b`) + WDC @ `ec8fd5c` (unchanged); the ENGINE's own vendors: OT @ **`17a19f8`** (the s18 re-pin at the engine's `574d8d3`; earlier `55c81c0` → `39003d3` → `17a19f8`) + WDC @ **`94f6460`** (the M60 docs-only wave at the engine's `b77909b`; was `83b8850`). **The R34 lockstep re-pin (`d28847c`, the vendor half — re-keyed R34): the app's engine vendor `574d8d3`→`60232ea` (the P-F4 code anchor) + BOTH OT mirrors `17a19f8`→`05d88d9` (the M59/M64 carrier round) + nle-ui `1e0c30b`→`cb04919` (747/747 — the DeliverPage supersede + the viewer-state-empty pair) + WDC `ec8fd5c`→`94f6460`; 426/426 + the 22-file roof UNCHANGED; the census register re-declared 43 = 33 zero-action (7 byte + 26 mechanical) + 8 carriers + 2 hosts (the CR-2 carrier FLIP — TimelineContextMenu byte-exact at the new pin, 9→8); the ENGINE's own vendors UNMOVED (OT `17a19f8` + WDC `94f6460` at `60232ea`).**

**GAP (the work — owner + phase per the D24 ladder, `IMPLEMENTATION-PLAN.md` §2; acceptance in parentheses; spec 14's phase register is retired — its runtime rows live HERE):**
- The event staircase playhead rows at full scope — every spec 15 §9.5 row published + consumed + pinned — **w2** (app; the W-media row — realness = walk) (acceptance: staircase suite green; the full-scope demo re-gates over the media registry).
- **K3 playback-behavior corpus** (the C2-behavior half; CORE-SEAMS S11 the playback-clock seam → **K3**): playhead ownership = engine clock truth — the mini's playback law families re-expressed app-side over the real engine (LAW-NET Part B rows: playhead scrub clamp, playback wrap/empty-guards/scrub-extents, R19 seekToClipHead walk-back, the R22 tick-mid-gesture live playhead — 12 tests) — **K3** (app; the store/policy halves author NOW against the injection-point contracts, ∥ the D26 census discipline, per the ordering law as re-stated by ARCH-R24) (acceptance: scrub/play/seek real, no mock clock; the corpus rows green against EngineMount's real engine). **Landed so far (R33, engine seal-round register P3-15):** the R30/R31 tranches — the scrub-session family (`scrub-session.test.tsx` 4 — "R31 Wave A, per docs/k3-map-timeline.md rows 47/42/10") + the K3 edit-verb families (`engineService.test.ts` 31 over k3-map rows 19/8/4/11/10/16/34) + `timeline-geometry` 38 + `history-laws` 15 — plus the R32 families (the doc-committing-session registry + D52's twin, `doc-committing-sessions.test.tsx` 7 @ `7664603`; K4's 12-leg e2e, `e2e-crawl.test.tsx` incl. the playback LEG 3); remaining: the rest of the LAW-NET Part B set (the stale "with D30's W-C in flight" phrasing dropped — W-C `f681d34` landed long since).
- **The app's JKL/transport-mirror convergence (D30 — LANDED R25-R27; the row RETIRED to the BASE app row (R33, engine seal-round register P2-2))**: the r1 residue this row held open — the OT mirror re-pin — is LANDED (`876f2b8`, the s17 re-pin, recorded five rows up in the consumer-pin note; the mirrors have since advanced to `17a19f8` at the current app pin), and the acceptance gate is green at the current pin: the live-registry wire-coverage gate (`src/wire-coverage.test.tsx:29` the live `WIRE_COMMAND_TYPES`/`WIRE_UI_EXCEPTION_VERBS` imports, `:577-582` THE GATE, 26 tests — 31 wire verbs = 28 routed + 3 exceptions at the current mirror). The wave record, preserved: W-C `f681d34` (the wire-dispatch seam + the M28R stateless ladder — the tap-accel window RETIRED, K = pause-only, Space resumes at the persisted rate, setPlaybackRate BEFORE play), W-D `78e7283` (the r/⇧R loop rows + the Z4 `[data-transport]` keymap guard — no shiftKey carve-out per 9-G1 P1-1) + nle-ui `755235c` (Z4 stopPropagation on the scrub row + ruler root) + `2646d80` (the R6 `r`-key yield-set — 'r' in both yield sets, the D28-A5 lockstep pattern's third execution), W-E the loop two-way echo-guarded bridge (EngineMount.tsx:332-409 — push :332-355, reverse :388-409, tick-domain echo keys both sides; re-based (R33, engine seal-round register P3-12)), W-F the coverage gate (the accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions, live — re-fenced to the LIVE registry `8a3eb35`, RC-V1's P2 fix, + the AR-2 gesture pin) — was **K3** (app; the coverage-gate instrument per ARCH-R24 D29.1(b)); acceptance DISCHARGED (the wave pins green — wire-routing observability, rate segments, the loop round-trip BOTH directions, the coverage gate's accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions, the gate consuming the exported registry live).
- The **viewer FRAME visuals** (the C2 visual half): the viewer honors the spec-18 §4.3 viewer state rows (loading/decode-fail+retry/offline) + the mini's implemented viewer state testids, side-by-side vs OT's `/view` gold sample — **w1** (app + OT runner; the grammar is human-fidelity work) (acceptance: the side-by-side VLM pass; the §4.3 rows + state testids enumerated in the acceptance lists at execution).
- The **K3 audio pins for playback** (the C3 audio-crawl row): offline-render + call-spy pins over the engine/WDC audio paths (the waveform/mute laws; the Node-venue law — no audio device in CI, meters are a live-push law per WDC's offline guard) — **K3** (app authors; **hard prerequisite MET (R25-R26)**: WDC's waveform seam contract — `waveform-contract.ts` (WAVEFORM_PEAKS_BUCKETS + WaveformPeaks + the three audio-registry functions) is the named cross-repo contract (the 14-pin contract test green inside the 777 at `ec8fd5c`); the app's consumer landed with 9 pins (`waveformPeaks.test.ts` @ `44e8049`) — remaining: the offline-render/call-spy pin audit over the mute laws under the Node venue) (acceptance: the pins green under the Node venue; the mute law = engine law, one owner).
- **Reverse-transport audio depth**: the JKL negative-rate half is BASE-silent by law (W3's silent-reverse J); FreeCut's reverse-shuttle grain plan (§8.4) remains the reference for full reverse-transport audio — **r2** (audio depth = run; spec 20's r2 family owns the mixer depth; this row is the transport-rate half) (acceptance: a grain-plan port or an equivalent not-needed ruling, registered + pinned either way).
- **The decode/scrub performance architecture (§5 + §7.3)**: the mediabunny decode-worker pool + VideoSourcePool adaptation + rVFC drift path, and the 3-tier scrub cache (GPU/RAM/LRU) — virtual media (the crawl's stand-in) needs none of them (the engine's single-tier scrub cache + prewarm are BASE); real media decode is **r4** (PENDING the user's D6 re-affirmation — deprioritized), the tiered caches + decoder prewarm ride the engine's own P2 backlog (**r6**, slips independently by design) (acceptance: per-item engine milestones green as queued; each tier's hit-rate law pinned at its landing).
- The app's **engine consumer re-pin — CLOSED at R9 W-A** (`45c00d1`: the app pins the engine @ `5036387`, ZERO lag, the behavior-delta gate PASS; the R23 one-commit debt PAID); the standing LAW survives — **K1-continuous** (S-app; the pin-lockset law: module lands first, app pin-bumps within one business day — every future module landing re-arms it) (acceptance: the app's suite green at every re-pinned HEAD).
- **The §12 clock-law families have NO engine-venue pin (R33, engine seal-round register P2-3)** — the carrier question, answered by filing the GAP: owner engine, no D24 phase (engine-venue test debt, the engine's own backlog). The §12 item-1 clock families (monotonic `now()`, seek re-anchor, `setRate` frame stepping, ceil-not-floor at negative rates, in/out-point range playback) + the item-5 50 ms scrub-latency goal are test-plan text only at the engine: the ONLY `Clock` pin in the engine's 789 (re-keyed R34, was 785) is the B7/R2-4 fps-re-anchor pin (`tests/vitest/engine/timeline-edit-ops.test.ts:120-148`, the fake-timer law); the W2 property tier (the 33 property sites over the timeline-math kernel / the undo soak / the transform resolver / the persistence round-trip, landed at `2a0ecf4` AFTER this register was filed — the tier UNCHANGED at `60232ea`; the P-F4 pins ride the nan-guard family, not the property files) does not touch `core/clock.ts`; the untested law bodies: the `_now()` time-source epoch/monotonic law (`core/clock.ts:572-593`), the rate-signed floor/ceil frame split (`:637-640`), the 100 ms `timeupdate` throttle (`:687`); the app's K4 LEG 3 (`e2e-crawl.test.tsx:563` — Space → play → the clock advances → pause frozen) is a coarse app-venue clock pin, not the §12 families (acceptance: the §12 item-1 bullets re-expressed as vitest pins over `core/clock.ts` with the R2-4 fake-timer law, plus the audio-clock spy pin — the §3 `AudioContext.currentTime` ground truth — and the 50 ms scrub-latency goal carried at a measured venue, registered + pinned either way).
- **NS-1 transport/render-clock coupling** — design queue, owner engine (the §0 pointer row, filed (R33, engine seal-round register P3-13): 03's dual-transport BASE rows above — OT's timer-free `advancePlayhead` + the app's audioService schedule — are coupled by app-owned glue no spec defines: the `lastScheduledTr` idempotence key + the composed law at `nle-test-app/src/audioService.ts:450/:585`; the engine's own filing `.agents/PLAN.md:470` — "who is master during playback … the segmentSignature/reschedule glue is app-owned and undocumented; without it the assembled app double-clocks or drifts"; see `IMPLEMENTATION-PLAN.md` S-engine (5), the NS-1..NS-5 queue — 03 had zero NS-1 mentions before this row, consistent with the R27 "plan's job" ruling).

**ACCEPTANCE & TEST PLAN:** §12 (the seed intent list, preserved verbatim; venue status (R33, engine seal-round register P2-3): the §12 clock-law families (item 1) + the 50 ms scrub-latency goal (item 5) have NO engine-venue pin — only the B7/R2-4 Clock.fps re-anchor pin exists in the engine's suite, the W2 property tier does not touch `core/clock.ts`, and the app's K4 LEG 3 is a coarse app-venue clock pin, not the families; the carrier is filed as the GAP row below) + the unnumbered `## Testing` section at file end (Tier 1/2/3 + property — frame accuracy, AV drift, scrub latency, varispeed pitch); spec 17 §13A facet rows (the §3.1 "Playback (frame-accurate)", "Varispeed (pitch preservation)", "Scrubbing latency" matrix rows); battery_r27's posture checks (BASE rows re-verified at the live pins — the regression role; the K2 combined-family census registers the engine-in-the-middle + WDC-integration families in spec 12/17).
...
> A clean-room port of FreeCut's Clock already exists at `nle-engine/src/lib/nle/core/clock.ts` — **BASE-pinned at `e3f55bd`** (code-anchor-stable; 764 LOC, unchanged since `b8c6f88` — the playback core is source-stable across the R24 re-pin; grown past the verbatim port: the R1-B7 Clock.fps re-anchor while playing (pinned in-suite, the B7 pin) + the R2-4 fake-timer RAF test pin (pinned in-suite); **R33 re-attribution (engine seal-round register P1-1): the R1-B8/R2-5 scrub-implies-pause + statechange laws live in `playback/player.ts:793-804`, NOT clock.ts (zero scrub matches in clock.ts) — and they are NOT pinned in-suite** (the block header `timeline-edit-ops.test.ts:115` says B7/B8 while containing only the B7 pin; the missing pins REGISTERED — fake-timer vehicle: beginScrub-while-playing pauses the clock, bumps `_transportGeneration`, emits `statechange({isPlaying:false})` before `scrubchange`; name-collision beware: the browser suite's R2-5 is the timeline addKeyframe guards, a different law)). It remains the plain-`number`-frames port, NOT the `MediaTime` boundary adaptation mandated above (still CORRECTIVE, §13E — the spec's boundary law is spec-side); see `19-code-references.md`.
...
> nle-engine ports all six plan functions and drift constants verbatim (`playback/video-sync.ts`, BASE at `2a0ecf4` — R33 re-key; the file is unchanged since `b8c6f88`, git-verified at the R33 pin; line numbers in §13E re-verified this round), but applies them to `HTMLVideoElement` via a DOM registry — the plan logic is the reference; the DOM application surface is the delta §5.1 corrects (decode-rate actuation, not element playbackRate). See `19-code-references.md`.
...
During transport-rate playback (JKL), every schedule term composes the transport rate `tr` with the element's own retime rate `segRate`: **`varispeedRate = segRate × tr`** (content per wall second); future anchors `(S−P)/tr`; played spans `remaining/tr`; **fades are timeline-authored but the envelope sweeps wall seconds — ÷ tr** (the R3-3 clamp computed in the timeline domain first); the mid-entry content offset is ELEMENT-domain (`offsetSec = trim + into × segRate` — the transport rate moves how fast you traverse, not WHERE in the clip you are); **one reschedule per rate change** (the `lastScheduledTr` idempotence key — the mirror's playRate echo must not re-fire); **`tr ≤ 0` is the honest silent-reverse degradation** (WebAudio cannot play backward; the clock runs backward in silence); the composed magnitude range is **[0.01, 20]** (element clamp [0.01,5] × ladder {1,2,4}) — the old "stays in `[1/32, 32]` by construction" claim was **false at the floor** and is retired (R33, engine seal-round register P3-10): the element clamp's opencut-legal floor sits BELOW the WSOLA floor, so composed rates in the dead zone **[0.01, 0.03125)** fall to the adapter's domain-guard fallback — warn + the pitch-affected legacy path (correct length, pitch-affected vs. author intent); the guard is the actual mechanism at the floor, not a net (`bridge/segment-strip-adapter.ts:537-550`; the [0.01,5] one-home `bridge/opencut-laws.ts:101-120` + the dead-zone SEAM NOTE `:112-113`; consumed via `clampOpencutRetimeRate` at `bridge/scene-to-segments.ts:326/:576/:610`; the constants lattice now also has an OT-side one-home leaf — `core/edit-domains.ts`, D43 @ the `17a19f8` mirror: the retime clamp re-exported verbatim + `WSOLA_RATE {1/32, 32}` — but this row's claim is the ENGINE's composed domain, not the lattice's). Reference implementation: `nle-test-app/src/audioService.ts:498-606` + `docs/design-jkl-audio-follow.md` §1; engine halves: `bridge/scene-mixer.ts:386-389` (element — `:386` the varispeedRate thread, `:387-389` the maintainPitch thread; re-anchored (R33, engine seal-round register P3-9) — the old `:365` cite now sits inside `scheduleSegments`' doc comment) + `audio/realtime-bridge.ts:328` (transport — still exact).

**The pitch half (R9-b — D38.1):** `el.preservePitch` is ElementJSON law (absent≡true — 09 §3.1A B2, D38.1) → the bridge projects `retime.maintainPitch` (scene-to-segments.ts:596/:623) → `seg.maintainPitch` → the adapter's WSOLA pre-retime branch (segment-strip-adapter.ts:526-550; the [1/32,32] guard + the >2ch fallback — tail re-anchored R33, engine seal-round register P3-10's verification); preservePitch=false leaves the legacy pitch-affected path. The SoundTouch VENUE is WDC's W2 (soundtouch.ts + varispeed.ts `retimeChannels`) — see §13E. The op-side companion law is 06 §5.12; the composed rate's pair-law consumers are 06:1670's linked-pair law + D31.7's fit-to-fill audio half.
...
2. **Per-item layer build** — for the frame's active items, build layers for **every item type** (video, composition, image, adjustment, text when it lands). The reference port's historical failure mode is instructive: nle-engine `playback/player.ts` (Round-7 era) dropped all non-video clips (`if (clip.type !== 'video') continue;`) — its own audit called this "the structural bottleneck" behind every per-type gap. **[R23 update: FIXED at the live pins]** — the engine's `scene-assembly.ts` now builds the per-type visual collection (video + image [Wave 4C P1.3] + text [Wave 4D-B glyph-atlas] + composition [the N1 family, nested-composition render depth]) + adjustment layers + stable-DOM tracks; the N1 composition-frame seam closed the structural bottleneck. Our loop keeps the per-type contract (spec 07 §4's type-agnostic `FrameItem[]`) as the law it always was.
...
### 13E. Code References — nle-engine (the historical register, re-verified at `f9ac806`, R27)

> The private **nle-engine** repo (github.com/bearachprema/nle-engine) is a clean-room FreeCut port that has since been ACCEPTED as the BASE playback core (§0: `f9ac806`, 748/748 vitest, tsc 0 — the R27 re-pin; the R22/R23/R24 pins and the Round-7-era "37,958 LOC, 124 tests, in-between reference" framing are superseded, kept here as history). Where engine code conflicts with this spec, **the spec wins** (the standing law). Full reconciliation: `19-code-references.md`. Line numbers below re-verified against the live tree this round — clock.ts 764 LOC; video-sync.ts 1,234 — unchanged; **player.ts 3,695** (the RA round's HW1-4 registry-auto-id/epoch-keyed still cache + HW1-10 grouping loop moved it; the cites below are stable — all 13 re-verified exact). New since the R24 register: bridge/opencut-laws.ts (the OV-01/02 one-home), scene-mixer.ts's M2 Wave-1 bus/sidechain/flush laws, the vendored re-pins OT `55c81c0` + WDC `83b8850` (re-keyed R29 — was `6e2b91a` + `ec8fd5c`).

| Spec section | Engine file:line (re-verified @ `f9ac806`, R27) | Verified quote | Status | Note |
|---|---|---|---|---|
| §3.2 `_now()` audio-clock | `src/lib/nle/core/clock.ts:576` | `? nextTimeSource.currentTime * 1000 : performance.now()` | ALIGNED | Verbatim FreeCut Clock port |
| §3.2 monotonic guard | `src/lib/nle/core/clock.ts:591` | `this._lastNowMs = Math.max(this._lastNowMs ?? normalizedNow, normalizedNow);` | ALIGNED | Single-offset + Math.max |
| §3.3 floor/ceil frame math | `src/lib/nle/core/clock.ts:638` | `return Math.floor(this._playbackStartFrame + framesElapsed);` | ALIGNED | Forward floor / reverse ceil |
| §3.2 timeupdate throttle | `src/lib/nle/core/clock.ts:687` | `if (now - this._lastTimeUpdateEmit >= this.TIME_UPDATE_INTERVAL_MS) {` | ALIGNED | 100ms throttle |
| §3.4 MediaTime adaptation | `src/lib/nle/core/clock.ts:92` | `export class Clock extends EventEmitter<ClockEvents> {` | CORRECTIVE | Plain number frames; spec mandates MediaTime at boundaries (spec-side, unchanged) |
| §3.5 StaticClock | — | COULD-NOT-VERIFY (no StaticClock class — re-verified at `2a0ecf4`: still absent; R33 re-key) | SPEC-ONLY | No render-entry clock |
...
| §6.1 six sync plans | `src/lib/nle/playback/video-sync.ts:926` | `export function planVideoFrameCallbackCorrection(input: {` | ALIGNED | All six plans ported as pure functions |
| §6.3 drift constants | `src/lib/nle/playback/video-sync.ts:181` | `export const LARGE_DRIFT_SECONDS = 0.2;` | ALIGNED | 0.2s / 0.15 cap / ±5%/±15% bands |
| §6.4 reverse coalescing | `src/lib/nle/playback/video-sync.ts:225` | `export const COALESCED_SEEK_EPSILON_SECONDS = 0.001;` | ALIGNED | 1ms epsilon |
| §5.1 mediabunny not `<video>` | `src/lib/nle/playback/video-sync.ts:1041` | `export function registerDomVideoElement(itemId: string, element: HTMLVideoElement): void {` | CORRECTIVE | Plans drive DOM elements; spec swaps actuator to decode-rate |
| §7.2 three-tier scrub | `src/lib/nle/playback/player.ts:280` | `When the user scrubs the timeline (beginScrub → frame seek → endScrub),` | ENGINE-GAP (narrowed) | Wave 3F landed the scrub cache + prewarm + the endScrub delay (single-tier LRU — the constants at player.ts:289-291: SCRUB_CACHE_MAX_SIZE 30 / END_DELAY_MS 1500 / PREWARM_RADIUS 3); Wave 4A's additions are the tracked prewarm promise (`_scrubPrewarmPromise`/`awaitScrubPrewarm`, player.ts:829/:840) + event forwarding; the FreeCut 3-tier GPU/RAM cache remains engine-side unbuilt (re-attributed R33, engine seal-round register P3-11) |
| §10 render loop | `src/lib/nle/playback/scene-assembly.ts:868-877` | `clip.type !== 'video' && clip.type !== 'composition' && clip.type !== 'image' && clip.type !== 'text'` | ALIGNED (was CORRECTIVE — FIXED) | The Round-7 non-video drop is closed: the N1 family + Wave 4C/4D-B build per-type (video/composition/image/text + adjustment + stable-DOM); one video-only `continue` remains in `collectFrameVideoCandidates` (scene-assembly.ts:1894 — correct there, the function's contract is video candidates) |
| §8.2 SoundTouch worklet | `src/lib/nle/audio/soundtouch-processor.worklet.ts:13` | `We implement a simpler granular pitch shifter:` | CORRECTIVE (superseded by W2) | The engine's own worklet stays the granular realtime insert path; the FULL WSOLA SoundTouch port the spec's §8.2 adoption called for LANDED in web-daw-core (W2 @ `de09c93`: `soundtouch.ts` + `varispeed.ts` `retimeChannels` + the worklet) — consumed by the engine bridge and the app's `maintainPitch` path (§0 BASE) |
