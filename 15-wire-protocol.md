...
**Status:** v-next (Round 23 fleet re-baseline — the wire is the S-round+T-round+S3-seam shape @ OT `222532c` (the **30-name** prefixed wire census) + the app consumer @ `70e99f0` + the engine service-slice home @ `b8c6f88`; the gap is C7/error-envelope/union-façade → r1, staircase → w2); NEW (TEST-02). Defines the runtime-operations layer that makes the engine fully data-driven.
...
- opencut-timeline (OT) @ `222532c` — **489/489** (359 in-page + 130 real-mouse across the 51 milestone suites, 14 of them real-mouse; the runner's report json is the count authority), tsc 0. The wire surface (`src/lib/timeline/headless/api.ts`): **the prefixed-name census is now 30** (exactly 30 `applyInner` case arms — 24 at the R15 charter; the family grew with the S2 `track.toggleLock` + the S1 transport trio `timeline.setPlaybackRate`/`timeline.setLoopRegion`/`timeline.advancePlayhead` + the D-T3 `track.setAllLocked` (M44) + the S3-seam `track.setAllMuted`; the R22-era "24/78" claim was already stale at its own pin — 28 actual @ `05584d8`). **S3 LANDED** (M46 @ `a4e971d`: `setTracksMuted` the mute-all batch op — the `setTracksLocked` D-T3 twin, ONE history entry, every-track-at-target NOOP predicate both directions — + the `track.setAllMuted` wire verb with the A9 `ok:true` + `data:{changed, muted}` echo, never a failure code for the idempotent set). **The F1 hardening LANDED** (F1B-4 malformed/null/array params → `INVALID_PARAMS` — 16/16 commands had been reporting `INTERNAL_ERROR`; F1B-2 update-patch no-overlap → `CONFLICT` (insert, move, AND update now enforced on the wire); F1B-5 `setLoopRegion` `clear:false` never clears; F1B-6 negative trims/starts rejected; F1B-7 failed `applyBatch` preserves the pre-batch redo stack; F1C-4 the strict wire-echo assert; the S2 lock pre-checks on 10 commands emitting `TRACK_LOCKED`). M45/M46 pins. 27 of the 78 union members now have live OT wire counterparts (the R15 24 + `toggleLock`/`setPlaybackRate`/`setLoopRegion`); 3 OT-side verbs have no union member yet (`advancePlayhead`/`setAllLocked`/`setAllMuted` — the C7-fold disposition rows in §13.15).
- nle-test-app @ `70e99f0` — **117/117**, tsc 0. The consumer side: the vendored OT core (mirror @ `a4e971d` via `vendor/nle-timeline`/UPSTREAM.lock, the S3 seam pin) + the **D28 edit-command dispatch** (`engineService.ts` — store action → core-method dispatch → the GluedShell mirror store) + the Wave-B S-round seam consumption (S1-S4 + D-S5) + W3 JKL; consumes `core.setTracksMuted`/`setTracksLocked` directly (the batch verbs are op-level consumers, pre-C7).
- nle-engine @ `b8c6f88` — **440/440** vitest, tsc 0. The service-slice home: the 19-op JSON-RPC dispatch (`src/lib/nle/headless/api.ts` — INTERNAL transport per Decision 12.2; the union façade is the r1 GAP row below) + the engine's own frozen-surface discipline (the 453-name `api-surface.frozen.ts` + `api-surface.test.ts` — the conscious-change gate the façade must flow through) + N2b keyframed volume LANDED @ `37cdd28`.
...
- Spec-internal registers: §4.1A (the routing-disposition table, 78 members) + §9.5 (the event staircase register) + §13.15 (the C7 worklist, R23-refreshed to 30 names) — pointed below, not duplicated.
...
- **C7 rename at r1 END** (was W-ops at END): the **30** prefixed wire names (§13.15's R23 census) → the bare spec-15 union + the one-business-day app migration + the nle-ui keymap-sync sub-gate (acceptance: §13.15 rows flip ALIGNED + the app migrated within one business day + keymap synced).
- **Error-envelope §6.3 refinement — r1 FIRST** (the plan's r1 row: the refinement lands before the C7 rename; was W-ops; acceptance: the ~24-code table + OT follows). *Annotated R23: the classification half is partially covered by the OT BASE — `TRACK_LOCKED` live exact-match (S2), `NOOP` live (R8), F1B-4's `INVALID_PARAMS` boundary law, F1B-2's `CONFLICT` on span patches; the coarseness itself (`NOT_FOUND`/`CONFLICT`/`INVALID_PARAMS` undifferentiated vs the ~24 fine-grained spec codes) remains the r1 refinement.*
- **Union façade — r1** (was W-ops; the retired spec-14 §4.2 row re-homed here): the engine's 78-union service slice — the JSON-RPC surface stays INTERNAL transport per D12.2; the façade owes the service subset through the engine's frozen-surface gate (acceptance: the ENGINE-home §4.1A rows dispatch through the façade + the `api-surface.frozen.ts` re-freeze lands consciously).
- **Element-level `toggleElementMuted`/`toggleElementVisibility` wire verbs — r1** (was the W-ops/A2-wave-1 wire addition; the retired spec-14 §4.1 row re-homed here — spec 06 §0 carries the ops-side half; the engine ops exist OT-side, routed today via `updateElements` patches): the dedicated wire pair is the r1 addition (acceptance: the two verbs on the wire + §4.1A's two Timeline rows flip).
- Event staircase at full scope — **w2** (was W-media; acceptance: every §9.5 row published + consumed + pinned — the staircase suite).
- Routing-disposition verification — **r1** (was W-ops; acceptance: every implemented §4.1A row cites a module pin; every DEFERRED row dispatches typed `NOT_IMPLEMENTED` — S1 dispatch-complete + honest).

**ACCEPTANCE & TEST PLAN:** §12 (Test Harness Usage) is this spec's battery; BASE acceptance = the cited suites at the cited pins (OT 489 — 359 in-page + 130 real-mouse; app 117; engine 440) — the regression role (K1). GAP acceptance is per-row above; facet rows in spec 17 §13A.
...
**What this is:** Decision 16's completeness instrument. Every union member is assigned ONE home: **OT** (opencut-timeline headless API — the editing SSOT), **ENGINE** (nle-engine union façade — the service slice), **APP** (nle-app-level semantics), or **DEFERRED** (typed `NOT_IMPLEMENTED` — see §4.1B). The app bus's exhaustive-switch dispatch compiles against THIS table; DEFERRED members return `{ok:false, code:'NOT_IMPLEMENTED'}` honestly rather than silently missing. The battery (spec 17) verifies: implemented rows cite a module pin SHA; DEFERRED rows cite a phase or a user-signed deferral. **Baseline (Round 15):** OT @`0412e41` implements 24 of 78 (with prefixed names pending C7 + form deltas in §13.15); the engine's JSON-RPC surface (19 edit ops) is INTERNAL transport (Decision 12.2-amended) and does not count toward this table. **R23 live reading (@ OT `222532c`):** the prefixed census is now **30** (§13.15's refreshed worklist) — 27 of the 78 union members have live OT wire counterparts (the R15 24 + `toggleTrackLock`←`track.toggleLock` S2 + `setRate`←`timeline.setPlaybackRate` + `setLoop`←`timeline.setLoopRegion` S1); 3 OT-side verbs await the C7-fold disposition (`advancePlayhead`/`setAllLocked`/`setAllMuted` — no union member). The phase column below is dual-tagged: the R23 tag per the D24 ladder (crawl K1-K4 / walk w1-w3 / run r1-r6) with the R15/R22-era tag it supersedes. **Member census (R15 fix):** the rows below sum to exactly 78 of 78 union members — Timeline 17 + Track 7 + Playback 5 + Project 7 + Scene 4 + Bookmark 4 + Media 2 + Selection 4 + Marker 3 + Effect 5 + Mask 4 + Transition 3 + Keyframe 4 + Clipboard 3 + Undo/redo 2 + Snapshot 1 + Export 3 = **78**.

| Category | Members (78) | Home | Status @ R15 → phase (R23 dual-tag) |
|---|---|---|---|
| **Timeline** | `insert`, `move`, `trim`, `split`, `delete`, `duplicate`, `updateElements` | OT | implemented (C7 rename pending) → **r1** (was A2) |
| | `ripple` | OT | implemented as the `rippleDelete` wrapper (spec keeps `delete{ripple:true}` canonical — §13.15) |
| | `roll`, `slip`, `slide`, `rateStretch` | OT (op-port) | NOT in OT (engine algorithms at timeline.ts:2984/4143/4246/3155) → **r1 wave 1** (was A2 wave 1) |
| | `retime`, `freezeFrame`, `rangeRemoval` | OT (op-port) | → **r1 wave 2** (was A2.5) |
| | `toggleElementVisibility`, `toggleElementMuted` | OT (wire addition) | engine-layer ops exist (timeline-core.ts:1341/:1385 — R23 re-pinned, W8-d/M40-pinned), not on the wire → **r1** (was A2 wave 1; the §0 GAP row) |
| **Track** | `toggleTrackMute`, `toggleTrackVisibility`, `addTrack`, `deleteTrack` | OT | implemented (C7 rename pending) |
| | `reorderTrack` | OT | → **r1** (was A2/A2.5) |
| | `toggleTrackSolo`, `toggleTrackLock` | OT | → **r1** (was A2 wave 1; `toggleTrackLock`'s OT counterpart `track.toggleLock` LANDED S2 — only the C7 fold remains; solo semantics per spec 20 §4.2 S/G placement — see 09's A4 amendment) |
| **Playback** | `play`, `pause`, `seek` | OT | implemented |
| | `setRate`, `setLoop` | APP (transport slice) | **the OT-side transport verbs LANDED S1** (`timeline.setPlaybackRate`/`timeline.setLoopRegion` — the spec forms ride the C7 fold at **r1**; was A3; the §13.15 N5 loop invariant is live OT-side) |
| **Selection** | `selectElements` | OT | implemented |
| | `selectTool`, `selectTrack`, `marqueeSelect` | APP (controller layer) | OT controllers implement the semantics (view-layer, not wire) → **K3** (was A3; app-behavior nets); marquee form decision at **K3 entry** |
| **Project** | `create/load/save/close/updateSettings/rename/delete` (7) | APP | project slice is app-level (Decision 16 law 4) → **w3** (was A5; save/load with ProjectJSON persistence) |
| **Scene** | `createScene/deleteScene/renameScene/switchScene` (4) | APP | app-level multi-scene → **w3** (was A5) |
| **Bookmark** | `toggleBookmark`, `removeBookmark`, `updateBookmark`, `moveBookmark` (4) | OT | renamed into the unified marker family per the A2 amendment (09 A2 + §13.15's C7 fold: `addMarker/updateMarker/deleteMarker` semantics — toggle≈add/delete, move≈update position); the union block retires at the next union-version bump → **r1** (was A2) |
| **Media** | `importMedia`, `deleteMedia` | APP | media registry + probe (engine `probeMedia` exists headless-side) → **w2** (was A3; W-media); real decode per D6 posture |
| **Marker** | `addMarker`, `updateMarker`, `deleteMarker` (3) | OT | bookmark family (toggle/remove/move) exists; the A2-amendment unifies Marker/Bookmark into ONE per-scene family → **r1** (was A2) |
| **Effect** | `addEffect` + 4 more (5) | ENGINE | engine effects registry exists (44 GPU effects); wire = engine façade + OT model extension (SceneTracks effect shapes — spec 06/07 gap work) → **DEFERRED pending the model extension** (was "past A3") |
| **Mask** | 4 | ENGINE | as effects → **DEFERRED pending the model extension** (was "past A3") |
| **Transition** | `addTransition`, `updateTransition`, `removeTransition` (3) | ENGINE (engine has the registry; OT models transitions element-hung — projector translation A1) | wire → **DEFERRED pending the model extension** (was "past A3") |
| **Keyframe** | `upsertKeyframes`, `removeKeyframes`, `retimeKeyframe`, `updateKeyframeCurves` (4; upsert/remove are plural batch forms, `retimeKeyframe` is singular per the union) | OT | per-key singular forms implemented for upsert/remove/retime (form delta in §13.15); `updateKeyframeCurves` has no OT form yet → **r1** (was A2 wave 1-2); batch engine-ops exist (timeline-core.ts:1567/:1662/:1902 — R23 re-pinned) |
| **Clipboard** | `copy`, `cut`, `paste` (3) | APP | greenfield (mock registers C19 → the K3 re-expression) → **r1 tail** (was A7a; the wire verbs land with the r1 dispatch-completeness pass or re-scope at r1 entry) |
| **Undo/redo** | `undo`, `redo` | OT | implemented |
| **Snapshot** | `snapshot` | APP (test seam) | readouts exist (OT out-of-band readouts; engine headless) → **K3** (was A2/A3; the app test seam) |
| **Export** | `exportFCPXML`, `exportMaster`, `exportFrame` (3) | ENGINE | engine export orchestrator exists (mediabunny A/V); FCPXML is **r5** greenfield (was A6) |

**Reading of the table at R15:** implemented = 24 (all OT) + the engine service surface exists but the union façade is A2 work. DEFERRED families (effects/masks/transitions/clipboard ≈ 15 members) are honestly typed `NOT_IMPLEMENTED` until their phase lands — the union's *contract* (§4.3 schemas) remains normative for all 78; only *dispatch* is phased. This is the difference between the spec being aspirational (union as wish-list) and honest (union as typed contract + phased dispatch map). **R23 reading (@ `222532c`):** implemented-with-counterpart = 27 of 78 (the growth: S2 `track.toggleLock`, S1 `setPlaybackRate`/`setLoopRegion`); the union façade is the **r1** GAP row (§0); the DEFERRED families' deferral basis is unchanged (the model extension gates them, not a phase date).
...
nle-engine (github.com/bearachprema/nle-engine, 37,958 LOC) is a clean-room FreeCut-port engine kept as an in-between de-risking reference. It is NOT conformant to this spec; it inherits FreeCut patterns this spec corrects. When engine and spec conflict, the spec wins. The valuable content is the delta, mapped below (citations verified Round 7; **R23 note — the engine pin moved from `f68ab8c` (the R22-era pin, history) to `b8c6f88` (the R23 pin) and the file grew: the 19-case dispatch is now at `headless/api.ts:773-785` (`applyOp` at :785, still exactly 19 case arms — re-verified R23), the line refs below are R7-era and drift accordingly; the engine's own 453-name api-surface freeze (`tests/vitest/engine/api-surface.frozen.ts` + `api-surface.test.ts`) is the discipline the r1 union façade must flow through**):
