...
**Status:** v-next (Round 24 — the R24 re-baseline: the scout protocol is the operating practice, executing per round — the R24 execution wave is its newest form: the module-card wave (5 read-only agents, one per REPO — the ground-truth cards feeding the ARCH-R24 rulings BEFORE the fleet) → the per-file audit fleet (20 agents, one per spec file — the R23 pattern, precedent, executing again) → the track/plan audit (the plan doc audited as its own artifact — the cold-executor re-simulation), the round closed by the adversarial review fold (×17 findings → ARCH-R24 v2); future charters are the PLAN's phase-entry decisions (`IMPLEMENTATION-PLAN.md`, per D23 — spec 14 is retired to the redirect stub, its phase ladders re-homed per the D24 mapping). Round 23 lineage: the R23 re-baseline — the per-file fleet's first execution (20 agents); Round 22 lineage: the §0 forward inventory + the R22 re-baseline — the same operating-practice ruling, at the R22 pins); Plan for dispatching sub-agents to refine the spec with code references
**Spec file:** `13-subagent-scout-plan.md`

---

## 0. FORWARD INVENTORY (the R22 posture law, D20 — what needs to be done; the BASE is accepted, not re-explained; re-baselined at R24 per the D24 ladder — the dual-vocabulary window closed at this fleet, D24 tags only)

**BASE (accepted, re-pinned 2026-09-08 @ the R24 fleet):**
- The protocol is EXECUTING — the §3 workflow (scout → audit → integration review) + §7 worklog IS the operating practice for per-round research agents: R15's four scouts (the A/B/N amendment set — landed as the "(Round 15 amendment)" markers across the specs), R22's two adversarial reviewers, the mockup stream's research contracts, and since the R22 re-baseline: **R23's two adversarial review rounds** (ARCH-R23 — the plan/bridge rulings review ×10 findings + the plan-doc executability review ×15, both GO-WITH-AMENDMENTS, both folded) and the R23 per-file audit fleet (the user's "one sub-agent per file… so they have focus" directive — precedent at R24, its protocol rows re-verified this round, no churn): 20 agents — 00-master + the 19 numbered specs (01-20 minus the retired 14) — each with repo-level focus, editing ONLY its own file, NO git (the orchestrator commits per batch), findings to `audits/fleet-r23/NN.report.md`; the battery re-baselines the pin world once, centrally, after the fleet (`IMPLEMENTATION-PLAN.md` §8 is the charter). **The R24 fleet — the newest pattern — adds two waves: (a) the MODULE-CARD wave — 5 read-only agents, one per REPO (nle-engine · opencut-timeline · web-daw-core · nle-ui · nle-test-app), each producing the ground-truth card (verified counts + the landing list since the prior pin + the seam surfaces + the queues/DECISIONS state + the census + the spec-staleness register) that feeds the rulings BEFORE the per-file fleet (`audits/fleet-r24/module-*.report.md`); (b) the TRACK/PLAN audit — the plan doc audited as its own artifact (`IMPLEMENTATION-PLAN.md`: the cold-executor re-simulation — every track row's first action executable TODAY at the current pins, the S-app/S-ot row re-writes, the estimate re-base), the wave that follows the per-file fleet.** The review-fold cycle continues per round (R24's count: ARCH-R24's adversarial round — ×17 findings, GO-WITH-AMENDMENTS, all folded → v2, after R23's two rounds ×10 + ×15) — THIS audit round is that protocol executing.
- The protocol's methodology carries the seal artifacts' rules (CORE-SEAMS / LAW-NET-INVENTORY, the R23 seal + the R24 mapping notes — the census → 24 routed + 6 exceptions (the R24 record; re-declared **31 = 28+3 @ `970948a`** at R27 — D-ARCH-6); the D25-bridge citations → D26's census discipline): the **evidence-citation rule** (an upstream-holds claim requires a REGISTERED evidence row — never a bare assertion), the **census counting rule** (declared == actual; the counts sum auditable against the live runner), and the **split rule** (a mixed-verdict unit splits into per-disposition rows — no unit carries two); battery_r23 and battery_r24 enforced the same spine (scrape-don't-substring — the live suite is scraped, never text-matched; count-discipline; battery_r24 added the census-coherence + routed/exceptions checks per D29; battery_r27 succeeds them — the live-registry + origin-attribution classes added).

**The standing process laws the R25-R27 fleets proved (absorbed at R27 from the SKILL learnings + the ARCH rulings — permanent law, cited by every future charter):** (1) **the briefing-hypothesis law** — a fleet briefing is a hypothesis; every agent verifies it against code, the code wins, discrepancies flagged (SKILL #107; the R27 BRIEFING.md embodies it). (2) **the count-authority law** — the artifact's own runner output is the count authority; scrape the report json, never the prose (SKILL #115; OT's `timeline-test-report.json`). (3) **the multi-pass wave law** — one agent per FEATURE for reference-material audits, the orchestrator reads every artifact BEFORE dispatching, multi-pass = verification + depth (SKILL #130). (4) **the integration-round law** — per-file agents cannot see cross-file contradictions; the integration/adversarial round is budgeted, not optional (SKILL #114). (5) **the cross-repo filing protocol** — a spec session FILES titled queue blocks, it never ORDERS another repo's code; FILE → EXECUTE → adversarial review → fold → re-pin, and a filing is a hypothesis the execution session tests first (SKILL #135/#136/#138). (6) **the live-scrape battery discipline** — declared == actual, scrape-don't-substring, and every check must distinguish 'instrument broken' (FAIL) from 'instrument absent' (visible skip) (SKILL #124).
- Its downstream product at the R24 pins (the specs this process produced, executing; 2026-09-08, the R24 fleet re-pin — 00-master v9.0's fleet table is the pin canon, D21: HEAD + consumer pins both): nle-engine @ `f9ac806` 748/748 (docs-only head over code anchor `50b91f5`; the E1/E2 fixes + the RC round — D12 ratification, OV-01/02/05/12, M2 Wave 1; the vendor re-pins — OT `6e2b91a`, WDC `ec8fd5c`), OT @ HEAD `55c81c0` 632/632 (**code pin `970948a`** — the trailing commits docs/runner-artifacts only, src diff empty; 72 milestone entries, the total only — the report json committed at the code pin is the count authority; the W11 complete-UI round grown: **31 wire verbs = 28 routed + 3 exceptions** (D-ARCH-6), the M49C coverage gate 28/3, op-parity 11/11), WDC @ `ec8fd5c` 777/777 (the S-series upstream wave — diagnostics/heartbeat, the rebuilt worklet, the rt-safety-gate fix), nle-ui @ `32abd58` 690 (R9-b preservePitch, F4 identity guards, FW-D setElementFieldAll, AW1-2 the Gain ceiling [−60,+20]), nle-test-app @ `c020b2a` 252/252 (the census 42 = 36 zero-action (6 byte + 30 mechanical) + 5 carriers + 1 host @ `6e2b91a`, register-equality CI-enforced; the R8/R9/R25-R27 waves — the 8-suite roof incl. wire-coverage + waveformPeaks; the adopt-wholesale convergence — 39 mirrors (32 zero-action + 7 carriers) + the host), ui-mock/shell-mini 495 in-flight (the R24-2 W6/W7 pending; the register re-keys at the WRAP) (the R23 seal 355 + the R24-mini nets: clip-box + mini-plus W0/W1/W2/W3 + fixes) + shell-variants 1,521+ (the live count at the fleet's window) (nle-core-spec). R23 lineage: engine `b8c6f88` 440/440, OT `222532c` 489/489, WDC `494f6ff` 759/759, nle-ui `85dcf57` 648, app `70e99f0` 117/117, mini 355 sealed + variants 1,521 — the fleet moved under every R23 pin. R22 lineage: engine `f68ab8c` 356, OT `05584d8` 459, WDC `fe05d85` 740, nle-ui `dba8d52` 640, app `e662759` 83, variants 1,334 — the fleet moved under every R22 pin.
>>>>>>> origin/main
...
- No standing gap — the no-gap ruling re-verified at the R23 and R24 fleet audits and it still holds: the protocol continues per round; the next chartered scouts (if any) are PHASE-ENTRY decisions, not standing charters: the FCPXML parser+corpus choice at **r5** entry (the interchange phase-entry artifact); the real-file decode corpus at user re-affirmation of D6 → **r4** (user-gated) (owner: the orchestrator's charter, per the plan's phase-entry gates — `IMPLEMENTATION-PLAN.md` §2) (acceptance: the phase-entry artifact recorded in the plan BEFORE the phase's work starts). The fleet protocol itself is NOT a gap row — it is executing (this audit is one of the R24 fleet's 20 agents; its only residue is the battery's central post-fleet re-baseline — **battery_r27** (the lineage: r15→r22→r23→r24→r25 123→r26 141→r27; `scripts/battery_r2N.py` the succession), S-spec's row).

**ACCEPTANCE & TEST PLAN:** no Testing section of its own — the test is process-shaped: every chartered round leaves its §7 worklog entry + audit trail in this repo BEFORE the phase's code work starts (the fleets' equivalent trail: the per-file findings reports in `audits/fleet-r23/` + `audits/fleet-r24/`, the module cards + the plan-audit report included); spec 17 §13A consumes the scouts' output via each spec's Testing section, not directly; battery posture (**battery_r27**, the successor): the pins above stay green (the specs this protocol produced keep executing), the live suites SCRAPED not substring-matched, declared == actual (count-discipline) — the dual-vocabulary window CLOSED at this fleet's end: the D24 set only (retired spec-14 terms survive solely in marked historical records).
