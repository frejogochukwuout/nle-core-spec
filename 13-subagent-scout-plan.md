...
- The protocol's methodology carries the seal artifacts' rules (CORE-SEAMS / LAW-NET-INVENTORY, the R23 seal + the R24 mapping notes — the census → 24 routed + 6 exceptions; the D25-bridge citations → D26's census discipline): the **evidence-citation rule** (an upstream-holds claim requires a REGISTERED evidence row — never a bare assertion), the **census counting rule** (declared == actual; the counts sum auditable against the live runner), and the **split rule** (a mixed-verdict unit splits into per-disposition rows — no unit carries two); battery_r23 and battery_r24 enforce the same spine (scrape-don't-substring — the live suite is scraped, never text-matched; count-discipline; battery_r24 adds the census-coherence + routed/exceptions checks per D29).
- Its downstream product at the R24 pins (the specs this process produced, executing; 2026-09-08, the R24 fleet re-pin — 00-master v9.0's fleet table is the pin canon, D21: HEAD + consumer pins both): nle-engine @ `5036387` 458/458 (the W2.5 effects-sidecar honest subset + the R8-REV blur-radius clamp; the fleet-coherence vendor re-pins — OT `c15a629`, WDC `494f6ff` docs-only-behind), OT @ `ded43c4` 536/536 (**code pin `c15a629`** — the last three commits are docs/runner-artifacts only, src diff empty; 386 in-page + 150 real-mouse per the fresh live-run report — the count authority; the W11 complete-UI round: 24 routed wire verbs + 6 documented exceptions, the M49C coverage gate, op-parity 11/11), WDC @ `85b81b0` 759/759 (docs-only over `494f6ff`; the seal-round wrap), nle-ui @ `fc4cc35` 674 (the D29 seams: `saveState`, DeliverPage `exportRequest`, the per-scene grade sidecar), nle-test-app @ `c885ece` 174/174 (the R8 real-wiring waves + R9/D30 mid-flight W-C..W-G; the adopt-wholesale convergence — 39 mirrors (32 zero-action + 7 carriers) + the host), ui-mock/shell-mini 396 (the R23 seal 355 + the R24-mini nets: clip-box + mini-plus W0/W1) + shell-variants 1,521+ (the live count at the fleet's window) (nle-core-spec). R23 lineage: engine `b8c6f88` 440/440, OT `222532c` 489/489, WDC `494f6ff` 759/759, nle-ui `85dcf57` 648, app `70e99f0` 117/117, mini 355 sealed + variants 1,521 — the fleet moved under every R23 pin. R22 lineage: engine `f68ab8c` 356, OT `05584d8` 459, WDC `fe05d85` 740, nle-ui `dba8d52` 640, app `e662759` 83, variants 1,334 — the fleet moved under every R22 pin.
