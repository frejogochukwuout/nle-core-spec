...
- The fleet's suites at the **R24 pins** (the K1 module nets — ALL GREEN; 00-master's fleet table is the pin canon, D21; the battery re-baselines the pin world once, centrally; consumer pins re-read live): **nle-engine** 458/458 @ `5036387` — 13 vitest suites (`tests/vitest/engine/`: timeline-edit-ops, video-sync, bridge-seams, planner, api-surface, load-validation, persistence, render-abort, timeline-math, transform-resolver, undo-serialize; `tests/vitest/`: nle-bridge, nle-bridge-realtime-midi; the vendored OT subset stays CORE-ONLY via the tsconfig/vitest alias; +18 since R23 — the W2.5 effects-sidecar pins + the R8-REV blur-clamp pin) — the app consumes @ `5036387` (re-pinned, fleet-coherent); **opencut-timeline** 536/536 @ `ded43c4` (code pin `c15a629`) — 386 in-page + 150 real-mouse across 59 milestone entries (M1-M49 + M49C, the W11 extensions M44-M49 + the M49T/H/R/G real-mouse suites; `download/timeline-test-report.json` the count authority; the runner `scripts/run-timeline-tests.mjs` = the in-page `window.__TIMELINE_TEST_REPORT__` scrape on `/` + 18 appended real-mouse entries on `/view` — the in-page UI-over-core runner with pixel reads, WYSIWYG state, `window.__VIEW_TEST__`); the app consumes the vendored lock-copy @ `c15a629` (byte-exact minus `testing/`; the engine's OT submodule also `c15a629`); **web-daw-core** 759/759 @ `85b81b0` (the one R24 commit is docs-only; 759 held, live-run verified) — 18 `src/test/` suites; the integration family = audio-integration, dsp-bounce-parity, dsp-effects-integration, dsp-null-unity, dsp-pdc, dsp-sidechain, nle-audio-core-derisk, offline-parity-wiring, real-audio-e2e, meter-tap, varispeed, fft-analyzer (+ the Goertzel detection oracle `lib/daw/dsp/metrics.ts` and the offline render — "the combined test the audio core has done"); the engine consumes @ `494f6ff` (docs-only behind HEAD); **nle-ui** 674 @ `fc4cc35` — 37 test files (consumed @ `fc4cc35`, the app at the package HEAD; +26 = the D29 saveState/exportRequest/grade-sidecar seams + R8-REV); **nle-test-app** 174/174 @ `c885ece` — 6 suites: engineSeam / sceneBridge / GluedShell / audioService + persistenceService / deliverService (the R8 D29 real-wiring wave — autosave, ducking, deliver export, effects/color preview — + the R9 W-A/W-B fleet-coherence waves); in-repo: **shell-mini 396** (10 test files / 141 census units — the R23 seal 355 + the R24-mini nets: clip-box + mini-plus W0/W1) + **shell-variants 1521+** (61 test files; the R23 W-A FX + W-B color waves; 00-master's 1,470 is its own pin-era figure — the battery's central re-baseline reconciles).
- The R23 seal artifacts + the VLM visual net (the K3/K4 tooling base): `ui-mock/shell-mini/docs/CORE-SEAMS.md` (the S1-S26 seam inventory + the store partition + the transport map — K3's re-expression worklist) + `LAW-NET-INVENTORY.md` (the 141-unit / 10-file / 396-test census: 33 HOLDS-on-OT / 363 app-side authoring / 134 GAP units + the 59-app-emitted-static + 15-templated testid census — K3's acceptance list) + `ui-mock/shell-variants/scripts/vlm-capture.mjs` / `vlm-review.mjs` / `vlm-run.sh` (the R23-G VLM visual-verification net — K3 adopts, see the GAP register).
...
- **K1 — module nets:** each repo's own suite at its own gates (the rows above; this spec's §17 CI configs + spec 17's three-tier methodology are the per-repo shape). ALL GREEN at the R24 pins — the OT K1 net grew 489→536 (the W11 milestone extensions: M44-M49 + the M49T/H/R/G real-mouse suites + the M49C coverage gate), engine 440→458, nle-ui 648→674, app 117→174.
- **K2 — combined nets (cross-module):** the seeds exist at HEAD — engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`: vendored OT ops × engine composition, within the 458 net), WDC's integration family (offline render + Goertzel detection, within the 759 net), OT's in-page UI-over-core runner (pixel reads + real-mouse + WYSIWYG state, the 386+150 net). **The D29 instruments (REGISTERED): OT's M49C coverage gate — the machine-checked complete-UI claim, accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions, node-side accumulation across the phases, tsc-lockstep both directions — is K2's OT-side completeness instrument (already green, 24/24); the wire census 24 routed verbs + 6 documented exceptions (`WIRE_COMMAND_TYPES` @ OT `api.ts` + the runner's exception registry) is the census the K2/K3 families key on.** The GAP register's census row formalizes the NAMED family.
- **K3 — app-behavior nets:** the app wiring driven headlessly over REAL modules — the LAW-NET corpus re-expression, the DOM-structural gate (three-way per ARCH-R24 D26.5: OT's frozen `data-test=` 68-site vocabulary / nle-ui's `shell-*` testids / the no-counterpart structural units — gated on the mini-unit mapping row + D30's W-C, not on a swap+testid layer), the offline audio pins (the Node-venue law), the VLM visual net (adopted from the variants). **The D30 W-F coverage-gate port is the REGISTERED K3 instrument (D29): the module-scope accumulator + the gesture sims + the final assertion (accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions) — the routed-verb completeness machine-check; the shell-path surface is excepted per D30 R2/R8 (the known blind spot, registered verbatim: the gate measures the PORT surface), with a compensating pin family driving the trusted-integrator verbs (⌘B, [ ]/⌥[ ], ,/., M/⇧M, ⌘M/⌘⇧M) directly through engineService — owner-tagged app-side (D29.1, review F14).**
- **K4 — the crawl exit gate:** the single automated e2e — import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export; zero mock paths, zero human input; the e2e drives the same routed wire verbs through the real UI (the shell-path exception applies identically — D29.1c).
