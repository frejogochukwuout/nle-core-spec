...
**Status:** v-next (Round 28 — the fleet re-pin + the TEST-LAW FOLD (S-spec(6)): engine **`074a2f6` 749/749** (25 test files; bridge-seams 151; code anchor **`74bef08`**) / OT HEAD **`55c81c0`** (code pin **`970948a`**) **632/632** / WDC **`83b8850` 777/777** (docs-only over `ec8fd5c`) / nle-ui **`32abd58` 690/690** / app **`85cff80` 252/252** (8-suite roof) / mini **495/12** (the register pin, sibling-maintained live-current) / variants **1,939 register-declared / 68 files** (the R28 WRAP re-key — the declared pair **1,950 runner-count / 126 stories** recorded, the collector-unit reconciliation; never a bare live count); the wire census **31 verbs = 28 routed + 3 exceptions** stands (with the ruled r1 projection **78→79→80** — 15:5037/:5039); **battery_r28 re-baselines the pin world once, centrally** (the R28 check classes at §0's ACCEPTANCE & TEST PLAN — incl. the §14.4 step-0 facet-completeness class and the suite-census coherence class); **the R28 test-law fold lands WITH battery_r28, never after it** — this spec's D42-D50 amendments: §0's BASE/GAP/K-tier re-key + the fold's GAP row, §5.5's D45 grade-seam parity rows, §6.4's re-key + §6.5's WDC-side retime laws, §8's D42 linkage invariants (I6-I8) + the D46 family invariants, §18/`## Testing`'s re-key — **the design law and the test law moved in the same round** (the W4 corpus fold `980920b` amended 14 law-side files and ZERO test-side files — the one-round lag `audits/fleet-r28/test-audit-coverage.md` §1.1 convicted by 17 §14.4 step-0's own rule; the fold closes it, the design per the T2 trio `test-law-d42-d44.md` / `test-law-d45-d47.md` / `test-law-d48-d50.md`). Round 27 — the fleet re-pin + the instrument re-key: engine **`f9ac806` 748/748** (25 test files; bridge-seams 151) / OT HEAD **`55c81c0`** (code pin **`970948a`**) **632/632** / WDC **`ec8fd5c` 777/777** / nle-ui **`32abd58` 690/690** / app **`c020b2a` 252/252** (8-suite roof) / mini 441 (R24-2 in flight) / variants 1,773 registered (64 files; live higher — re-keys at the sibling's WRAP); the wire census is now **31 verbs = 28 routed + 3 exceptions** (D-ARCH-6 `970948a`; the exported registry consumed LIVE by both gates); battery_r26 141 checks (135/141 at the R27 pins — pure pin-lag) → battery_r27 this round; the K2/K3 instruments re-keyed (M49C 28/3, origin-attributed; the W-F port LANDED R25 with the AR-2 live-registry consumption + gesture pin). Round 24 — the fleet re-pin + the D29 instrument registration: the K1-K4 net-layers are this spec's crawl verification tiers (the spec-side home of the ladder's testing vocabulary — §0's tier map below); **the K2 combined-family census + registration lands HERE (and spec 17) at K2 entry** — per `audits/ARCH-R24-timeline-strategy-and-topology.md` D29, OT's M49C coverage gate is the REGISTERED K2 OT-side completeness instrument and the app's D30 W-F coverage-gate port is the REGISTERED K3 instrument (the R24 census — 24 routed + 6 exceptions — is superseded by the 28+3 re-declaration above); the dual phase-tag window CLOSED at this fleet — the D24 K/w/r set only (the transitional "was C/W/R" tags stripped). Round 23 — the fleet re-pin + the D24 verification ladder: every phase tag re-keyed C/W/R → K/w/r per `audits/ARCH-R23-plan-and-bridge.md` D24; spec 14 retired, its testing-domain §4 rows re-homed to this §0 GAP register, the plan is `IMPLEMENTATION-PLAN.md`; the R23 pin set (engine 440 / OT 489 / nle-ui 648 / app 117) is superseded by the R24 set. Round 22 — the §0 forward inventory + the R22 re-baseline: the methodology is landed and executing via spec 17 v1.3; the R22 fleet pins were the baseline then, the law-net corpus was the next gap — that corpus LANDED at the R23 seal); Refined (SCOUT-12) — open questions resolved (§12), corrections verified (§14), CI/runner configs + asset scripts audited (audits/12-testing-strategy.audit.md — PASS-WITH-CAVEAT, issues applied). Round-8 note: the umbrella methodology (tier definitions, facet coverage matrix, NFR verification recipes, error-path census, a11y spot suite) lives in `17-test-plan.md` v1.1 §13A — this spec owns the INFRASTRUCTURE layer (virtual framebuffer, pixel/audio verification mechanics, CI configs) that 17's recipes execute on; NFR measurements per 00-master §6A run through the jobs defined here.
...
## 0. FORWARD INVENTORY (the R22 posture, R24-re-based — what needs to be done; the BASE is accepted, not re-explained)

**BASE (accepted, pinned 2026-09-08 — the R24 re-pin; re-keyed 2026-09-16 at the R28 pins; the R23/R27 pins superseded):**
- The methodology landed — this spec's INFRASTRUCTURE layer: virtual framebuffer (§3.2), pixel verification (§5), audio verification (§6), WYSIWYG (§7 — **status-annotated R33**: one engine; the export path IS the cloud venue today; the two-venue pixel gate + the realtime-vs-offline NULL rig ride r2/r3 — §7's own status note), property-based testing (§8 — **re-keyed R33**: the ENGINE's class-API property tier LANDED @ `2a0ecf4` [`tests/vitest/property/` — 4 test files + `arbitraries.ts`, `fast-check@4.10.1`, 33 sites, the 2 real catches P-F1/P-F2; the randomized undo soak over the 61-mutator `execute()` surface] — an ENGINE-side disposition per the register's preferred reading, NOT this spec's §8 catalogue: §8's D42/D46 families + the r1-ported surface ride r1 per 17 §13A.8; §13.7's SPEC-ONLY row was the honest pre-R33 record), CI configs (§11/§17), asset scripts (§15/§16 — **re-keyed R33**: r4/r5-phase content — virtual media + computed oracles is the engine's Tier-2 canon until the real-file corpus [r4] and a real-GPU venue land; §15/§16 execute there — their status notes). The umbrella methodology is superseded by spec 17; the domain specifics here (workers/audio mechanics) feed 01/02/20's Testing sections. (R33, engine seal-round register P2-23 + P2-24 + P2-25.)
- EXECUTING via spec 17 v1.3 §0A (the acceptance-executability law) — this spec's mechanics are what 17's recipes run on.
- The fleet's suites at the **R29 pins** (the K1 module nets — ALL GREEN; 00-master's fleet table is the pin canon, D21; the battery re-baselines the pin world once, centrally — **battery_r28 re-baselines after the fleet**; consumer pins re-read live — the row's R28 label re-keyed R29): **nle-engine** 749/749 @ `e3f55bd` (code anchor `74bef08` — the R29 re-pin chore-only, the engine's own code UNCHANGED) — **25 vitest test files** (the 13 R24 suites + the R25-R28 growth: timeline-linked-source-edit (the E1/E2 13-test pin file), timeline-locked-track, timeline-mode-clamps, timeline-nan-guard, timeline-relink-fixes, upstream-ports (G1, 25), m2-wave1-mixer-surface (M2 Wave 1, 9), compositor-clear, mask-manager-view, audio-codec-ladder, media-registry, stable-video-grouping, the R28 CR-F1 unconditional-connect pin (spec-20's 9→10 re-key — the 749th); bridge-seams **151**, nle-bridge **99**; the vendored OT subset stays CORE-ONLY via the tsconfig/vitest alias) — the app consumes @ `e3f55bd` (re-converged at the engine HEAD R29 — the R28 vendor≠HEAD divergence (`74bef08` vs `074a2f6`, BY DESIGN per R17: the post-anchor commits are engine-repo-only) RESOLVED by the chore-only re-pin) — **R33: `2a0ecf4` — 785/785, 29 vitest files** (749 + the W2 property tier's 33 sites [`tests/vitest/property/`: timeline-undo-soak 11 + timeline-math 10 + persistence-roundtrip 6 + transform-resolver 6, + `arbitraries.ts`] + the 3 P-F2 nan-guard pins; the W0-W4 exhaustive-testing waves — the count gate `gate-vitest-count.mjs` [PINNED_TOTAL 785, both drift directions, fail-closed, + standing self-tests] + the parallel `coverage (report artifact)` job [report-artifact-first, thresholds at W5] + `perf.yml` the fleet's first scheduled workflow; the code anchor moves `74bef08`→`28aa387` — the P-F1/P-F2 fixes (`28aa387` the last src-touching commit, `2a0ecf4` the docs-only HEAD — re-keyed R34, was `→2a0ecf4`); register P2-1's record at 01 §0's R33 layer; the app's engine vendor re-keyed `e3f55bd`→`574d8d3` at the app's own lockstep wave `7664603`, the W0-W4 landings engine-repo-only beyond it); the engine's OT submodule @ **`17a19f8`** + WDC @ **`94f6460`** (re-keyed R34 — was `55c81c0`/`83b8850`: the twin re-pin b77909b [OT→`39003d3` + WDC→`94f6460`, docs-only] + the s18 lockstep 574d8d3 [OT→`17a19f8`, the M58R fix]; gitlinks verified at `2a0ecf4`); **opencut-timeline** 632/632 @ `3e18722` (code pin `970948a` — the trailing seal18 W1+W2 reviews/docs only) — **72 milestone entries (M1-M58 + the *R re-runs — the total only per the R27 counting convention; `download/timeline-test-report.json` committed at `970948a` the count authority); the runner `scripts/run-timeline-tests.mjs` = the in-page `window.__TIMELINE_TEST_REPORT__` scrape + the appended real-mouse entries on `/view` — the in-page UI-over-core runner with pixel reads, WYSIWYG state, `window.__VIEW_TEST__`); the app consumes the vendored lock-copy @ `05d88d9` (re-keyed R34 [wrap], was `17a19f8` at the W2 census moment → `05d88d9` at the W3b re-pin; both OT mirrors moved) (byte-exact minus `testing/`; the `970948a` re-pin LANDED R29 — the D-ARCH-6 s17 consumer duty @ `876f2b8`; the engine's OT submodule also `17a19f8` (re-keyed R34, was `55c81c0`)); **web-daw-core** 777/777 @ `94f6460` (docs-only over `ec8fd5c` — the ENG-2 S-series docs wrap; lock `f5011b3` unchanged; the S-series absorbed — the diagnostics/heartbeat law, the rebuilt dsp-effects-worklet, the rt-safety-gate fix; `waveform-contract.test.ts` 14 pins byte-verbatim) — **19 `src/test/` suites, 34 total** (the waveform-contract promotion made src/test 19 at R26; consumers: engine @ `94f6460` (re-keyed R34, was `83b8850`) + app @ `94f6460` (re-keyed R34 [wrap], was `ec8fd5c` — the W3b re-pin) — WDC's own ENG-2 docs wrap LANDED @ `94f6460` (the HANDOFF truth-up, 777/777, '4 deferred' retired)); the integration family = audio-integration, dsp-bounce-parity, dsp-effects-integration, dsp-null-unity, dsp-pdc, dsp-sidechain, nle-audio-core-derisk, offline-parity-wiring, real-audio-e2e, meter-tap, varispeed, fft-analyzer (+ the Goertzel detection oracle `lib/daw/dsp/metrics.ts` and the offline render — "the combined test the audio core has done"); the engine consumes @ `94f6460` (docs-only, WDC's HEAD pin; re-keyed R34 — was `494f6ff`, three pins stale); **nle-ui** 691 @ `6754979` — 37 test files (R9-b preservePitch, F4 identity guards, FW-D `setElementFieldAll`, AW1-2 the Gain ceiling + AW1-4 the D-ARCH-6 pool seam + ENG-1; the app vendored @ `cb04919` (re-keyed R34 [wrap], was `1e0c30b` at the W2 census moment — itself was `6754979`; the W3b re-pin moved it) — the AW1-2 absorb LANDED, the vendor re-converged at HEAD) — **R30: 701 @ `5779295`** (83af958 the C16 package twin + the 4 package-gap pins; 5779295 the K3 FIX-2 dead-entry class) — **R31: 701 @ `dc3644b`** (docs-only: the D29 DECISIONS first-party record + the C0 MiniShell row filing; 701 UNCHANGED); **nle-test-app** 256/256 @ `876f2b8` — **8 suites: GluedShell 104 / audioService 42 / sceneBridge 41 / deliverService 20 / persistenceService 20 / wire-coverage 13 / engineSeam 7 / waveformPeaks 9** (the R8 D29 real-wiring + the R9/D30 W-A/W-F waves + the RA/RC rounds + the R29 D-ARCH-6 s17 consumer duty; census 42 = 36 zero-action + 5 carriers + host @ `55c81c0`, register-equality CI-enforced) — **R30: 377/377 @ `8f12cf9` — 12 files: GluedShell 124 / wire-coverage 26 / audioService 42 / deliverService 20 / keybindings-repeat 4 / waveformPeaks 9 / history-laws 15 / persistenceService 20 / engineService 31 / sceneBridge 41 / timeline-geometry 38 / engineSeam 7** (the K3 corpus round: the four re-expression maps + the store/policy + geometry tranches + the C16 guards; the census register UNCHANGED — the K3 files are additive test files) — **R31: 418/418 @ `f58147c` — 21 files (+9: dom-structural 8 / scrub-session 4 / clip-lifecycle 3 / trim-gesture 3 / edge-autoscroll 2 / zoom-anchor 2 / dnd-fallback 3 / mid-drag-yield 5 / e2e-crawl 11 = 41 new tests)** (the DOM-structural tranche: ALL 13 RE-EXPRESS-PORT rows LANDED + the D26.5 four partial rulings as code + D51 the mid-drag keyboard yield + the K4 e2e legs begun — the capstone chain commits exactly six wire verbs, each ok, each origin ui; the census register UNCHANGED — the tranche is additive test files + the two attr fixes) — **R32: 426/426 @ `ce7cfc3` — 22 files (+1: doc-committing-sessions 7 = the session registry 3 + the D52 shell twin 4; + the W1 K4 move-drag LEG 2b in e2e-crawl)** (the K4 exit verdict LANDED + the w1-entry: the human-rounds protocol + the annotakit review loop + the side-by-side fixtures + the VLM net 7/7 captured / 0 REAL-BUG + the D25.3a port absorption — the census register re-declared 43 = 32 zero-action + 9 carriers + 2 hosts) — **R33: `7664603` — 426/426 UNCHANGED, the 22-file roof unchanged** (the lockstep re-pin wave: the engine vendor `e3f55bd`→`574d8d3` + BOTH OT mirrors `39003d3`→`17a19f8`; the census register amended [pin re-key only, classes unchanged 43 = 32 zero-action + 9 carriers + 2 hosts]); **nle-ui R32: 743/743 @ `1e0c30b`** (the C0 MiniShell composition LANDED 701→737, 78 stories + the ConfirmProvider/MiniShell exports; the D52 twin 737→743 — the optional TimelineRouter.isInteractionActive + the 9 doc-mutating row guards + the 6-pin family); **OT R32: 649/649 @ `39003d3`** (the code pin MOVED off `970948a` — the D25.3a mini-theme wave: theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins; 632 classic + 17 M60) — **R33: OT @ `008e7f3` 675/675 — the seal18 close** (code pin `344123a`, src diff empty to HEAD [the wrap docs/artifacts + the m58 script past it]; 77 milestone entries = 632 classic + 17 M60 + 11 M61 + 11 M63 + 3 M62 + 1 M62-gate, the report json the count authority; the M58R flake fix — the engine's 09-15 OV-12 CI red root-caused + closed [the un-awaited wireReset race]; the census 31 = 28 routed + 3 exceptions UNCHANGED) — **R34 re-pin: OT HEAD `8f96ab2` (the code pin MOVED — the M64 fps param + the L2 carrier families M59-M59D; 714/714 @ 83 entries) + app `d28847c` (426/426 unchanged; the R34 re-pin: vendor engine→`60232ea` + both OT mirrors→`05d88d9` + nle-ui→`cb04919` + WDC→`94f6460`; the CR-2 carrier FLIP — the census 43 = 33 zero-action + 8 carriers + 2 hosts) + nle-ui `cb04919` (747/747 — the DeliverPage supersede contract + the viewer-state-empty CR pair)**; in-repo: **shell-mini 495/12 (the register pin — the sibling's R24-2 wrap re-key, sibling-maintained live-current; census 163 units / 150 GAP / 462 authored; supersedes the R27 read's 441-in-flight / 10 files / 149 census units)** + **shell-variants WRAPPED — 1,939/68 register-declared (the R28 WRAP re-key; the declared pair 1,950 runner-count / 126 stories recorded — the collector-unit reconciliation: a collector difference is a battery-recorded pair, never a drift; supersedes 64 files / 1,796 registered).** **The R28 round statement: the design law and the test law moved in the same round** — the W4 corpus fold (`980920b`, 14 law-side files) + this test-law fold (the GAP register's R28 fold row below) are ONE round; battery_r28 re-baselines the pin world once, centrally (per ARCH-R28 R17), and the suite-census coherence class checks THIS row's figures against 00-master's fleet table (749/632/777/691/256; variants 1,939/68 + the declared pair 1,950/126; mini 495/12). **R35 layer (the GATE-1 P1 fold): engine `2bae99a` docs-only over `60232ea` 789/789 (the CI GREEN on the aivs home) · OT `8f287c7` docs-only (`8f96ab2` 714/714 @ 83) · WDC `dc0ca32` docs-only 777/777 · nle-ui `45d42fb` 754/754 (the C-1a edit-styles batch) · app the edit-styles wave (the census 45 = 33+8+4) · the fleet re-homed to github.com/aivs-tech (the layered convention: 00's fleet table carries the same layer).**
- The R23 seal artifacts + the VLM visual net (the K3/K4 tooling base): `ui-mock/shell-mini/docs/CORE-SEAMS.md` (the S1-S26 seam inventory + the store partition + the transport map — K3's re-expression worklist) + `LAW-NET-INVENTORY.md` (the 150-unit / 10-file / 445-test census: 33 HOLDS-on-OT / 412 app-side authoring / 137 GAP units + the 59-app-emitted-static + 15-templated testid census — K3's acceptance list) + `ui-mock/shell-variants/scripts/vlm-capture.mjs` / `vlm-review.mjs` / `vlm-run.sh` (the R23-G VLM visual-verification net — K3 adopts, see the GAP register).

**The D24 verification ladder, spec-12-side (the K-tiers are the crawl's verification tiers; `IMPLEMENTATION-PLAN.md` §2 is the executable gate table — this §0 is their spec home):**
- **K1 — module nets:** each repo's own suite at its own gates (the rows above; this spec's §17 CI configs + spec 17's three-tier methodology are the per-repo shape). ALL GREEN at the R29 pins — the K1 net grew engine 458→748→**749**→**785** (R33 — the exhaustive-testing waves: the W2 property tier's 33 sites + the 3 P-F2 pins; engine seal-round register P2-1) / OT 536→**632** / WDC 759→**777** / nle-ui 674→690→**691** / app 174→252→**256** across R25-R29 (the E1/E2 fix + FW/OV/RC/M2 waves + the R28 CR-F1 unconditional-connect re-pin at the engine; D-ARCH-6 + session-16 at OT; the S-series at WDC; R9-b/F4/FW-D/AW1-2/AW1-4 at nle-ui; the RA/RC rounds + the R29 D-ARCH-6 consumer duty at the app) — the gate's test-law citation: §0's BASE rows above equal 00-master's fleet table (**battery_r28's suite-census coherence class** — both collector figures recorded, the runner-vs-line-start reconciliation a RECORDED pair, never a drift).
- **K2 — combined nets (cross-module):** the seeds exist at HEAD — engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`: vendored OT ops × engine composition, within the 458 net), WDC's integration family (offline render + Goertzel detection, within the 759 net), OT's in-page UI-over-core runner (pixel reads + real-mouse + WYSIWYG state, the 386+150 net). **The D29 instruments (REGISTERED, R27 re-key): OT's M49C coverage gate — the machine-checked complete-UI claim, accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions, **origin-attributed (ONLY `origin:"ui"` dispatches count — HA-4-2; test-handle dispatches never count)**, node-side accumulation across the phases, tsc-lockstep both directions — is K2's OT-side completeness instrument (**green, 28 routed + 3 exceptions** @ `344123a` — re-keyed R33, was `970948a`); the wire census **31 verbs = 28 routed + 3 exceptions** (`WIRE_COMMAND_TYPES` @ OT `headless/api.ts:268-300` + the **EXPORTED** `WIRE_UI_EXCEPTIONS` registry @ `:353-360` (both cites re-keyed R33 — was `:243-275`/`:328-335`; the census UNCHANGED 31 = 28 routed + 3 exceptions), an AR-1/D-ARCH-3 tsc-asserted export consumed LIVE by both gates — never a hand-mirrored copy, the AR-2 consumption law) is the census the K2/K3 families key on.** The GAP register's census row formalizes the NAMED family. **R28 projection note:** the census re-declares mechanically at the r1 landings (D29-F8) — the ruled projection **78→79→80** as insertBatch + replace mint union members (15:5037/:5039; D47/D46-B5); the K2/K3 families' r1 stage-exit gates cite the consolidated **r1-port acceptance protocol** (17 §13A.8's five additions — see the GAP register's census row below).
- **K3 — app-behavior nets:** the app wiring driven headlessly over REAL modules — the LAW-NET corpus re-expression, the DOM-structural gate (three-way per ARCH-R24 D26.5: OT's frozen `data-test=` 68-site vocabulary / nle-ui's `shell-*` testids / the no-counterpart structural units — gated on the mini-unit mapping row + D30's W-C, not on a swap+testid layer), the offline audio pins (the Node-venue law), the VLM visual net (adopted from the variants). **The D30 W-F coverage-gate port is the REGISTERED K3 instrument (D29; **LANDED R25** — the app's `wire-coverage` suite, 13 tests in the 256 (9 at the R25 landing); the gate consumes the LIVE `WIRE_UI_EXCEPTION_VERBS` from the vendored barrel — the hand-mirrored copy went stale at the `6e2b91a` re-pin, the live-registry consumption law — and carries the AR-2 gesture pin: the REAL volume-line double-click → `timeline.upsertKeyframe` origin 'ui'): the module-scope accumulator + the gesture sims + the final assertion (accumulated ⊇ `WIRE_COMMAND_TYPES` − the exported exceptions, live-barrel) — the routed-verb completeness machine-check; the shell-path surface is excepted per D30 R2/R8 (the known blind spot, registered verbatim: the gate measures the PORT surface), with a compensating pin family driving the trusted-integrator verbs (⌘B, [ ]/⌥[ ], ,/., M/⇧M, ⌘M/⌘⇧M) directly through engineService — owner-tagged app-side (D29.1, review F14).** **R28 additions (the D48-D50 K3 surface, per `audits/fleet-r28/test-law-d48-d50.md` §1-§3):** the **page-key tests** (the app-side ⌘1-⌘5 page-key suite — Edit/Color/Audio-focus/FX/Deliver; the ⌘4 re-click toggle + the Esc exit + the per-page view-state memory + the `page==='fx'` density discriminator — named as **XMOCK-6's registered r5/K3 impl half** per 16:28's own registration: "Remaining at K3: the app-side page-key tests + the nle-ui FX-page absorption") + the **completeness machine-check cited as the pattern for new closed surfaces** (the module-scope accumulator + the LIVE-imported union + the direct array diff that NAMES the drifted member + the size identity — the `Page` union's 5/5 gate's shape; closed sets are consumed as live imports, never hand-mirrored — the RC-V1 stale-mirror incident the law's own text cites; the same law serves the D48 bridge-drift gate and the D49 burn-in app-twin).
- **K4 — the crawl exit gate:** the single automated e2e — import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export; zero mock paths, zero human input; the e2e drives the same **28 routed wire verbs** through the real UI (the shell-path exception applies identically — D29.1c).
- **Walk/run (the depth ladder):** w1-w3 the real-app tiers with the human classes (every human-required gate registered [H:reason] in the plan — crawl discipline continues through the walk: findings land as programmatic pins wherever possible); r1-r6 the run depth — the parity pins below are r2/r3, the decode corpus r4, the fixture corpus r5.

**GAP (the work — owner + phase per the D24 ladder; the R22 C/W/R tags re-keyed via ARCH-R23's old→new mapping table, spec 14 retired; the transitional dual tags stripped at the R24 fleet — the D24 K/w/r set only, the window closed; the testing-domain §4 rows re-homed HERE per the posture law; acceptance in parentheses):**
- LAW-NET-INVENTORY (owner: nle-core-spec): the crawl's acceptance corpus distilled from the mini's 445-test net. **LANDED at the R23 seal round** — the D24 mapping's pre-C1 status flip: now **K3's acceptance list** (the K3/K4 gates consume it). The census: 150 law-family census units / 10 files / 445 tests; the disposition totals (33 HOLDS-on-OT tests; 412 app-side authoring tests / 137 units); the 59-app-emitted-static + 15-templated testid census (the 60th static id is harness-only — `LAW-NET-INVENTORY.md` §2.2).
- **The K2 combined-family census + registration (THE new K2 row — the plan's S-spec work item, at K2 entry)** (owner: nle-core-spec): formalize the cross-module corpus as a NAMED suite family with a census + a register in spec 12/17 — engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`), WDC's integration family (`audio-integration`/`dsp-bounce-parity`/`dsp-effects-integration`/`nle-audio-core-derisk` + the `offline-parity-wiring`/`real-audio-e2e` pair), OT's in-page UI-over-core runner (the 386 in-page + 150 real-mouse M-suites, report-json authority; the engine-in-the-middle 4 within the 458 net, WDC's family within the 759 net); close the missing cross-module pairs — the OT-ops→engine-decode round-trips at the app's seam shapes (S-engine's worklist item) (acceptance: the family census green + registered here and in spec 17 §13A.7 + battery-checked thereafter; the missing-pair suites green at the engine). **Registration state (R24): still PENDING — K2 entry hasn't happened; this row (and spec 17 §13A.7) is the registration home. **R34 re-key (R34-W1-b): LANDED — the census + register recorded at this row's end; the register home is spec 17 §13A.7's re-tier row.** The D29 instruments pre-register the gate: M49C is the K2 OT-side completeness instrument (green 28/28 @ `970948a`) and the wire census **31 = 28 routed + 3 exceptions** is the census the family keys on — the K2 entry lands the census count rows at the R27 pins (748/777/632) as the registered family.** **R28 additions (the five r1-port protocol items — the consolidated r1-port acceptance protocol, 17 §13A.8's charge + one §13A.7 facet row pointing at it, per `audits/fleet-r28/test-law-d45-d47.md` §4): the family's r1 stage-exit gates gain (1) D42's pairwise re-key (the carried E1/E2/F6 pins re-keyed to assert link values through undo/redo/serialization + rider 9's count reconciliation); (2) D43's Stage-0 zero-behavior-change fence (the verbatim-re-export byte-compare + the battery's leaf-only import check widened to the C1-fold edge); (3) D44's 9 lockPreCheck riders + the never-guard/tsc-lockstep compile checks + the census re-key; (4) D46's transition-remap probe at Stage-4 entry (the two-exit gate) + ripple-overwrite's downstream-set diff check; (5) D47's union-version-bump migration (the old-replay deprecation rows + the counterpart re-derivation 29→31→32-of-80) — the base protocol (the plan's r1 row) + these five = the executor's work-order gate content.** **R34 census record (R34-W1-b — the K2 landing at the live pins; the R24/R27/R28 layers above stay verbatim as the pre-landing record): (1) engine-in-the-middle @ engine `d8b97b4` (CI-only over the R33 seal `2a0ecf4`; 785/785, 29 vitest files, the PINNED_TOTAL=785 count gate + the 5-job CI venue) — `bridge-seams` 151 + `planner` 52 + `video-sync` 31 + `timeline-edit-ops` 13 = 247 it( cites (the companion bridge-carrier pair: `nle-bridge` 99 + `nle-bridge-realtime-midi` 8); (2) WDC's integration family @ WDC `94f6460` (777/777; 34 .test.ts = 19 src/test + 15 src/lib) — `audio-integration` 27 + `real-audio-e2e` 12 + `dsp-bounce-parity` 9 + `dsp-effects-integration` 8 + `nle-audio-core-derisk` 7 + `offline-parity-wiring` 2 = 65 it( cites across the 6 named suites; (3) OT's in-page UI-over-core runner @ OT `c4c04f1` — the report-json authority `download/timeline-test-report.json`: **687/687 at 78 entries** (M1-M64 + the R re-runs + `M49C` + `M62-gate`; 675/675 @ `008e7f3` pre-M64 — the fps family), total-only per this row's law; **M49C green 28/28 @ code pin `344123a`** — the wire census **31 = 28 routed + 3 exceptions** stands (the `970948a` cite above superseded by the R33 re-key); (4) the missing cross-module pairs (the OT-ops→engine-decode round-trips at the app's seam shapes) — carried TODAY at the app-carried venue: `wire-coverage` 26 + `engineSeam` 7 @ app `28f3229` (docs-only over `7664603`; 426/426, the 22-file roof) — honestly stated: NO engine-resident round-trip suites are claimed by this census; engine-side authoring beyond the app-carried shapes stays a forward row (S-engine's worklist), NOT a K2 blocker. Per-file figures are static it( cites; the runtime nets are the declared count-gate pins. Acceptance state: family census green + registered HERE + spec 17 §13A.7 (this round) + battery-checked at the next battery fork.**
- **The R28 test-law fold — the D42-D50 rows in 12/17 (THE new GAP row — S-spec(6)'s spec-side half, the plan carve's owner)** (owner: nle-core-spec; phase: **W5/W6 — landing WITH battery_r28, never after it**): the corpus's test law catches up to its design law, ONCE — the W4 corpus fold (`980920b`) amended 14 law-side files and ZERO test-side files; by 17 §14.4 step-0's own rule ("a facet with no row anywhere is a spec bug") every D42-D50 facet was a spec bug (`audits/fleet-r28/test-audit-coverage.md` §1.1 + §5's P1 list — this row is its 12-side fix). THIS spec's half (this fold): §0's BASE/GAP/K-tier re-key to the R28 pins + this row + the battery-posture row's R28 class list (D47's counterpart arithmetic class among them) + §5.5's D45 grade-seam parity rows + §6.4's re-key/§6.5's WDC-side retime laws + §8's D42 linkage invariants (I6-I8) + the D46 family invariants + §18/`## Testing`'s re-key; the 17-half lands in the same fold (17 §3.1's eight new matrix rows + §13A.7's facet rows + §13A.4.1's 78→**80** re-key + §2.5 rule-8's two-tier census re-key + the new §13A.8) with the model-fold prereq sweep (09:310-:370 marker interface; 15:1566-:1632 params; 09:119-:124 captions; 10:811 flip; 18 §4.4/§4.8/:41; 08:2168 + 04:2293 ruled-out rows) (acceptance: **the 17 §14.4 step-0 audit green — every D42-D50 facet has a matrix/facet row, machine-checked by battery_r28's step-0 facet-completeness class; battery_r28 green with the R28 check classes** — the design law and the test law moved in the same round).
- The crawl app's corpus authoring — the exact scope is now counted (owner: nle-test-app, **K3** — split per D24: the corpus-check half is K3, the e2e half is K4, the human side-by-side → w1-entry): 322 app-side authoring tests across 115 law-family census units per `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` §2.3-§2.4 (supersedes the earlier ~130-180 law estimate; per D25.5 the units land where their owner lives — OT-suite extensions for OT-side laws, app-side tests for app-policy/engine seams, CORE-SEAMS §3 the transport map; the DOM-structural half lands AFTER the data-test mapping row + D30's W-C — the ordering law, re-pointed per ARCH-R24 D26.5 — while the store/policy halves may author ∥ against the injection-point contracts; includes the composed ripple family — 4 census units / 16 tests composed app-side over `timeline.trim`/`rippleDelete`, r1 graduating the OT interval-diff family) (acceptance: the inventory's corpus checked row-by-row, green over REAL modules, zero mock paths).
- **The K3 offline audio pins** (owner: nle-test-app + WDC, **K3** — C3's audio-crawl substance, unchanged): the offline-render + call-spy pins (waveform/mute laws) per the Node-venue law (the Node venue has no audio device — WDC's own law; this spec's §6 offline-render mechanics are the venue) (acceptance: the pins green; **hard prerequisite: S-wdc's waveform seam contract** — the only K3 sub-half with an external prerequisite).
- **The K3 VLM visual net** (owner: nle-test-app, **K3** — new R23 row: the variants' R23-G tooling, adopted not forked): port `ui-mock/shell-variants/scripts/vlm-capture.mjs` / `vlm-review.mjs` / `vlm-run.sh` (the capture → review → gate pipeline) onto the app's `.storybook` + OT's runner (acceptance: the VLM pass gate executes against both surfaces; the w1 side-by-side and the `/view` gold-sample verification reuse it). **R28 re-key (the two-tier visual net with exit gates — the VLM-net/K3 law, per `audits/fleet-r28/test-law-d48-d50.md`'s grounding law 3):** the VLM story net runs at the registered **1280×800 floor** + console-clean mounts + battery zero-error + fresh 0-P1-0-P2 probes before WRAP — reserved for the layout/legibility class the jsdom battery cannot see; **BOTH collector figures recorded** (runner vs line-start — the 1,950/1,939 reconciliation law: a collector difference is a battery-recorded pair, never a drift). **R34 re-key: the K3 VLM net LANDED @ the app's `57dce6a` (the w1-entry's instrument — 7/7 captured [5 app stories + 2 OT /view targets, ZERO console errors], the triage 0 REAL-BUG [2 hallucinations disproven by measurement, 4 design-intents, 1 category confusion], GATE=PASS via `vlm-gate.json`; K4-ratification condition #2's spec-side record — the pair at 17:29/17:31).**
- **The K4 automated e2e** (owner: nle-test-app, **K4** — C4's e2e half per the D24 split; the crawl's completion proof): the app's single named suite — import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export, asserted programmatically (acceptance: green end-to-end, zero mock paths, zero human input; w2's full-scope demo re-gates by extending this suite over the media registry).
- **The NFR/perf family's venue + carriers (THE new GAP row — R33, engine seal-round register P1-8): 17 §13A.1's recipes have ZERO app-side carriers and no app-side venue** (owner: nle-test-app + nle-engine; phase: the §13A.1 recipes at their ladder rungs): no perf-envelope test exists app-side (no FPS/scrub-latency/memory/render-time assertions — ProgramCanvas telemetry `durationMs` only) and the app CI is push+PR only; **the ENGINE nightly venue LANDED R33-recorded** (the perf tier PERF-1..3 @ the engine's `perf.yml` (+ the randomized-seed property pass, `FC_SEED=random` — the exploration venue) — the fleet's FIRST scheduled workflow, telemetry-mode first; the op-time/offline-mixdown/heap-leak envelopes, NOT the app-side recipes; closes the testing design's audit F-1 "nightly venue that no repo defines"); an app-side nightly job (or the per-PR smoke subset) is the venue precondition — budgets are pass/fail per §13A.1's own law, and until the venue exists the §13A.1 facet rows dangle without an executable path (17 §0A rule 2's own spirit).
- The offline-parity threshold pins + the null rig (owner: app+WDC+engine, **r2**) (acceptance: max deviation ≤ −60 dBFS any channel).
- Grade-math parity pins (owner: app+engine, **r3**) (acceptance: the mock's W4 math vs the engine's output on the same fixtures — max delta ≤ 1 LSB-equivalent). **R28:** the D45/R9-c instantiation at §5.5 rides this row's r3 phase — the compose-then-filter seam's parity twin (monitor ≡ export) + the fragment-pass parity family + `buildGradeFilterString`'s string law, per `audits/fleet-r28/test-law-d45-d47.md` §1.2-§1.4.
- Real-file decode corpus (owner: nle-engine, **r4**; PENDING the user's D6 re-affirmation, deprioritized) (acceptance: decode round-trip pins on the real-file corpus).
- FCPXML fixture corpus (owner: nle-test-app, **r5**; the phase-entry artifact is the parser + fixture-corpus choice) (acceptance: exports validate vs the chosen reference parser on the corpus).

**ACCEPTANCE & TEST PLAN:** §18 (the meta plan) + the `## Testing` meta-test section (infra regressions caught FIRST per 17 §9.3's CI ordering); spec 17 §13A is the LIVE facet matrix — this spec points there, not to a static table (the sibling fleet round re-baselines its §13A.7 re-tier row to the R29 pins); battery posture: **the R29 suite pins above stay green** (the K1 regression role — **the battery re-baselines the pin world once, centrally after the fleet** (battery_r28 re-based the R28 world @ `965308c`; the R29 pins ride the next fork): engine `e3f55bd`/749 (code anchor `74bef08`) — **R33 re-key: the engine pin moves to `2a0ecf4`/785** (the W0-W4 exhaustive-testing waves — the property tier + the count gate + the coverage job + the perf nightly; engine seal-round register P2-1; the BASE row's R33 layer above the record; the suite-census coherence class re-baselines at the R33 battery fork), OT `3e18722`/`970948a`/632, WDC `94f6460`/777, nle-ui `6754979`/691, app `876f2b8`/256 — **R34 W2-era re-key (superseded by the wrap re-key below): OT `008e7f3`/675 + nle-ui `1e0c30b`/743 + app `7664603`/426; census 43 = 32+9+2 — **R34 WRAP RE-KEY: OT `8f96ab2`/`8f96ab2`/714 @ 83 entries (the M64 fps param + the M59-M59D CR families) + nle-ui `cb04919`/747 (the DeliverPage supersede + viewer-state-empty CR pair) + app `d28847c`/426 (the R34 re-pin wave; the CR-2 carrier FLIP — the census 43 = 33 zero-action + 8 carriers + 2 hosts; the OT mirror lock 05d88d9); WDC `94f6460`/777 unmoved** + the consumer pins + the census-coherence check **31 = 28+3** (lineage: 24→28→30→24+6→25+5→28+3) per D29/D-ARCH-6, with the ruled r1 projection **78→79→80** re-keying the sweep at the bump (15:5037/:5039); the R27 classes carried forward (the **registry-export check** — the AR-2 live-registry consumption law, a consumer gate consumes the exported registry never a copy; the **line-pin re-base map as ONE consolidated battery-able input** (spec-19's §2); the app census arithmetic 42 = 36+5+1 @ `55c81c0`; the origin-attribution clause on the M49C/W-F instrument checks); **the R28 check classes** (the five named + the T2 additions, per `audits/fleet-r28/test-law-d48-d50.md` §4.4-4.5 + `test-law-d45-d47.md` §§1-4): **D43-A3** (check-2 widened to the C1-fold core-file→OT-leaf edge — bridge/ + the folded timeline-math.ts site, still leaf-only) · **D44-A6** (the 25-code class-tag completeness census, scraped from 15 §6.3's registry table) · **D47** (the 78→79→80 counterpart arithmetic + the insertBatch-ripple/replace §13.15 rows) · **D50-site-19** (the register's round-final OPEN table — rows 5-8 closed via D48/D49/D50 → 0 open design rows) · **R16** (the dead-constants drop) · **D45** (the §13A.5-extension presence + the parity-oracle rows — §5.5 here) · **D46** (the four families' facet rows + the RE-3 probe row + the §8 invariants here) · **the r1-protocol class** (17 §13A.8's five additions each named with its stage slot) · **the §14.4 step-0 facet-completeness class** (THE load-bearing one — every D42-D50 facet has a matrix/facet row, machine-checked) · **the suite-census coherence class** (12 §0/17 §0A's BASE rows equal the R29 pin set 749/632/777/691/256; variants 1,939/68 with the declared pair 1,950/126 recorded; mini 495/12 — the runner-vs-line-start reconciliation law as a RECORDED figure, never a drift) · **the 182-row keymap coherence class** (16 Appendix A ≡ §16 matrix ≡ §0 census at 182; 15 §13.5 re-keyed 181→**182** by the W6 cross-file sweep — R33, engine seal-round register P3-45) · **the ruled-out-claimant negative class** (08:2168/04:2293's ⌘1-⌘9 claimant rows carry the resolved wording, the bare claimant forms absent) · **the signoff-rider freshness class** (the R28 re-rider's re-keyed numbers: 31=28+3 + the 78→79→80 projection; the 182-row registry; the 25-code two-tier registry)); the crawl's tier gates: K2 the census + the missing pairs, K3 the corpus row-by-row + the audio pins + the VLM net + the D30 W-F coverage-gate port (the registered K3 instrument), K4 the e2e — `IMPLEMENTATION-PLAN.md` §2 is the executable gate table.
...
### 5.5 The grade-seam parity rows (R28 — the D45/R9-c consumption at r3)

The compose-then-filter final pass is a parity-twin surface (P2's one-law-both-venues, instantiated at the seam — 08:24's R9-c fold; per `audits/fleet-r28/test-law-d45-d47.md` §1.2-§1.4):

- **monitor ≡ export (the construction pin):** the monitor path (ProgramCanvas) and the export path (deliverService) both route through the ONE `paintCompositionFrame(ctx, ops, media, {grade})` helper — a wiring spot-check (P20); a future "fix" that re-inlines one venue's final pass trips THIS test. Both venues pass the SAME **`{grade}` object** (the consumer-preview tier's one-home — the string mapping lives at the seam, never at either venue).
- **The ORDER pin (compose-then-filter):** the ctx-stub call log (P12) shows clear → N composite `drawImage` calls (the scratch compose) → **ONE** filtered `drawImage` onto the target — the filter is applied exactly once, to the composited frame, never per-element (the discrimination fixture: two overlapping elements, a non-neutral grade — per-element filtering would differ).
- **The r3 fragment-pass parity (the venue's grade home):** the fragment passes over the scene-linear `rgba16float` working texture (08:26) — the mock's CPU color math (the spec-08-exact oracle, never the production venue) vs the engine's output on the **SAME shared fixtures** (the §5.3 committed-asset rule — one fixture set, both sides pinned on it, never forked); **max per-channel delta ≤ 1 LSB-equivalent** in 10-bit code values (the metric pinned as a literal); SHA-256 bit-exactness for same-input re-renders per side (determinism before parity); the sequential-pass law ([clipGrade] → [timelineGrade] — later passes see earlier output; the A∘B ≠ B∘A discrimination fixture FAILS under a params-merge "optimization"). **The instrument self-test precondition** (the P-WDC-4 law generalized to the visual family): the comparison instrument is proven against closed forms BEFORE any parity claim — a known identity grade ⇒ delta exactly 0; a known ±1-code-value perturbation ⇒ the metric reads exactly 1; treat failures here as instrument failures, not test noise.
- **`buildGradeFilterString`'s honest-CSS subset pins (the string law):** the canonical-order table (every expected string **hand-derived**, never built by the module under test — `{contrast:2, saturation:0.5}` → `'contrast(2) saturate(0.5)'`, the fixed component order; `hue-rotate(90deg)`'s unit law); all-neutral ⇒ `''` (the empty string, not a chain of identity functions; each neutral component omitted individually); the invalid-CSS hostile fold — NaN / ±Inf / negative where CSS forbids ⇒ the poisoned component is **DROPPED** (the remaining valid components still apply; never a thrown error, never an invalid filter string reaching `ctx`); the per-element save/restore bracket (`ctx.filter = s; drawImage; ctx.filter = 'none'` — the assignment never leaks into a subsequent unfiltered draw); **the closed-set census** — the emitted filter-function vocabulary is frozen **{`contrast`, `saturate`, `brightness`, `hue-rotate`}** (grade path) **+ {`blur`}** (the element-effects sidecar path, the R9-d honest subset with the R8-REV radius clamp carried); a new function joining the string builder without a spec row fails the census (derived from the builder's own output over the full input census, not a hand-copied list).
...
> **R28 re-key (D46 — the audio-half law):** §6.4's varispeed row is the SEED of the WDC-side retime family — the fit-to-fill/ripple-overwrite audio-half law (r1 Stage 3, the composites ladder — append → fit-to-fill → ripple-overwrite; ripple-overwrite's audio half rides the same element-retime defaults), instantiated at §6.5 per `audits/fleet-r28/test-law-d45-d47.md` §2.4(d).

### 6.5 The WDC-side retime laws (R28 — the D46 audio half)

The fit-to-fill composite's audio half is a RULED acceptance family (06 §5.9F + ARCH-R28 §4 D46), all rendered through the Node `OfflineAudioContext` venue (the `web-audio-api` package — real DSP in CI, zero devices, P-WDC-1; per-test 30 s timeouts, never global; serial workers — the ~1.8 GB lesson):

- **The null-test convention (the untouched-lane law):** fit-to-fill retimes ONE lane; every OTHER lane renders **`nullDepthDb ≥ 60 dB`** vs the pre-op render (indistinguishable — the AES null-test convention, the W-F3 threshold family; the retimed lane's `onsetDeltaMs` pinned — shifted content lands where the arithmetic says). **The oracle self-test is the precondition** (P-WDC-4, adopted verbatim): the comparison instrument is proven against closed forms BEFORE any null-test claim — and the silent-reference guard: `refEnergy === 0` reads −∞ and must NOT read as a perfect match.
- **The exact-duration-no-tolerance law:** the retimed audio's output length == `round(frames / rate)` **EXACTLY** at the accepted rates (the F1 flush law — a no-flush regression drops ~90 ms at rate 2; the internal sizing law `flushPadFrames ≥ inputChunkSize` is the regression catcher); the timeline-side twin is fit-to-fill's exact-fill (§8's invariant below) — no tolerance on either side.
- **The [1/32, 32] domain guards:** the empirically-fatal out-of-domain rates (1e-6 → spin; >1e5 → 350 GB OOM) degrade to IDENTITY — never spin, never throw; the DOMAIN BOUNDS work exactly (rate 1/32 → ×32 length; rate 32 → ÷32). The ACCEPTANCE domain itself is the computed intersection **[0.1, 5]** (06 §5.9F/D43.10 — `[MIN_RETIME_RATE, MAX_RETIME_RATE] ∩ NATIVE_VENUE_SPEED ∩ WSOLA_RATE`; out-of-domain REFUSES `INVALID_PARAMS`, never clamps — exact-fill has no tolerance); the WSOLA engine domain [1/32, 32] is the guard family above.
- **The goertzel pitch oracle:** 440/660 Hz survive the retime (bin > 0.4) while the chipmunk frequency (input × rate) stays < 0.05 — no pitch shift on any accepted fit (the §6.3 FFT family's content twin; WDC's `lib/daw/dsp/metrics.ts` detection oracle).
...
**R33 two-venue status note (engine seal-round register P2-25):** as written, §7 reads as standing law — the current truth: **one engine; the export path IS the cloud venue today.** The landed halves are real: m24's encode→decode→pixel-parity vs `renderFrameOffscreen` (engine `src/app/page.tsx:248` + the m24.5 milestone pin `:7983-7996`), m29's bridge-mux ≡ direct-mix sample-level parity (`:272-273`), and N2b's offline-render sample-level parity pins. **The two-venue pixel gate (browser-render == cloud-render, both engines live in one run) + the realtime-vs-offline NULL rig are net-new rig work riding the r2/r3 parity rows** — 17 §17A S4's honest note (`17:2790`: "the true realtime-vs-offline NULL gate is net-new rig work" — the engine's CR-A #6 one-engine-per-instance guard blocks the cheap route); the r2 parity corpus's randomized scenes reuse the property tier's `arbitraries.ts` (the engine design's pre-registration). Until those land, this section is the TARGET contract, not a landed gate. (R33, engine seal-round register P2-25.)
...
**The R28 invariant additions (the D42 linkage + D46 edit families — this section's first growth since the seed six; per `audits/fleet-r28/test-law-d42-d44.md` §D42.4-D and `test-law-d45-d47.md` §2/§5):**

**D42 — the linkage closure properties (three invariants, swept after EVERY op over the seeded-PRNG fuzz net — the M25 shape):**

1. **The closure-shape property (I6):** for every state reachable by the seeded op sequence, the derived runtime group is **symmetric, live-id-only, order-stable, cycle-free** — x∈group(y) ⇒ y∈group(x); dangling pointers dropped (read-as-absent); the group's member order is the scene's element order, never map-iteration order; the derivation terminates on the hostile zoo (self-pointers, directed 3-cycles, one-way pointers).
2. **NO dead `linkedTo` after ANY op (I7):** every **non-null** `linkedTo` on every live element points at a live id (null = the cleared state, legal per 15:962-:971's patch-key law) — after every op in the sequence (the dead-pointer sweep; the invariant that catches a forgotten prune in any FUTURE verb — the class the curated per-verb tables miss).
3. **Undo/serialize preserves the field (I8):** `base + edit + undo ≡ base` (serialized compare, `linkedTo` fields included — the snapshot algebra); `toJSON → fromJSON → toJSON` byte-stable carrying the field.

**D46 — the four edit families' invariants (the families join the no-overlap/undo families above):**

- **replace:** the keyframe-prune law (the removed clip's keyframes pruned — zero `KEYFRAME_ORPHAN` warnings, no orphans in the serialized state) + the A9 honest echo (`data:{insertedClipId, removedClipId, durationInFrames, sourceEnd}` — a same-source replace still commits, ONE entry, changed ids; **NOOP never**).
- **append:** the append point evolves INSIDE the op — `t_{i+1} = t_i + dur_i` accumulated per member, never pre-computed as N × t₀; ONE history entry for the whole multi-append; the playhead never read (a fed playhead asserts invariance).
- **ripple-overwrite:** the downstream-set diff == the computed set — the shifted set is exactly `{target-track elements with startTime ≥ span.end} ∪ {transition-right-neighbors of removed clips}`, direct-diffed so a wrongly-shifted or wrongly-skipped member is NAMED (never a hand-listed copy — the E1-a derivation risk turned into the pin's own oracle).
- **fit-to-fill:** the computed-intersection domain property — `FIT_TO_FILL_RATE == [MIN_RETIME_RATE, MAX_RETIME_RATE] ∩ NATIVE_VENUE_SPEED ∩ WSOLA_RATE ⇒ [0.1, 5]`, the expected side hand-computed in the test from the three LIVE imports (the anti-tautology law applied to a derivation; `AUTHORING_SPEED`'s [0.1, 4] is the named trap, NOT an input); out-of-domain REFUSES (`INVALID_PARAMS`, constraint `domain`), never clamps — the exact-fill output-duration law has no tolerance.
- **insertBatch (the TOTAL-span property):** `insertBatch(elements, {ripple:true})` pushes every downstream element by the block's **TOTAL span** (the first-element anchor + the relative offsets composed — insert's D31.6 full-duration law mirrored); the one-element degenerate ≡ the single insert — one law, both verbs.
...
> nle-engine's test reality **at the SCOUT-12 era** was a **single-tier, in-app harness** (124 tests in `src/app/page.tsx`, results DOM-scraped by `scripts/run-nle-tests.mjs`) — the exact pattern this spec + spec 17's three-tier methodology correct. The engine's own 113-item test charter (`gaps/audit/G-test-coverage.md`) was the re-tier worklist. Spec wins; full reconciliation: `19-code-references.md`.
>
> **R23 annotation (2026-09-07, the fleet re-verify):** the table below is the HISTORICAL SCOUT-12 record — superseded in substance: the engine LANDED the vitest tier (`"test:vitest": "vitest run"` in `package.json`; `tests/vitest/` — 13 suites, **440/440 @ `b8c6f88`**, incl. the K2 engine-in-the-middle seeds `timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner` + the nle-bridge pair). The in-app harness page survives as a legacy surface, not the engine's primary net; the live reconciliation is spec 19's R23 re-baseline. **R24 re-base (2026-09-08):** the tier holds at 13 suites, **458/458 @ `5036387`** (+18 since R23 — the W2.5 effects-sidecar pins + the R8-REV blur-clamp pin, bridge-seams now 113); the engine's OT submodule re-pinned `a4e971d`→`c15a629` (the W11 wire surface importable at the test seam, zero engine mediation by design). **R27 re-base:** 25 test files, **748/748 @ `f9ac806`** (code anchor `50b91f5`; bridge-seams **151**, nle-bridge **99**, upstream-ports 25, m2-wave1 9); the OT submodule @ **`6e2b91a`** + WDC @ **`ec8fd5c`**. **R33 note (engine seal-round register P2-23):** the table's §8 property row ("COULD-NOT-VERIFY (no fast-check) — SPEC-ONLY") was the honest record at its vintage — the engine's CLASS-API property tier LANDED @ `2a0ecf4` (W2: `tests/vitest/property/` — 4 test files + `arbitraries.ts`, `fast-check@4.10.1`, 33 sites, the 2 real catches P-F1/P-F2; §0 BASE's re-keyed bullet the record) — a VENUE-TIER landing, not this spec's §8 catalogue (which rides the r1 port per 17 §13A.8); the §15 fixtures row below stays SPEC-ONLY at its r4/r5 phases (§15/§16's status notes).
...
> **R33 status note (engine seal-round register P2-24):** §15 is **r4/r5-phase content, target-not-current** — no ffmpeg/ffprobe invocation, no `tests/fixtures/` media tree, no test-asset manifest.json, and no generate-assets/regen workflow exists in ANY repo today (verified at engine `2a0ecf4` + app `7664603` — re-verified R34 @ engine `60232ea` + app `d28847c`; the app's `vlm-*/manifest.json` is the VLM capture net's own bookkeeping, a different family). The engine's Tier-2 canon is **procedural virtual media + computed oracles** (`src/lib/nle/media/virtual-video.ts` + `virtual-audio.ts`; the deterministic `VirtualVideoSpec`/`VirtualAudioSpec` fixtures + expected pixels computed from math) — a stronger canon for virtual media (no golden drift possible); the only fixtures tree the engine carries is the perf tier's in-code `tests/perf/fixtures/reference-composition.ts` (REF-COMP-1, built through the public Timeline API — the same procedural canon). §15's ffmpeg scripts execute when the real-file corpus (r4, owner nle-engine — §0's GAP row) and the real-GPU venue land; §16's reference-regen process is the r5-side twin. (R33, engine seal-round register P2-24.)
...
> **R33 status note (engine seal-round register P2-24):** §16's reference-regen machinery is **r4/r5-phase content, target-not-current** — zero reference PNGs, regen scripts, reference meta files, or per-platform reference directories exist in any repo (the greps at §15's status note); the engine's pixel verification is computed-oracle (expected pixels from math — no golden drift by construction). The process below is the TARGET workflow for the real-GPU venue + the reference-render corpus. (R33, engine seal-round register P2-24.)
...
**R33 coverage status note (engine seal-round register P2-27 — DISCHARGE-SHAPED):** the canonical config above (the FreeCut 48/45/55/52 thresholds ratchet) is **target-not-current**. **Engine side: coverage LANDED at W3** (`2a0ecf4`; re-keyed R34: the pin `60232ea` — the job's runs GREEN since the R34 variable fix) — `@vitest/coverage-v8@3.2.7` (version-locked to the resolved vitest), v8 provider, include **`src/lib/nle/**` ONLY** (the browser milestone suite owns `src/app` — its venue is measured by nobody in vitest terms), a PARALLEL CI job `coverage (report artifact)` (never a flag on the serial real-DSP vitest job — the 15-min budget), **report-artifact-first: NO thresholds in v1** (per-directory LINES-only ratchets at measured−3 land at W5 from the recorded artifacts, the design §C.2 globs) + **the precedence law in the yaml: coverage SUPPLEMENTS the count/census pins — it never replaces them** (it cannot see oracle quality; its value is the zero-reach module census). **App side: instruments but UNGATED** (`vitest.config.ts` coverage include `src/**`, no thresholds) — a coverage regression still cannot fail any app build. **What remains open (the register's either/or, resolved by registration): the thresholds floor, both repos, at W5** — until then this note is the "target-not-current" annotation so the ratchet is never assumed to exist. (R33, engine seal-round register P2-27 + design-exhaustive-testing §C.)
...
7. **The R28 re-key (this spec's own test plan reflects the round's additions):** §0's BASE/GAP/K-tier rows re-keyed to the R28 pins + the R28 test-law fold's GAP row (the D42-D50 amendments: §5.5's D45 grade-seam parity rows, §6.4's re-key + §6.5's WDC-side retime laws, §8's D42/D46 invariant families, the battery-posture row's R28 class list); the fold's gate is **battery_r28 green with the R28 check classes + the 17 §14.4 step-0 audit green** (every D42-D50 facet carries a matrix/facet row, machine-checked by the step-0 facet-completeness class) — the design law and the test law moved in the same round (per `audits/fleet-r28/test-audit-coverage.md` §5 + `test-law-d48-d50.md` §4-§5).
...
>
> **Note on the R28 re-key (2026-09-16):** this spec's own test law moved
> with the round — §0's pin/GAP/battery-posture re-key, §5.5's grade-seam
> parity rows, §6.4's re-key + §6.5's WDC-side retime laws, §8's D42/D46
> invariant families, §18's fold item above. The meta-tests below are
> unchanged (the infrastructure is round-independent); the round's
> additions are LAW-side rows the battery checks: **battery_r28 is the
> round's gate** (the R28 class list at §0's ACCEPTANCE & TEST PLAN),
> including the suite-census coherence class that checks THIS spec's §0
> BASE rows against 00-master's fleet table (both collector figures
> recorded — a collector difference is a battery-recorded pair, never a
> drift) and the §14.4 step-0 facet-completeness class (every D42-D50
> facet has a matrix/facet row in spec 17).
...
**R33 execution-status register (engine seal-round register P2-26 — each row marked below):** canary → **CARRIED** (the engine runner's WebGPU canary probe, `run-nle-tests.mjs:171-194` — aborts before any milestone runs if the venue is missing); count-snapshot → **CARRIED, now BOTH repos** (engine-side since W1/W2: the count gate `gate-vitest-count.mjs` — the per-file pin table + `PINNED_TOTAL 785`, both drift directions, fail-closed — plus its standing self-tests `gate-vitest-count.test.mjs`, the F-5 close upgraded to standing teeth; app-side: the census register + `census-check.mjs` + the mutation gate; spec-side: the battery's suite-census coherence class); mutation-teeth → **CARRIED app-side** (`census-mutation-gate.mjs` — the 6 doctor classes, clean-passes-AND-every-doctored-copy-fails) + engine-side (the count gate's synthetic self-test fixtures); negative-test-always-fails / self-run-suite-against-itself / flaky-test-detector → **SPEC-ONLY** (the expected-failures project + the runner-swallow negative + the `--repeat-each` flake census are REGISTERED against the engine's runner as the programmatic close of the silent-green class — zero `--repeat-each` anywhere today; mitigation: the suites are deterministic by design, the property tier fixed-seeded at 20260921). (R33, engine seal-round register P2-26.)

- `canary-test-always-passes` — **[CARRIED: the WebGPU canary probe, run-nle-tests.mjs:171-194]** — a test that asserts `1 === 1`. This test
...
- `negative-test-always-fails` — **[SPEC-ONLY — the expected-failures project (the inverted pass/fail project) is the registered programmatic close, not yet built]** — a test that asserts `false === true`.
...
- `self-test-runs-suite-against-itself` — **[SPEC-ONLY — nearest live carriers: the count gate's fail-closed report parse + its self-tests; the self-run negative itself uncarried]** — the entire `tests/unit/12-testing-strategy/`
...
- `flaky-test-detector-bounds-flake-rate` — **[SPEC-ONLY — deterministic-by-design mitigation (fixed seeds; zero `--repeat-each` venues today; the nightly property-explore pass the registered exploration knob)]** — for a known-deterministic
...
- `snapshot-of-meta-suite-pass-count` — **[CARRIED engine-side: `gate-vitest-count.mjs` + its self-tests (W1/W2 — the pin table IS the snapshot, both directions); app-side: the census register + census-check + the mutation gate]** — the meta-suite itself has a known
