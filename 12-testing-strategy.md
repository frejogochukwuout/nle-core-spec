...
- The fleet's suites at the R22 pins (spec 17 §13A.7's re-tier row): nle-engine 356/356 @ `f68ab8c`; OT 459/459 @ `05584d8`; WDC 740/740 @ `fe05d85` (consumed @ `5570321`); nle-ui 640 @ `dba8d52` (the app consumes @ `752991d`); nle-test-app 83/83 @ `e662759`; ui-mock/shell-mini 357 + shell-variants 1,334.

**GAP (the work — owner + phase per spec 14; acceptance in parentheses):**
- LAW-NET-INVENTORY (owner: nle-core-spec, gates pre-C1/pre-C4): the crawl's acceptance corpus distilled from the mini's 357-test net (acceptance: `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` exists + the C1/C4 gates consume it). LANDED at the R23 seal round: the census (129 law-family census units / 8 files / 357 tests) + the disposition totals (33 HOLDS-on-OT tests; 324 app-side authoring tests / 116 units) + the 60-static + 15-templated testid census.
- The crawl app's test authoring — the exact scope is now counted (owner: nle-test-app, phase C4): 324 app-side authoring tests across 116 law-family census units per `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` §2.3-§2.4 (supersedes the earlier ~130-180 law estimate) (acceptance: the inventory's corpus checked row-by-row).
