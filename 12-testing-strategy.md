...
2. **NO dead `linkedTo` after ANY op (I7):** every `linkedTo` on every live element points at a live id — after every op in the sequence (the dead-pointer sweep; the invariant that catches a forgotten prune in any FUTURE verb — the class the curated per-verb tables miss).
