...
- The fleet's suites at the **R28 pins** (the K1 module nets — ALL GREEN; 00-master's fleet table is the pin canon, D21; the battery re-baselines the pin world once, centrally — **battery_r28 re-baselines after the fleet**; consumer pins re-read live — the row's R27 label re-keyed R28): **nle-engine** 749/749 @ `074a2f6` (code anchor `74bef08`) — **25 vitest test files** (the 13 R24 suites + the R25-R28 growth: timeline-linked-source-edit (the E1/E2 13-test pin file), timeline-locked-track, timeline-mode-clamps, timeline-nan-guard, timeline-relink-fixes, upstream-ports (G1, 25), m2-wave1-mixer-surface (M2 Wave 1, 9), compositor-clear, mask-manager-view, audio-codec-ladder, media-registry, stable-video-grouping, the R28 CR-F1 unconditional-connect pin (spec-20's 9→10 re-key — the 749th); bridge-seams **151**, nle-bridge **99**; the vendored OT subset stays CORE-ONLY via the tsconfig/vitest alias) — the app consumes @ `74bef08` (the code anchor, one step behind engine HEAD `074a2f6` — BY DESIGN per R17: the post-anchor commits are engine-repo-only); the engine's OT submodule @ **`6e2b91a`** + WDC @ **`ec8fd5c`**; **opencut-timeline** 632/632 @ `55c81c0` (code pin `970948a`) — **72 milestone entries (M1-M58 + the *R re-runs — the total only per the R27 counting convention; `download/timeline-test-report.json` committed at `970948a` the count authority); the runner `scripts/run-timeline-tests.mjs` = the in-page `window.__TIMELINE_TEST_REPORT__` scrape + the appended real-mouse entries on `/view` — the in-page UI-over-core runner with pixel reads, WYSIWYG state, `window.__VIEW_TEST__`); the app consumes the vendored lock-copy @ `6e2b91a` (byte-exact minus `testing/`; the `970948a` re-pin the filed queue; the engine's OT submodule also `6e2b91a`); **web-daw-core** 777/777 @ `83b8850` (docs-only over `ec8fd5c`; lock `f5011b3` unchanged; the S-series absorbed — the diagnostics/heartbeat law, the rebuilt dsp-effects-worklet, the rt-safety-gate fix; `waveform-contract.test.ts` 14 pins byte-verbatim) — **19 `src/test/` suites, 34 total** (the waveform-contract promotion made src/test 19 at R26; consumers: engine + app both @ `ec8fd5c` — WDC's own docs-wrap never landed, their HANDOFF still says 759); the integration family = audio-integration, dsp-bounce-parity, dsp-effects-integration, dsp-null-unity, dsp-pdc, dsp-sidechain, nle-audio-core-derisk, offline-parity-wiring, real-audio-e2e, meter-tap, varispeed, fft-analyzer (+ the Goertzel detection oracle `lib/daw/dsp/metrics.ts` and the offline render — "the combined test the audio core has done"); the engine consumes @ `494f6ff` (docs-only behind HEAD); **nle-ui** 690 @ `32abd58` — 37 test files (R9-b preservePitch, F4 identity guards, FW-D `setElementFieldAll`, AW1-2 the Gain ceiling; the app vendored @ `83ff8a8`, the AW1-2 absorb queued); **nle-test-app** 252/252 @ `85cff80` — **8 suites: GluedShell 104 / audioService 42 / sceneBridge 41 / deliverService 20 / persistenceService 20 / wire-coverage 9 / engineSeam 7 / waveformPeaks 9** (the R8 D29 real-wiring + the R9/D30 W-A/W-F waves + the RA/RC rounds; census 42 = 36 zero-action + 5 carriers + host @ `6e2b91a`, register-equality CI-enforced); in-repo: **shell-mini 495/12 (the register pin — the sibling's R24-2 wrap re-key, sibling-maintained live-current; census 163 units / 150 GAP / 462 authored; supersedes the R27 read's 441-in-flight / 10 files / 149 census units)** + **shell-variants WRAPPED — 1,939/68 register-declared (the R28 WRAP re-key; the declared pair 1,950 runner-count / 126 stories recorded — the collector-unit reconciliation: a collector difference is a battery-recorded pair, never a drift; supersedes 64 files / 1,796 registered).** **The R28 round statement: the design law and the test law moved in the same round** — the W4 corpus fold (`980920b`, 14 law-side files) + this test-law fold (the GAP register's R28 fold row below) are ONE round; battery_r28 re-baselines the pin world once, centrally (per ARCH-R28 R17), and the suite-census coherence class checks THIS row's figures against 00-master's fleet table (749/632/777/690/252; variants 1,939/68 + the declared pair 1,950/126; mini 495/12).
...
- **K1 — module nets:** each repo's own suite at its own gates (the rows above; this spec's §17 CI configs + spec 17's three-tier methodology are the per-repo shape). ALL GREEN at the R28 pins — the K1 net grew engine 458→748→**749** / OT 536→**632** / WDC 759→**777** / nle-ui 674→**690** / app 174→**252** across R25-R28 (the E1/E2 fix + FW/OV/RC/M2 waves + the R28 CR-F1 unconditional-connect re-pin at the engine; D-ARCH-6 + session-16 at OT; the S-series at WDC; R9-b/F4/FW-D/AW1-2 at nle-ui; the RA/RC rounds at the app) — the gate's test-law citation: §0's BASE rows above equal 00-master's fleet table (**battery_r28's suite-census coherence class** — both collector figures recorded, the runner-vs-line-start reconciliation a RECORDED pair, never a drift).
