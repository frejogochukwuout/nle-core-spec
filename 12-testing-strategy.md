...
**BASE (accepted, pinned 2026-09-07 — the R23 re-pin; the R22 pins superseded):**
...
- The fleet's suites at the **R23 pins** (the K1 module nets — ALL GREEN; 00-master's fleet table is the pin canon, D21; the battery re-baselines the pin world once, centrally; consumer pins re-read live): **nle-engine** 440/440 @ `b8c6f88` — 13 vitest suites (`tests/vitest/engine/`: timeline-edit-ops, video-sync, bridge-seams, planner, api-surface, load-validation, persistence, render-abort, timeline-math, transform-resolver, undo-serialize; `tests/vitest/`: nle-bridge, nle-bridge-realtime-midi; the vendored OT subset stays CORE-ONLY via the tsconfig/vitest alias) — the app consumes @ `4ef0147` (behind, the consumer re-pin wave queued); **opencut-timeline** 489/489 @ `222532c` — 359 in-page + 130 real-mouse across 51 milestone suites (`download/timeline-test-report.json` the count authority; the runner `scripts/run-timeline-tests.mjs` = the in-page `window.__TIMELINE_TEST_REPORT__` scrape on `/` + 14 appended real-mouse suites on `/view` — the in-page UI-over-core runner with pixel reads, WYSIWYG state, `window.__VIEW_TEST__`); the app consumes the vendored mirror @ `a4e971d` (the engine's OT submodule also `a4e971d`); **web-daw-core** 759/759 @ `494f6ff` — 18 `src/test/` suites; the integration family = audio-integration, dsp-bounce-parity, dsp-effects-integration, dsp-null-unity, dsp-pdc, dsp-sidechain, nle-audio-core-derisk, offline-parity-wiring, real-audio-e2e, meter-tap, varispeed, fft-analyzer (+ the Goertzel detection oracle `lib/daw/dsp/metrics.ts` and the offline render — "the combined test the audio core has done"); the engine consumes @ `f446512`; **nle-ui** 648 @ `85dcf57` — 37 test files (consumed @ `85dcf57`, the app at the package HEAD); **nle-test-app** 117/117 @ `70e99f0` — engineSeam / sceneBridge / GluedShell / audioService (the W3 JKL audio half + RR1-B landed); in-repo: **shell-mini 355** (8 test files / 128 census units — the R23 seal) + **shell-variants 1,542-live** (60 test files; the R23 W-A FX + W-B color waves; 00-master's 1,470 is its own pin-era figure — the battery's central re-baseline reconciles).
...
- **K1 — module nets:** each repo's own suite at its own gates (the rows above; this spec's §17 CI configs + spec 17's three-tier methodology are the per-repo shape). ALL GREEN at the R23 pins.
- **K2 — combined nets (cross-module):** the seeds exist at HEAD — engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`: vendored OT ops × engine composition), WDC's integration family (offline render + Goertzel detection), OT's in-page UI-over-core runner (pixel reads + real-mouse + WYSIWYG state). The GAP register's census row formalizes the NAMED family.
- **K3 — app-behavior nets:** the app wiring driven headlessly over REAL modules — the LAW-NET corpus re-expression, the testid-census DOM-structural gate (downstream of the D25 bridge's testid-emission half — the ordering law), the offline audio pins (the Node-venue law), the VLM visual net (adopted from the variants).
- **K4 — the crawl exit gate:** the single automated e2e — import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export; zero mock paths, zero human input.
...
**GAP (the work — owner + phase per the D24 ladder; the R22 C/W/R tags re-keyed via ARCH-R23's old→new mapping table, spec 14 retired; the testing-domain §4 rows re-homed HERE per the posture law; acceptance in parentheses):**
...
- **The K4 automated e2e** (owner: nle-test-app, **K4** — was C4's e2e half; the crawl's completion proof): the app's single named suite — import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export, asserted programmatically (acceptance: green end-to-end, zero mock paths, zero human input; w2's full-scope demo re-gates by extending this suite over the media registry).
- The offline-parity threshold pins + the null rig (owner: app+WDC+engine, **r2** — was W-audio) (acceptance: max deviation ≤ −60 dBFS any channel).
- Grade-math parity pins (owner: app+engine, **r3** — was W-color) (acceptance: the mock's W4 math vs the engine's output on the same fixtures — max delta ≤ 1 LSB-equivalent).
- Real-file decode corpus (owner: nle-engine, **r4** — was W-n5; PENDING the user's D6 re-affirmation, deprioritized) (acceptance: decode round-trip pins on the real-file corpus).
- FCPXML fixture corpus (owner: nle-test-app, **r5** — was R-fcpxml; the phase-entry artifact is the parser + fixture-corpus choice) (acceptance: exports validate vs the chosen reference parser on the corpus).

**ACCEPTANCE & TEST PLAN:** §18 (the meta plan) + the `## Testing` meta-test section (infra regressions caught FIRST per 17 §9.3's CI ordering); spec 17 §13A is the LIVE facet matrix — this spec points there, not to a static table (the sibling fleet round re-baselines its §13A.7 re-tier row to the R23 pins); battery posture: **the R23 suite pins above stay green** (the K1 regression role — battery_r23 keys engine `b8c6f88`/440, OT `222532c`/489, WDC `494f6ff`/759, nle-ui `85dcf57`/648, app `70e99f0`/117 + the consumer pins); the crawl's tier gates: K2 the census + the missing pairs, K3 the corpus row-by-row + the audio pins + the VLM net, K4 the e2e — `IMPLEMENTATION-PLAN.md` §2 is the executable gate table.
