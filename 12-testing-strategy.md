...
**BASE (accepted, pinned 2026-09-08 — the R24 re-pin; the R23 pins superseded):**
...
- **The K3 VLM visual net** (owner: nle-test-app, **K3** — new R23 row: the variants' R23-G tooling, adopted not forked): port `ui-mock/shell-variants/scripts/vlm-capture.mjs` / `vlm-review.mjs` / `vlm-run.sh` (the capture → review → gate pipeline) onto the app's `.storybook` + OT's runner (acceptance: the VLM pass gate executes against both surfaces; the w1 side-by-side and the `/view` gold-sample verification reuse it).
...
- Grade-math parity pins (owner: app+engine, **r3**) (acceptance: the mock's W4 math vs the engine's output on the same fixtures — max delta ≤ 1 LSB-equivalent).
