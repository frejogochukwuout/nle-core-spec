...
- LAW-NET-INVENTORY (owner: nle-core-spec): the crawl's acceptance corpus distilled from the mini's 355-test net. **LANDED at the R23 seal round** — the D24 mapping's pre-C1 status flip: now **K3's acceptance list** (the K3/K4 gates consume it). The census: 128 law-family census units / 8 files / 355 tests; the disposition totals (33 HOLDS-on-OT tests; 322 app-side authoring tests / 115 units); the 59-app-emitted-static + 15-templated testid census (the 60th static id is harness-only — `LAW-NET-INVENTORY.md` §2.2).
