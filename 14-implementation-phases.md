...
| `nle-core-spec` | the canon (this repo) + the two mock/review surfaces (annotakit) | `31d659d`→R22 | — | battery |
| `ui-mock/shell-mini` + `shell-variants` | design references + law registers + live review surfaces; the mini carries the law net (358 tests: Timeline 99 / App 59 / useMini 108 / geometry 58 / MediaPool 19 / timecode 8 / waveform 7) and ~60 `mini-*` testids | (same repo) | — | 358 + 1334 tests |
...
**Pre-C1 (the inventory, spec-side — REQUIRED BEFORE C1 entry):** the **mini law-net inventory** distilled from the mini's 358-test net into the two checklists C1/C4 consume: (a) the *timeline-law subset* (from Timeline 99 + useMini 108 + geometry 58 — every law marked HOLDS-on-OT / GAP-with-owner) BEFORE C1; (b) the *full corpus* (adding App 59 + MediaPool 19 + timecode 8 + waveform 7 + the ~60-testid census) BEFORE C4. Deliverable: `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` (the C1/C4 acceptance lists). Also pre-C1: **spec 18 §16.2's stale R19 drag-law paragraph is corrected to the R18k clamp law** (the R21 user-P0 revert) so C1's executor reads ONE law, not two contradictory ones.
...
| **C4 mini-parity gate** | app + spec | THE gate: the law-net corpus re-expressed as APP-side tests (**the app implements; the spec's LAW-NET-INVENTORY is the acceptance list**); export wiring (engine-side today, absent in app src — the topbar CTA → engine export call); the annotakit review loop config for the app (the strongest reusable mock asset; acceptance: pins created on the crawl app land in the store + mirror); battery; demo | **DEMO at mini scope: import(virtual) → cut (drag/trim/split/ripple) → play → export; zero mock paths; side-by-side final vs the mini mock; the full corpus checked row-by-row.** Law-net arithmetic: 235 raw view/chrome-side mini tests − the OT-covered op-semantics subset = the app-side authoring count (the inventory states the exact number; bounded by ~150-200); every violation the laws expose is FIXED or registered | 3-5 |

**Crawl total: ≈ 10-14 wk solo / 7-9 wk two-dev.** The drag law is the user-P0'd R18k clamp (OT-SEAMS row 3; the tombstoned escape rows re-open only by USER request).
...
- Row 6 (trim ghost edges — view-layer affordance): **app port config, PRE-DECIDED** (OT's `view/` is utils-only — there is no OT React layer to make additive; the ghosts ride the app's port tree).
...
| Trim ghost edges (mini law — view affordance; **app port config, pre-decided**) | app port tree | C1 |
...
| Gap | Where it lands | Phase |
|---|---|---|
| **LAW-NET-INVENTORY.md** (timeline-law subset before C1; full corpus + testid census before C4) | `ui-mock/shell-mini/docs/` | **pre-C1 / pre-C4 (BEFORE the consuming phases)** |
| Spec 18 §16.2 drag-law paragraph corrected to R18k (the R21 user-P0 revert) | spec 18 | pre-C1 |
