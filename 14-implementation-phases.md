...
| `ui-mock/shell-mini` + `shell-variants` | design references + law registers + live review surfaces; the mini carries the law net (**355 tests — the R22 drag-law retirement held (the verbatim R18k law) + the R22-R23 review/seal nets; 8 files**) and 60 static `mini-*` testids + 15 templated families (the census: LAW-NET-INVENTORY §2.2) | (same repo) | — | 355 + 1334 tests |
...
**Pre-C1 (the inventory, spec-side — REQUIRED BEFORE C1 entry):** the **mini law-net inventory** distilled from the mini's 355-test net (post the R22 user directive: the full drag-machinery retirement — the R18k clamp + single-edge live-field magnet + last-preview-seal is the WHOLE drag law; ghosts / commit-at-UP / pending-gesture window / tick freeze / scrub edge auto-scroll are RETIRED) into the two checklists C1/C4 consume: (a) the *timeline-law subset* (every law marked HOLDS-on-OT / GAP-with-owner) BEFORE C1; (b) the *full corpus* (adding the chrome/App/MediaPool/timecode/waveform laws + the ~60-testid census) BEFORE C4. Deliverable: `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` (the C1/C4 acceptance lists). **STATUS: LANDED at the R23 seal round** (the census: 8 files / 355 tests / 128 law-family census units; the disposition totals: 33 HOLDS-on-OT tests, 322 app-side authoring tests / 115 units — §2.3/§2.4 supersede the ~130-180 estimate; the testid census: 60 static + 15 templated families; the split-rule + evidence-citation rules stated in §0). Also pre-C1: **spec 18 §16.2's stale R19 drag-law paragraph is corrected to the R22 law** (the R18k law verbatim — the R21 partial revert + the R22 full retirement) so C1's executor reads ONE law, not two contradictory ones — **DONE at R22 (the §16.2 R22 notice governs; the struck R19 paragraph is retained as marked history)**.
