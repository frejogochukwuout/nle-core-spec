...
**Written:** 2026-09-07 (Round 22, the spec-stream audit round) — **v2: adversarial review round 1 folded (REVIEW-R22-ARCH verdict GO-WITH-AMENDMENTS, 10 amendments + 7 registered risks, §7).**
...
| **C4 mini-parity gate** | app + spec | THE gate: the crawl app passes the mini's law net re-expressed as APP-side tests + battery + demo. **Law-net arithmetic (REVIEW-1 amendment 4): the mini's 358 tests ≈ op-semantics subset (covered by OT's 459) + ~150-200 view/chrome/window/policy laws that OT does NOT cover (drag clamp as app policy, ghosts, gap-fit, window/binding, pending-gesture, zoom ladder, keyboard surface, a11y) — these are WRITTEN app-side, not carried; export wiring (absent in the app's src today — engine-side only) is C4 work** | **DEMO at mini scope: import(virtual) → cut (drag/trim/split/ripple) → play → export; zero mock paths; side-by-side final; the law-net checklist (spec-side inventory, see Ruling E amendment 6) checked off row-by-row** |
...
**Estimates (REVIEW-1 amendment 8, re-baselined):** C0 ≈ 1 wk; C1 ≈ 3-5 wk (fixture bridge + window/binding embedding + qc- restyle itemized); C2 ≈ 1 wk; C3 ≈ 1-1.5 wk; C4 ≈ 2-3 wk (law-net written app-side + export wiring + battery + demo). **Crawl total ≈ 8-11 wk solo / 6-8 wk two-dev.**
...
**Critical path:** C0 → C1 → C2/C3 (∥) → C4 → walk's W-ops/W-media (the demo re-gate at full scope) → W-audio/W-project/W-color (∥ where two-dev) → run. Solo: the chain is serial (8-11 wk crawl); two-dev: S-ot (gap rows + op ports) rides parallel to S-package/S-app through the crawl (the 6-8 wk two-dev track) and cuts the walk by ~4 wk.
