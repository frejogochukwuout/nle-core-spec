...
- **The wire-row leg:** 15 §4.3.22's `AddTrackCommand` carries `'caption'` + `language?` + the corrected six-kind enumeration comment (the stale-`'video'|'audio'|'overlay'` enumeration cannot reappear as the ACTIVE comment — a negative substring **SCOPED to skip the sweep note's own historical quote** at 15:1092-:1094 ("this comment read `'video' | 'audio' | 'overlay'`"): assert the corrected enumeration `'video' | 'text' | 'audio' | 'graphic' | 'effect' | 'caption'` PRESENT and the stale form ABSENT outside the quoted historical context — the unscoped negative false-positives on the landed note's own text).
...
- **(c) the fx density exemption → THE DISCRIMINATOR PIN** (the two-doors-one-key law): with `page==='fx'` → the FULL-Timeline density exemption applies (the ViewOptionsPopover **compact-tracks sub-group's DOM-ABSENCE law** — `queryByTestId('shell-menu-tl-view-options-compact-…')` null for every compact id, the registered surface; the popover itself stays on every page, so the ABSENCE assertion targets the compact sub-group only; the store-level twin `resolveTimelineCompactScope` (`page==='fx'` ⇒ `'off'`, useUiStore.ts:259-261) is the T1 half of the same law. **The originally-named `shell-view-options` testid DOES NOT EXIST in the mock** — the real testid family is `shell-menu-tl-view-options` + the `…-compact-*` sub-group; the nonexistent name is retired from this design); with `fxMode===true` but `page==='edit'` (the FX tool armed on Edit) → NO exemption (the compact sub-group present, the compact entry stays). The pin's law sentence: **the density exemption reads the page key only — `fxMode` is written by both doors but read by neither the density nor any other page-scoped surface** (the one-flag law's read-side half).
...
**Class-census scope, named:** the ten classes below are **the T2-c ten** — NOT the fleet-level census. The fleet's battery_r28 class list is **THIRTEEN** (12 §0's battery-posture row: the five named + T2-c's five + T2-b's D45/D46/r1-protocol classes), and T2-a's `D42-test-law` check (test-law-d42-d44.md §D42.6) is named in NEITHER the plan's ten nor 12's thirteen — one count, one home (12 §0's battery-posture row), the plan cites by reference. A battery author encoding "ten" as the fleet total undercounts by three-plus-one.
...
10. **The signoff-rider freshness class — PER-FILE assertion set** — the two twins carry DIFFERENT payloads, so the class asserts per file, never one shared number set: **TESTABILITY-SIGNOFF.md** carries the R28 re-rider WITH the three re-keyed numbers (31=28+3 + the 78→79→80 projection; the 182-row registry; the 25-code two-tier registry — its R7-era stale-number riders are the reason those figures exist); **FINAL-SIGNOFF.md** carries the four-condition state (condition (1) LANDED incl. the T3 test-law fold; (3) MET 0-open) + the **D8-D50** decisions-corpus census + the certification clause (test-law completeness) — FINAL has NO stale-number riders to re-key, so asserting the number set there would false-fail. The re-stamp's precondition is machine-checked per file.
