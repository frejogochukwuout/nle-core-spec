...
**Round:** R25 (2026-09-09). **Status:** v1 → adversarial review (2 fresh-context reviewers: design-consistency 15 findings — 4 P1 / 6 P2 / 5 P3, GO-WITH-AMENDMENTS; corpus-faithfulness ~50 claims re-derived live — 5 fold-ins + 6 precision fixes, GO-WITH-AMENDMENTS) → **v2 (all folded)**.
...
- **30.3 — the matrix is a ladder gate (the satisfiable form):** at **K2**, every matrix row is either LANDED (ported + pinned where r1 has landed it by then) or carries a live r1-scheduled row with owner + acceptance — **zero orphan rows** (the completeness is over the REGISTER, not over landed-ness — the ports land at r1 per D30.2, and K2 checks the register, not pre-dated landings); at **K3**, the app column matches the LANDED set with pins. The gate's machine check = the per-mode pinned tests exist at the named files (the mode reports' acceptance rows) for every LANDED row, and every non-LANDED row resolves to a live scheduled row — not prose.
...
- **31.1 — the two-semantics split is named law (and taught EVERYWHERE it appears):** **placement-insert** (the drag-drop/pool insert: OT's `insertElements` — reject-not-shift, the main-track zero-anchor, F1A-3 homogeneous batches) and **insert-edit** (the DaVinci source-mode splice: split-at-playhead + intrinsic push-down + multi-clip sequential) are DISTINCT operations with DISTINCT homes. Placement-insert is LANDED (OT, canonical). Insert-edit is the engine's 3-point family (`performInsertEdit` :4702) porting to OT's ops layer at r1 per Decision 12.3. 06 §5.9 states both + the boundary sentence: "the placement surface places (rejecting or displacing per strategy); the source-edit surface splices (splitting and pushing)." **The same teaching lands at every ripple-flag site: 15 §4.3.9's `insert{ripple}` param and 15:4901's r1 row re-key to the D31.6 composite law; 16:183's Option+R text gains the precision note (the flag routes delete + the placement surface's displacement — the source-edit family's push is intrinsic, never flag-routed).**
...
- **31.3 — §5.9B OVERWRITE EDIT gains its prose:** the covered-clip law from the engine's verified implementation: fully-contained → removed; straddle-both → split ×2, middle removed, left+right remnants survive; head/tail straddle → trimmed remnant; zero downstream movement; transitions referencing removed clips drop; ONE atomic undo. The phantom `placement:'overwrite'` rows (06:3110, 06:3175) re-key to this family; **07:367 is NOT one of them** — it is the ElementTransition params-keys patch-semantics naming collision (P8's hazard note stands).
...
- **31.6 — RIPPLE OVERWRITE (new family §5.9E):** the delta law: the target is fully replaced; the source's own duration stands (NO out-adjustment — that is replace's law); downstream shifts by `newDuration − oldDuration` (push if positive, PULL if negative — pull leaves no gap); upstream untouched; one atomic undo; **the target's transitions drop (the replacement breaks both edges — the incoming clip enters bare, the same law performOverwriteEdit applies to removed clips); the zero-floor law: the delta never shortens any downstream clip below its minimum duration; the companion law per D32.1's ripple-overwrite row.** **The classic §12 diff is asymmetric — it produces the PULL exactly but cannot produce the PUSH (freed = ∅, the intermediate state is overlap-rejected)** — so the composite is `delete + move + insert` (three routed verbs, applyBatch, one undo — F1A-2's intra-batch overlap guard passes because the trajectory is delete-then-move-then-insert, never overlapping), NOT "overwrite + the ripple flag". 06:1509's "ripple-insert via the diff" row and :3120/:3175's phantom test rows are corrected by this law.
...
### Decision 31A — the dedicated-vs-composite criterion (one stated rule, not ad hoc)

A new family gets a **DEDICATED op** when its semantics cannot be expressed as a guarded composition of LANDED verbs without breaking atomicity / transition / keyframe / companion invariants — replace (the transition-remap + sever semantics need single-op atomicity). A family stays a **COMPOSITE** when a batch of landed verbs already preserves the invariants — append (`insertElements` relative-offset batch), ripple-overwrite (delete+move+insert), fit-to-fill (insert+retime). The criterion is stated in 06 §5.9's preamble so the next family proposal derives the same split.
...
- **32.1 — the fan-out table (ALL TEN modes):** 06 §5.0 carries the mode × situation matrix: linked pair present / transitions at edges / sync-locked tracks. Per-mode: slip/slide/roll/ripple-trim — the pair rides the SAME op (one history entry); insert — the pair splits at the same frame, each half re-pairs, the right half shifts (the left-half-never-moves law of 31.2); overwrite — the pair's coverage mirrors the covered-clip law (removed → cascade; trimmed survivor → the companion trims to match); replace — SEVERS (31.4); append — the linked-append law (31.5); fit-to-fill — the pair takes the SAME rate; **ripple-overwrite — the removed set's companions cascade (the removeItems law), the move set applies to BOTH tracks' linked sets at the same delta (the A/V pair moves together), the incoming clip enters unlinked (the replacement severs, 31.4's law).**
...
- **32.4 — the split-link ruling (the conflict stated, then decided):** 06 §5.1's quoted FreeCut split code severs the link on a single-side split; the engine relinks both halves. **Relink-both-halves is the law** (Resolve-faithful: a split clip's halves both stay linked to the companion; the engine implements this; representable in 09's pairwise `linkedTo` as both halves carrying the link). The 06 §5.1 quote is REFERENCE-CENSUS (the R15-era FreeCut reference), not law — the amendment adds the explicit note row. The nle-ui/mini R14 right-half-sever is a registered divergence — the D26 carrier-reduction program's reconciliation row, not an immediate change.
...
- **34.1 — the primary chords are source-mode-gated:** `,` insert-edit / `.` overwrite-edit / **`⇧⌥.` (Shift+Option+period) ripple-overwrite** (the plain `Shift+.` is TAKEN — 16:235 slip-10-frames / :283 nudge-10; every conflict resolved explicitly per 16 §6) / `E` append (Premiere/FCP grammar; the C46 gate; zero live risk — no shipped shell has a source viewer yet). The F9-F12 block lands as documented DaVinci-parity ALTERNATES (F9 insert / F10 overwrite / F11 replace / F12 append) with the preventDefault + browser-hostage law (F11 fullscreen; F1 help; macOS fn-defaults). **`Shift+F10` is NOT available** (18 §4.9 owns it as the context-menu route — the ripple-overwrite alternate is F10 within the documented family, no shift-chord).
- **34.2 — the `,`/`.` context-split is ratified as written law** (it resolves via 16's own multi-descriptor law + §6.2's order: source-mode (panel context) > slip tool (tool context)): 16 §3.4A gains the descriptors; §6.1's row 2 becomes the four-meaning row (source-mode insert/overwrite | slip-by-frame | slide-by-frame | plain nudge) — **AND §6.1's `Shift+`/`Option+` split families (slip-10 / nudge-10) gain their resolution rows too (the ⇧⌥. chord sits in the same table) — no conflict left implicit.**
...
- **35.1 — `REFERENCE-REGISTER.md` at the spec root** (cross-spec ownership — not 18 §0): one row per mock surface family: `surface | mock home (component+store) | test pins (count) | owning spec | citation form | divergence rule`. Seeded with the 19 censused families (11 aligned / 3 diverging / 8 with mock-only content — overlapping classes per the §2.4 census note).
...
- **36.1 — S-app re-written to the post-D30 state:** the landed waves fold to history; the live rows become: the app's `.agents/` bootstrap (OVERDUE ×2 — before any further app work), the **census-CI mechanism** (D26.2 claims "CI-enforced register-equality" but the app's ci.yml runs no census check — the law now carries its mechanism requirement as a **QUEUE FILING into the app's repo** per the cross-repo protocol: a titled block in the app's queue specifying a `scripts/census-check.ts` step in ci.yml, owner S-app, gate = the register equality fails the build; the spec round FILES it, the app session executes it), K3's store/policy halves (unblocked, unstarted), the W-G filing reconciliation (below).
- **36.2 — the W-G filing reconciliation (a QUEUE FILING into the engine's repo, this round):** the engine's R9 queue (3989506) carries 3-of-4 stale dispositions that predate ARCH-R24's F1 amendments — re-filed as a titled amendment block in the engine's queue: R9-a transform-sidecars → FUTURE-EXTENSION; R9-b maintainPitch → LANDED engine-side (the residual is the app's engineService hardcode — re-filed app-side); R9-c scene-grade → DECLINED-BY-LAW (not filed); R9-d preview-widening → aligned, stands. **E1/E2/E3 (the defect rows) are likewise QUEUE FILINGS into the engine's queue doc (titled blocks with the fix contracts + the pin requirements) — the spec round files, the engine session executes.**
- **36.3 — the WDC waveform filing re-venues:** the R24 promotion was **never executed in WDC** — it stayed spec-side only (no filing ever landed in any WDC venue; R24's own docs named the correct target). The actual venue is the **repo-ROOT `HANDOFF.md`** (WDC's `.agents/` holds only SKILL.md) — the promotion files THIS round as a titled block at the root HANDOFF with the de-facto contract citation (audio-registry.ts:24-74). Second round this filing has missed; the plan's S-wdc row carries the corrected venue + a filing-verification gate (the row is not done until the target repo's queue SHOWS it).
- **36.4 — OT's R9 seam queue absorbs into S-ot** (the mapping: R9-a already spec-side per 15 §13.15; R9-c's hunks fold into the carrier-reduction program; **the W8 `!shiftKey` playhead-guard hole is STILL OPEN OT-side** — only the app-side Z4 twin + the hole-documentation landed at fdb771c (a PLAN-only commit); the OT-side fix stays a live row; WIRE_UI_EXCEPTIONS-export is a new small row). **The still-open nle-ui filings (the C0 row, the D29 package-side rows, the carrier-reduction work order) stay in the plan's S-package row — not dropped.**
- **36.5 — the mode-matrix ladder rows enter the plan** (per 30.3): the K2 zero-orphan-rows gate + the K3 landed-set pins + the per-track routing of the fleet's gap rows (~70-80 rows by table class; the amendment fleet consolidates them into the owning specs' registers — S-spec authoring THIS round; S-engine the defect-fix + mode-exposure filings pre-r1; S-ot the r1 port families; S-app the K3 pins).
...
| P11 | 15 §4.3.9 + 15:4901 + 16:183 | The insert{ripple} param + the r1 row + the Option+R text still teach the flag-push path D31.6 kills | Re-key all three to the composite law + the precision note (D31.1's everywhere-clause) |
...
4. **The cross-repo filings THIS round executes** (per the filing protocol — titled blocks in the target repos' queue docs): the WDC root-HANDOFF waveform promotion (D36.3) + the engine R9 re-file + the E1/E2/E3 defect filings + the app's census-CI mechanism block (D36.1).
5. **The integration review + the wrap.**
