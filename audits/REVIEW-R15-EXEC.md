...
**Task ID:** R15-PR2b · **Date:** 2026-09-06 · **Reviewer:** same senior-engineer execution reviewer (fresh verification pass, not memory): re-read ARCH-R15 v2 in full; re-verified in-repo the claims v2 newly added (event-surface citations, mock-CI claim, OT playhead mechanism, dropped framing, arithmetic closure of the phase table).

### Per-finding verdict table

| Finding (this review, v1) | v2 disposition | Verdict |
|---|---|---|
| **C1** corrected bottom-up calendar | §3.4 adopted verbatim: week −1 (0.5-1, both spikes as GATES), A0=1, A1=4-5, A2=5-6 with my sub-item decomposition (C7 0.5-1 / façade ~1 / op wave 3-4 / refresh 0.5), A3=5-6 with sub-items incl. `/dev/view-fixture` 0.5-1, A4-v1/v2 split, A5=1.5-2, A6=2-3 ("pure greenfield — zero engine refs"), A7a/A7b split; totals 22-27 solo / 13-16 two-dev; demo figures; serial-bottleneck + two-dev-decay notes all present | **LANDED** (one arithmetic-consistency caveat → N1 below) |
| **C2** S3 relabel + fixture cost + drift | §3.2 S3(a) = "view-component verification in app context … NOT app-path integration"; (c) = command-capture "THE app-path integration surface"; fixture line-itemed in A3 (0.5-1); risk #5 = "fixture maintained, not assumed free" | **LANDED** |
| **C3** config as A0 artifact + spikes as gates + HMR in A0 exit gate | §2.1: "REVIEW-R15-EXEC §2.2 — adopted verbatim as the A0 artifact" (insertion-ordered regex aliases, reserved namespaces, engine strictness, TS-major-by-spike, dedupe + never-install-in-vendor); A0 exit gate = "TimelineView renders in-app + HMR round-trip on a vendored file"; week −1 has both spikes as gates | **LANDED** |
| **M1** mock-port re-scope | S3(b) ~200 view-state/chrome "ported near-verbatim"; (c) ~60 capture "replaces the mock's doc-slice tests, which do not survive the store swap"; "half of 596" — grep: **zero hits** in v2 | **LANDED** |
| **M2** A4-v1/v2 split | A4-v1 (1-2, offline parity + behavioral pins) / A4-v2 (~1, later-parallel, "net-new … CR-A #6 blocks the cheap route"); S4 row says the same | **LANDED** |
| **M3** A7a/A7b split | A7a app polish (1-1.5) / A7b engine P2 (2-2.5, "ENGINE-repo items under engine gates, app pin bumps, slips independently", scopes ~1 + secondary qualifier ~1-1.5) | **LANDED** |
| **M4** CI specifics | §2.5 "HEAD-follow opens a bump PR … never pushes main" (write credential distinct from read PAT); §3.2 CI = engine ci.yml:39-53 recipe verbatim (persist-credentials:false + ::add-mask::), Xvfb:99 + SwiftShader flag triplet copied verbatim, "measured at A0, not asserted", fast lane "measured", "PAT runbook written before first rotation (read PAT spans 3 repos)" | **LANDED** |
| **M5** CSS law | §2.1 CSS law: OT globals.css FIRST / app+Tailwind SECOND, 3-5 screenshot-parity rows, OT view.css/runner.css split request; risk #4 | **LANDED** |
| **Minors** (m1-m8 + week-(-1) items) | A0=1 ✓; env-var dual-set "sets BOTH TIMELINE_VIEW_URL and TIMELINE_TEST_VIEW_URL (m17 vs the other 12 — verified inconsistency) or fixes m17 in OT (one line)" ✓; app-side wrapper "imports the 13 phase modules directly … keeps artifacts OUT of the submodule tree" ✓; worklet copy at A0 + root-base law ✓; FCPXML parser at week −1 + A6 ✓; never-install-in-vendor + dedupe + exact mediabunny 1.50.8 ✓; engine cosmetic folds ✓; SCOUT docs as onboarding + W8-f/W9 panel booking ✓; test-attr dual-selector ✓; error-envelope-now-not-later ✓; state-boundary contract (view-state-only store, selectors over `core.getScene()`) ✓ | **LANDED** |

### New-defect scan of the v2 rewrite

Checked for new execution-plan defects; the v2's *new* material was verified against code, not taken on trust:

- **v2's new citations are accurate.** `orchestrator.ts:111` `onProgress?: (progress: NleRenderProgress) => void` — exact. `realtime-engine.ts:44-58` masterAnalyser/masterMeter wiring — exact. §0.1's "mock has no CI workflow" — TRUE (no `.github/workflows` in `ui-mock/shell-variants`). `player.ts:587-608` — verified in my v1 pass.
- **The routing-disposition table (§2.3) does NOT un-do the calendar.** It is bounded and honestly costed (0.5 wk inside A2's 5-6: 78-row spec-15 table + a typed `NOT_IMPLEMENTED` code + S5 battery check), and its DEFERRED-members approach *strengthens* A2's exit gate — "dispatch-complete + NOT_IMPLEMENTED honest" is a better gate than v1's ambiguous "routes 100% of implemented union". The ~20+ DEFERRED rows each citing a phase is a standing S5 obligation, not new serial work.
- **Engine-home projector (§2.2) does NOT damage A1's estimate handling — it improves it.** A1 keeps 4-5 wk; the change moves the work into the engine repo under D9's additive rule, which is estimate-*positive*: the parity gates now run in engine CI's **proven** 8-min real-WebGPU venue instead of my assumed Vite+SwiftShader app harness (the app's own S4 nightly still needs SwiftShader — unchanged), the in-repo consumer re-point list (19 headless ops, `editProject`, `buildTimelineData`, `player.ts:3092`, persistence, the 265-row runner, 7 vitest suites) justifies the estimate rather than inflating it, and the two-dev split survives (A1 engine-repo ∥ A2 OT-repo+spec). The re-typed "retirement = permanent internal test substrate" is more honest than v1's deletion promise.
- **N1 (MINOR — editorial, required on landing): the headline calendar numbers were inherited, not re-derived, after v2 added A2.5.** The phase table is now internally inconsistent at two points: (a) the demo figure "≈ 11-13 wk solo" is underivable from the table (week −1 + A0 + A1 + A2 + A3 sums to 15.5-19; it reaches 11-13 only under an unstated reduced-scope reading — the demo needs the translator + C7 + façade, NOT op wave 1 or the full parity corpus, which is true but never said); (b) the totals "22-27" close only if A7b (2-2.5), week −1 (0.5-1), and A4-v2 (~1) sit outside the serial path — defensible (A7b "slips independently") but unstated; the stated-inclusive sum is ~24.5-32. **Full disclosure: this is partly MY defect** — the demo figure was already underivable from my own §5 rows (they summed 15.5-19 to A3); the 11-13 came from an implicit reduced-scope reading I never wrote down, and v2 faithfully copied it. Fix is two sentences: state the demo's dependency scope (A1-translator + C7 + façade, op wave 1 explicitly after the demo), and state what the totals exclude; or restate the totals serial-inclusive (~24-31).
- **N2 (MINOR — editorial, required on landing): the §2.3bis "imperative playhead API" is real, needed, and NOT line-itemed.** Verified: OT's view playhead renders from `core.getCurrentTime()` (`TimelinePlayhead.tsx:50`), so under the (correct) playhead-ownership ruling — engine Clock is truth during playback, no per-frame `core.seek` — **the OT view's playhead would be frozen during playback** unless a NEW in-direction OT surface exists (imperative playhead handle, or an external time-source prop). §2.7's punch list line-items only the OUT-direction (`onViewStateChange`, ~0.5d) and A3's "view mount" sub-item doesn't mention the in-direction surface. Fix: one punch-list row (~0.5d additive OT prop) folded into A3's view-mount sub-item.
- Checked-and-clean: A2 sub-items sum 5-6.5 against a 5-6 phase (rounding, fine); A3 sub-items 4.5-7 against 5-6 (fine); mock "ported, not vendored" is consistent with the 3-submodule topology and my §2.2 config; the §2.5 pin-lockset rule (from PR1) is sound and subsumes my m5 double-vendoring disposition; §3.1's "mock retires as a repo after A7" is consistent with §4's "UX review surface through A7".

### Final verdict

All eight mandate points LANDED; the two new defects are one-line editorial corrections (N1: state the demo's dependency scope + totals-exclusion reading; N2: line-item the imperative-playhead OT surface) that change no scope, no structure, and no feasibility — fold both into the landing commit of the ARCH-R15/00-master/14 set.
