...
- **the K3 VLM visual net** (new R23 row; owner: nle-test-app): adopt the variants' R23-G tooling — `vlm-capture.mjs`/`vlm-review.mjs`/`vlm-run.sh` — onto the app's `.storybook` + OT's runner, adopted not forked (acceptance: the VLM pass gate executes against both surfaces; the w1 side-by-side and the `/view` gold-sample verification reuse it);
