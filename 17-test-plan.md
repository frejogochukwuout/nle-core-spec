...
**Status:** v1.6 (Round 28 — the test-law fold: **the design law and the test law moved in the same round** — D42-D50 were ruled at the W4 corpus fold (`980920b`) AND their test law lands THIS round: §3.1's eight new matrix rows (linked-companion ops · edit-domains · the two-tier error envelope · the NS-4 render-plan projector · the four absent edit families · the param trio · markers v2 · caption tracks) + §13A.7's D42-D50 facet rows + the protocol row + the NEW §13A.8 (the r1-port acceptance protocol) + §2.5 rule-8's two-tier census re-key + §13A.4.1's union re-key + §13A.5's NS-4 extension, per `audits/fleet-r28/test-audit-coverage.md` §5 + the three test-law designs (d42-d44 / d45-d47 / d48-d50) — the R27 narrative below is lineage); **the wire census figures re-keyed (R28/D47 + D46-B5): the union projection 78 → 79 → 80** — `insertBatch` the 79th member (minted at the r1 fold), `replace` the 80th (no counterpart until `timeline.replace` lands) — **and the counterparts 29 → 31 → 32-of-80** (29 + `retimeKeyframes`' replacement slot + `insertBatch`'s own 79th-member slot = 31 of the post-bump 79-member union; 32 of 80 once `replace` lands; the ruled r1-fold projection per spec 15 §0-BASE/:5021/:5023/:5025 — the landed census stays 31 = 28 routed + 3 exceptions until the bump re-declares mechanically per D29-F8). v1.5 (Round 27 — the fleet re-pin + the instrument flips (the R24 narrative below is lineage): **engine 748/748 @ `f9ac806`** (code anchor `50b91f5`; 25 vitest test files — the 13 R24 suites + audio-codec-ladder, compositor-clear, mask-manager-view, media-registry, stable-video-grouping, timeline-linked-source-edit (the E1/E2 13-test pin file), timeline-locked-track, timeline-mode-clamps, timeline-nan-guard, timeline-relink-fixes, upstream-ports (G1, 25), m2-wave1-mixer-surface (M2 Wave 1, 9); bridge-seams 151, nle-bridge 99; tsc 0, fence 52, probes 9/9; the OV-12 vendored-tree re-pin gate in CI) — consumes OT @ `6e2b91a` + WDC @ `ec8fd5c`; **OT 632/632 @ `55c81c0` / code `970948a`** — **the total only** (72 milestone entries, M50-M58 + the *R re-runs since R24; `download/timeline-test-report.json` the authority; **WIRE_COMMAND_TYPES = 31 = 28 routed + 3 exceptions** — selectElements/advancePlayhead/trim — M49C machine-checked 28/3, origin-attributed; the M58/M58R batch-verb pin family: insertBatch/removeKeyframes/retimeKeyframes, one history entry each, the singular remove/retime verbs RETIRED — the R28/D47.3 correction: `insert` stays singular, neither deprecated nor aliased); **WDC 777/777 @ `ec8fd5c`** — 19 `src/test/` suites + the colocated family (34 `.test.ts` total; the S-series upstream wave: the diagnostics EngineDiagnostic bus + heartbeat monitor, the rebuilt dsp-effects-worklet, the rt-safety-gate heritage fix; `waveform-contract.test.ts` 14 pins byte-verbatim); **nle-ui 690/690 @ `32abd58`** (AW1-2 the Gain ceiling +4→+20 — the [−60,+20] fleet domain; F4 identity guards; FW-D `setElementFieldAll` @ `83ff8a8`); **app 252/252 @ `c020b2a` — the 8-suite roof** (GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + wire-coverage 9 + engineSeam 7 + waveformPeaks 9; the AR-2 gesture pin + the LIVE-registry law; the census register 42 = 36 zero-action + 5 carriers + 1 host @ `6e2b91a`; the census-CI mutation gate clean+M1-M6b); **mini 441 in-flight (R24-2 W3; census 149/136/408 — REGISTER-PENDING, re-keys at the sibling's wrap)**; **variants 1,733/64 at the R24-wrap `3db130f`, higher and moving (REGISTER-PENDING — re-key at the sibling's round-wrap only, per the coordination law)**; the K3 instrument rows below flipped LANDED (the coverage gate; the audio pins @ `44e8049`); battery_r24 → **battery_r27**. Round 24 — the per-file audit fleet's re-verify against the R24 pin world (ARCH-R24 v2, D26-D29; the R24 pin set: engine 458 @ `5036387`, OT 536 @ `ded43c4`/`c15a629`, WDC 759 @ `85b81b0`, nle-ui 674 @ `fc4cc35`, app 174 @ `c885ece` — the then-6-suite roof; mini 355; variants 1521+ (61 files) — all superseded by the R27 set above): the D29 instruments registered in the ladder — M49C = K2's OT-side completeness instrument, the D30 W-F coverage-gate port = K3's (in flight), K4 drives the routed verbs; the K3 DOM-structural half re-keyed on the D26.5 three-way data-test mapping row; the shell-path blind spot + the compensating trusted-integrator pin family registered; the app's 9-A2 zero-no-op census (177 controls: ~127 real / 13 degraded / 21 honest-mock / 16 no-op incl. 9 unexpected-dead) + its 26-pin plan + Z1-Z7 registered as K3's app-side worklist rows (ALL PENDING — zero landed); battery_r23 → battery_r24; the dual-tag window CLOSED (D24 K/w/r set only). v1.4 (Round 23 — the per-file audit fleet's re-verify against the live repos: the R22 counts re-pinned to the R23 HEADs (**engine 440/440 @ `b8c6f88` — 13 suites; OT 489/489 @ `222532c` (359 in-page + 130 real-mouse, the report json the authority); WDC 759/759 @ `494f6ff`; nle-ui 648 @ `85dcf57`; app 117/117 @ `70e99f0`; mini 355 sealed; variants 1,542-live**); the phase vocabulary re-keyed per ARCH-R23 D24 (C1/C4→K3/K4, W-*→r1-r4, R-fcpxml→r5 — dual-tagged through the fleet window); the K-tier map (K1-K4) added at §0A + the K2 combined-family census row added to the §0A GAP register (spec 12's twin — the S-spec deliverable at K2 entry); the N2b bridge/parity pins + the W2 soundtouch/varispeed + the VLM net + the seal artifacts reflected; battery_r22 → battery_r23; the plan is `IMPLEMENTATION-PLAN.md`, spec 14 retired to the redirect stub. v1.3 (Round 22 — the finality round: §0A the acceptance-executability law (the posture law's test-side half: every BASE row carries its regression role — suite + count + pin; every GAP row carries its acceptance gate + facet row); §13A.7 + §17A + §18.5 re-baselined to the 2026-09-07 fleet counts (engine 356; OT 459 incl. 130 real-mouse; WDC 740; nle-ui 640; app 83; mini 355 (current-tracked — the R23 seal census; 333 at the R22 re-baseline); variants 1334) + the crawl's law-net-corpus gate family (LAW-NET-INVENTORY, C1/C4) + the app-promotion (nle-test-app IS the roof). v1.2 (Round 15 — §17A added: the app-tier roof suites (Decision 17 / ARCH-R15 §3 — S1-S5 table, regression-continuity law, CI composition, pin-lockset assertion); §13A.7 re-tier row re-baselined to the R15 verified counts; §13A.5 audio-seam citation corrected post-M1.6; §18.5 header re-baselined. v1.1 (Round 8 — §13A added: facet coverage matrix, NFR verification recipes, a11y spot suite, absorbed-semantics fixtures, wire-protocol conformance suite, seam property tests, UI-shell v1.1 facet tests; §2.5 gains the error-path-census + NFR-recipe rules). Supersedes the methodology portions of `12-testing-strategy.md`
...
1. **BASE rows state their regression role.** Every accepted-code row in every spec's §0 carries: the pin (repo@SHA), the suite that pins it, and the count at that pin (e.g. "OT @ `ded43c4` (code pin `c15a629`), 536/536 — 386 in-page + 150 real-mouse; `download/timeline-test-report.json` the count authority"). The suite is the acceptance EXECUTION for existing code — the spec does not re-describe what the suite already proves.
2. **GAP rows state their acceptance gate.** Every remaining-work row carries a testable gate (the phase exit in the plan — `IMPLEMENTATION-PLAN.md` §2's K/w/r ladder; spec 14 is retired, its "spec 14 §…" citations resolve via the stub's §-redirect table) + a facet row here (§13A) — a gap row with neither is a spec bug.
3. **The crawl's law-net corpus is a first-class gate family:** `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` (the corpus-phase deliverable — **LANDED at the R23 seal**) is the acceptance list the crawl app's re-expressed tests must satisfy: the census (10 files / 495 tests / 150 law-family census units — 33 HOLDS-on-OT tests / **412 app-side authoring tests across 137 GAP units**; the 59-app-emitted-static + 15-templated testid census, the 60th static id harness-only — §2.2) + `CORE-SEAMS.md` (the S1-S26 seam inventory + the store partition + the transport map — K3's re-expression worklist). Per D24's split, the corpus exit is **K3** (the corpus checked row-by-row over REAL modules — mini-parity, ARCH-R22 Ruling B) + **K4** (the automated-e2e completion proof) + w1-entry (the human side-by-side).
4. **Count discipline is battery-enforced (battery_r24 — the battery_r23 successor):** declared == actual at the current pins; stale counts are failures (the R15→R22 drift — engine 274+265+318→356, OT 423→459, WDC 721→740, mock 596→640+333+1334+83 — and the R22→R23 drift — engine 356→440, OT 459→489, WDC 740→759, nle-ui 640→648, app 83→117, variants 1334→1542-live — and the R23→R24 drift — engine 440→458, OT 489→536, nle-ui 648→674, app 117→174, variants 1542-live→1521+ (WDC held 759 @ `85b81b0`, docs-only) — are exactly the drift class the battery catches; the count-consistency check keys engine 458 + OT 536 across 17+19+00).

**The R28 posture (Round 28 — the test-law fold, 2026-09-16):** the W4 corpus fold ruled D42-D50 law-side and touched ZERO test-side files — by rule 2 above and §14.4 step 0's own law, every D42-D50 facet was a spec bug (`audits/fleet-r28/test-audit-coverage.md` §1.1/§2). This round's fold closes the lag IN the round it was found: **the design law and the test law moved in the same round.** Landed here: §3.1's eight new matrix rows (linked-companion ops · edit-domains · the two-tier error envelope · the NS-4 render-plan projector · the four absent edit families · the param trio · markers v2 · caption tracks) + §13A.7's D42-D50 facet rows + the protocol row + the NEW §13A.8 (the r1-port acceptance protocol — the consolidated r1 recipe) + §2.5 rule-8's / §13A.4.2's / §13A.1's two-tier census re-key + §13A.4.1's union re-key + §13A.5's NS-4 extension. **The wire census figures re-keyed with them (R28/D47 + D46-B5): the union projection 78 → 79 → 80** — `insertBatch` the 79th member (minted at the r1 fold), `replace` the 80th (no counterpart until `timeline.replace` lands) — **and the counterparts 29 → 31 → 32-of-80** (29 + `retimeKeyframes`' replacement slot + `insertBatch`'s own 79th-member slot = 31 of the post-bump 79-member union; 32 of 80 once `replace` lands; the ruled r1-fold projection per spec 15 §0-BASE/:5021/:5023/:5025 — the landed census stays 31 = 28 routed + 3 exceptions until the bump re-declares mechanically per D29-F8; battery_r28's D47 class machine-checks the arithmetic against the convention re-derived from the spec text).

**BASE (accepted, pinned 2026-09-08 — the R24 re-pin, re-keyed R27, re-keyed R29; 00-master's fleet table is the pin canon, D21):** the three-tier methodology + matrix + template (§2-§11); the facet matrix §13A; the roof-suite law §17A — all EXECUTING (the **K1 module nets** are the proof, ALL GREEN at the R29 pins: **engine 749/749 @ `e3f55bd`** (code anchor `74bef08` — the R29 re-pin chore-only, the engine's own code UNCHANGED; the live-run venue is GHA CI) — **25 vitest test files** (the 13 R24 suites + the R25-R27 growth: timeline-linked-source-edit (the E1/E2 13-test pin file, R26 `8188d4e`), timeline-locked-track, timeline-mode-clamps, timeline-nan-guard, timeline-relink-fixes, **upstream-ports (G1 @ `3a28b6e`, 25 tests)**, **m2-wave1-mixer-surface (M2 Wave 1 @ `50b91f5`, 9 pins)**, compositor-clear, mask-manager-view, audio-codec-ladder, media-registry, stable-video-grouping; **bridge-seams now 151, nle-bridge now 99**; **N2b LANDED @ `37cdd28`** — 18 bridge pins + 2 REAL offline-render sample-level parity pins; the RR1-A fade-clamp SPAN round; the 455-name API freeze) — **R33 re-pin: engine 785/785 @ `2a0ecf4` (29 vitest files — the exhaustive-testing waves, engine seal-round register P2-1: the W2 property tier [33 sites, the 2 real catches P-F1/P-F2] + the W1 count gate [PINNED_TOTAL 785, both directions, fail-closed, + standing self-tests] + the W3 coverage job [report-artifact-first] + the W4 `perf.yml` nightly [the fleet's first scheduled workflow]; the code anchor moves `74bef08`→`2a0ecf4` — the P-F1/P-F2 fixes; 01 §0's R33 layer the full record; the app's engine vendor re-keyed `e3f55bd`→`574d8d3` at the app's own lockstep wave `7664603`, the W0-W4 landings engine-repo-only beyond it; app/OT/WDC/nle-ui pins UNCHANGED by this wave)**; **OT 632/632 @ `3e18722` (code pin `970948a` — the trailing seal18 W1+W2 reviews/docs only)** — **the total only** (72 milestone entries, M50-M58 + M51R/M55R/M57R/M58R; `download/timeline-test-report.json` the count authority; the T-round + W10F hardening + the W11 complete-UI round grown — **28 routed wire verbs + 3 documented exceptions**, machine-checked by the M49C coverage gate 28/3, origin-attributed; the canonical React timeline UI tree per D25/D26); **WDC 777/777 @ `94f6460`** (docs-only over `ec8fd5c` — the ENG-2 S-series docs wrap; the S-series upstream wave: the diagnostics/heartbeat law, the rebuilt dsp-effects-worklet, the rt-safety-gate heritage fix) — **19 `src/test/` suites + the colocated family (34 total)** (the integration family + the Goertzel detection oracle; **W2 soundtouch/varispeed LANDED**; the drift-gate now fails-on-drift); **nle-ui 691 @ `6754979`** (F3/F7 real-gain + the kind-aware mute-all pin + the D29 seam waves — the saveState lifecycle, the DeliverPage `exportRequest`, the per-scene grade sidecar; AW1-2 the Gain ceiling + AW1-4 the D-ARCH-6 pool seam + ENG-1; boundary-script-gated); **app 256/256 @ `876f2b8`** (the **8-suite roof**: GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + **wire-coverage 13** + engineSeam 7 + **waveformPeaks 9**; the R8 D29 real-wiring wave — autosave/persistence, sidechain ducking, deliver export, effects/color preview — plus RR1-B and the W3 JKL audio half standing; the R9/R25-R27 waves: W-C/W-D/W-E landed, the AR-2 gesture pin + the LIVE-registry consumption law in the wire-coverage suite); **mini 495 in-flight (the R24-2 W6/W7 pending; the register re-keys at the WRAP) in-flight** (R24-2 W0-W3 landed: 357→377→396→415→428→441; census 149 units / 136 GAP / 408 authored — REGISTER-PENDING, re-keys at the sibling's wrap); **variants 1,733/64 at the R24-wrap, higher and moving** (64 test files; the R23 W-A FX + W-B color waves + **the VLM net** `scripts/vlm-capture.mjs`/`vlm-review.mjs`/`vlm-run.sh`; 00-master's reconciliation note stands); consumer pins re-read live (the app consumes engine @ `e3f55bd` — re-converged at the engine HEAD R29, the R28 one-step-behind divergence resolved by the chore-only re-pin; the OT lock-copy @ `55c81c0`, byte-exact minus `testing/` — the D-ARCH-6 s17 consumer duty landed @ `876f2b8`; nle-ui @ `6754979` — the AW1-2 absorb LANDED; WDC @ `ec8fd5c` (unchanged); the engine's OT submodule @ `55c81c0`); all CI-green; **R30 re-pin: app 377/377 @ `8f12cf9` (the 12-file roof — the K3 corpus round: the four re-expression maps + the store/policy + geometry tranches + the C16 guards) + nle-ui 701/701 @ `5779295` (83af958 the C16 package twin + the 4 package-gap pins; 5779295 the K3 FIX-2 dead-entry class); engine/OT/WDC pins UNCHANGED** — **R31 re-pin: app 418/418 @ `f58147c` (the 21-file roof — the DOM-structural tranche: all 13 RE-EXPRESS-PORT rows + the D26.5 four partial rulings as code + D51 the mid-drag keyboard yield + the e2e-crawl suite begun) + nle-ui 701 @ `dc3644b` (docs-only: the D29 DECISIONS first-party record + the C0 filing) + OT HEAD `31f2764` (docs-only — the R31 queue filing: the carrier-reduction work order + the D25.3b worklist + the data-test freeze ask; the code pin `970948a` UNMOVED); engine/WDC pins UNCHANGED** — **R32 re-pin: app 426/426 @ `ce7cfc3` (the 22-file roof — the K4 exit verdict + the move-drag LEG 2b + the doc-committing session registry + the D52 shell-twin family + the w1-entry deliverables: the human-rounds protocol, the annotakit review loop, the side-by-side fixtures, the VLM net 7/7 / 0 REAL-BUG) + nle-ui 743/743 @ `1e0c30b` (the C0 MiniShell composition LANDED 701→737 + the D52 twin 737→743) + OT HEAD `39003d3` (the code pin MOVED off `970948a` — the D25.3a mini-theme wave: theme.ts + waveform + Tier-2 + the M60 pins; 649/649 = 632 classic → R33: OT @ `008e7f3` 675/675, the seal18 close, the census 31=28+3 UNCHANGED, re-keyed R33 + 17 M60); engine/WDC pins UNCHANGED**). **R33 app-layer addendum (recorded R34 — the layer was missed in the R33 fold): app `7664603` 426/426 (the lockstep re-pin: vendor engine→`574d8d3` + both OT mirrors→`17a19f8`; the census register re-declared 43 = 32 zero-action + 9 carriers + 2 hosts; the 22-file roof unchanged) — the w1-entry deliverables of the R32 layer above are all AT `7664603`.** — **R34 re-pin: OT HEAD `8f96ab2` (the code pin MOVED — the M64 fps param + the L2 carrier families M59-M59D; 714/714 @ 83 entries) + app `d28847c` (426/426 unchanged; the R34 re-pin: vendor engine→`60232ea` + both OT mirrors→`05d88d9` + nle-ui→`cb04919` + WDC→`94f6460`; the CR-2 carrier FLIP — the census 43 = 33 zero-action + 8 carriers + 2 hosts) + nle-ui `cb04919` (747/747 — the DeliverPage supersede contract + the viewer-state-empty CR pair)** — **R35 layer: OT `8f287c7` docs-only (the code pin `8f96ab2` stands) · app the edit-styles wave (batches A/B/C; the census 45 = 33 zero-action + 8 carriers + 4 app-host; the nle-ui vendor `cb04919`→`45d42fb`) · nle-ui `45d42fb` 754/754 · WDC `dc0ca32` docs-only · engine `2bae99a` docs-only (789/789; the CI GREEN on the aivs home) — the fleet re-homed to github.com/aivs-tech**

**GAP (the work — owner + phase per the D24 ladder in `IMPLEMENTATION-PLAN.md`; the vocabulary is the D24 K/w/r set ONLY — the dual-tag window CLOSED at this fleet's end (the R22 C/W/R tags remain in the R23 ledger as history); the retired spec-14 §4 testing-domain rows are re-homed HERE per the posture law):**
- the LAW-NET-INVENTORY corpus (**LANDED R23**; the K3/K4 gates now consume it — the seal's census is K3's acceptance list);
- **the K2 combined-family census + registration (the K2 row — spec 12's twin, THIS file the plan twin; owner: nle-core-spec, at K2 entry; registration state at R24: still PENDING — K2 entry hasn't happened, the instruments + counts below pre-register the gate; **R34 re-key (R34-W1-b): LANDED — the census record at this row's end; spec 12 §0's twin row the registration home + the §13A.7 re-tier row the register**)**: formalize the cross-module corpus as a NAMED suite family — engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`: vendored OT ops × engine composition — the 458 net), WDC's integration family (`audio-integration`/`dsp-bounce-parity`/`dsp-effects-integration`/`nle-audio-core-derisk` + the `offline-parity-wiring`/`real-audio-e2e` pair — the 759 net), OT's in-page UI-over-core runner (the 72-entry M-suite register — the 632 net, total-only) — with a census + a register in spec 12 + here; **M49C is K2's OT-side completeness instrument (REGISTERED per D29.1a: the accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions gate — the 31-name census = 28 routed + 3 exceptions (selectElements/advancePlayhead/trim, api.ts:353-360 — re-keyed R33), tsc-lockstep both directions — green 28/28 at `344123a` (re-keyed R33, was `970948a`))**; close the missing cross-module pairs (the OT-ops→engine-decode round-trips at the app's seam shapes — S-engine's worklist item) (acceptance: the family census green + registered in spec 12/17 §13A.7 + battery-checked thereafter); **R34 census record (R34-W1-b, the K2 landing at the live pins; the R24 nets above — 458/759/632 + the 72-entry register — stay verbatim, superseded): engine-in-the-middle @ `d8b97b4` (CI-only over the R33 seal `2a0ecf4`; 785/785, 29 vitest files, PINNED_TOTAL=785 + the 5-job CI venue) — `bridge-seams` 151 / `planner` 52 / `video-sync` 31 / `timeline-edit-ops` 13 (247 it( cites; the companion `nle-bridge` pair 99 + 8); WDC's integration family @ `94f6460` (777/777; 34 .test.ts = 19 src/test + 15 src/lib) — `audio-integration` 27 / `real-audio-e2e` 12 / `dsp-bounce-parity` 9 / `dsp-effects-integration` 8 / `nle-audio-core-derisk` 7 / `offline-parity-wiring` 2 (65 it( cites); OT's in-page UI-over-core runner @ `c4c04f1` (the report-json authority: **687/687 at 78 entries** — M1-M64 + the R re-runs + M49C + M62-gate; 675/675 @ `008e7f3` pre-M64), total-only; **M49C green 28/28 @ `344123a`** — 31 = 28 routed + 3 exceptions stands; the missing pairs — the OT-ops→engine-decode round-trips at the app's seam shapes — carried at the app venue (`wire-coverage` 26 + `engineSeam` 7 @ `28f3229`, docs-only over `7664603`; 426/426, the 22-file roof): honestly stated — no engine-resident round-trip suites claimed; engine-side authoring beyond the app-carried shapes stays a forward row (S-engine's worklist), NOT a K2 blocker; per-file figures are static it( cites, the runtime nets are the count-gate pins; registered in 12 §0 + HERE + the §13A.7 re-tier row re-keyed same round; battery-checked at the next fork.**
- the crawl app's corpus authoring (**K3** — the corpus-check half of the D24 split: the e2e half is K4, the human side-by-side → w1-entry; owner: nle-test-app): the exact counted scope — 322 app-side authoring tests / 115 law-family census units, per LAW-NET-INVENTORY §2.3-§2.4 (supersedes the ~130-180 estimate); includes the composed ripple family (4 census units / 16 tests composed app-side over `timeline.trim`/`rippleDelete` — r1 graduates the OT interval-diff family) and the DOM-structural half now keyed on the **D26.5 three-way data-test mapping row** (OT's frozen `data-test=` 68-site convention for the timeline units / nle-ui's already-emitting `shell-*` testid family for the chrome units / the no-counterpart structural units as the additive-component worklist — the mini's 59-static + 15-templated census mapped unit-by-unit onto those vocabularies), landing after the D30 W-C wave (the ordering law re-stated per D26: gated on the mapping row + W-C, not on a swap + a testid-emission layer) while the store/policy halves author ∥ against the injection-point contracts (acceptance: the corpus green row-by-row over REAL modules, zero mock paths);
- **the K3 offline audio pins** (unchanged in substance; owner: nle-test-app + WDC): the offline-render + call-spy pins (waveform/mute laws) per the Node-venue law (acceptance: the pins green; **prerequisite MET and the pins LANDED**: the WDC waveform contract survived byte-verbatim (14 pins, `ec8fd5c`) AND the app's K3 audio pins LANDED @ `44e8049` — `src/waveformPeaks.test.ts`, 9 tests, consuming WDC's EXPORTED contract names; remaining: the offline-render/call-spy pin audit over the mute laws under the Node venue);
- **the K3 VLM visual net** (new R23 row; owner: nle-test-app): adopt the variants' R23-G tooling — `vlm-capture.mjs`/`vlm-review.mjs`/`vlm-run.sh` — onto the app's `.storybook` + OT's runner, adopted not forked (acceptance: the VLM pass gate executes against both surfaces; the w1 side-by-side and the `/view` gold-sample verification reuse it) — **R34 re-key (recorded as K4-ratification condition #2's spec-side landing): LANDED @ the app's `57dce6a` (the w1-entry's own instrument — `vlm:capture`/`vlm:review`/`vlm:gate` on the app's package.json; the 7/7 capture [5 app stories + 2 OT /view targets, ZERO console errors], the review triage by measurement [2 VLM hallucinations disproven live, 4 design-intents, 1 category confusion → 0 REAL-BUG], GATE=PASS [`vlm-gate.json`: collectorPair 5/5 + 2/2, console-clean, specCommit 95dd53c]); the pair also recorded at 17:31's K4 R32 layer**;
- **the K3 app-side zero-no-op worklist (new R24 row; owner: nle-test-app / S-app — the D30 W-F/Z6 waves)**: the 9-A2 app-flow census — 30 surface groups / **177 controls: ~127 real / 13 degraded / 21 honest-mock / 16 no-op (incl. 9 unexpected-dead)** — plus its P1-P26 26-pin hardening plan + the Z1-Z7 fixes; **LANDED at the app's R9 round (206/206)**: the W-F coverage gate (`wire-coverage.test.tsx`) + the Z6 dead-zone sweep (P8/10/11/12/13/14/19/22/25 pinned) + the zero-no-op closure (9 unexpected-dead → 4 closed by the waves, 4 pinned honest, 1 CR'd); re-fenced R27 to the LIVE registry (`8a3eb35`) + the AR-2 gesture pin (the REAL volume-line dblclick → `timeline.upsertKeyframe` origin 'ui', `wire-coverage.test.tsx:328-377`); the census-CI mutation gate now runs **8 scenarios** (clean + M1-M6b); remainder: the census re-run's residual set rides the D-ARCH-6 re-pin round;
- **the K4 automated e2e** (the crawl's completion proof; owner: nle-test-app): import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export, asserted programmatically — **driving the same 28 routed wire verbs through the real UI (D29.1c; the shell-path exception applies identically)** (acceptance: green end-to-end, zero mock paths, zero human input; w2's full-scope demo re-gates by extending this suite) — **[R32: the K4 EXIT VERDICT LANDED — the crawl's completion proof COMPLETE]** (the interpretation ruling ratified adversarially: the '28 routed wire verbs' numeral is the R27 W6-A census DECLARATION, not a gate strengthening — the e2e's 11 wire-visible verbs + 2 shell-path rows are the chain's shape, the FULL 28-verb completeness instrument is R8's wire-coverage gate (already green, the live-registry law); the cut clause now 4-of-4 LITERALLY — the W1 move-drag LEG 2b landed (the e2e 11→12 tests); zero-mock-path re-verified (ONE venue-law canvas.toBlob stub per design-r8-d29 W2.4, zero module mocks); zero-human-input held; the VLM collector pair recorded 7/7 stories captured / 0 REAL-BUG (2 hallucinations disproven by measurement, 4 design-intents, 1 category confusion) — the ruling + evidence chain in the app repo's docs/decision-k4-exit-verdict.md);
- the offline-parity threshold pins + the null rig (**r2**; pass: max deviation ≤ −60 dBFS any channel);
- the grade-math parity pins (**r3**);
- the real-file decode corpus (**r4**; PENDING the user's D6 re-affirmation, deprioritized);
- the FCPXML fixture corpus (**r5**; the phase-entry artifact);
- the battery's posture checks (**R24 — battery_r24**: the pin/count key set + the plan/stub checks + the routed/exceptions coherence check per D29 + the per-file-audit residue sweep).

**The D24 verification ladder, spec-17-side (where acceptance executes — `IMPLEMENTATION-PLAN.md` §2 is the executable gate table; this §0A is the tier vocabulary's spec home; the D29 instruments are the ladder's gates now — the repos' own W11 verification vocabulary, adopted, not parallel-prescribed):**
- **K1 — module nets:** each repo's own suite at its own gates (the BASE rows above; this spec's three-tier methodology §2 is the per-repo shape). ALL GREEN at the R29 pins — OT's milestone register grew to **72 entries** (M44-M58 + the M49T/H/R/G + M51R/M55R/M57R/M58R real-mouse families + the M49C coverage gate), the engine net 440→458→748→**749**→**785** (R33 — the exhaustive-testing property tier, register P2-1), nle-ui 648→674→690→**691**, the app roof 117→174→252→**256 (8 suites)**.
- **K2 — combined nets (cross-module):** the named family (the GAP register's census row) — engine-in-the-middle, WDC's integration family, OT's in-page runner (pixel reads + real-mouse + WYSIWYG state). The seeds exist at HEAD; the census + register land at K2 entry. (R34: LANDED — the census record at the GAP row above + spec 12 §0's twin.) **K2's OT-side completeness instrument is M49C (REGISTERED, D29.1a):** the accumulated ⊇ `WIRE_COMMAND_TYPES` − exceptions gate (the 31-name census = **28 routed + 3 exceptions**, tsc-lockstep both directions) — green 28/28 at `344123a` (re-keyed R33, was `970948a`).
- **K3 — app-behavior nets:** the LAW-NET corpus re-expression (322 authored + 33 projection pins), the DOM-structural gate (now keyed on the D26.5 three-way data-test mapping row: OT's `data-test=` 68-site convention for the timeline units / nle-ui's `shell-*` testids for the chrome units / the no-counterpart structural units as the worklist), the offline audio pins, the VLM visual net — over REAL modules, driven headlessly. **K3's routed-verb completeness machine-check is the D30 W-F coverage-gate port (REGISTERED, D29.1b — **LANDED**): the gate consumes the LIVE `WIRE_UI_EXCEPTION_VERBS` export (never a hand-mirrored copy — the 00 §2A standing law) + the AR-2 keyframe-authoring gesture pin (the volume-line dblclick → `timeline.upsertKeyframe` origin 'ui'); the offline audio pins LANDED @ `44e8049`; the DOM-structural row's data-test basis re-keyed to 69 occurrences / 59 unique names (the unique-names convention, R4-iii; 05's audit + 19 §10 the statement)**: the module-scope accumulator + the gesture sims + the final assertion — with the **shell-path blind spot registered** (the gate measures the PORT surface, excepted per D30 R2/R8) and **the compensating pin family** driving the trusted-integrator verbs (⌘B, [ ]/⌥[ ], ,/., M/⇧M, ⌘M/⌘⇧M) directly through engineService — owner-tagged app-side (D29.1b/review F14); the zero-no-op worklist row above is K3's app-side hardening set.
- **K4 — the crawl exit gate:** the single automated e2e (import → cut → play → export) — zero mock paths, zero human input; the crawl's completion proof; **drives the same 28 routed verbs through the real UI (D29.1c — the shell-path exception applies identically)**.
- **w1-w3 / r1-r6:** the walk's real-app tiers with the human classes (every human-required gate registered [H:reason] in the plan — the crawl discipline continues through the walk: findings land as programmatic pins wherever possible) and the run's depth tiers — the GAP rows' r2/r3/r4/r5 tails are the r-phase parity/decode/corpus pins.
...
8. **Error-path census (Round 8; re-keyed Round 28/D44 — the two-tier
   class+fine taxonomy)**: every one of the **25 fine codes** the feature
   can emit (spec 15 §6.3's registry: the 24 + `RATE_OUT_OF_DOMAIN`) must
   have at least one test that *triggers* it — asserting the code AND its
   **class** tag (per §6.3's class-tag table, exactly one class per code,
   the three A-6 restorations `TRIM_BEYOND_SOURCE`→INVALID_PARAMS/`source`,
   `JOB_QUEUE_FULL`→CONFLICT, `PROJECT_DIRTY`→CONFLICT as load-bearing rows)
   plus the constraint type where declared (`'domain'` joins the union) —
   not just `ok: false`. **The never-silent law:** a refusal returns
   `{ok:false, code}` — `NOOP` included (the compliant benign refusal); the
   sanctioned `ok:true` non-mutations are the **benign-echo family** ONLY
   (the A9 set-alls `data:{changed:false,…}`; the zero-delta
   `retimeKeyframes`; the D-T7 same-position `moveElements` — zero history
   entry), each pinned with an honest echo and never counted as refusals;
   any other `ok:true` non-mutation is a spec bug. The **9 lockPreCheck
   riders** (roll/slip/slide/rateStretch/retime/freezeFrame/rangeRemoval/
   replace + the widened placement-target) each carry a `TRACK_LOCKED`
   trigger BEFORE the op runs. A code that can be emitted but never
   triggered in the suite is a coverage hole, not a nicety.
...
| Linked-companion linkage (`linkedTo` pairwise field + runtime closure — D42, R28) | ✅ | — | ✅ | ✅ | — | — |
| Edit-domains one-home lattice (`edit-domains.ts` constants + the computed intersection — D43, R28) | ✅ | — | — | ✅ | — | — |
| Two-tier error envelope (the 25-code class-tag census + the benign-echo family — D44, R28) | ✅ | — | ✅ | — | — | — |
| NS-4 render-plan projector (SceneTracks-blind `project(scene) → TimelineData` — D45, R28) | ✅ | — | ✅ | ✅ | — | — |
| The four absent edit families (replace / append / ripple-overwrite / fit-to-fill — D46, R28) | ✅ | — | ✅ | ✅ | — | — |
| Flat keyframe forms + insertBatch ripple (the param trio — the 78→79→80 union members — D47, R28) | ✅ | — | ✅ | ✅ | — | — |
| Markers (point + range v2 — `duration` + `notes`; ⇧M start-match — D48, R28) | ✅ | — | ✅ | — | — | — |
| Caption tracks (kind + `language` + burn-in exclusion + `ElementJSON.text` — D49, R28) | ✅ | ✅ | ✅ | — | — | — |
...
> **R33 status note (engine seal-round register P2-24):** §5's real-media fixture set (mp4/wav/LUTs/fonts) is **r4/r5-phase content** — no fixture files, manifest, or generation pipeline exist in any repo today; the engine's Tier-2 canon is procedural virtual media + computed oracles (`virtual-video.ts`/`virtual-audio.ts`; spec 12 §15's status note the fuller record). The names below are the canonical vocabulary the r4/r5 corpora adopt. (R33, engine seal-round register P2-24.)
...
> **R33 status note (engine seal-round register P2-24):** the reference-regen machinery is **r4/r5-phase content** — zero reference PNGs / regen scripts / per-platform reference dirs in any repo (verified at engine `2a0ecf4` + app `7664603`); the engine's pixel oracles are computed (expected pixels from math — no golden drift possible). The process below is the TARGET workflow for the real-GPU venue (12 §16's twin note). (R33, engine seal-round register P2-24.)
...
> **R33 status note (engine seal-round register P2-24):** `generate-assets.mjs`, the manifest, and the bucket pipeline below are **r4/r5-phase content — none exist yet** (the greps at 12 §15's status note); the engine's venue is the in-code `VirtualVideoSpec`/`VirtualAudioSpec` builders + computed oracles. The pipeline is the TARGET for the real-file corpus round (r4, owner nle-engine). (R33, engine seal-round register P2-24.)
...
Each NFR budget → a concrete, CI-runnable measurement. NFR tests run in the nightly job (§9.3) + the shell-mount smoke subset per-PR; budgets are pass/fail, not advisory. **R33 venue-status note (engine seal-round register P1-8): the nightly venue LANDED engine-side first** (the perf tier PERF-1..3 @ the engine's `perf.yml`, telemetry-mode first — the fleet's first scheduled workflow; re-keyed R34: the PERF family is 3 files/10 tests (PERF-1 timeline-ops 6 + PERF-2 audio-mixdown 2 + PERF-3 heap-leak 2 — the W1-c verification; the '1..4' was the off-by-one) + the workflow now PARSES (the perf.yml:79 unquoted-colon fix + the two §B.5 steps restored: the perf count-gate `--table perf` mode + the `test:property:explore` randomized-seed pass — engine `d8b97b4`); **the app-side recipes' venue (a nightly job or the per-PR smoke subset) is NOT YET LANDED — the family's GAP row (spec 12 §0, R33) owns the venue + carriers; the Tier/job column below is the TARGET model, not the current venue record.**
...
| Error-path discipline | §13A.4 error-path census (T1) | T1, per-PR | every one of the **25 fine codes** in spec 15 §6.3's two-tier registry (class-tagged per its class-tag table, R28/D44) has ≥ 1 triggering test asserting the code AND its class; a coverage script (codelist × grep of test IDs) enforces it |
...
1. **Schema sweep (T1)**: for each of the union members — **78 today; the R28/D47 + D46-B5 ruled projection 78 → 79 → 80** (`insertBatch` mints the 79th member at the r1 fold; `replace` the 80th — no counterpart until `timeline.replace` lands; the counterparts re-derive **29 → 31 → 32-of-80** per the convention stated at spec 15 §0-BASE/:5035-:5042) — a valid example round-trips through the spec 15 §11 Zod schemas; a deliberately-invalid variant fails with the expected issue path. The examples are generated from spec 15 §5's example table (single source, no test-local forks); the two new sweep examples (bare `insertBatch` + `replace`) are generated from the same table at the bump — the count re-keys WITH the landing, never ahead of it.
2. **Error-path census (T1; re-keyed Round 28/D44 — the two-tier class+fine taxonomy)**: every one of the **25 fine codes** in §6.3's registry (the 24 + `RATE_OUT_OF_DOMAIN`) is triggered by at least one test asserting the code AND its **class** tag (per §6.3's class-tag table — exactly one class per code; the A-6 restorations `TRIM_BEYOND_SOURCE`→INVALID_PARAMS/`source`, `JOB_QUEUE_FULL`→CONFLICT, `PROJECT_DIRTY`→CONFLICT as load-bearing rows) plus the constraint type where declared (`'domain'` in the union); the **benign-echo family is asserted `ok:true` with an honest echo** (the A9 set-alls; the zero-delta `retimeKeyframes`; the D-T7 same-position `moveElements`) and never counted as refusals (the never-silent law — `NOOP` is `ok:false`); the **9 lockPreCheck riders** each carry a `TRACK_LOCKED`-before-the-op trigger; the census script fails if a registered code has zero triggering tests (§2.5 rule 8's R28 re-key).
...
### 13A.5 Projector property tests (T1 — the projector's mandatory deliverable per spec 14 §3's A1 row at R22 (resolved via the retired stub's §-redirect table to `IMPLEMENTATION-PLAN.md`'s ladder — the projector-parity corpus is the K-tier row in §0A's GAP register); "spec 14 P1" at R9; re-typed R9 from the retired bidirectional adapter per Decision 12.1)
...
- **Audio-domain seam properties (spec 20 §5, web-daw-core's suites as the executable reference)**: merge laws (same-media adjacent segments merge phase-continuously; fades block; rate boundaries never merge), null-parity ≥60 dB offline-vs-realtime, canonical send/return (no dry leak), pan law −3 dB at strip level — pinned by WDC's `nle-audio-core-derisk.test.ts` (H0-H7 incl. H3; **759/759 @ `85b81b0` — the R24 re-verify, `85b81b0` docs-only over the R23 `494f6ff`; the W2 soundtouch/varispeed family + the Goertzel detection oracle landed since the R22-era 740**) + the bridge suite NOW AT nle-engine `tests/vitest/nle-bridge.test.ts` (the H8-H17 family — 37 tests at the R22 citation, grown to 96 pins at the R24 pin `5036387` by the S/W rounds + **N2b LANDED @ `37cdd28`** (18 bridge pins + 2 REAL offline-render sample-level parity pins, the dB-domain gain-automation law); its twin `tests/vitest/engine/bridge-seams.test.ts` now at 113 pins — the W2.5 effects-sidecar family +17 and the R8-REV blur-radius clamp +1 landed at `5036387`; relocated verbatim with the bridge per WDC's M1.6 — the old in-WDC nle-bridge citation is superseded) + the sibling `tests/vitest/nle-bridge-realtime-midi.test.ts` (the H18 scored-notes/MIDI bridge family, the realtime half)
- **The render-plan projector's four properties (R28/D45 — the NS-4 extension, `project(scene) → TimelineData` per 08:24)**: **deterministic** — same scene in, byte-identical `TimelineData` out (SHA-256; no timestamp/id-counter/wall-clock leakage); **stable-under-reprojection** — the one-way projector's operative idempotence (projecting the same scene content twice yields byte-identical IRs — the `idSeed` id-counter discipline); **never-loss** — total element count preserved through projection, per track-kind, including the honest-skip classes (the sticker/graphic exclusion list asserted as the DOCUMENTED exclusions and ONLY those — a dropped member outside the filed list NAMES itself in the failure, the direct-diff law); **no-readback as venue-blindness — TWO independent assertions**: (i) the dependency census (static — the venue stack's frozen file set imports ZERO SceneTracks-typed modules; the layer-fence poisoned-edge class; a new edge lands only WITH a spec amendment) AND (ii) the P12 Proxy/call-counter probe (dynamic — the projected IR handed to `setTimeline` holds no live scene reference: mutate the scene post-projection ⇒ the held IR is byte-stable). Plus NS-5's array-identity invalidation boundary (same tracks-array reference ⇒ NO reprojection; a NEW equal-content reference ⇒ reprojection fires — identity, never content, never frame-count), NS-1's clock-agnostic confinement (the same scene at three playhead/clock states ⇒ byte-identical IR), and the translation laws (per-track ordering, z-order/visibility, transition descriptors, tick arithmetic hand-derived from the bridge's documented laws — the anti-tautology sentence in the suite header verbatim). Phase: r1-entry (the adapter is THE r1-entry architecture decision — S-engine).
- **The {grade} seam pins at r3 (R28/D45 — the R9-c fold, 08:24)**: the construction pin (the monitor path AND the export path both route through the ONE `paintCompositionFrame(ctx, ops, media, {grade})` helper — a future "fix" that re-inlines one venue's final pass trips THIS test); the ORDER pin (the ctx-stub call log: clear → N composite `drawImage` calls → ONE filtered `drawImage` onto the target — the filter applied exactly once, to the composited frame, never per-element; the neutral grade ⇒ the direct paint path, zero filter assignments); `buildGradeFilterString`'s honest-CSS-subset pins (the canonical-order table with hand-derived string literals, the neutral `''` law, the hostile-drop disposition per field, the per-element save/restore bracket, the closed set {`contrast`, `saturate`, `brightness`, `hue-rotate`} + `blur` — derived from the builder's output over the full input census, never a hand-copied list); the app-side wrapper-retirement census (ProgramCanvas + deliverService retire their wrapper pairs + the app-side string mapping — ZERO app-side final-pass homes; the migration's DOM observable stays `data-grade`; pre/post-migration app snapshots identical). Phase: r3 (the color venue work — S-engine for the seam/string/parity, S-app for the migration).
...
|---|---|---|---|---|---|
| 78-command union (schema validity) | 15 §4 | F | T1 | §13A.4.1 schema sweep | 78/78 round-trip today (the ruled r1 projection 78 → 79 → 80 — §13A.4.1's R28 re-key; the 79th/80th sweep examples join at the bump); invalid variants fail |
...
| SceneTracks ↔ flat seam | 00 D11 (R8; superseded — the one-way projector per D12.1/D16, spec 14 §3 A1 → resolved via the retired stub's §-redirect to the plan) | F | T1 | §13A.5 property tests | both identity laws + never-loss |
...
| Projector parity vs engine-native Timeline oracle (Round 15) | 00 D12.1, 14 §3 A1 (retired → the plan's ladder), 05 §16.5A | F | T2 | §17A S4: parity corpus, pixel-exact vs the engine `Timeline` oracle — primary venue is ENGINE CI (its ~8-min real-WebGPU milestone runner + vitest); app S4 nightly adds the Xvfb+SwiftShader venue | parity green on the corpus (audio + stills) |
...
| Reference-repo re-tiering (**R24 re-baseline: engine 458, OT 536 (386 in-page + 150 real-mouse), WDC 759, nle-ui 674, nle-test-app 174, mini 495 (R24-current — the R23 seal 355 + the R24-mini nets), variants 1521+**) | 19 §12, 00 D14 | NF (process) | — | re-baselined to the **2026-09-08 verified counts (the R24 pins; 00-master's fleet table the canon, D21)**: engine 458 vitest @ `5036387` (13 suites: `tests/vitest/engine/` 11 + `tests/vitest/` nle-bridge pair; static-census-matched — the live-run venue is GHA CI) → T1/T2 (re-keyed R33, engine seal-round register P3-37: 3-job at the R24 pin → **the 5-job venue @ `2a0ecf4`** — typecheck/vitest/milestones + the OV-12 `vendored-timeline` re-pin gate (`ci.yml:293-393`) + the W3 coverage job); OT 536 @ `ded43c4` / code pin `c15a629` (386 in-page M1-M49 + 150 real-mouse across the 18-entry real-mouse family incl. M49T/H/R/G — 59 milestone entries, the report json the count authority; the M49C coverage gate) → T1/T2/T3 (the real-mouse phases ARE Tier-3-grade, in-repo; the in-page runner is a K2 seed); WDC 759 @ `85b81b0` → T1 (null-test gates are the audio T1; the 18-suite integration family is the K2 seed); nle-ui 674 @ `fc4cc35` → T3-package (engine-free chrome tier, boundary-script-gated; the D29 seams — saveState/exportRequest/grade sidecar — pinned); **nle-test-app 174 @ `c885ece` → THE ROOF** (the app tier: the 6-suite seam/wired-whole set — GluedShell 70 + audioService 40 + sceneBridge 25 + persistence 17 + deliver 15 + engineSeam 7 — promoted from the retired "mock 596" row: the mock split into nle-ui (674) + the crawl-app corpus (K3, LAW-NET-INVENTORY)); mini 495 + variants 1521+ → design-reference tier (local + annotakit; retire per the port-then-swap law — post-K4/w1); count discipline (declared == scraped, battery_r24-enforced) | zero count drift |
| K2 combined-family census + registration (Round 23 — spec 12's twin row; **re-verified R24, counts re-based**) | ARCH-R23 D24, `IMPLEMENTATION-PLAN.md` S-spec item 2 | NF (process) | T1/T2/T3 (cross-module) | the named cross-module family censused + registered in spec 12 + HERE (§0A's GAP register row — spec 12 owns the registration, this row is 17's plan twin): engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner` — the 458 net), WDC's integration family (the 759 net), OT's in-page runner (the 386 in-page + 150 real-mouse, 59-entry register); the missing pairs (OT-ops→engine-decode round-trips at the app's seam shapes) authored at the engine; **M49C is K2's OT-side completeness instrument (D29.1a — registered, already green 24/24)**; registration state R24: still PENDING — K2 entry hasn't happened (the census rows land at 458/759/536) — **R34 re-key (R34-W1-b): LANDED — the census rows at the live pins: engine-in-the-middle within engine 785/785 @ `d8b97b4` (bridge-seams 151 / planner 52 / video-sync 31 / timeline-edit-ops 13), the WDC family within 777/777 @ `94f6460` (65 it( cites across the 6 suites), OT's register 687/687 at 78 entries @ `c4c04f1` (M49C green 28/28 @ `344123a`)** | census green + registered + battery-checked; the missing-pair suites green — **R34: carried at the app's seam shapes (`wire-coverage` 26 + `engineSeam` 7 @ `28f3229`); no engine-resident round-trip suites claimed — forward row (S-engine's worklist), not a K2 blocker** |
| Linked-companion linkage (Round 28 — D42) | 06 §5.0/§5.9B (rider 12's uniform predicate :411; E1-c :440; OW-4 :1603; F4 :1698) + 09 §3.1A B1 + 15 §4.3.2 (the `linkedTo` patch key :962-:971) | F | T1/T3 | the closure battery (symmetric / live-id-only / order-stable / cycle-free / locked-partner-skip / sync-lock-skip — the ONE shared validator) + the per-verb fan-out tables + the named regression pins in 06's GAP-row acceptance vocabulary: **E1-c** (the mixed-split fixture — the untouched third member's ORIGINAL link survives verbatim; the split halves re-pair by side, fresh pointers), **OW-4 prune/CASCADE** (the companion cascades WHOLE; zero dead `linkedTo` on survivors), **relink-both-halves**, **duplicate-severs** (F4 — the copy lands `linkedTo === undefined`, the `{...element}` phantom-carry trap), replace-severs (`syncLinked:false`), and the patch family (**null-clears** both sides; `TRACK_LOCKED` on a locked-track patch; dead-id → `ELEMENT_NOT_FOUND`) | every pin green at the r1 Stage-0 port (S-ot's ONE additive change); the fuzz invariants I6-I8 sweep the net (closure-shape after ANY op; NO dead `linkedTo` after ANY op; undo/serialize preserves the field — 12 §8's three properties); the carried 24 re-keyed with the P16 disposition + rider 9's count reconciliation |
| Edit-domains one-home lattice (Round 28 — D43) | 06 §5.9F/:1672 (the live three-way intersection) + 06 §11.7 (the domain split) | F | T1 | the per-domain grids (interior + bounds + the [0.01, 0.02, 1/32] dead zone + NaN/±Inf hostile; literals never module-vs-module); the computed-intersection property — `FIT_TO_FILL_RATE == [MIN_RETIME_RATE, MAX_RETIME_RATE] ∩ NATIVE_VENUE_SPEED ∩ WSOLA_RATE ⇒ [0.1, 5]`, the expected side hand-recomputed from the three LIVE imports (the [0.1,4] AUTHORING_SPEED trap's mutation probe; the narrows-refusal probe that kills static text); `minDurationTicks(fps)` + the dead-zone predicate; the `NATIVE_VENUE_SPEED` fence in bridge-seams (the deep-imported leaf ≡ engine MIN/MAX_SPEED + the C1 wiring proof); the nle-ui local twin pinned **⊂** (pin failure ⇒ align nle-ui to the one-home, NEVER edit the module); the Stage-0 **verbatim** re-export check (volume-dB + retime leaves re-exported verbatim — zero behavior change) | the grids + the intersection property green at r1 Stage 0; the battery's check-2 widening (the C1-fold leaf-only import edge: bridge/ + the folded timeline-math.ts site) green; the cross-file [0.1,5] coherence (06 §5.9F + §11.7 + this row) battery-checked |
| Two-tier error envelope (Round 28 — D44) | 15 §6.3 (the class-tag table + the never-silent law :3002-:3014) | F | T1 | the 25-row census table DERIVED from the live registry (the 24 + `RATE_OUT_OF_DOMAIN`, each code tagged exactly ONE class; the A-6 restorations `TRIM_BEYOND_SOURCE`/`JOB_QUEUE_FULL`/`PROJECT_DIRTY` as load-bearing rows; constraint types incl. `'domain'`); the never-silent law (`NOOP` = `ok:false`) + the benign-echo family each pinned `ok:true` with an honest echo (the A9 set-alls; the zero-delta `retimeKeyframes`; the D-T7 same-position `moveElements`); the closed-family audit; the 9 lockPreCheck riders (each `TRACK_LOCKED`-before-the-op + the no-commit trio) + the 2 applyBatch-inherited riders; the `TRACK_LOCKED`-before-`NOT_FOUND` precedence pin; the tsc-lockstep class enum | the census re-key landed with battery_r28 (§2.5 rule-8 + §13A.4.2 + §13A.1 re-keyed to the two-tier shape); the runtime arm + the lockstep map at r1 Stage 0; the riders ride their verbs' r1 stages (replace's rider at Stage 4) |
| NS-4 render-plan projector + the {grade} seam (Round 28 — D45) | 08:24 (the R9-c fold + `buildGradeFilterString` engine-side + the app migration registered) | F | T1 (r1-entry) + r3 seam | §13A.5's R28 extension: the four properties on `project(scene) → TimelineData` + venue-blindness as TWO assertions (the dependency census + the Proxy/call-counter probe) + NS-5's array-identity boundary + NS-1's clock-agnostic confinement + the translation laws; the {grade} seam pins at r3 (the construction pin — both venues route through the ONE `paintCompositionFrame(ctx, ops, media, {grade})`; the ORDER pin — ONE filtered draw, never per-element; `buildGradeFilterString`'s honest-CSS-subset closed set) | the projector suite green at r1-entry (S-engine); the seam + the fragment-pass parity + the app migration (ProgramCanvas + deliverService retire their wrapper pairs — zero app-side final-pass homes) at r3 |
| The four absent edit families + the RE-3 probe (Round 28 — D46) | 06 §5.9C-F (RE-1 :1622; RE-3 :1624; AP-2 :1643; RO-1/RO-2/RO-3 :1662-:1664; FF-1/FF-2 :1682-:1688) | F | T1/T3 | the per-family M58 law sets (ONE history entry; NOOP honesty; the no-commit trio; the shared validator; the error table): replace's Tier-1 pins (exact-length, no-move, undo atomicity, transition survival, companion untouched; `NOOP` never), append's M16-class set (append-at-track-end, playhead-ignored, multi-append order + one-entry, the linked-append law, never-a-push), ripple-overwrite's delta/pull/order/zero-floor/companion laws + THE DOWNSTREAM-SET DIFF CHECK (the shifted set == the COMPUTED set — the direct diff naming the drifted member), fit-to-fill's 4-point census + the computed-intersection domain property + the WDC audio half (the null-test on untouched lanes, exact-duration-no-tolerance, the goertzel pitch oracle, the [1/32,32] guards); the RE-3 transition-remap probe (06 §5.9C's GAP row — the two-exit gate: the pinned `transitionOut` endpoint-remap test green ⇒ replace proceeds; the redesign verdict filed as a new D-entry ⇒ the cut fires — the probe cannot silently fail) | the families land r1 Stages 2-4 per the leverage map; the probe at Stage-4 entry with exactly ONE exit evidenced at Stage-4 exit — an unevidenced probe is a spec bug |
| The param trio (Round 28 — D47) | 15 §11 (the flat zod schemas :4349-:4392) + 15 §13.15/:5035-:5042 (the counterpart arithmetic + the insertBatch ripple ruling) + 16 §3.12 (the flat keymap rows) | F | T1/T3 | the flat-form wire tests (the round-trip sweep; the self-addressing two-element-one-command pin; the mint/hoist/min-1 laws; the no-parallel-arrays negative; the old-replay deprecation rows — the grouped forms + the singular-absolute retire at the bump, a named stable envelope, never silent acceptance, never a crash); the both-verbs law (`insert` stays singular — the ruling's own negative; insertBatch's M58 batch set + the anchor-required conditional schema law); THE INSERTBATCH RIPPLE TOTAL-SPAN PIN (the push == d1+d2+d3, the block's TOTAL span — the one-element degenerate ≡ insert's D31.6, the equivalence that ties both verbs to one law); the counterpart count as a battery class (29→31→32-of-80 re-DERIVED from the convention); the 16-side keymap dispatch coherence (the emitted command validates against the spec-15 zod schemas; the `<runtime>` deltaTicks pair) | the re-keys land NOW with battery_r28 (§13A.4.1's 78→79→80 + the header's singular-retirement fix); the pins ride the r1 union-version bump (the param-alignment wave, Stage 2-3 with insertBatch's fold) |
| Markers v2 — point + range, `duration` + `notes` (Round 28 — D48) | 09 §3.1A (the `Marker` interface) + 05:722 (the scene-shrink law) + 16:342/:351 (the ⇧M start-match law + the ⌥M inspector field set) + 10 §4.7 (the export rows :586-:619) | F | T1/T2/T3 | the validation table (point/range by `duration`; `end = time + duration`, ≥ 1 frame, ≤ scene duration); the D2e field-law round-trip (`notes` owns OT's `Bookmark.note`; `label` doc-side-synthesized — the id pattern; the sceneBridge :375/:557 forward+backward `duration` round-trip); the scene-shrink keep-and-display-clamp pin (the marker array referentially unchanged; the DISPLAY clamps to the tail; re-grow restores); the ⇧M START-match window table (±1 frame; strictly-inside-span deletes nothing — the no-commit trio); the id-addressed remove grammar (playhead-independent); the export string-law rows (start relative to the clip; range → the REAL `duration` attr; `note` iff `notes`; `value` = `label`); the keyword as a REGISTERED deviation (register row 5's ~3-pin churn — a register-check, not a behavior pin) | the model-fold prereqs (09 §3.1A + 15 §4.3.49-51 + 18 §4.4) are the W6 delta sweep's charge; the pins land with the W6 fold + the r1 wire surface (the `updateMarker` `Partial<Marker>` payload rides the union bump if the model fold lands there); the clip-marker bundle (the 4 Clip.test pins + the insertPlan re-offset laws) is r5-entry as ONE gate |
| Caption tracks — the track-kind hybrid (Round 28 — D49) | 15 §4.3.22 (addTrack `'caption'` + `language?` :1092-:1110) + 10:544-:548 (`ElementJSON.text`) + 05 §12.1 (the order law) | F | T1/T2/T3 | the six-kind TrackType union (tsc-lockstep both directions); the BCP-47 validation grid; the DESCRIPTIVE duplicate-language law (addTrack does NOT reject — `ok:true` + a second track minted, the benign-family discrimination; the export-merge determinism pin; the r5 enforcement registered with its migration note); the burn-in exclusion as the TWO-SIDED one-law pin (`elementAtTime` scans `['overlay','main']` only + the `data-ops` invariance; the burn-in hit PRESENT — the chip text + language badge); `ElementJSON.text` round-trip + the `params?.text` stand-in negative; the lane-position/z-mirror law (TOP, above `overlay`); the four-item OT r1 cost acceptance set (TrackType + `SceneTracks.captions` + the `TextElement` body + the addTrack machinery branches) | the W6 prereq sweep (09 §119-:124 + 10:811 + 18 §4.4/§4.7 + 05 §7.3/§12.1/§12.2) then the facet rows; the OT rows at r1 (the register flip's stated cost — the four-item acceptance); the SRT/ASS/VTT file-per-language export corpus at r5 |
| The five pages — the page system (Round 28 — D50) | 16 §3.8/:365-:379 (the five-page law) + 16 §6.1 row 11 (the three-claimant resolution) + 18 §4.8 | F | T3 | the page-key completeness machine-check VERBATIM (the module-scope accumulator + the LIFO fail-loud words + the direct array diffs against the LIVE-imported `Page` union — `missing`/`stale` both `[]` + the size identity 5); the Audio page-vs-mode BOTH-families law (membership + the activation toggle — ⌘4 enters, re-click/Esc exits — neither substituting); THE FX-DENSITY DISCRIMINATOR PIN `page === 'fx'` NOT `fxMode` (the ViewOptionsPopover DOM-absence law; the Edit-page FX tool keeps the compact entry); the per-page view-state memory (`pageTimelineView` seeded per page, the isolation row); the ⌘6-⌘9 reserved rows; the three-claimant resolution's inert-claimant pins (the ruled-out sets never dispatch) | the corpus re-keys land at W6 (18 §4.8/:41 + 08:2168 + 04:2293's ruled-out rows); these facet rows are the K3 gate's acceptance key (16:28's registration — the DECISION closed; the IMPL window registered, D50/F6); the app-side page-key suite + the nle-ui FX-page absorption ride XMOCK-6's r5/K3 impl half |
| The r1 port acceptance protocol (Round 28 — the consolidated recipe) | spec 15 §13.15 + 06 §0 + the plan's r1 row (`IMPLEMENTATION-PLAN.md`:67) | F | T1 | §13A.8 (the base protocol + the five R28 additions — the D42 pairwise re-key, the D43 Stage-0 fence, the D44 riders + census re-key, the D46 probe + downstream-set diff, the D47 bump migration — each with pins/order-slot/battery-check) | the five additions each evidenced at their stage exits; a stage exit that fired without its evidence cited is a spec bug |
...
### 13A.8 The r1-port acceptance protocol (Round 28 — the consolidated recipe)

The plan's r1 row (`IMPLEMENTATION-PLAN.md`:67) states the base protocol for every port; R28's rulings widened it with **five protocol elements that existed nowhere as test law** — without consolidation the executors' work orders carry the Stage ladder but not the R28-widened gate content. This section is the consolidated recipe the r1 executor follows at every stage exit — an ORDERING law (five additions × stage slots × battery checks), citing the owning facets (§3.1/§13A.7's D42-D47 rows) rather than duplicating them:

| # | The addition | The pins that land | The order slot | The battery check |
|---|---|---|---|---|
| 0 | **The base** (the plan's r1 row): the carried engine tests green in OT · the census self-re-declares (D29.2, proven thrice) · the 05 §8A grammar row · the D32 fan-out row · the mode-matrix re-derive | per port, per stage | every stage | the mode-matrix re-derive + the census lineage |
| 1 | **D42 — the pairwise re-key:** the carried E1/E2/F6 pins re-keyed to assert link values (`linkedTo`) through undo/redo/serialization (09 §3.1A B1); rider 9's count reconciliation (the carried corpus = 13+13+20+12+table+11, the decomposition recorded) | the re-keyed pin family runs green in OT with the pairwise field asserted at every undo/serialize boundary; the reconciliation recorded in the port's design note | Stage 0 (the field + closure + four-verb laws are S-ot's filed first step) | the D42 class (12 §8's closure invariants + the 06↔17 pin-vocabulary coherence, drifted members NAMED) |
...
| 3 | **D44 — the riders:** the 9 lockPreCheck riders each with a `TRACK_LOCKED` trigger (roll/slip/slide/rateStretch/retime/freezeFrame/rangeRemoval/replace + the widened placement-target) · the never-guard/tsc-lockstep compile checks · §2.5 rule-8's error-path census re-keyed to the 25-code two-tier shape (each code × class + the benign-echo family as `ok:true`) | one rider row per verb (the P3 table-loop form); the compile checks fire at the verb landings; the census script re-keyed | the riders ride their verbs' stages (replace's rider at Stage 4); the census re-key lands with battery_r28 | the D44-A6 class-tag completeness class (25 codes × one class each, scraped from 15 §6.3's table) |
| 4 | **D46 — the probe + the downstream-set diff check:** the transition-remap probe at Stage-4 entry (two exits — §13A.7's D46 row: the pinned `transitionOut` endpoint-remap test green ⇒ replace proceeds; the redesign verdict filed as a new D-entry ⇒ the cut fires) · ripple-overwrite's downstream-set diff check (the E1-a condition as B2: the shifted set == the COMPUTED downstream set, the direct diff naming the drifted member) | the probe's pinned test + the RE-3 disposition; the diff-check pin | the probe at Stage-4 entry; the diff-check with RO's port (Stage 3) | the D46 class (the probe-row presence + the disposition + the mode-matrix rows) |
| 5 | **D47 — the bump migration:** the union-version bump's old-replay deprecation path (the grouped forms + the singular-absolute retire on the same bump — §2-law-5's convention: a named, stable, non-crashing envelope, never silent acceptance) + the counterpart re-derivation 29→31→(32 of 80) | the old-replay deprecation rows + the count re-keys (§13A.4.1's 78→79→80) | the param-alignment wave (Stage 2-3, with insertBatch's fold) | the D47 class (the counterpart arithmetic + the stale-projection check) |
...
|---|---|---|---|
| 01 | Core engine | Undo/redo, Project save/load, Worker lifecycle | `01-core-engine.md` `## Testing` (≈2184) |
| 02 | Workers & threading | Worker lifecycle, OPFS persistence | `02-workers-threading.md` `## Testing` (≈2551) |
| 03 | Playback engine | Playback frame accuracy, Scrub latency, Varispeed | `03-playback-engine.md` `## Testing` (≈2437) |
| 04 | Renderer & color | Color space conversion, Transfer functions, YUV/RGB, Multi-track blend, Opacity, Masks | `04-renderer-color.md` `## 17. Testing` (≈2132) |
| 05 | Timeline | Keyboard shortcuts, Mouse interactions, Drag-and-drop | `05-timeline.md` `## Testing` (≈1627) |
| 06 | NLE ops | Split, Trim, Ripple, Roll, Slip, Slide, Delete, Insert, Rate stretch, Retime, Freeze frame, Range removal | `06-nle-ops.md` `## Testing` (≈3198) |
| 07 | Composition | Multi-track blend, Transitions, Masks | `07-composition.md` `## 17. Testing` (≈1760) |
| 08 | Color grading | Color wheels, Curves, LUT, Qualifier, Scopes | `08-color-grading.md` `## 19. Testing` (≈2135) |
| 09 | Project model | Project save/load, Project schema migration, OPFS persistence | `09-project-model.md` `## Testing` (≈2622) |
| 10 | FCPXML export | FCPXML export, FCPXML import (manual) | `10-fcpxml-export.md` `## 16. Testing` (≈1981) |
| 11 | Cloud render | Cloud render WYSIWYG, Memory ceiling (8K), Render time (4K, 8K) | `11-cloud-render.md` `## Testing` (≈2444) |
| 12 | Testing infrastructure | (this spec references 12; 12 references this spec) | `12-testing-strategy.md` `## Testing` (≈2468) |
...
> This section lands ARCH-R15 §3 (Ruling 3) as canon. **The principle — four walls, one roof:** module repos keep their full gates UNDILUTED (**R24 counts**: engine: CI + 458 vitest; OT: 536 incl. the 150 real-mouse tests; WDC: 759 + tsc behind its `gate` job; nle-ui: 674 + boundary script; **the app repo (nle-test-app) is the roof: 256/256 seam/wired-whole tests — the 8-suite inventory (R29 re-key, @ `876f2b8`): GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + wire-coverage 13 + engineSeam 7 + waveformPeaks 9 — the census register 42 = 36 zero-action + 5 carriers + 1 host; + the crawl's K3 law-net corpus**; the mocks — mini 495 (R24-current) / variants 1521+, local + annotakit — are design references that retire per the port-then-swap law (post-K4/w1)). The app does NOT re-test module internals — with the one sanctioned exception of S3(a), which re-runs OT's real-mouse phases specifically to verify the VIEW COMPONENT in the app's host context (bundler/CSS/React), not the engine semantics. The app tests ONLY the seams and the wired whole. All counts below are the PR2-corrected figures from ARCH-R15 §3.2 (the S-suite SIZES are R15-era estimates; the LIVE fleet counts are the §0A/§13A.7 rows).

### 17A.1 The five suites

| Suite | Tier (this spec's naming) | What it pins | Size |
|---|---|---|---|
| **S1 seam contracts** | Tier-1 (vitest) | Projector laws (per 00 D12.1 + 14 §3 A1 (retired → the plan's ladder) + SCOUT-A §4): timebase round-trips, transition-window translation, keyframe normalization; one-wayness of EDITING state (telemetry excluded — the event staircase is the UP seam); bus routing completeness (every union member → one home or typed `NOT_IMPLEMENTED`); event-completeness + mapping; S/G/E trackId-only invariant | ~100-140 |
...
| **S5 spec-conformance battery** | mechanical | App-layer extension of the battery lineage (**`battery_r24.py` today** — the battery_r9 successor line, `nle-core-spec/scripts/`): routing-disposition table check; testid census; keymap ledger; **pin-lockset assertion** (parse the engine's `git submodule status` at the app's engine SHA and compare against the app's OT/WDC pins — the app's pins must be ≥ the engine's and move only WITH the engine pin); gap-register freshness (every row cites a pin SHA) | ~35 checks |
...
nle-engine (github.com/bearachprema/nle-engine — **R24 re-baseline @ `5036387`: 458/458 vitest (static-census-matched; the live-run venue is GHA CI) + tsc 0 (13 suites incl. the nle-bridge pair + nle-bridge-realtime-midi); the N1-N4 seam family + W1 meter bridge + N2b gain-automation (`37cdd28`) + the RR1-A fade-clamp SPAN round + the W2.5 effects sidecar (`buildElementFilterString`) + the R8-REV blur-radius clamp landed; vendor pins: OT @ `c15a629`, WDC @ `494f6ff`; the 455-name API freeze + layer fence stand**) is a clean-room
FreeCut-port **in-between reference, NOT canon**. Its R7-era test reality was single-tier (one
Playwright-driven in-app harness — the pattern this spec's three-tier methodology corrects; its own audit independently recommended the same split); since Round 15 the engine's CI runs three venues (Node vitest / browser milestones / probes) — the crawl re-tiering (the e2e half → K4 per the D24 split) has de facto converged engine-side, and the remaining re-tier surface is the app's (§17A). Where engine and spec conflict,
**the spec wins**. Full reconciliation: `19-code-references.md`.

| Spec 17 section | nle-engine file:line | Verified quote | Status | Note |
|---|---|---|---|---|
| §2.1 Tier 1 (Vitest) | `scripts/run-nle-tests.mjs:90` | `browser = await chromium.launch({` | CORRECTIVE (R7-era; **superseded at Round 15** — the engine has run a Node vitest tier since: 274 tests then, **458/458 across 13 suites at the R24 pin**, 3-job CI) | Was: all tests in one browser page, no Node tier |
