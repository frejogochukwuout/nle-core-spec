...
**BASE (accepted, pinned 2026-09-08 — the R24 re-pin; 00-master's fleet table is the pin canon, D21):** the three-tier methodology + matrix + template (§2-§11); the facet matrix §13A; the roof-suite law §17A — all EXECUTING (the **K1 module nets** are the proof, ALL GREEN at the R24 pins: **engine 458/458 @ `5036387`** (static-census-matched — the live-run venue is GHA CI) — 13 vitest suites (`tests/vitest/engine/`: timeline-edit-ops, video-sync, bridge-seams, planner, api-surface, load-validation, persistence, render-abort, timeline-math, transform-resolver, undo-serialize; `tests/vitest/`: nle-bridge (96 pins), nle-bridge-realtime-midi; bridge-seams now 113 — the W2.5 effects-sidecar +17 and the R8-REV blur-clamp +1; **N2b LANDED @ `37cdd28`** — 18 bridge pins + 2 REAL offline-render sample-level parity pins; the RR1-A fade-clamp SPAN round; the 455-name API freeze); **OT 536/536 @ `ded43c4` (code pin `c15a629` — the last three commits docs/runner-artifacts only)** — 386 in-page + 150 real-mouse across 59 milestone entries (`download/timeline-test-report.json` the count authority, live-run re-verified at the R24 audit; the T-round + W10F hardening + the W11 complete-UI round — 24 routed wire verbs + 6 documented exceptions, machine-checked by the M49C coverage gate; the canonical React timeline UI tree per D25/D26); **WDC 759/759 @ `85b81b0`** (docs-only over `494f6ff`) — 18 `src/test/` suites (the integration family + the Goertzel detection oracle; **W2 soundtouch/varispeed LANDED**; the drift-gate now fails-on-drift); **nle-ui 674 @ `fc4cc35`** (F3/F7 real-gain + the kind-aware mute-all pin + the D29 seam waves — the saveState lifecycle, the DeliverPage `exportRequest`, the per-scene grade sidecar; boundary-script-gated); **app 174/174 @ `c885ece`** (the 6-suite roof: GluedShell 70 + audioService 40 + sceneBridge 25 + persistence 17 + deliver 15 + engineSeam 7; the R8 D29 real-wiring wave — autosave/persistence, sidechain ducking, deliver export, effects/color preview — plus RR1-B and the W3 JKL audio half standing; the R9 D30 W-A/W-B re-pins mid-flight); **mini 396** (the R23 seal 355 + the R24-mini nets: clip-box + mini-plus W0/W1); **variants 1521+** (61 test files; the R23 W-A FX + W-B color waves + **the VLM net** `scripts/vlm-capture.mjs`/`vlm-review.mjs`/`vlm-run.sh`; 00-master's reconciliation note stands); consumer pins re-read live (the app consumes engine @ `5036387` — fleet-coherent, the R9 W-A re-pin; the OT lock-copy @ `c15a629`, byte-exact minus `testing/`; nle-ui @ `fc4cc35`; WDC @ `85b81b0` — opportunistic docs-only ahead of the engine's `494f6ff`; the engine's OT submodule @ `c15a629`); all CI-green).
...
- **the K3 offline audio pins** (unchanged in substance; owner: nle-test-app + WDC): the offline-render + call-spy pins (waveform/mute laws) per the Node-venue law (acceptance: the pins green; hard prerequisite: S-wdc's waveform seam contract — the only K3 sub-half with an external prerequisite; D29.4 promotes the contract statement into WDC's session-queue scope — spec 20 §0 owns the row);
...
- **the K3 app-side zero-no-op worklist (new R24 row; owner: nle-test-app / S-app — the D30 W-F/Z6 waves)**: the 9-A2 app-flow census — 30 surface groups / **177 controls: ~127 real / 13 degraded / 21 honest-mock / 16 no-op (incl. 9 unexpected-dead)** — plus its P1-P26 26-pin hardening plan + the Z1-Z7 fixes; **ALL PENDING at `c885ece`** (zero landed since the audit — only the W-A/W-B vendor/file re-pins, which change zero behavior); the W-F coverage-gate module + the Z6 dead-zone pin sweep are the K3-relevant halves, the rest harden the real-control surface the K3 corpus authors over (acceptance: the P-series + Z-series pins land with the D30 waves; the census re-run closes the unexpected-dead set);
- **the K4 automated e2e** (the crawl's completion proof; owner: nle-test-app): import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export, asserted programmatically — **driving the same 24 routed wire verbs through the real UI (D29.1c; the shell-path exception applies identically)** (acceptance: green end-to-end, zero mock paths, zero human input; w2's full-scope demo re-gates by extending this suite);
