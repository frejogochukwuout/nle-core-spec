...
- **the K4 automated e2e** (the crawl's completion proof; owner: nle-test-app): import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export, asserted programmatically — **driving the same 28 routed wire verbs through the real UI (D29.1c; the shell-path exception applies identically)** (acceptance: green end-to-end, zero mock paths, zero human input; w2's full-scope demo re-gates by extending this suite);
