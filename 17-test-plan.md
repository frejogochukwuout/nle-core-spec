...
3. **The crawl's law-net corpus is a first-class gate family:** `ui-mock/shell-mini/docs/LAW-NET-INVENTORY.md` (the pre-C1/pre-C4 deliverable — **LANDED at the R23 seal**) is the acceptance list the crawl app's re-expressed tests must satisfy: the sealed census (8 files / 355 tests / 128 law-family census units — 33 HOLDS-on-OT tests / **322 app-side authoring tests across 115 GAP units**; the 59-app-emitted-static + 15-templated testid census, the 60th static id harness-only — §2.2) + `CORE-SEAMS.md` (the S1-S26 seam inventory + the store partition + the transport map — K3's re-expression worklist). Per D24's split, the old C4 exit is now **K3** (the corpus checked row-by-row over REAL modules — mini-parity, ARCH-R22 Ruling B) + **K4** (the automated-e2e completion proof) + w1-entry (the human side-by-side).
4. **Count discipline is battery-enforced (battery_r23 — the battery_r22 successor):** declared == actual at the current pins; stale counts are failures (the R15→R22 drift — engine 274+265+318→356, OT 423→459, WDC 721→740, mock 596→640+333+1334+83 — and the R22→R23 drift — engine 356→440, OT 459→489, WDC 740→759, nle-ui 640→648, app 83→117, variants 1334→1542-live — are exactly the drift class the battery catches; the count-consistency check keys engine 440 + OT 489 across 17+19+00).

**BASE (accepted, pinned 2026-09-07 — the R23 re-pin; 00-master's fleet table is the pin canon, D21):** the three-tier methodology + matrix + template (§2-§11); the facet matrix §13A; the roof-suite law §17A — all EXECUTING (the **K1 module nets** are the proof, ALL GREEN at the R23 pins: **engine 440/440 @ `b8c6f88`** — 13 vitest suites (`tests/vitest/engine/`: timeline-edit-ops, video-sync, bridge-seams, planner, api-surface, load-validation, persistence, render-abort, timeline-math, transform-resolver, undo-serialize; `tests/vitest/`: nle-bridge, nle-bridge-realtime-midi; **N2b LANDED @ `37cdd28`** — 18 bridge pins + 2 REAL offline-render sample-level parity pins; the RR1-A fade-clamp SPAN round; the 455-name API freeze); **OT 489/489 @ `222532c`** — 359 in-page + 130 real-mouse across 51 milestone suites (`download/timeline-test-report.json` the count authority; the S-round + F1 whole-codebase hardening + M45/M46; the canonical React timeline UI tree per D25); **WDC 759/759 @ `494f6ff`** — 18 `src/test/` suites (the integration family + the Goertzel detection oracle; **W2 soundtouch/varispeed LANDED**; the drift-gate now fails-on-drift); **nle-ui 648 @ `85dcf57`** (F3/F7 real-gain + the kind-aware mute-all pin; boundary-script-gated); **app 117/117 @ `70e99f0`** (the W3 JKL audio half + RR1-B); **mini 355** (sealed); **variants 1,542-live** (the R23 W-A FX + W-B color waves + **the VLM net** `scripts/vlm-capture.mjs`/`vlm-review.mjs`/`vlm-run.sh`; 00-master's 1,470 is its pin-era figure — the battery's central re-baseline reconciles); consumer pins re-read live (the app consumes engine @ `4ef0147` — behind, the consumer re-pin wave queued; the OT vendored mirror @ `a4e971d`; WDC @ `f446512`); all CI-green).

**GAP (the work — owner + phase per the D24 ladder in `IMPLEMENTATION-PLAN.md`; the R22 C/W/R tags re-keyed via the D24 mapping table, dual-tagged through the fleet window; the retired spec-14 §4 testing-domain rows are re-homed HERE per the posture law):**
- the LAW-NET-INVENTORY corpus (pre-C1/pre-C4 — **LANDED R23**; the K3/K4 gates now consume it — the seal's census is K3's acceptance list);
- **the K2 combined-family census + registration (the new K2 row — spec 12's twin; owner: nle-core-spec, at K2 entry)**: formalize the cross-module corpus as a NAMED suite family — engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`: vendored OT ops × engine composition), WDC's integration family (`audio-integration`/`dsp-bounce-parity`/`dsp-effects-integration`/`nle-audio-core-derisk` + the `offline-parity-wiring`/`real-audio-e2e` pair), OT's in-page UI-over-core runner (the 359 in-page + 130 real-mouse M-suites) — with a census + a register in spec 12 + here; close the missing cross-module pairs (the OT-ops→engine-decode round-trips at the app's seam shapes — S-engine's worklist item) (acceptance: the family census green + registered in spec 12/17 §13A.7 + battery-checked thereafter);
- the crawl app's corpus authoring (**K3** — was C4, split per D24: the corpus-check half is K3, the e2e half is K4, the human side-by-side → w1-entry; owner: nle-test-app): the exact counted scope — 322 app-side authoring tests / 115 law-family census units, per LAW-NET-INVENTORY §2.3-§2.4 (supersedes the ~130-180 estimate); includes the composed ripple family (4 census units / 16 tests composed app-side over `timeline.trim`/`rippleDelete` — r1 graduates the OT interval-diff family) and the DOM-structural half (the 59+15 testid census) landing AFTER the D25 swap + OT's testid-emission layer (the ordering law) while the store/policy halves author ∥ against the injection-point contracts (acceptance: the corpus green row-by-row over REAL modules, zero mock paths);
- **the K3 offline audio pins** (was C3's audio crawl, unchanged in substance; owner: nle-test-app + WDC): the offline-render + call-spy pins (waveform/mute laws) per the Node-venue law (acceptance: the pins green; hard prerequisite: S-wdc's waveform seam contract — the only K3 sub-half with an external prerequisite);
...
- **the K4 automated e2e** (was C4's e2e half — the crawl's completion proof; owner: nle-test-app): import(virtual) → cut (drag/trim/split/ripple) → play (engine clock) → export, asserted programmatically (acceptance: green end-to-end, zero mock paths, zero human input; w2's full-scope demo re-gates by extending this suite);
- the offline-parity threshold pins + the null rig (**r2** — was W-audio; pass: max deviation ≤ −60 dBFS any channel);
- the grade-math parity pins (**r3** — was W-color);
- the real-file decode corpus (**r4** — was W-n5; PENDING the user's D6 re-affirmation, deprioritized);
- the FCPXML fixture corpus (**r5** — was R-fcpxml; the phase-entry artifact);
- the battery's posture checks (**R23 — battery_r23**: the pin/count key set + the plan/stub checks + the per-file-audit residue sweep).

**The D24 verification ladder, spec-17-side (where acceptance executes — `IMPLEMENTATION-PLAN.md` §2 is the executable gate table; this §0A is the tier vocabulary's spec home):**
- **K1 — module nets:** each repo's own suite at its own gates (the BASE rows above; this spec's three-tier methodology §2 is the per-repo shape). ALL GREEN at the R23 pins.
- **K2 — combined nets (cross-module):** the named family (the GAP register's census row) — engine-in-the-middle, WDC's integration family, OT's in-page runner (pixel reads + real-mouse + WYSIWYG state). The seeds exist at HEAD; the census + register land at K2 entry.
- **K3 — app-behavior nets:** the LAW-NET corpus re-expression (322 authored + 33 projection pins), the testid-census DOM-structural gate, the offline audio pins, the VLM visual net — over REAL modules, driven headlessly.
- **K4 — the crawl exit gate:** the single automated e2e (import → cut → play → export) — zero mock paths, zero human input; the crawl's completion proof.
...
- **Audio-domain seam properties (spec 20 §5, web-daw-core's suites as the executable reference)**: merge laws (same-media adjacent segments merge phase-continuously; fades block; rate boundaries never merge), null-parity ≥60 dB offline-vs-realtime, canonical send/return (no dry leak), pan law −3 dB at strip level — pinned by WDC's `nle-audio-core-derisk.test.ts` (H0-H7 incl. H3; **759/759 @ `494f6ff` — the R23 re-run; the W2 soundtouch/varispeed family + the Goertzel detection oracle landed since the R22-era 740**) + the bridge suite NOW AT nle-engine `tests/vitest/nle-bridge.test.ts` (the C1/H8-H17 family — 37 tests at the R22 citation, grown to ~96 pins at the R23 pin by the S/W rounds + **N2b LANDED @ `37cdd28`** (18 bridge pins + 2 REAL offline-render sample-level parity pins, the dB-domain gain-automation law); relocated verbatim with the bridge per WDC's M1.6 — the old in-WDC nle-bridge citation is superseded) + the sibling `tests/vitest/nle-bridge-realtime-midi.test.ts` (the H18 scored-notes/MIDI bridge family, the realtime half)
...
| Reference-repo re-tiering (**R23 re-baseline: engine 440, OT 489 (359 in-page + 130 real-mouse), WDC 759, nle-ui 648, nle-test-app 117, mini 355 (sealed, R23), variants 1,542-live**) | 19 §12, 00 D14 | NF (process) | — | re-baselined to the **2026-09-07 verified counts (the R23 pins; 00-master's fleet table the canon, D21)**: engine 440 vitest (13 suites: `tests/vitest/engine/` 11 + `tests/vitest/` nle-bridge pair) → T1/T2 (3-job CI); OT 489 (359 in-page M1-M46 + 130 real-mouse across the 14-suite R-family — 51 milestone suites, the report json the count authority) → T1/T2/T3 (the real-mouse phases ARE Tier-3-grade, in-repo; the in-page runner is a K2 seed); WDC 759 → T1 (null-test gates are the audio T1; the 18-suite integration family is the K2 seed); nle-ui 648 → T3-package (engine-free chrome tier, boundary-script-gated); **nle-test-app 117 → THE ROOF** (the app tier: the seam/wired-whole suite — promoted from the retired "mock 596" row: the mock split into nle-ui (648) + the crawl-app corpus (K3, LAW-NET-INVENTORY)); mini 355 + variants 1,542-live → design-reference tier (local + annotakit; retire per the port-then-swap law — post-K4/w1); count discipline (declared == scraped, battery_r23-enforced) | zero count drift |
| K2 combined-family census + registration (Round 23 — spec 12's twin row) | ARCH-R23 D24, `IMPLEMENTATION-PLAN.md` S-spec item 2 | NF (process) | T1/T2/T3 (cross-module) | the named cross-module family censused + registered in spec 12 + HERE (§0A's GAP register row): engine-in-the-middle (`timeline-edit-ops`/`video-sync`/`bridge-seams`/`planner`), WDC's integration family, OT's in-page runner; the missing pairs (OT-ops→engine-decode round-trips at the app's seam shapes) authored at the engine | census green + registered + battery-checked; the missing-pair suites green |
...
| **S5 spec-conformance battery** | mechanical | App-layer extension of the battery lineage (**`battery_r23.py` today** — the battery_r9 successor line, `nle-core-spec/scripts/`): routing-disposition table check; testid census; keymap ledger; **pin-lockset assertion** (parse the engine's `git submodule status` at the app's engine SHA and compare against the app's OT/WDC pins — the app's pins must be ≥ the engine's and move only WITH the engine pin); gap-register freshness (every row cites a pin SHA) | ~35 checks |
...
nle-engine (github.com/bearachprema/nle-engine — **R23 re-baseline @ `b8c6f88`: 440/440 vitest + tsc 0 (13 suites incl. the nle-bridge pair + nle-bridge-realtime-midi); the N1-N4 seam family + W1 meter bridge + N2b gain-automation (`37cdd28`) + the RR1-A fade-clamp SPAN round landed; vendor pins: OT @ `a4e971d`, WDC @ `f446512`; the 455-name API freeze + layer fence stand**) is a clean-room
FreeCut-port **in-between reference, NOT canon**. Its R7-era test reality was single-tier (one
Playwright-driven in-app harness — the pattern this spec's three-tier methodology corrects; its own audit independently recommended the same split); since Round 15 the engine's CI runs three venues (Node vitest / browser milestones / probes) — the crawl re-tiering (the old C4→K4 mapping per D24) has de facto converged engine-side, and the remaining re-tier surface is the app's (§17A). Where engine and spec conflict,
**the spec wins**. Full reconciliation: `19-code-references.md`.

| Spec 17 section | nle-engine file:line | Verified quote | Status | Note |
|---|---|---|---|---|
| §2.1 Tier 1 (Vitest) | `scripts/run-nle-tests.mjs:90` | `browser = await chromium.launch({` | CORRECTIVE (R7-era; **superseded at Round 15** — the engine has run a Node vitest tier since: 274 tests then, **440/440 across 13 suites at the R23 pin**, 3-job CI) | Was: all tests in one browser page, no Node tier |
