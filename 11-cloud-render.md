...
**BASE (accepted, pinned 2026-09-07; line-pins re-verified at the R23 HEADs and re-verified unchanged at the R24 HEADs, 2026-09-08; the R27 re-verify at engine `f9ac806` moved four pins — DECISIONS :226→:242, orchestrator :524→:544, api.ts 2,757→2,777, export/ 1,785→1,806 — see §14R's R27 note; the R33 re-verify at engine `2a0ecf4`: the four pins HOLD verbatim (DECISIONS :242, orchestrator :544, api.ts 2,777, export/ 1,806 — the R33 src movement is timeline.ts-only; R33 re-key)):**
...
- **Zero cloud-SERVICE code re-verified at the R23 pins and again at the R24 pins** (engine `5036387` 458/458 static-census-matched · OT HEAD `ded43c4` code `c15a629` 536 · WDC `85b81b0` 759 · nle-ui `fc4cc35` 674 · app `c885ece` 174 — grepped; the R8/R9 engine rounds were W2.5 effects + fleet re-pins, no cloud code): no Node render driver/server, no render queue, no ffmpeg subprocess, no S3/RemoteStorage, no Dockerfile/RunPod surface, no real-GPU flag set anywhere. The engine's own driver is Xvfb+SwiftShader software-WebGPU **by design** (its Decision 12); its P3 "Node-side HTTP headless driver" gap is still open (`gaps/audit/MASTER.md:166`). Nothing shipped, nothing to regress.
- Historical context for the unchartered posture (D27.3 — reference only, no dependency; the primary row lives in 00-master §0): the dormant predecessor **cloudcut-nle** (June-July 2026, dispositioned DORMANT) carried the one REAL server-side render path this fleet carries none of — `mini-services/render-worker/index.ts` (419 LOC: Node `child_process` FFmpeg with atomic job claim, 10s heartbeat, timeout-kill, retry) + the `/api/render` queue routes — a built-but-dormant §6.1/§8-§10-shaped cloud half; its successor notes (`docs/planning/NOTES_FOR_SUCCESSOR.md`) disposition it as history, and the r6 re-scope remains the user's call.
- **The design's browser-side halves are REAL in the engine at HEAD (new since the R22 pin; the honest refinement of the old "fleet has ZERO cloud code" claim):** (a) the headless harness — `src/lib/nle/headless/api.ts` (2,777 LOC at the R27 pin `f9ac806`; the 9-method `window.freecut`-shaped `NleHeadlessApi` with `AbortSignal` support + `HARNESS_READY_PREDICATE`, mounted in-page with the REAL render adapter at the m24.9/m26.10 browser milestones); (b) the in-browser encode path = this spec's §11.4 **Path A** — `src/lib/nle/export/` (orchestrator 957 / settings 368 / audio-mixdown 369 / contracts 92 + index 20 LOC — 1,806 total; Wave 5B Export M1 + Stage 2 audio + m29 A/V mux; WebCodecs+mediabunny; decode-verified incl. pixel parity vs `renderFrameOffscreen` — see `gaps/audit/MASTER.md` rows 5B/5B-S2/5B-S2C). Path B (the ffmpeg raw-frame pipe, §11.4/§17), the queue/server/storage layer (§8-§10), the real-GPU flag set + RunPod/Dockerfile (§4.1/§18) remain SPEC-ONLY.
...
- **NONE scheduled.** **r6 — unchartered** (the retired spec-14 §3.3 RUN row, re-pointed through the stub's §-redirect to `IMPLEMENTATION-PLAN.md` §2's r6 row: "cloud render UNCHARTERED unless the user re-scopes"; D24's non-blocking tail; the pre-D24 phase tag retired at the R24 fleet window-close), a non-goal until the user re-scopes it. Acceptance: n/a until re-scoped by the user. Do not invent work. [This row IS the retired spec-14 §4 cloud row's home — the unchartered ruling re-homed per D23.3/D24; the battery's spec-11 (with 13) no-gap-rows exemption stays honored: this row is the honest register. Owner at activation time: n/a (a new cloud service repo or an app-side seam — the re-scope decides).]
...
| File | LOC | Cross-ref |
|---|---|---|
| `00-master-spec.md` | 863 | Master decisions (WebGPU-only, 10-bit scene-linear, cloud render) |
| `01-core-engine.md` | 2272 | `createRenderEngine()` entry point — SCOUT-01 verified FreeCut headless harness exists but didn't fully read it; SCOUT-11 read in full |
| `04-renderer-color.md` | 2398 | `WebGPURenderer.readPixels()` — SCOUT-04 verified FreeCut has no readback (uses mediabunny VideoSample directly); our spec must add readback for Path B (ffmpeg pipe) |
| `02-workers-threading.md` | 2649 | SCOUT-02 verified AudioWorklet `numberOfInputs: 0` (NOT 1) and SoundTouch source-pushed-via-port; vendored soundtouchjs v0.2.3 LGPL v2.1 |
| `03-playback-engine.md` | 2792 | SCOUT-03 verified mediabunny UrlSource exists and the pixelFormat: 'P010' option does NOT exist |

### 14R. Code References — nle-engine (reference, NOT canon)

> nle-engine has **no cloud-SERVICE render** (re-verified at `5036387` in the R24 round, previously `b8c6f88` — see §0 BASE) — but it DOES carry both browser-side halves this spec designs against: `src/lib/nle/headless/api.ts` (2,777 LOC, the harness API) + `src/lib/nle/export/` (this spec's §11.4 Path A encode path, decode-verified). The engine's Xvfb+SwiftShader headless setup (its Decision 12) is an ALIGNED reference for this spec's headless-Chrome infrastructure; the cloud pipeline (driver/queue/ffmpeg/server/S3) remains SPEC-ONLY. Full reconciliation: `19-code-references.md`.
...
| Xvfb prerequisite | `.agents/DECISIONS.md:242` | `` `--headless=new` alone returns `null` from `requestAdapter()`. `` | ALIGNED (CI) | Xvfb mandatory even headless; spec §4.1 gains the CI-fallback note |
...
> **R24 re-verification (2026-09-08, `5036387` 458/458 static-census-matched):** the two-halves truth re-verified — zero cloud-service code by grep across all five R24 HEADs (`5036387` · `ded43c4` · `85b81b0` · `fc4cc35` · `c885ece`; same false-positive classes as R23: the S3-seam wave-tags, WDC's FFmpeg-bake design comments, the mock DeliverPages); the engine's only src deltas since `b8c6f88` are the W2.5 effects sidecar + the R8-REV blur-radius clamp (composition-frame.ts + bridge-seams.test.ts) + the two fleet-coherence re-pins (OT `c15a629`, WDC `494f6ff`) — NO cloud code. Both browser-side halves unchanged and still landed: headless/api.ts 2,757 LOC; export/ 5 files, 1,785 LOC (936/368/369/92/20); every quoted line-pin re-verified in place — `NleHeadlessApi` :111, the frame-grab doc :118, MASTER's P3 row :166, the SwiftShader flag list :66-72, `run-nle-tests.mjs:8`, `DECISIONS.md:29/:226`, `G-test-coverage.md:26`, `createRenderAdapter` (orchestrator.ts:524), render-abort's 9 pins, and the m24.9/m26.10 harness mounts in the engine's `src/app/page.tsx` (:8117/:9382). New since R23, browser-side only: nle-ui's W2.4/D29 `exportRequest` Deliver seam + the app's W2.4 real deliver export (deliverService.ts, mediabunny, in-browser — the absent-world mock stays verbatim) — Path-A-adjacent surface, zero cloud-service work. The activation-time ENGINE-GAPs are unchanged (P3 HTTP driver, Stage-3 worker/OPFS, Stage-5 smart-copy per `gaps/audit/C-export-encode.md`).

> **R27 re-verification (2026-09-14; at R27 the pins were: engine `f9ac806` 748/748 · OT `55c81c0`/`970948a` · WDC `ec8fd5c` · nle-ui `32abd58` · app `c020b2a`):** zero cloud-service code re-verified by grep across all five R27 pins (same false-positive classes: the S3A wave-tags, WDC's FFmpeg-bake design comments, the mock DeliverPages); the RC round's engine deltas (the D12 ratification, OV-01/02/05/12, M2 Wave 1's materializeStrips, the NS queue) are laws + docs, NO cloud code. The two browser-side halves unchanged and still landed: headless/api.ts **2,777** LOC (NleHeadlessApi :111, frame-grab :118, HARNESS_READY_PREDICATE :2771 — frozen in api-surface.frozen.ts:62); export/ 5 files **1,806** LOC (orchestrator **957** — `createRenderAdapter` now **:544**, `renderCompositionCore` :187; settings 368 / audio-mixdown 369 / contracts 92 / index 20). Moved pins: `.agents/DECISIONS.md` :226→**:242** (the D11/D12 entries); `run-nle-tests.mjs:8/:66`, `MASTER.md:166`, `G-test-coverage.md:26`, `DECISIONS.md:29`, the page.tsx m24.9/m26.10 mounts (:8117/:9382) all HOLD. Browser-side additions to the Path-A-adjacent surface: the app's `deliverService.ts` (418 L, D29 W2.4 export host — in-browser mediabunny, blobUrl, no server) now carries **Z2** (D30 W-E: the scene grade reaches the export, :300) and consumes the M2 Wave 1 audio law (a FRESH SceneMixer.renderOffline over the real segment pipeline incl. foldSidechains; both paths build through `materializeStrips` — scene-mixer.ts:50-51). The activation-time ENGINE-GAPs unchanged (P3 HTTP driver, Stage-3 worker/OPFS, Stage-5 smart-copy per `gaps/audit/C-export-encode.md`).

> **R33 re-verification (engine `2a0ecf4` 785/785 · OT HEAD `008e7f3`/code pin `344123a` 675/675 · WDC `94f6460` 777/777 · nle-ui `1e0c30b` 743/743 · app `7664603` 426/426):** zero cloud-service code re-verified by grep across all five R33 pins (same false-positive classes: the S3A wave-tags, WDC's FFmpeg-bake design comments, the mock DeliverPages); the R33 engine movement is the exhaustive-testing waves (the W2 property tier + the P-F1/P-F2 fixes — timeline.ts-only, NO cloud code); the s18 seal round (OT) is lib-tree + flake-fix code, NO cloud code. The two browser-side halves unchanged and still landed: headless/api.ts 2,777 LOC (`NleHeadlessApi` :111, the frame-grab doc :118, `HARNESS_READY_PREDICATE` :2771); export/ 5 files 1,806 LOC (orchestrator 957 — `createRenderAdapter` :544; settings 368 / audio-mixdown 369 / contracts 92 / index 20). The activation-time ENGINE-GAPs unchanged (P3 HTTP driver, Stage-3 worker/OPFS, Stage-5 smart-copy per `gaps/audit/C-export-encode.md`). (R33 re-key)
...
- `render-progress-updates-via-websocket` — after `Enter`, the dialog subscribes via `POST /api/engine/subscribe` (SSE/WebSocket per spec 15 §8.7); the test injects synthetic `RenderProgress` events (frame 10/100, 50/100, 100/100); the dialog's progress bar updates to `10%`, `50%`, `100%`
