...
- opencut-timeline @ HEAD `008e7f3` / **code pin `344123a`** — **675/675, 77 milestones** (the report json's own breakdown: **632 classic + 17 M60-theme + 11 M61 + 11 M63 + 3 M62 + 1 M62-gate** — the M63 figure is 11 per `download/timeline-test-report.json`; the seal18-FINAL commit message's "10 M63" is a typo; re-keyed R33, engine seal-round register P3-19; history: the R24 reading 536/536 @ HEAD `ded43c4` / code pin `c15a629`, docs-only src diff — re-based R27; the R29 re-key `55c81c0`→`3e18722` docs-only; the register's own interim re-key `31f2764` w/ code pin `970948a` UNCHANGED was itself lagged — the R32-a M60 mini-theme wave (`670ef59`..`a6846a6`) + the seal18 L-waves touched `src/`, so the code pin moved `970948a`→`344123a` and the count 632→675) — **the placeholder compositor** (re-verified present in the tree: `src/lib/timeline/render/placeholder-compositor.ts`: Canvas2D solid-color composition behind `setTracks()` + `renderFrame(t)`, pixel-verified by M12's `readCenterPixel` in `testing/milestones-view.ts`; the file's own header names THIS spec's renderer as the real engine that plugs in behind the same contract — only the painter changes). The app consumes the byte-exact lock-copy (`vendor/nle-timeline` = the lock-declared `17a19f8` minus `testing/` — byte-identical at HEAD `008e7f3` too (the two commits past `17a19f8` touch only `download/`+`scripts/`), plus the `vendor/nle-timeline-ui` census-reference mirror at the SAME upstream SHA (D26.2's port-census law, never imported by app code); was `c15a629` — two-plus generations stale; live-verified R33).
