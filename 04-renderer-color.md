...
- nle-engine @ `5036387` — **458/458 vitest, tsc 0** (was 440 @ `b8c6f88` at the R23 pin; +18 = the W2.5 +17 + R8-REV #5's +1) — the renderer stack: the WebGPU compositor (`gpu/compositor.ts`), the **44-effect registry** (43 in `effects/pipeline.ts` + `gpu-lut` in `effects/lut.ts` — re-verified), real A/V export; vendors OT @ `c15a629` + WDC @ `494f6ff` (git submodules — the R9-W-A fleet-coherence re-pins; CORE-ONLY alias discipline). **Landed since the R23 pin (the round's only engine source deltas, all inside the 458) — the W2.5 D29 effects sidecar + filter string (engine commits `8a0b7fe` + `4b2dfd1`):** `CompositionElementParams.effects` (the `{name, params}` bag — the consumer's Inspector stack minus the consumer-local id/enabled fields; **enabled-only is the consumer-domain law**, filtered BEFORE the seam) + **`buildElementFilterString`** (`bridge/composition-frame.ts:223` — the pinnable law) resolving **the honest CSS-filter subset**: **Gaussian Blur → `blur(radius px)`** (radius from params, default 4, non-finite→4, negative clamped ≥ 0 per the R8-REV #5 — invalid CSS would no-op the whole `ctx.filter` assignment); **everything else honest-skips to the GPU registry's fidelity** (the 44-effect registry renders Vignette/Glow/Film Grain/Motion Blur at fidelity — a fake Canvas2D approximation would preview what the render never ships); compatible effects join with spaces; identity `''`. The `CompositionDrawOp.filter` field (:161) carries the string; the painter assigns `ctx.filter` per element inside its save/restore bracket (Safari < 18 silently no-ops — graceful) — **the renderer-side preview v1**. The N1 composition-frame bridge family (`buildCompositionFrame` pure composition math + `paintCompositionFrame` thin Canvas2D painter; `bridge-seams.test.ts` now 113) + the R22-era landing list (the **W1 meter bridge** — `bridge/realtime-engine.ts`'s real `onMeter` push + `onBusMeter`; the RR1-A fade-clamp SPAN + toSorted consumer-compat rounds; N2b keyframed volume (`37cdd28`); the S3 seam-audit wave; the K2 seeds: `timeline-edit-ops` / `video-sync` / `bridge-seams` / `planner`) — all re-verified at the new pin.
