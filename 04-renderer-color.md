...
**Status:** v-next (Round 24 — the per-file audit fleet's re-verify: the §0 BASE re-pinned to the R24 pins (engine `5036387` 458/458; OT `ded43c4` code-pin `c15a629` 536; app `c885ece` 174; nle-ui `fc4cc35` 674) and the **W2.5 D29 effects/filter landing promoted to BASE** (engine `8a0b7fe` + R8-REV `4b2dfd1` — the effects sidecar in CompositionElementParams + `buildElementFilterString`'s honest CSS-filter subset; app `41f5bc9` — ProgramCanvas's el.effects mapping + the per-scene SCENE GRADE final pass; nle-ui `7280372` — the grade sidecar seam + ColorPage LIVE sliders); the register gains the **scene-grade DECLINE decision row (D29.5c)** and the **CSS-filter widening OPEN gap (D29.5d)**; the phase tags are the D24 set only (the dual-vocabulary window closed this round); the R23 re-verify's facts stand (the N1 composition-frame family, the OT placeholder compositor, the app ProgramCanvas, the variants' W-B color wave in BASE; §13D's engine refs re-verified — the Wave 4A round fixed the port's two P0s, two rows flipped); the forward work: the CSS-filter widening (S-engine, D29.5d) + the color instruments (S-engine, consumed at r3) + the grade-math binding (r3) + the projector parity corpus (S-engine ∥ crawl) + CPU transition fallback (r6)); Refined spec — sub-agent scout SCOUT-04 has verified all claims against FreeCut + OpenCut-classic source
...
**BASE (accepted, re-verified @ the R24 pins — engine `5036387`, OT `ded43c4` (code pin `c15a629`), app `c885ece`, nle-ui `fc4cc35`, variants `e89cd65`):**
- nle-engine @ `5036387` — **458/458 vitest, tsc 0** (was 440 @ `b8c6f88` at the R23 pin; +18 = the W2.5 +17 + R8-REV #5's +1) — the renderer stack: the WebGPU compositor (`gpu/compositor.ts`), the **44-effect registry** (43 in `effects/pipeline.ts` + `gpu-lut` in `effects/lut.ts` — re-verified), real A/V export; vendors OT @ `17a19f8` + WDC @ `94f6460` (git submodules — re-keyed R33, engine seal-round register P3-20, verified via the engine's gitlinks @ `2a0ecf4`; the row's prior `c15a629`+`494f6ff` were the R24-era R9-W-A fleet-coherence re-pins, two-plus moves behind — the register's audit-time `55c81c0`+`83b8850` and the M60 re-pin `39003d3` interim generations superseded; CORE-ONLY alias discipline). **Landed since the R23 pin (the round's only engine source deltas, all inside the 458) — the W2.5 D29 effects sidecar + filter string (engine commits `8a0b7fe` + `4b2dfd1`):** `CompositionElementParams.effects` (the `{name, params}` bag — the consumer's Inspector stack minus the consumer-local id/enabled fields; **enabled-only is the consumer-domain law**, filtered BEFORE the seam) + **`buildElementFilterString`** (`bridge/composition-frame.ts:257` — the pinnable law) resolving **the honest CSS-filter subset**: **Gaussian Blur → `blur(radius px)`** (radius from params, **default 10 — the GPU registry's declared default per the F2-5 cross-module coherence pin** (the old default-4 previewed a blur the render never shipped), non-finite→10, negative clamped ≥ 0 per the R8-REV #5 — invalid CSS would no-op the whole `ctx.filter` assignment); **everything else honest-skips to the GPU registry's fidelity** (the 44-effect registry renders Vignette/Glow/Film Grain/Motion Blur at fidelity — a fake Canvas2D approximation would preview what the render never ships); compatible effects join with spaces; identity `''`. The `CompositionDrawOp.filter` field (:179) carries the string; the painter assigns `ctx.filter` per element inside its save/restore bracket (Safari < 18 silently no-ops — graceful) — **the renderer-side preview v1**. The N1 composition-frame bridge family (`buildCompositionFrame` pure composition math + `paintCompositionFrame` thin Canvas2D painter; `bridge-seams.test.ts` now **151 @ `f9ac806`** (the F2-5/F2-6/F2-7/OV-05/P1 hardening waves)) + the R22-era landing list (the **W1 meter bridge** — `bridge/realtime-engine.ts`'s real `onMeter` push + `onBusMeter`; the RR1-A fade-clamp SPAN + toSorted consumer-compat rounds; N2b keyframed volume (`37cdd28`); the S3 seam-audit wave; the K2 seeds: `timeline-edit-ops` / `video-sync` / `bridge-seams` / `planner`) — all re-verified at the new pin.
- opencut-timeline @ HEAD `8f96ab2` / **code pin `8f96ab2`** — **714/714, 83 milestones** (re-keyed R34, was `008e7f3`/`344123a`/675 @ 77: the M64 fps-param wave + the M59/M59A/B/C/D CR carrier families — CR-2's TimelineContextMenu functional form + `EMPTY_CONTEXT_ITEMS`, CR-3 `scrollAnchorId`, CR-4 the CancelRegistry barrel, CR-5 `performElementDelete` + the confirm-delete carrier, CR-8 `surfaceOwnedKeys` + the exported `SURFACE_OWNED_KEYS`, CR-9 the DragEventLike JSDoc; the report json at the pin the count authority; `src/lib/timeline/{ops,headless}` byte-unchanged — the wire census 31 = 28 routed + 3 exceptions UNCHANGED. The R33 layer: the report json's own breakdown: **632 classic + 17 M60-theme + 11 M61 + 11 M63 + 3 M62 + 1 M62-gate** — the M63 figure is 11 per `download/timeline-test-report.json`; the seal18-FINAL commit message's "10 M63" is a typo; re-keyed R33, engine seal-round register P3-19; history: the R24 reading 536/536 @ HEAD `ded43c4` / code pin `c15a629`, docs-only src diff — re-based R27; the R29 re-key `55c81c0`→`3e18722` docs-only; the register's own interim re-key `31f2764` w/ code pin `970948a` UNCHANGED was itself lagged — the R32-a M60 mini-theme wave (`670ef59`..`a6846a6`) + the seal18 L-waves touched `src/`, so the code pin moved `970948a`→`344123a` and the count 632→675) — **the placeholder compositor** (re-verified present in the tree: `src/lib/timeline/render/placeholder-compositor.ts`: Canvas2D solid-color composition behind `setTracks()` + `renderFrame(t)`, pixel-verified by M12's `readCenterPixel` in `testing/milestones-view.ts`; the file's own header names THIS spec's renderer as the real engine that plugs in behind the same contract — only the painter changes). The app consumes the byte-exact lock-copy (`vendor/nle-timeline` = the lock-declared `05d88d9` minus `testing/` (re-keyed R34, was `17a19f8` — the app's R34 lockstep re-pin `d28847c`; the lib tree byte-unchanged past it — the `05d88d9..8f96ab2` delta touches `src/components/timeline/` only), plus the `vendor/nle-timeline-ui` census-reference mirror at the SAME upstream SHA (D26.2's port-census law, never imported by app code); was `c15a629` — two-plus generations stale; live-verified R33).
- nle-test-app @ `c885ece` — 174/174 — **ProgramCanvas** (`src/ProgramCanvas.tsx`): the app's program monitor hosting the engine's N1 composition seam (the live core scene + D2e sidecars + the hash-color virtual-media resolver; jsdom pins the composition math via the data attributes). **The W2.5 app-side consumer rows landed (R8 `41f5bc9`):** ProgramCanvas maps `el.effects` (**enabled-only — the consumer-domain law**; the id field dropped at the seam) via the engine's `buildElementFilterString`, **+ the per-scene SCENE GRADE as a FINAL PASS** — compose-then-filter: the frame composes on a scratch canvas, then ONE filtered `drawImage` onto the monitor (canonical order contrast/saturate/brightness/hue-rotate, all four present when non-neutral; **the grade string is `''` when neutral — the direct paint path stays the default**); the ops' filter strings + the grade string are the jsdom pin surface (`data-filters` / `data-grade`). Honest-mock (by design): the LUT/wheels preview stays mock — the GPU fidelity path (the nle-ui ColorPage ledger below).
- nle-ui @ `fc4cc35` — 674/674 (live-run) — the engine-free shell package; **the W2.5 grade sidecar seam** (`7280372`, +8 pins): the store's `color: Record<sceneId, SceneGrade>` ({contrast, saturation, brightness, hue} — the CSS-filter domain, 1/1/1/0 neutral) + `setGrade` (merges into the neutral-seeded record; view-state law — no history entry, never in SceneJSON; the app's persistence carries the snapshot) + **ColorPage LIVE sliders** — Contrast + Saturation store-driven (−100..100 ⇄ 0..2), following scene switches. Honest-mock ledger v1: Pivot + Qualifier hue stay honest-local (marked, no store write); brightness/hue are contract-complete but slider-less; **the LUT select + the four wheels stay honest-mock** ("the preview lands with the render round, spec 08") — the GPU fidelity path.
- MOCK-side design reference (pinned in-repo, NOT shipping code): `ui-mock/shell-variants` @ `e89cd65` — **1,939 it-blocks / 68 test files** (the R28 WRAP re-key per R16 — ARCH-R28 §4; the battery's line-start method; the declared pair recorded alongside: **1,950 runner-count / 126 stories**, the collector-unit reconciliation — 1,939 line-start + 7 REPLAY-case fan-outs + 4 AA5 loop iterations = 1,950 exactly; this row's prior 1,521 and 00-master's 1470 are point-in-time figures — the battery's central post-fleet re-baseline reconciles) — the W4 wave (`src/lib/color/`: colorSpace / gradeMath / qualifierMath / scopesMath + gradedImage; spec-08-exact math + real scopes data) **+ the R23 W-B color wave LANDED (`db58d8d`)**: ScopesDock (the four scope TABS — Luma WFM / RGB Parade / Vector / Histogram — in the timeline-area console row), the nodes-in-viewer surface (ColorNodeGraph owns the viewer region), StillsPanel (the DaVinci-style Gallery of clip-level grade records), and the `timelineCompact` auto|on|off density toggles — **the mock-side reference half of r3's parity gates**. Negative result (the mini's `CORE-SEAMS.md` audited-absent register): the mini carries NO color instruments — S-engine's scope (GAP row 1).

**GAP (the work — owner + phase per the D24 ladder / `IMPLEMENTATION-PLAN.md` §2; acceptance in parentheses):**
- **The color instruments — scopes (waveform/vectorscope), secondary qualifier, power window — have ZERO engine code today** (re-verified @ `5036387`: **still zero progress** — the only engine mentions are the FreeCut wire-schema strings `gpu-power-window` / `gpu-secondary-qualifier` in `headless/api.ts`'s `GPU_EFFECT_TYPES` (:1492 the const, :1529 `gpu-power-window`, :1535 `gpu-secondary-qualifier` — the byte-identical wire contract, NOT implementations; the wire↔registry census (R33, engine seal-round register P2-5): `GPU_EFFECT_TYPES` advertises **54** vs the **44-id registry** — **11 advertised-but-unimplemented** (`gpu-ascii`, `gpu-blocks`, `gpu-box-blur`, `gpu-dither`, `gpu-droste`, `gpu-pixel-sort-hq`, `gpu-power-window`, `gpu-secondary-qualifier`, `gpu-threshold`, `gpu-trigger-wave`, `gpu-wave` — each passes the `z.enum(GPU_EFFECT_TYPES)` gates at `api.ts:1637/:1773` then degrades via the warn-once-then-skip at `pipeline.ts:5044-5051`) **+ `gpu-tint` implemented-but-unadvertised** (`pipeline.ts:716`, wire-rejected) = the 11+1 mismatch, re-verified live @ `60232ea` (re-keyed R34, was `2a0ecf4` — `headless/api.ts` + `effects/pipeline.ts` byte-unchanged through the P-F4 round, every cite above exact); the two instruments above are the color-relevant subset of the 11) plus the param-metadata comment at `effects/pipeline.ts:12`; the engine's P2 backlog still lists "scopes (waveform/vectorscope)"). Engine; **S-engine (∥ crawl, 4-6 wk — the ARCH-R22 reviewed register; acceptance: each instrument's engine milestone green, consumed + pinned at r3 — S-engine builds, r3 consumes; scopes plot engine data, qualifier + window apply in the linear-light pipeline)**.
- **The grade-math binding + parity** — bind the W4 mock math (spec-08-exact) to the engine's real pixels. App+engine; **r3** — the plan's r3 row (now `plan:71`, the D40 form — quote refreshed R33, engine seal-round register P3-18, was the pre-D40 "W4" wording): "the grade-math binding + parity; the S-engine instruments consumed (scopes/qualifier/power-window)"; **acceptance: grade-math parity pins — the mock's W4 math vs the engine's output on the same fixtures, max delta ≤ 1 LSB-equivalent**. The math-side row is spec 08 §0's binding row; THIS row owns the renderer-pipeline surface it lands on (the §5.2/§16.3 linear-light working textures + the §6.2 color-pipeline shaders). (The W2.5 grade final pass is NOT this row: it is the honest CSS-filter consumer preview v1; this row targets the engine's linear-light pipeline.)
- **The scene-grade DECLINE (D29.5c — a DECISION row, not a gap):** the engine seam **declines the scene-grade promotion into composition/export BY DESIGN** — "the scene GRADE stays a consumer-side FINAL PASS by design (hue-rotate/saturate are non-linear under per-element alpha compositing — per-element grade filters ≠ composited grade; the app host composes first, filter-draws once)" (the engine's `8a0b7fe` design law). **The consumer-side final pass is THE LAW** — realized in the app's W2.5 (ProgramCanvas's compose-then-filter pass, BASE above); the app's Z2 grade→export fix **LANDED @ R25 (D30 W-E)** — `deliverService.ts:308-338`: the export grades via the same compose-then-filter final pass ProgramCanvas uses (`renderCtx.filter = gradeFilter` at `:332`); the monitor↔export divergence is CLOSED. **R33 amendments (engine seal-round register P1-2): (a) the history chain corrected** — the seam-side proposal was re-filed spec-side R25 into the engine's PLAN (`.agents/PLAN.md:156-162`), dispositioned DECLINED-BY-LAW (ARCH-R24 F1, `:186-189`), then RATIFIED as D29.5c's registered evolution by D45 (ARCH-R28 §4) — the engine-PLAN disposition re-file pending; **(b) the D45-ordered 04-side flip EXECUTED (was un-executed since R28): this row's decline is RATIFIED-AS-EVOLUTION, r3-consumable** — the compose-then-filter ORDER stays the math law; the venue moves consumer→seam at r3. Twin row: spec 08 carries the grade-side twin of this decision.
- **The CSS-filter widening (D29.5d — OPEN):** only the **Gaussian-Blur subset** landed at `8a0b7fe` (`buildElementFilterString`'s honest map = one effect); **Vignette / Glow / LUT-preview v1 — the Canvas2D-reachable subset — remain open** (the app's D30/R10 CR re-scoped per D29.5d). **S-engine** (acceptance: each widened effect either renders a faithful Canvas2D approximation in `buildElementFilterString` or stays an honest skip — no fake previews; the engine pins extend the bridge-seams filter-string suite; the LUT preview coordinates with spec 08's GPU-fidelity path).
...
- **CPU transition renderers as export fallback** (~1,450 LOC per the engine's P2 backlog). Engine; **r6, non-blocking** — the tail that also carries the ShapeItem/Lottie/Subtitle/Controller surfacing (acceptance: the per-item engine milestone, written when queued, green).
- Register: `IMPLEMENTATION-PLAN.md` §2 (r3's row + the D24 mapping in ARCH-R23) + §3 (the S-engine workstream) + ARCH-R24 D29.5 (the DECLINE + widening rulings) — the worklist; acceptance lives here (spec 14 retired to the redirect stub; its §4.2 runtime rows are re-homed above).
...
> **Canonical `FrameDescriptor` shape:** Defined in `07-composition.md §4` — `interface FrameDescriptor { width, height, clear, displayMode, items: FrameItem[] }` where `type FrameItem = Layer | SceneEffect`. Each item carries a `type` discriminator (`'layer'` or `'sceneEffect'`); both `Layer` and `SceneEffect` expose `effectPassGroups: EffectPass[][]`. The renderer iterates `descriptor.items` in authored order and dispatches by `type`.
...
nle-engine (clean-room FreeCut port) reproduces this 8-bit baseline — **46 `rgba8unorm` sites across 10 files under `src/lib/nle` @ `5036387`** (32 of them in the 7 gpu/-family files: gpu/compositor, gpu/mask-manager, gpu/texture-pool, gpu/glyph-atlas-text-pipeline, gpu/row-alignment, effects/pipeline, effects/lut; the R22-era figure was 29 across 7 — the engine grew 440→458 tests, the 8-bit baseline did not move); the corrective mapping is in §13D and `19-code-references.md`.
...
❌ **CORRECTION** (confirms SCOUT-03 finding, `03-playback-engine.md` §14.D):
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine) is a clean-room FreeCut-port **in-between reference, NOT canon** — it inherits FreeCut patterns this spec corrects (its GPU pipeline is 8-bit `rgba8unorm` throughout: **46 sites across 10 files @ `5036387`** — 32 in the 7 gpu/-family files). Where engine code conflicts with this spec, **the spec wins** (Decision 5). Full reconciliation: `19-code-references.md`.
>
> *(R23 fleet re-verify: the repo moved from the SCOUT-04 snapshot — 37,958 LOC / 124 tests — to **440/440 vitest, tsc 0, ~54,700 LOC under `src/lib/nle`**; the line refs below are refreshed to `b8c6f88`, the scout-era refs live in git history. Two rows flip status: the engine's own Wave 4A round fixed the port's P0.1/P0.2 — noted inline.) R24 fleet re-verify: **458/458, 54,791 LOC under `src/lib/nle` @ `5036387`** — the W2.5 round touched only `bridge/composition-frame.ts` + `bridge-seams.test.ts`, so every file:line below re-verified unchanged (the W2.5 bridge surface is new BASE in §0, not a §13D row). R27 fleet re-verify: **748/748, 57,064 LOC under `src/lib/nle` @ `f9ac806`** (a docs-only head over code anchor `50b91f5`) — the E1/E2 + RC/OV waves moved the GPU files since, so the "every file:line below re-verified unchanged" clause is superseded: three rows are re-anchored below (compositor `:1825` with `uploadRgba` `:1818` + the `player.ts:1641/:1647` callers, mask-manager `:542-557`, player `:1580`), while the 46-site `rgba8unorm` count and the pass-discipline anchor `effects/pipeline.ts:5204` re-verified exact. R33 re-base (engine seal-round register P3-16, @ `2a0ecf4`, 785/785 — read-verified line-by-line): the R29-R33 waves moved mask-manager again — the mask pass-discipline anchor `mask-manager.ts:647-670` (the R27 reading) is now **`:657-682`** (the getMaskResult doc block + signature; the combine call at `:734`), the invert-once row below is re-anchored **`:542-557`→`:735`**, and the R27 note's other anchors — compositor `:1825` + `uploadRgba` `:1818` + the `player.ts:1641/:1647` callers + player `:1580` + `pipeline.ts:5204` + the 46-site `rgba8unorm` count — re-verified EXACT. R34 re-key: the anchors hold @ `60232ea` (789/789) — the P-F4 round's src delta is `timeline.ts` + the nan-guard pin file only, every §13D file:line below re-verified exact again.*

| Spec section | Engine file:line | Verified quote | Status | Note |
|---|---|---|---|---|
| §5.1 r16uint Y/U/V planes | `src/lib/nle/gpu/compositor.ts:1825` | `format: 'rgba8unorm',` | CORRECTIVE | 8-bit source uploads (`uploadRgba` at :1818, called from `playback/player.ts:1641/:1647`); spec's 10-bit per-plane r16uint path wins |
...
| §8.4 mask invert-once | `src/lib/nle/gpu/mask-manager.ts:735` | `invertNext: mask.shape.shape.maskInvert === true,` | **PORT-FIXED (Wave 4A)** | The CPU+shader double-invert P0.2 is GONE — invert travels exactly once (the flag on the single-mask path / `invertNext` in the combine chain); per-call combine outputs kill the cross-clip share (cite re-anchored R33, engine seal-round register P3-16, was `:542-557` @ `f9ac806`) |
| §7.1 all item types | `src/lib/nle/playback/player.ts:1580` | `if (clip.type !== 'video' && clip.type !== 'image') continue;` | CORRECTIVE | Dispatch now covers video/image/text/composition (Waves 4C/4D); sticker/graphic still skipped; spec's dispatch wins |
...
**End of `04-renderer-color.md`.** Next: `05-timeline.md` (18-ui-shell and 19-code-references follow the 00-17 set).
