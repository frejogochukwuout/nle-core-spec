...
- **Worklet asset serving** (the retired spec-14 §4.3 row, re-homed here per the posture law): the app serves WDC's 3 worklet bundles (`public/worklets/dsp-effects-worklet.js`, `biquad-processor.js`, `capture-worklet.js`) from its own `public/` — owner: nle-test-app, **K3** (acceptance: the 3 files copied at the root-base-only path the `DSP_WORKLET_URL_CANDIDATES` law resolves first; a K3 browser-venue pin proves the worklet-backed inserts (compressor/limiter/stereo-widener) run the REAL DSP, not the unity-passthrough fallback — today the app maps inserts but serves no worklets, so they silently passthrough; re-verified at `c020b2a`: the app's `public/` directory does not exist at all).
