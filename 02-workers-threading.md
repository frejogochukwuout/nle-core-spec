...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine; **458/458 vitest @ `5036387`, vendors OT @ `c15a629` + WDC @ `494f6ff`** — the counts re-verified at the R24 pins) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, single-tier tests, procedural media, zero Web Workers). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`.
...
| §8.1 decode.worker | `src/lib/nle/playback/player.ts:1629` | `const bytes = this._registry.renderVideoFrame(clip.source.sourceId, sourceFrame);` | ENGINE-GAP | Decode is a synchronous procedural call |
...
| §11 Q12 mediabunny | `src/lib/nle/media/metadata.ts:83` | `a custom decoder path (mediabunny + AudioSampleSink)` | ENGINE-GAP | The MEDIA/DECODE path is still comment-only (2 mentions, zero imports — re-verified R23 and R24 @ `5036387`). NOTE: the engine is no longer mediabunny-free overall — the EXPORT path genuinely imports it (`export/orchestrator.ts:232,822`, `export/settings.ts:302,332`; mediabunny@1.50.8, the engine's only runtime dep, lazy-imported strictly inside the export module boundary) — that is the M1 export surface, not the §8.1 decode worker |
