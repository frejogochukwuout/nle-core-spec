...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine; **785/785 vitest @ `2a0ecf4`** — the R33 exhaustive-testing waves [the property tier + the count gate + the coverage job + the `perf.yml` nightly; the code anchor moved `74bef08`→`2a0ecf4` for the P-F1/P-F2 fixes], vendors OT @ `55c81c0` + WDC @ `83b8850` UNMOVED — the counts re-verified at the R33 re-key (749/749 @ the R29 re-pin `e3f55bd`, a chore-only re-pin over the then-unmoved code anchor `74bef08`; the R27-era counts held at `f9ac806`, a docs-only head over code anchor `50b91f5`)) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — "single-tier tests" dropped from the list: the engine is multi-tier at `2a0ecf4` [785 vitest / 29 files + the 4-file property tier + the 9 CI-wired probes + the browser-milestones job + the OV-12 vendored-tree gate]; spec 01's scout-era snapshot parenthetical carries the historical form (R33, engine seal-round register P3-4)). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`.
