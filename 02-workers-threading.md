...
**Status:** v-next (Round 24 — the audit-fleet re-verification @ the 2026-09-08 pins (a LOW-DELTA round for this domain: the R24 deltas — the engine's W2.5/R8-REV composition-frame + fleet-re-pin rounds, OT's W11 wire surface, WDC's docs-only seal, the app's R8/R9 wiring waves — touch NONE of the worker/threading surfaces; the R23 truth re-verified intact): the AUDIO-THREAD half of this spec is landed + in-suite fleet-wide — the W2 SoundTouch varispeed round (WDC `soundtouch.ts`/`varispeed.ts` + the engine's adapter pre-retime + the app's `el.speed` consumption), the W1 meter-tap bridge, the drift-correction law, N2b gain automation on the audio thread; the WEB-WORKER half (§3/§4/§5/§8) remains at ZERO workers in the engine by design (procedural media, D5) — the honest split the R22 wording blurred; the remaining worker work is r6/r4/K3/r2-class, thin); Refined spec (sub-agent scout SCOUT-02)
...
## 0. FORWARD INVENTORY (the R24 posture — what needs to be done; the BASE is accepted, not re-explained)

**BASE (accepted; re-pinned to the R24 pins, 2026-09-08 — zero material worker/threading delta since R23; every row re-verified):**
- nle-engine @ `e3f55bd` — 749/749 vitest, tsc 0 — the audio-thread half of this spec landed + tested: §7's fleet-real form (the `nle-pitch-shifter` AudioWorklet, Blob-URL loaded, 1-in/1-out + k-rate `pitchRatio` — `src/lib/nle/audio/soundtouch-processor.worklet.ts`; the W2 maintainPitch pre-retime through the vendored WDC WSOLA — `bridge/segment-strip-adapter.ts` consuming `retimeChannels`/`RETIME_RATE_MIN/MAX` from `@/lib/daw/varispeed`, the three-site schedule law + the byte-capped 96 MB LRU retime span cache, pins W2a–W2k), the W1 meter-tap bridge on the realtime handle (`onMeter`/`onBusMeter`/`reattachBusMeters` — the wireBuses sever-all restore law), the H16 drift-correction law (`getScheduledClipSourceTime` + hard seek, buffer-relative frame of reference), and N2b gain automation riding the segment strip (`gainAutomation`, the dedicated automation gain node + the 2 REAL offline-render sample-level parity pins). §4/§5/§8 remain UNIMPLEMENTED — the engine still has ZERO `new Worker(` sites (re-verified by grep at this pin — comment mentions only, in `mixer.ts`/`export/index.ts`/`video-sync.ts`; the export path is main-thread by Stage-3 design, `gaps/audit/C-export-encode.md`). Vendors OT @ `55c81c0`, WDC @ `83b8850`. (R24: the +18 pins since R23's 440 are the W2.5/R8-REV composition-frame rounds — visual-side, zero audio-thread/worker surface; this row's surfaces re-read unchanged at `5036387`.) (R27 @ `f9ac806`, a docs-only head over code anchor `50b91f5`: audio-adjacent movement now exists — the RC round + M2 Wave 1 landed the bridge bus-law/sidechain parity (`bridge/scene-mixer.ts`) + the engine's `public/worklets` git-symlink serving venue; this row's named audio-thread surfaces re-read unchanged.) (R29 @ `e3f55bd` — a chore-only re-pin over the unmoved code anchor `74bef08`, own code unchanged, vendors re-keyed: this row's surfaces re-read unchanged.) (R33 @ `2a0ecf4` — the exhaustive-testing waves LANDED: **749→785** [the W2 property tier's 33 sites + the 3 P-F2 pins], 29 vitest files, the count gate + the coverage job + the `perf.yml` nightly — 01 §0's R33 layer the full record (engine seal-round register P2-1); this row's audio-thread/worker surfaces re-read UNCHANGED — the property tier soaks the native Timeline/timeline-math/persistence/transform-resolver domains, ZERO new worker code; the engine still has ZERO `new Worker(` sites at `2a0ecf4`.)
- web-daw-core @ `94f6460` (a docs-only head over the unmoved code anchor `ec8fd5c` — the S-series sync + wrap `83b8850`→`94f6460`: HANDOFF/PLAN/SKILL/worklog only, zero src/ bytes; the pin re-keyed R33, engine seal-round register P3-7) — 777/777 vitest (34 test files), tsc 0 — the audio-thread DSP half of this spec is WDC-owned and landed: **W2** (`src/lib/daw/soundtouch.ts` — the typed LGPL WSOLA port, bit-exact vs freecut's reference; `src/lib/daw/varispeed.ts` — `retimeChannels` with the F1 flush law EXACT round(N/rate), the [1/32, 32] retime rate domain, the degenerate-setter guards, the general `daw-varispeed` worklet source + `loadVarispeedWorklet` (Node blob-URL venue PROVEN, W3C createBuffer order, sampleRate honored, CR8 option-path validation); COPYING.LESSER/THIRD-PARTY-NOTICES ship per the LGPL obligation), **W1** (`src/lib/daw/meter-tap.ts` — the tap loop, the split-dispose law, the OFFLINE guard via the startRendering marker, the CR-P1 re-entrancy chain-token guard), and the worklet-serving surface (`public/worklets/`: dsp-effects-worklet.js, biquad-processor.js, capture-worklet.js; `DSP_WORKLET_URL_CANDIDATES` root-base-first law; the in-flight addModule episode + bounded-retry + unity-passthrough fallback contract in `effects.ts`). The worklet↔offline parity contract is PINNED (W2-CR: string-eval classes bit-exact vs the port; quantum-fed prefix bit-exact to the flush-trim region). (R27: the S-series upstream wave — 19 refreshed files + the 18-file closure (diagnostics.ts, the REBUILT `dsp-effects-worklet.js` bundle with the `DspDiagnosedProcessor` mixin, the rt-safety-gate transitive-heritage fix, wam-instrument+state, the MIDI family, the catalog split); the R24 "worklet law files re-verified unchanged" parenthetical is superseded — §7.4's R27 amendment carries the law.)
- nle-test-app @ `876f2b8` — 256/256, tsc 0 — the audio-thread-side consumption proof, now four-fold: real audio crossfades + the WDC audio host running in the app (`audioService.ts` — one ctx, one engine handle, one SceneMixer); the W1 meters consumed end-to-end (per-strip `onMeter` + engine-pushed master/bus taps into the package registry); the W2 consumption (`el.speed` → engine `retime {rate, maintainPitch: true}` → the flattener's varispeedRate + maintainPitch — the wiring pins in `sceneBridge.test.ts`); the W3 JKL audio half (rate-change reschedule, one idempotence key, composed varispeedRate = segRate × transport rate, silent-reverse J) + RR1-B (the speed-identity hole closed — el.speed = retime.rate even at 1). The R8/R9 waves since `70e99f0` add two further audio-thread consumptions, still zero worker code: the W2.1 sidechain ducking — now under the SINGLE-OWNER law (M2 Wave 1, `1aebbd5`: the app's `rewireSidechains` DELETED; the bridge's `SceneMixer.materializeStrips` collects + connects the duck wires through the shared `collectSidechainWires` contract on BOTH the realtime and offline paths — `audioService.ts:319-322` carries the replacement note), and the W2.4 deliver export running the engine's offline mixdown — which SINCE M2 Wave 1 (§1a, `deliverService.ts:260`) flushes worklet module loads before `startRendering` (`workletFlushMs: sceneHasWorkletBackedInserts() ? 250 : 0`; the app-reachable worklet-backed insert family = compressor/gate/deesser/limiter, `:162-177` (the CR-F2 note at :170-176); a worklet-backed insert whose module load races `startRendering` renders PASSTHROUGH — the offline-duck inertness class this law exists to close; both cites re-based to the `7664603` tree (R33, engine seal-round register P3-5)).

**GAP (the work — owner + phase per the D24 ladder, `IMPLEMENTATION-PLAN.md` §2; the spec-14 phase vocabulary retired at R23; the R23 dual-tag transition closed at R24 — D24 tags only):**
- Worker-pool surfacing for the new P2 engine features (ShapeItem/Lottie/Subtitle/Controller, CPU transition renderers) — **r6** (engine, non-blocking) (acceptance: per-item milestones when queued, each new off-main-thread surface reuses the §4 abstraction when the first worker lands).
- The §8.1 decode worker — rides **r4** (PENDING the user's D6 re-affirmation, deprioritized) (acceptance: the real-decode → VirtualMediaAsset round-trip pins on the real-file corpus; the worker follows the N5 registry, §8.1's protocol lands with it).
- **Worklet asset serving** (the retired spec-14 §4.3 row, re-homed here per the posture law): **RESOLVED at R27 in a different shape than specced** — the app serves the vendored worklets at their expected URL via the vite `serve-vendored-worklets` plugin (vite.config.ts:28-92: dev middleware + `closeBundle` dist copy of `vendor/web-daw-core/public/worklets` — `closeBundle()` at :72, the CR-F5 missing-source warning at :82, the block's end cite re-based to the `7664603` tree (R33, engine seal-round register P3-5); ZERO byte duplication — the files live only in the WDC submodule; the app's own `public/` directory still does not exist, and does not need to). The ENGINE's venue is the `public/worklets` git SYMLINK to `../vendor/web-daw-core/public/worklets` (M2 Wave 1, `50b91f5`) — both serve the first `DSP_WORKLET_URL_CANDIDATES` root-base candidate (`/worklets/dsp-effects-worklet.js`, effects.ts:157-161; the varispeed worklet needs NO asset — blob-URL). What remains owner nle-test-app, **K3**: the real-BROWSER venue pin — a Playwright/headless-Chrome test proving the worklet-backed inserts (compressor/limiter/stereo-widener) run the REAL DSP, not the unity-passthrough fallback (the app's suite is vitest/jsdom-only today; the flush-args pins at `deliverService.test.ts:438-471` cover the ARGUMENTS, not the venue).
- **Export-render worker offload** (§8.5's worker — engine export Stage 3): the engine's export path is main-thread by design today; the worker + main-thread fallback port is sequenced — owner: nle-engine, **r6** (acceptance: the ~520-LOC worker + managed-worker + render-pipeline port per `gaps/audit/C-export-encode.md` Stage 3, the fallback trigger family §8.5 documents, pinned in the engine's suite; until then the export path's UI-freeze is the documented Stage-3 cost).
- **The async pre-retime queue** (registered at the R23 pins; re-verified OPEN at R24 in both owning queues — the engine's G2 + WDC's HANDOFF #3, design round FIRST) — a threading-discipline gap this spec owns: the adapter's maintainPitch pre-retime is a SYNCHRONOUS O(span) WSOLA compute on the play edge — a 3-min stereo span at rate 0.5 ≈ seconds of main-thread jank (accepted for the validation app; the fix is specced in WDC `docs/design-w2-varispeed.md` §4: async pre-retime + legacy-fallback-while-pending) — owner: nle-engine (the adapter), design contract WDC — **r2** (acceptance: the retime compute leaves the play edge; the legacy pair plays while the retimed span is pending; a jank pin bounds the play-edge main-thread work).

**ACCEPTANCE & TEST PLAN:** §12 (the six test classes) + the unnumbered `## Testing` section at file end (Tier 1 mocked-worker logic / Tier 2 real-worker browser rows); spec 17 §13A facet rows (the §3.1 "Worker lifecycle" + "AudioWorklet message protocol" matrix rows); the R24 battery's posture checks (the fleet counts at the pins — 748/748 @ `f9ac806`, 777/777 @ `ec8fd5c`, 252/252 @ `c020b2a` — the regression role) **[R30 re-pin: app `8f12cf9` — 377/377 (the 12-file roof; the K3 corpus round); nle-ui `5779295` — 701/701 (the C16 twin + the K3 FIX-2); engine/OT/WDC pins UNCHANGED]**. The landed audio-thread surfaces carry their own in-suite pins today: engine W2a–W2k + W1 + H16 + N2b (785 @ `2a0ecf4` — the R33 layer above), WDC W2/W2l/W2o/W2p/W2q/W2r + W1k (777 @ `94f6460`), app W2 wiring + W3 JKL (426 @ `7664603`, the 22-file roof) — the trailing counts re-based to the live pins (R33, engine seal-round register P3-6).
...
> The private nle-engine port currently runs **zero** Web Workers (verified by grep — no `new Worker(` anywhere in its src; **re-verified at `b8c6f88` (R23) and again at `5036387` (R24, 2026-09-08 — still zero sites) and again at `f9ac806` (R27, 2026-09-13 — still zero sites)**), executing all decode/audio/waveform work on the main thread over procedural media. It is a corrective counter-example for this spec, not a reference implementation; see `19-code-references.md`. Its AUDIO-THREAD half, however, is real and landed (the pitch-shifter worklet + the W2/W1/N2b surfaces — §0 BASE) — the corrective line scopes to the WEB-WORKER half only.
...
> **R23 FLEET REALITY (landed — the in-fleet implementation of this section's DSP):** WDC's W2 round shipped the SoundTouch law core-owned: `src/lib/daw/soundtouch.ts` (the typed LGPL WSOLA port — `Stretch`/`RateTransposer`/`SoundTouch`/`FifoSampleBuffer`, bit-exact vs freecut's vendored reference, LGPL header + COPYING.LESSER/THIRD-PARTY-NOTICES shipped) + `src/lib/daw/varispeed.ts` (`retimeChannels` — the offline form, F1 flush law EXACT round(N/rate), the [1/32, 32] rate domain, degenerate-setter guards; the general `daw-varispeed` worklet source + `loadVarispeedWorklet`, Blob-URL loader idempotent per ctx, never rejects — resolves `false` and the caller degrades). The engine consumes the OFFLINE form (`retimeChannels`) for `maintainPitch` — a pre-retime through the adapter's span cache (W2a–W2k) — NOT the worklet; its own `nle-pitch-shifter` worklet (the simplified granular shifter, §13B) serves the `pitchSemitones` insert path. The worklet↔offline parity contract is pinned WDC-side (string-eval classes bit-exact vs the port; quantum-fed prefix bit-exact to the flush-trim region). The remaining §7.1 gap for OUR target form: the engine's pre-retime is a synchronous O(span) main-thread compute on the play edge — the async pre-retime + legacy fallback is the §0 GAP row (r2). **R24 re-verification: unchanged** — WDC's `85b81b0` delta since `494f6ff` is docs-only, the engine's audio-thread files are untouched by its W2.5/R8-REV rounds, and the r2 row is still open in both owning queues (engine G2 / WDC HANDOFF #3).
...
> **R23 FLEET REALITY (the two landed worklets differ from this target — both are 1-in/1-out, input-bus-fed, Blob-URL loaded):** (a) the engine's `nle-pitch-shifter` (`soundtouch-processor.worklet.ts`) — 1 input → 1 output, k-rate `pitchRatio` AudioParam, source connects → worklet → destination; loaded via Blob URL from an exported source string (no bundler worker config needed). (b) WDC's `daw-varispeed` (`varispeed.ts` `VARI_SPEED_WORKLET_SOURCE`) — 1-in/1-out, interleaves input-bus samples into `SoundTouch`, de-interleaves the output; port messages `set-tempo`/`set-pitch`/`set-rate`/`flush` (all validated, clamped ≥ 0.01); zero-fills EVERY output channel (CR8); returns `true` always (Correction #17's law); `loadVarispeedWorklet` Blob-URL loader, idempotent per ctx. NEITHER implements the 0-input + `append-source` port-push form above — the spec's target remains the FreeCut law for the offline/cloud-render parity form (§7.3); the landed 1-in/1-out forms ride the standard input bus, which is adequate for the insert-seam uses they serve today (the engine's H8 insert seam; WDC's test venue). When the §7.1 scrub/varispeed preview surface is built, choose per §7.2. **R24 re-verification: unchanged** — the fleet's worklet census is still exactly these two Blob-URL processors plus WDC's three served `public/worklets/` assets; no repo added worker/worklet code at the R24 pins (grep-verified engine / WDC / OT / nle-ui).
...
> **SCOUT FINDING (cloud render)**: FreeCut's `prepareAudioBufferForSoundTouchPreview` (`soundtouch-preview-worklet.ts:128-139`) uses `OfflineAudioContext` for resampling when `OfflineAudioContext` is available, falling back to a JS linear resampler otherwise. The worklet itself is codec-agnostic; OfflineAudioContext's `startRendering()` drives `process()` calls. This is the same pattern we'll use. The WDC offline-render readiness gate (`offline-render.ts:736-768` — await every effect's `ready`, bounded 2 s) is the landed analog of this §7.3 pattern, and its flush discipline is exactly what the app's flush-args law (§0's K3 row, §7.4's law-4 consumer) consumes.

### 7.4 Audio-thread observability & realtime-safety discipline (the WDC S-series law — LANDED at `ec8fd5c`)

> **Round 27 amendment** — the audio-thread discipline the fleet landed but no spec carried (corpus-wide fact #7; this section is the law's MASTER home — spec 20 §11's file-map rows, spec 17 §13A's "AudioWorklet message protocol" facet, and spec 11's heartbeat-disambiguation note all cite back here; the four citing specs carry the same anchors). Five laws:
...
**Status:** LANDED WDC-side at `ec8fd5c` (inside the 777/777); the engine consumes it via the vendored tree (re-pinned `480a216`); the app's K3 browser-venue row (§0 below) is where a consumer pin lands. Cross-refs: spec 20 §11's file-map rows (the audio-side registration — the same four anchors: `diagnostics.ts:1-376`, `dsp-effects-worklet.js:315-380`, `effects.ts:441-507`, `rt-safety-gate.ts:140-187`); spec 17 §13A's "AudioWorklet message protocol" facet gains the diagnostics messages (the heartbeat/NaN/silence protocol rows); §12's test class 5 (AudioWorklet) absorbs the protocol rows — the heartbeat-stall bypass pin, the NaN-permanent-bypass pin, and the gate-lethality (poisoned-fixture) pin.
...
// (see `03-playback-engine.md` §5.2 and `04-renderer-color.md` §14.C) lists
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine; **789/789 vitest @ `60232ea`** — the R33 exhaustive-testing waves [the property tier + the count gate + the coverage job + the `perf.yml` nightly; the code anchor moved `74bef08`→`2a0ecf4` for the P-F1/P-F2 fixes] + the R34 P-F4 round [+4 nan-guard pins — the nightly exploration venue's first catch, fixed; the gate re-pinned 785→789 same-commit; the code anchor `28aa387`→`60232ea`; the nightly + the 5-job CI GREEN] — re-keyed R34, was 785/785 @ `2a0ecf4`; vendors OT @ `17a19f8` + WDC @ `94f6460` UNMOVED — re-keyed R34, the R33 read's `55c81c0`/`83b8850` was two re-pins behind; the counts re-verified at the R33 re-key (749/749 @ the R29 re-pin `e3f55bd`, a chore-only re-pin over the then-unmoved code anchor `74bef08`; the R27-era counts held at `f9ac806`, a docs-only head over code anchor `50b91f5`)) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — "single-tier tests" dropped from the list: the engine is multi-tier at `60232ea` (re-keyed R34, was `2a0ecf4`) [789 vitest / 29 files + the 4-file property tier + the 9 CI-wired probes + the browser-milestones job + the OV-12 vendored-tree gate]; spec 01's scout-era snapshot parenthetical carries the historical form (R33, engine seal-round register P3-4)). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`.

| Spec section | Engine file:line | Verified quote | Status | Note |
|---|---|---|---|---|
| §3 worker inventory | (grep src, re-verified @ `b8c6f88` and `5036387`) | no `new Worker(` matches | ENGINE-GAP | Zero workers; entire §3 inventory unimplemented — spec wins |
| §4 ManagedWorker | — | COULD-NOT-VERIFY (no abstraction exists) | SPEC-ONLY | Greenfield |
| §8.1 decode.worker | `src/lib/nle/playback/player.ts:1646` | `const bytes = this._registry.renderVideoFrame(clip.source.sourceId, sourceFrame);` | ENGINE-GAP | Decode is a synchronous procedural call |
| §7.1 SoundTouch varispeed | `src/lib/nle/audio/soundtouch-processor.worklet.ts:13` | `We implement a simpler granular pitch shifter:` | CORRECTIVE | Simplified shifter vs vendored SoundTouch; spec wins. **R23: HALF-SUPERSEDED by W2** — the real WSOLA now exists in the vendored WDC (`lib/daw/soundtouch.ts`) and the engine consumes it for `maintainPitch` as the adapter's offline pre-retime (`retimeChannels`); the engine's own worklet remains the simplified shifter, now scoped to the `pitchSemitones` insert path only |
| §7.2 worklet topology | `src/lib/nle/audio/soundtouch-processor.worklet.ts:47` | `NLE_PITCH_SHIFTER_PROCESSOR_NAME = 'nle-pitch-shifter';` | CORRECTIVE | 1-in/1-out + k-rate param vs spec's 0-input + port-push (OfflineAudioContext parity); spec wins. **R23: the landed fleet has TWO 1-in/1-out Blob-URL worklets (engine `nle-pitch-shifter`; WDC `daw-varispeed`) — see §7.2's fleet-reality note; the 0-input port-push form stays the target for the §7.1 preview surface** |
...
| §11 Q12 mediabunny | `src/lib/nle/media/metadata.ts:83` | `a custom decoder path (mediabunny + AudioSampleSink)` | ENGINE-GAP | The MEDIA/DECODE path is still comment-only (2 mentions, zero imports — re-verified R23 and R24 @ `5036387`). NOTE: the engine is no longer mediabunny-free overall — the EXPORT path genuinely imports it (`export/orchestrator.ts:232,836`, `export/settings.ts:302,332`; mediabunny@1.50.8, the engine's only runtime dep, lazy-imported strictly inside the export module boundary) — that is the M1 export surface, not the §8.1 decode worker |
...
> **R33 execution-status note (engine seal-round register P2-1):** the engine's property tier LANDED @ `2a0ecf4` in the CLASS-API shape (the native-Timeline undo soak + timeline-math + persistence + transform-resolver — 01 `## Testing`'s R33 note the record) — it does NOT carry this section's worker-pool invariants: the engine still has ZERO worker code (`new Worker(` grep clean at `2a0ecf4`), so the acquire/release/terminateAll algebra below remains **r6-content** — these properties activate with §4's worker-pool surfacing per §0's GAP rows, and they are the contract that tier will soak.
