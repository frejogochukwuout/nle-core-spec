...
**Status:** v-next (Round 23 — the audit-fleet re-baseline @ the 2026-09-07 pins: the AUDIO-THREAD half of this spec is landed + in-suite fleet-wide — the W2 SoundTouch varispeed round (WDC `soundtouch.ts`/`varispeed.ts` + the engine's adapter pre-retime + the app's `el.speed` consumption), the W1 meter-tap bridge, the drift-correction law, N2b gain automation on the audio thread; the WEB-WORKER half (§3/§4/§5/§8) remains at ZERO workers in the engine by design (procedural media, D5) — the honest split the R22 wording blurred; the remaining worker work is r6/r4-class, thin); Refined spec (sub-agent scout SCOUT-02)
...
- **The async pre-retime queue** (NEW at the R23 pins — a threading-discipline gap this spec owns): the adapter's maintainPitch pre-retime is a SYNCHRONOUS O(span) WSOLA compute on the play edge — a 3-min stereo span at rate 0.5 ≈ seconds of main-thread jank (accepted for the validation app; the fix is specced in WDC `docs/design-w2-varispeed.md` §4: async pre-retime + legacy-fallback-while-pending) — owner: nle-engine (the adapter), design contract WDC — **r2** (was W-audio-class) (acceptance: the retime compute leaves the play edge; the legacy pair plays while the retimed span is pending; a jank pin bounds the play-edge main-thread work).

**ACCEPTANCE & TEST PLAN:** §12 (the six test classes) + the unnumbered `## Testing` section at file end (Tier 1 mocked-worker logic / Tier 2 real-worker browser rows); spec 17 §13A facet rows (the §3.1 "Worker lifecycle" + "AudioWorklet message protocol" matrix rows); the R23 battery's posture checks (the fleet counts at the pins — 440/440 @ `b8c6f88`, 759/759 @ `494f6ff`, 117/117 @ `70e99f0` — the regression role). The landed audio-thread surfaces carry their own in-suite pins today: engine W2a–W2k + W1 + H16 + N2b (440), WDC W2/W2l/W2o/W2p/W2q/W2r + W1k (759), app W2 wiring + W3 JKL (117).
...
| §11 Q12 mediabunny | `src/lib/nle/media/metadata.ts:83` | `a custom decoder path (mediabunny + AudioSampleSink)` | ENGINE-GAP | The MEDIA/DECODE path is still comment-only (2 mentions, zero imports — R23 re-verified). NOTE: the engine is no longer mediabunny-free overall — the EXPORT path genuinely imports it (`export/orchestrator.ts:232,822`, `export/settings.ts:302,332`; mediabunny@1.50.8, the engine's only runtime dep, lazy-imported strictly inside the export module boundary) — that is the M1 export surface, not the §8.1 decode worker |
