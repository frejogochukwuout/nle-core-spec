...
- **The D25 bridge (S-ot, the crawl window — was spec 14 §4.1's OT-side halves, re-homed).** The view-config surface — zoom-ladder config (the ± keys' ladder constant) + ripple-toggle semantics exposure (the ⌥R mode's engine-side semantics). The keyboard-side consumers: §3.8's zoom rows + §3.2's Option+R row. Owner: S-ot (the bridge work order's item 3).
