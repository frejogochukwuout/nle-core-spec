...
| `,` / `.` = insert / overwrite edit (source → timeline; the FreeCut-only comparison rows, §19:1496-1497) | `,` / `.` = source-mode-gated insert-edit / overwrite-edit (§3.4A, the C46 gate) | ✅ agreed — ratified R25 (D34.2): the variants' R20-W2 C46 implementation becomes spec law; the timeline meanings (slip/slide/nudge) are untouched by the gate (§6.1 row 2's four meanings) |
