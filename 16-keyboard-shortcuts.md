...
- nle-test-app @ `876f2b8` — **256/256** (the 8-suite roof: GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + wire-coverage 13 + engineSeam 7 + waveformPeaks 9), tsc 0 (was `c885ece` 174 at R24, `70e99f0` 117/117 at R23, `e662759` 83/83 at R22). The app's landed key surface: (a) the PORT keymap (`src/timeline-port/hooks/use-keybindings.ts` — the **27-row** port map (OT's 28 minus the 3 undo rows **plus the ⇧L/⇧J fixed-2× pair** — r/⇧R is the upstream W11 pair (OT :48-49), un-withheld at K2; R33, engine seal-round register P3-44): Space/K play, L/J JKL shuttle, ⇧L/⇧J fixed 2×, ←/→ frame-step, ⇧←/→ 5 s jump (the "1 s/5 s" pair's 1 s half is dead config — the shift branch always takes `LONG_JUMP_SECONDS = 5`; R33, engine seal-round register P2-29), Home/Enter/End, S split, Q/W split-side, Backspace/Delete + ⇧ ripple-delete, N snap, ⌘A select-all, ⌘D duplicate, B bookmark, Escape cancel-then-deselect; the port's ctrl+z / ctrl+shift+z / ctrl+y rows are REMOVED — the shell owns undo, D22b; **the R30 C16 auto-repeat guard** (`if (event.repeat) return;` at use-keybindings.ts:230, the comment block :221-229 above it — ALL keys, zero exemptions; the package twin at useShortcuts.ts:52; pinned src/keybindings-repeat.test.ts + the package's useShortcuts.test.tsx; the stale :220 pin collapsed to :230 R33, engine seal-round register P3-47) **[R30 re-pin: `8f12cf9` — 377/377, the 12-file roof (GluedShell 124 + wire-coverage 26 + audioService 42 + deliverService 20 + keybindings-repeat 4 + waveformPeaks 9 + history-laws 15 + persistenceService 20 + engineService 31 + sceneBridge 41 + timeline-geometry 38 + engineSeam 7); the K3 corpus round — the four re-expression maps + the store/policy + geometry tranches + the C16 guards; consumers nle-ui @ `5779295` (701/701) + engine `e3f55bd` + OT lock `55c81c0` + WDC `ec8fd5c` unchanged]** **[R31 re-pin: `f58147c` — 418/418, the 21-file roof (+9 NEW: dom-structural 8 + scrub-session 4 + clip-lifecycle 3 + trim-gesture 3 + edge-autoscroll 2 + zoom-anchor 2 + dnd-fallback 3 + mid-drag-yield 5 + e2e-crawl 6); **D51 — the mid-drag keyboard yield LANDED** (the mini useKeys.ts:60 `dragActive` law's engine-world twin: an `isInteractionActive` gate after e.repeat, before SURFACE_OWNED_KEYS — every verb yields during an active element-interaction session [element drag / handle trim / bookmark drag, the three view-level families] EXCEPT Escape [the interaction's own cancel]; scrub NEVER engages [the stateless-surface law — Space mid-scrub toggles play]; pinned src/mid-drag-yield.test.tsx — the 10-leg shape: the s/Space/Backspace yields + no error chip, the Escape carve-out, the commit + release legs, the trim twin, the scrub exclusion, the positive control; the adversarial review's five amendments folded — the two-keymap residual REGISTERED: the shell's useShortcuts fires wire-invisible router verbs mid-drag [⌘Z rewinds the doc under a live gesture, probe-proven], the D52 candidate = the optional TimelineRouter.isInteractionActive twin — **RESOLVED, the residual flipped candidate→LANDED (R33, engine seal-round register P2-31): D52 LANDED @ R32** (nle-ui `1e0c30b` 743/743 — the optional twin `timelineRouter.ts:87` + the 9 doc-mutating row gates in useShortcuts; the app R32-W3-a `b5fe3b3` 422/422 + `ce7cfc3` 426/426 — the OR-composed getter + the doc-committing-sessions registry; the R32 note below); the ruling + review live in the app repo's docs/decision-mid-drag-keyboard-yield.md + docs/review-mid-drag-keyboard-yield.md); consumers nle-ui @ `dc3644b` (701 — docs-only: the D29 DECISIONS first-party record + the C0 filing) + OT HEAD `31f2764` (docs-only over the code pin `970948a` — the R31 queue filing: the carrier-reduction work order + the D25.3b structural-gap worklist + the data-test freeze ask) + engine `e3f55bd` + WDC `ec8fd5c` unchanged]** **[R32: the D52 two-keymap yield LANDED — the twin became REAL: nle-ui at its R32 pin `1e0c30b` gains the OPTIONAL TimelineRouter.isInteractionActive member (true = the shell's 9 DOC-MUTATING rows yield SILENTLY — no preventDefault, no verb: ⌘Z/⇧⌘Z/⌘Y · ⌘B · ⌘M/⌘⇧M · ⌥[/⌥] · ⌥⇧M · m/⇧m · ,/. · [/]; the view/transport/tool rows NEVER consult it — the narrow-gate law; back-compat: a router without the member never yields) + the 6-pin family; the app `ce7cfc3` (R32's pin of record; `d28847c` at R34 — the flag store unmoved) implements the member over the app-layer flag store (src/interaction-active.ts — ONE source TWO listeners: D51's port gate + D52's shell twin read the SAME OR-composed getter — the live mousedown-synchronous flags FIRST, the registry term second) + the 4-test describe on the real glued app (probe C / the NEW census rows / the scrub twin / the no-session classic-default); the per-element session families LANDED with it: the doc-committing session registry (the ~12-line module — keyframe-drag + volume-line join the gate; scrub/marquee/box-select EXCLUDED by ruling); consumers nle-ui @ `cb04919` (747/747 — re-keyed R34, was `1e0c30b`/743: the DeliverPage supersede contract + the viewer-state-empty CR pair; the C0 MiniShell + the D52 twin beneath) + OT HEAD `39003d3` (the code pin MOVED off `970948a` — the D25.3a mini-theme wave, 649/649) → R33: OT @ `008e7f3` **675/675** (the seal18 close, the census 31 = 28 routed + 3 exceptions UNCHANGED, the code pin `344123a`; re-keyed R33) + engine/WDC unchanged]**; (b) **the W3 JKL half landed** (`3f351bc`, design `docs/design-jkl-audio-follow.md` v1.1 — **the AUDIO half is now SEALED WDC-side (the S4 close: composed varispeed, one reschedule per rate change, silent reverse; only the R4 ⇧K dead-key residue remains, a P3 doc item)**): the port's tap-accel ladder (1×→2×→4× inside 500 ms; K pauses with the rate persisted — the engine law; ⇧L/⇧J bypass at 2×) drives `core.setPlaybackRate` directly (capture-phase) — **SUPERSEDED (landed — the R5 row below): the M28R stateless ladder LANDED at W-D (`f681d34` — use-keybindings is one of the census's 9 carriers at the R32-W3-a registry round, the count re-keyed 5 → 9 R33, engine seal-round register P3-47: 43 port files = 32 zero-action (6 byte + 26 mechanical) + 9 documented carriers + 2 app-hosts @ `17a19f8`, `docs/port-census.md:31`)**; the audio scheduler composes the transport rate (varispeedRate = elementRate × rate; one stop+reschedule per rate change; r < 0 = silent reverse); the routed `setShuttle` round-trips through the mirror (`engineService.setPlaybackRate`; contract surface — no production caller, the port's keymap owns JKL in the engine world); (c) the shell-owner undo law (nle-ui's useShortcuts ⌘Z/⇧⌘Z/⌘Y → the timelineRouter facade: engine history first, store domain second).
- The mini's editing keys — `ui-mock/shell-mini` (in flight — R24-2 W0-W2+W2-fix landed; 428 tests @ the `fb05540` re-pin, census 147 units / 134 GAP / 395 authored; W3+ pending) + the R24-2 W2 tool radio: **V/X = select/transition** (miniPlus-gated, `useKeys.ts:55-70`; `trimTool: 'select'|'roll'|'slip'|'slide'|'transition'`, `useMini.ts:320` — X a NEW corpus key, unbound in Appendix A) + the law registers: `docs/CORE-SEAMS.md` **S9** (useKeys: Space/S/[/]/Del/±/0/Home/Esc/V/X + the form-control skip + the C1/C16 laws) + `docs/LAW-NET-INVENTORY.md`'s App.test.tsx row (the keyboard-on-shell laws: **C1** Space-yields-to-native-button, **C16** auto-repeat gate, **C46** clip-is-a-button, **C49** the advertised zoom keys). The mini's `[`/`]` = cutHead/cutTail (trim-to-playhead, ripple-pref-aware — the R18e form; §3.4's ⌥[ / ⌥] ripple chords live in the nle-ui/variants shells instead).
- nle-ui @ `6754979` — **691/691** (the R9-b preservePitch + F4 identity guards + FW-D `setElementFieldAll` + AW1-2 the Gain ceiling [−60,+20] + AW1-4 the D-ARCH-6 pool seam + ENG-1). **[R30: `5779295` — 701/701 (83af958 the C16 package twin + the 4 package-gap pins; 5779295 the K3 FIX-2 — the 3-verb dead-history-entry class)]** **[R31: `dc3644b` — 701 UNCHANGED, docs-only: the D29 DECISIONS first-party record + the C0 MiniShell row filing]** The package's shell keymap: `src/hooks/useShortcuts.ts` + `src/lib/shortcutMap.ts` (**53 cheat-sheet rows — re-verified at `32abd58` (R27)** — the honest implemented-set ledger, §8.6 single-source discipline) + the CheatSheet modal (§7.3, auto-generated). The **engine-owned yield law (D22b)**: router-attached → the port keymap owns the plain NLE editing keys (incl. the shifted ⇧J/⇧L/⇧Backspace/⇧←→ — the W3/F5 double-fire fix); the shell keeps the ⌘-family, pages, markers, ⌥-family, F6. **The yield-set's third row LANDED (`2646d80`): 'r' sits in `engineOwnedPlain` + `shiftedYields` (`useShortcuts.ts:98/:114` at the then-current pin `1e0c30b` (R34: `cb04919` — the cites re-verified exact, the D52 block unmoved by the CR pair) — re-keyed from :69/:85 R33, engine seal-round register P3-47: the D52 block :81-95 was inserted above them; the :69/:85 figures remain the `2646d80`-landing vintage, as cited in the R6 row below); the ripple tool keeps r in the mock world only.**
...
- OT @ HEAD **`008e7f3`** — **675/675** (77 milestone entries — the total only, — **R34: OT @ `8f96ab2` — 714/714 @ 83 entries** (the M64 /view fps param + the M59/59A/59B/59C/59D carrier families [CR-1/2/3/4/5/8/9 LANDED]; the wire census 31 = 28+3 UNCHANGED — the carriers are props, not verbs), the report json committed at the pin the count authority; the R33 census, verified at the report json: **632 classic + 17 M60 (the D25.3a theme tokens) + 11 M61 (seal18 L1 — the wire-classification hardening: F1B-5 + the HA-2 family + D44-OT `ERROR_CODES` + HA-2-11) + 11 M63 (the D42 linkage model) + 3 M62 (the functionality-census gate) + 1 M62-gate = 675** — re-keyed R33, engine seal-round register P2-30). The code pin MOVED off `970948a` at R32 (the D25.3a mini-theme wave, `39003d3` — 649/649) → the seal18 lib-tree wave `17a19f8` (linkage.ts NEW — D42; edit-domains.ts NEW — D43; api.ts +254, timeline-core.ts +144, split.ts +17, types +12 — all additive, the components tree byte-unchanged, **the wire census UNCHANGED 31 = 28 routed + 3 exceptions; `ERROR_CODES` exported at api.ts:234**) → the M58R flake fix at HEAD `008e7f3` (the engine's 09-15 OV-12 CI red root-caused + closed: the un-awaited wireReset race in the delete-seam phase). (R31 history: HEAD `31f2764` — 632/632 UNCHANGED, docs-only over the code pin `970948a` — the R31 queue filing.) **The CANONICAL keymap now lives in the tree, W11-rewritten (DECISIONS #25 — a CONSCIOUS divergence from classic's j/k/l jump rows)**: `src/components/timeline/hooks/use-keybindings.ts` (`:23-81` at the R27 pin) — the 28-row map: the JKL rows are the **M28R stateless rate-based ladder** (rate ∈ {0, ±1, ±2, ±4}; same-direction doubling capped; opposite-direction reset; **K = pause-ONLY** — `timeline.pause`; Space resumes at the persisted rate; **setPlaybackRate BEFORE play** — the S1 rate-conditioned restart), the jump gestures survive on ⇧Arrow (±5 s — the 1 s half is dead config, the shift branch always takes `LONG_JUMP_SECONDS = 5`; R33, engine seal-round register P2-29), **r / ⇧R = loop set/clear** (`setLoopFromSelection` / `clearLoopRegion` — the W11 pair, both previously unbound), the ctrl+z / ctrl+shift+z / ctrl+y undo rows OT keeps; the in-page real-keyboard suites (**M24**: S split, Space + rAF ticker, follow-scroll, arrows + Home/End, Delete + Ctrl+Z, B bookmark, text-input guard; **M28R** — the actions real-mouse suite: the JKL ladder pins (l → +1, l l → +2, the +4 cap, K-pause-with-rate-persisted, Space-resume-at-persisted-rate, j → −1, j j → −2, opposite-direction reset) + ⇧arrow 5 s, q/w split sides, Enter, Backspace, n snap, ⌘a, Escape, ⌘d, ⌘y/⌘⇧z redo; **M49T**: rate control, loop region, wrap laws).
...
- **K3 — the OT-side keyboard-laws census (registered R23, re-shaped R24; re-verified R27 @ `970948a`; re-keyed R33 @ `008e7f3` — engine seal-round register P2-29/P2-30).** The three R23 divergences re-verify as: **K and J/L RESOLVED OT-side** — W11's M28R rewrote OT's rows to K = pause-ONLY + the shuttle rate ladder (both now match §3.1's rows; the ladder mechanism is stateless rate math off the live engine state, not §8.7's tap-count closure — the observable ladder is the same); the app-side K divergence CLOSED with the W-D adoption (R5, LANDED below); **B remains the sole live divergence after the R33 ⇧Arrow re-base** (OT + the app port: `b` = bookmark-at-playhead vs §3.2's razor tool — the nle-ui/variants mocks implement B = razor per this spec). *(R33, engine seal-round register P2-29: the census had under-counted live divergences by a full family — the ⇧Arrow rows contradicted BOTH engine-world keymaps (spec §3.1 said a 10-frame step; OT + the app port jump ±5 s — the W11/M28R law). Resolved by RE-BASING §3.1/§6.1 #8/Appendix A to the dual-world law (mock ±10-frame step / engine-world ±5 s jump) rather than registering a standing divergence — the cheat sheet's `engine:` annotation pattern, `shortcutMap.ts:36-37`.)* The port's keymap deltas are the D26 census's documented carriers (the keymap surgery in use-keybindings.ts — one of the 3 merge files; the yield-set port-local; the Z4 guard to come) — no longer a D25-bridge PORT-LOCAL set. The mini's **C16 auto-repeat gate (`e.repeat`) is STILL absent from the OT keymap** — the app-port guard + the package twin LANDED R30 (`8f12cf9`: `if (event.repeat) return;` at the port's use-keybindings.ts:230 — ALL keys, comment block :221-229; the nle-ui twin useShortcuts.ts:52; pinned by the app's keybindings-repeat.test.ts — the four legs :78/:99/:111/:149 — + the package's useShortcuts.test.tsx; re-scoped OT-only R33, engine seal-round register P2-30 — the old "STILL in neither" wording directed S-app work at a landed item and understated the app-side coverage; OT's 247-line keymap re-verified grep-clean at `008e7f3`) — a held S still machine-guns splits on the canonical OT tree (live-proven in the mini; OT needs the gate or the app-side re-expression pins it). Disposition: the B row + the mechanism note ride the D26 carrier-reduction program's review cycles + the D30 R10(k) spec ask (record the divergent keymap laws as decisions, not silent divergences); K3 re-points the app-side key tests at the canonical tree. Owner: S-ot (carrier program) + S-app (K3).
...
- **r1 — keymap surfaces for the new op families** (slip/slide/roll/rateStretch wave 1; retime/freezeFrame/rangeRemoval wave 2). Acceptance: keymap rows + nle-ui sync (the S-package workstream's r1 queue row). *(R33, engine seal-round register P3-48 — inventory note: the engine's native r1 twins pre-exist the wire surface, so the wave is keymap rows + wire routing, NOT op authoring — slip `timeline.ts:4988`, slideItem :5129 (one history entry via the `execute()` reentrancy fold :5150-5153, neighbor trims by construction, the linked default `options.linked ?? true` :5165), rollingTrimItems :3356, rateStretchItem :3872 + the rateStretchWithRipple wrapper :7699, freezeFrameAtPosition :8706, the range-removal composite :8248+ (removeSilenceFromItems :8565 / removeTranscriptRangesFromItems :8641) — cites live at engine `2a0ecf4`, re-keyed from the register's pre-property-tier reads (+32/+54 drift); the slide span-preserved/one-history-entry pin is the queued acceptance leg — today the op is pinned by adjacency only: slide-G3 clamps `timeline-mode-clamps.test.ts:376`, the nan no-op `timeline-nan-guard.test.ts:120`, the locked-track gate `timeline-locked-track.test.ts:93`; zero slide-specific span-invariant pins exist (the property-tier soak exercises slip/slide under the generic 10-law battery only).)*
- **The carrier-reduction program (S-ot, the crawl window — the old D25-bridge view-config halves, re-pointed by D26).** The view-config surface — zoom-ladder config (the ± keys' ladder constant) + ripple-toggle semantics exposure (the ⌥R mode's engine-side semantics) — **re-verified ABSENT at `970948a` (R27)** (rippleMode is an internal `useState` at TimelineView.tsx:240 — re-pinned from :237 R33, engine seal-round register P3-47, the +3 drift landed with OT's M60 theme wave, verified at `008e7f3`; zoom is continuous-clamped, no exposed ladder). The keyboard-side consumers: §3.8's zoom rows + §3.2's Option+R row. Owner: S-ot (ONE carrier-reduction work order, D26.3 — no longer a bridge prerequisite: K3 gates on the coverage vocabulary per D29).
...
4. **Modifier keys for variants.** `Shift+arrow` for 10-frame nudge vs `arrow` for 1-frame (the mock world's law — the engine world's ⇧Arrow is the ±5 s jump, §6.1 #8's dual-world resolution; R33, engine seal-round register P2-29). `Shift+J`/`Shift+L` for 2× shuttle vs `J`/`L` for 1×. `Cmd+Shift+B` for split-all-tracks vs `Cmd+B` for split-focused. The pattern: **Shift = "more"** (10× frames, 2× speed, all tracks), **Cmd = "global"** (file, project, all-tracks).
...
| `Shift+Left` | 10 frames back (mock world) / 5 s back (engine world) | mock: `{ type: 'seek', params: { time: <currentFrame - 10> } }` · engine world: `{ type: 'seek', params: { time: <currentTime − 5 s> } }` — **the dual-world ⇧Arrow law (R33, engine seal-round register P2-29)**: the nle-ui/variants mocks keep the ±10-frame step (`useShortcuts.ts:153-160` — `nudgePlayhead(±10)`); OT's canonical keymap + the app port jump ±5 s (the W11/M28R law — OT `use-keybindings.ts:44-45/:206-210`, app `:50-51/:278-282`, `jumpBy(±LONG_JUMP_SECONDS)`; the 1 s branch is dead config — the jump bindings are shift-only; §6.1 #8) | Always | Shift+Left |
| `Shift+Right` | 10 frames forward (mock world) / 5 s forward (engine world) | mock: `{ type: 'seek', params: { time: <currentFrame + 10> } }` · engine world: `{ type: 'seek', params: { time: <currentTime + 5 s> } }` (the dual-world law, R33, engine seal-round register P2-29) | Always | Shift+Right |
...
**Phasing (the census law):** every command shape in this subsection carries the **(r1-scheduled)** marker — the source-edit verbs land in spec 15 at r1 with the D31 family law (06 §5.9 (preamble) + §5.9B-§5.9F, the R25 restructure), and **15's landed union is untouched by this table**; the keymap rows + the Appendix-A `kbd-source-*` enumeration + the cheat-sheet rows ride the r1 keymap wave (§0's r1 GAP row) under the D30 R5 lockstep law. The `,`/`.` rows themselves are **ratified now** — the variants have implemented + tested the gate since R20-W2 (C46; the deviation-register entry retires: the rows are spec law, no longer a registered deviation — the retirement EXECUTED R33 in the ledger, `.agents/SPEC-REVISION-CANDIDATES.md` §I's C45/C46 keyboard-facing halves annotated RETIRED, the 18 §4.3 chrome-contract halves staying live there until 18 absorbs the cluster; R33, engine seal-round register P3-46). The rows are dormant in every shipped shell by construction (no source viewer exists — 18 §8.5 defers dual viewers to v2; zero live-behavior risk).
...
| 8 | `Shift+Left` / `Shift+Right` | Jump 5 sec (OpenCut-classic) | Nudge 10 frames (FreeCut) | **Dual-world law (R33, engine seal-round register P2-29): the mock world (nle-ui + variants `useShortcuts` — `nudgePlayhead(±10)`) keeps the 10-frame step (more useful day-to-day); the engine world (OT's canonical keymap + the app port — W11/M28R, DECISIONS #25's conscious divergence) jumps ±5 s (`jumpBy(±5)`; the shift branch is ALWAYS 5 s — `JUMP_SECONDS = 1` is dead config on the shift-only binding).** The cheat sheet carries the honest dual annotation (`shortcutMap.ts:36-37`); `PageUp`/`PageDown` remain the edit-point navigation route. | §3.1 (this spec) vs §19 (OpenCut-classic) — resolved R33 as a world-split, not a single-world win |
...
| `shift+arrows` = nudge 1px (FreeCut) | `Shift+Left/Right` = 10-frame nudge (mock world) / ±5 s jump (engine world — §6.1 #8's dual-world law, R33, engine seal-round register P2-29) | Frames > pixels — pixel nudging is a UI concept, frame nudging is a media concept; frame-based is more useful and testable |
...
|---|---|---|---|
| Phase 2 (timeline MVP) | **K3** (the app-behavior nets; the surface already exists — the app's port keymap + the shell) | Playback + basic edit | §3.1 (playback), §3.2 (tools: V, B), §3.3 (Tab/Esc), §3.4 (Cmd+B, Delete, Backspace), §3.10 (undo/redo) — LIVED: the port's 27-row map + the W3 JKL half + the D22b undo law |
...
The old "Phase 2 exit criteria" ("Keyboard shortcuts work") is answered by the K3 gate today: the app's key surface at `876f2b8` (256/256 — the S1 JKL pins, the S2 delete/undo round-trips; the M28R-law pins LANDED with D30 W-D) + the mini's law corpus (in flight, 428→441) + the OT in-page real-keyboard suites (M24/M28R at `3e18722`, code pin `970948a` — 632/632) are the crawl's evidence (R31: the OT HEAD `31f2764` — the code pin UNMOVED, docs-only); the REMAINING rows are §0's GAP register (the K3 re-expression, the OT-side census, the D30 W-D family — R5/R6/Z4 — the r5 long tail, the r1 surfaces).
...
kbd-frame-jump-back         | Shift+Left         | 10 frames back (mock) / 5 s back (engine — the dual-world law; R33, engine seal-round register P2-29) | playback  | Always
kbd-frame-jump-fwd          | Shift+Right        | 10 frames forward (mock) / 5 s forward (engine — same P2-29 law) | playback  | Always
...
nle-engine has NO keyboard layer — it is engine + headless API only (its own Decision 9: "No React dependency in engine core"). Everything in this spec is SPEC-ONLY relative to the engine; the rows below document what the engine does provide that an implementer will attach a keyboard handler to. Where engine and spec conflict, **the spec wins**. Full reconciliation: `19-code-references.md`. **[R27 re-verified at `f9ac806` (748/748) — every verdict STANDS: api.ts:793/:919, run-nle-tests.mjs:167, page.tsx:5794 exact; TWO line pins drifted: player.ts `:994-995`→**:999-1000** and the ratechange cite `:606-607`→**:196** (the event map; the clock-forwarding comments :603-605). R24 note (history): re-verified at `5036387` — every verdict STANDS (all six rows re-checked live; the pin moved `b8c6f88`→`5036387` via a submodule-only OT re-pin a4e971d→c15a629, zero engine source delta). R23 re-verification at `b8c6f88` kept as lineage; the file:line citations unchanged (the playback/player and headless/api files live under `src/lib/nle/` now):]**
...
| §3.1 JKL / setRate | `src/lib/nle/playback/player.ts:999-1000` | `setPlaybackRate(rate: number): void { this._clock.playbackRate = rate;` | ALIGNED (underneath) | The rate machinery a JKL resolver would drive exists in Player (the `ratechange` event forwards from the clock, player.ts:196 — the event map; the clock-forwarding comments :603-605); the key→command layer is missing engine-side — the APP's port keymap drives it through the vendored OT (`core.setPlaybackRate`, W3-verified), and OT's canonical keymap now drives the same machinery through the wire verbs (`timeline.setPlaybackRate` / `timeline.play` / `timeline.pause` — the W11 M28R ladder, dispatch order setPlaybackRate-BEFORE-play) |
