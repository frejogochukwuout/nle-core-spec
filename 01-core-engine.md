...
- (b) `maintainPitch` — **LANDED** (BASE above; scene-to-segments.ts:246/:611) — the residual is the app's own toggle wiring (app-owned; the app's Inspector toggle is mock-local per its own zero-no-op audit).
...
| §4.1/§4.2 Command + undo | `src/lib/nle/timeline/timeline.ts:5225` | `execute<T>(label: string, fn: () => T): T {` | SUPERSEDED | Scout-era: zero callers (engine P1.5). CLOSED since (P1.14, 2026-09-04): 59 ops wrapped via `execute()` — undo-by-default, one user op = one undo step; probe-p114 (318 checks) CI-wired; 61 `execute(` call sites in timeline.ts at `b8c6f88` |
