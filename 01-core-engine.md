...
**Status:** v-next (Round 23 — the R23 per-file re-audit: engine core @ `b8c6f88` 440/440, the frozen API surface (A5/D9 — 455 names, +2 additive) + layer fence, the N1-N4 + W1 + W2 seams and **N2b LANDED**, accepted as BASE; forward = the S-engine parity corpus + K2's engine-side pairs, the r1 union façade, r4 N5 decode, the color instruments, the r6 P2 remainder. R22 lineage: the R22 re-baseline accepted engine @ `f68ab8c` 356/356 with forward = the S-engine parity corpus, the W-ops union façade, W-audio N2b, W-n5 decode, R-engine-p2 items — of which N2b has since landed); Refined by sub-agent scout (SCOUT-01) — open questions answered with source code references
...
- nle-engine @ `b8c6f88` — 440/440 vitest, tsc 0 — this spec's core architecture contract landed: the frozen API surface + layer fence (A5/D9 — freeze opened at 453 names 2026-09-04; now **455** via two D9-legal additive names: `clampAuthorFadeToSpan` (RR1-A), `foldVolumeKeyframesToAutomation` (N2b)); the N1-N4 bridge seam family (composition-frame → the app's real ProgramCanvas, flattener volume/mute, cut-centered transition windows → real audio crossfades, av-link); W1 meter bridge; W2 maintainPitch varispeed on the bridge (the retime 3-site schedule law + the byte-capped recency-LRU span cache, 10 pins W2a-j — consumes WDC's typed LGPL WSOLA port); **N2b keyframed volume LANDED** (`37cdd28`, seam round S2 — the flattener folds `animations["volume"]` to gain-automation breakpoints (lane REPLACES the clip static, merge-blocking) + the adapter's dedicated automation gain node (exp-ramp exact for linear-in-dB, mid-entry/rate/hold, disposal at all 3 sites) + the native core path (`foldVolumeKeyframesToAutomation`, `ScheduleClipOptions.gainAutomation`); 18 bridge pins + 2 REAL offline-render sample-level parity pins; design doc `gaps/audit/design-n2b-gain-automation.md` v2 — audit-REJECTED once, 3 P0s folded); the RR1-A fade-clamp SPAN law (`clampAuthorFadeToSpan`, one shared law at BOTH native twins — the flattener and the player/export; `d421446` + the review round `b8c6f88`) with the A1 merged-run tail-fade carry and the A2 out-of-domain retime legacy fallback; the S3 engine seam-audit wave (`eda6e2f` — 2 P1 + 4 P2 + 6 P3, 23 tests); the consumer-compat `toSorted` fix (`798104e` — the vendored tree compiles under CONSUMER lib targets); the cross-module combined suites (`tests/vitest/engine/timeline-edit-ops` / `video-sync` / `bridge-seams` / `planner` — the K2 seeds). Vendors OT @ `a4e971d`, WDC @ `f446512` (W2 + the drift-gate-that-FAILS-on-drift live at that pin).
- nle-test-app @ `70e99f0` — 117/117, tsc 0 — the consumption proof: EngineMount + ProgramCanvas render real frames through the N1 seam; real audio crossfades + av-link dispatch; W3 JKL transport-rate audio follow (both rate-change entry paths reschedule, one idempotence key — `3f351bc`) + RR1-B; the N2b app-side glue (scheduleAll threads the automation lane + the segmentSignature gate, 2 pins). Consumes engine @ `4ef0147`, nle-ui @ `85dcf57` (the consumer re-pin wave — the app trailing engine HEAD by one commit — is the live S-engine queue item).

**GAP (the work — owner + phase in the D24 ladder vocabulary per `IMPLEMENTATION-PLAN.md`; acceptance in parentheses):**
- The projector parity corpus — the N1 composition-frame family's formal S4 suite: SceneTracks→ingestion translator + parity corpus — S-engine, crawl (the engine-CI K1/K2-class parity suite; was ∥ crawl) (acceptance: parity green on the corpus, engine CI).
- K2's engine-side combined pairs — OT-ops→engine-decode round-trips at the app's seam shapes (extends `tests/vitest/engine/timeline-edit-ops` / `video-sync`) — S-engine, K2 (crawl; NEW at R23/D24 — the combined-family formalization, registered in spec 12/17's K2 census) (acceptance: the round-trip pairs green in engine CI + present in the K2 census).
- The union façade — the 78-union service slice, JSON-RPC re-typed internal per D12.2 — r1 (was W-ops; engine-additive, OT's op families graduate in-phase) (acceptance: dispatch-complete + typed NOT_IMPLEMENTED).
- N5 real media decode (registry + decode → VirtualMediaAsset) — r4 (was W-n5; engine), PENDING user re-affirmation of D6 (acceptance: decode round-trip pins on a real-file corpus).
- The color instruments — scopes (waveform/vectorscope), the secondary qualifier, the power window as color-page instruments — S-engine authors ∥ crawl, consumed at r3 (was S-engine → consumed at W-color; NOT an r3 deliverable — S-engine builds, r3's grade-math consumes; zero scopes code today — the effects pipeline's `powerWindow`/`secondaryQualifier` GPU-effect params are the seed, not the instruments) (acceptance: per-instrument engine milestones green as queued; r3's scopes-plot-engine-data + grade-math parity pins consume them).
- ShapeItem/Lottie/Subtitle/Controller surfacing + CPU transition renderers — r6 (was R-engine-p2; engine, non-blocking) (acceptance: per-item milestones when queued).

**ACCEPTANCE & TEST PLAN:** §8 (the intent list) + the unnumbered `## Testing` section at file end (Tier 1/2/3 + property — the executable contract); spec 17 §13A facet rows (§13A.4 wire-protocol conformance for the union façade; §13A.5 projector property tests for the parity corpus); the battery's posture checks (battery_r23 — BASE rows re-verified at their pins — the regression role).
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — 54,739 src/lib LOC, 440 vitest + 9 CI-wired engine probes @ `b8c6f88`) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — still true at HEAD). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`. (The scout-era snapshot read 37,958 LOC / 124 tests / single-tier tests; the table below is that point-in-time record — rows carry supersession notes where the repo has since moved.)
