...
**D29.5 DISPOSITIONS (the engine-repo record vs the app's D30 R10 asks — per ARCH-R24 D29.5; all re-verified at `074a2f6`):**
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — 57,085 src/lib LOC / 63 files, 749 vitest / 25 test files + 9 CI-wired probes @ `074a2f6`) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — still true at HEAD). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`. (The scout-era snapshot read 37,958 LOC / 124 tests / single-tier tests; the table below is that point-in-time record — rows carry supersession notes where the repo has since moved.)
