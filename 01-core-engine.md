...
**ACCEPTANCE & TEST PLAN:** §8 (the intent list) + the unnumbered `## Testing` section at file end (Tier 1/2/3 + property — the executable contract); spec 17 §13A facet rows (§13A.4 wire-protocol conformance for the union façade; §13A.5 projector property tests for the parity corpus); the battery's posture checks (battery_r24 — BASE rows re-verified at their pins — the regression role).
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — 57,085 src/lib LOC / 63 files, 749 vitest / 25 test files + 9 CI-wired probes @ `e3f55bd`) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — still true at HEAD). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`. (The scout-era snapshot read 37,958 LOC / 124 tests / single-tier tests; the table below is that point-in-time record — rows carry supersession notes where the repo has since moved.)
