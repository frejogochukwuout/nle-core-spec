...
**Status:** v-next (Round 24 — the R24 per-file re-audit: engine core @ `5036387` 458/458 — the W2.5 D29 effects sidecar + `buildElementFilterString` (`8a0b7fe`, +17) + the R8-REV #5 blur-radius clamp (`4b2dfd1`, +1) + the fleet-coherence OT re-pin a4e971d→c15a629 (submodule-only) — the frozen API surface (A5/D9 — 455 names, unmoved) + layer fence, the N1-N4 + W1 + W2 seams and **N2b LANDED**, accepted as BASE; forward = the S-engine parity corpus + K2's engine-side pairs, the r1 union façade, r4 N5 decode, the color instruments, the CSS-filter widening (Vignette/Glow/LUT-preview v1), the r6 P2 remainder. R23 lineage: the R23 re-baseline accepted engine @ `b8c6f88` 440/440 with this same forward set — none of it landed since; the W2.5/R8-REV/re-pin landings arrived off-forward; R22 lineage: engine @ `f68ab8c` 356/356 — N2b the landed member). **R27 re-audit: engine @ `f9ac806` 748/748 — the D12 venue ratification + the OV convergence absorbed (this round's fix-list)**; Refined by sub-agent scout (SCOUT-01) — open questions answered with source code references
...
## 0. FORWARD INVENTORY (the R22 posture law, D20 — what needs to be done; the BASE is accepted, not re-explained; re-baselined at R24 per the D24 ladder; the dual-vocabulary window is closed — D24 tags only)

**BASE (accepted, pinned 2026-09-07):**
- nle-engine @ **`e3f55bd`** (chore-only re-pin over the unmoved code anchor `74bef08` — vendored OT→`55c81c0` + WDC→`83b8850`, own code unchanged) — **749/749 vitest, tsc 0, 25 vitest files** (live list: bridge-seams **151** + nle-bridge **99** + planner 52 + timeline-math 42 + transform-resolver 40 + load-validation **33** + video-sync 31 + timeline-nan-guard 31 + upstream-ports 25 + persistence 25 + timeline-mode-clamps 20 + undo-serialize 15 + timeline-linked-source-edit 13 + timeline-edit-ops 13 + timeline-locked-track 12 + timeline-relink-fixes 11 + m2-wave1-mixer-surface 10 (re-keyed 9→10 — the R29 re-key moved the total but not the per-file list; R33, engine seal-round register P3-3) + render-abort 9 + realtime-midi 8 + stable-video-grouping 5 + media-registry 5 + audio-codec-ladder 5 + mask-manager-view 4 + compositor-clear 3 + api-surface 3 — the R25-R27 waves: E1/E2, N3/N4, F5/F6, the RC round, M2 Wave 1; table-driven rows expand at runtime; live-run venue = GHA CI) — this spec's core architecture contract landed: the frozen API surface + layer fence (A5/D9 — freeze opened at 453 names 2026-09-04; now **455** via two D9-legal additive names: `clampAuthorFadeToSpan` (RR1-A), `foldVolumeKeyframesToAutomation` (N2b) — **unmoved at `f9ac806`**: W2.5's `buildElementFilterString` is bridge-level (`bridge/composition-frame.ts`), NOT re-exported through the barrel `src/lib/nle/index.ts` (the barrel's only `bridge`-adjacent export is `audio/realtime-bridge`'s `RealtimeAudioBridge`; the bridge is app-level G-layer code per D8, deep-imported) — D9 intact; the layer fence = `scripts/lint-layering.mjs` + `gaps/audit/LAYER-SNAPSHOT.txt`, **52 pinned cross-layer edges, CI-wired — held exactly at `f9ac806` (⭐ re-verified; + the OV-12 `vendored-timeline` CI re-pin job, ci.yml:293-393 @ `2a0ecf4`, re-based (R33, engine seal-round register P3-5))**); the N1-N4 bridge seam family (composition-frame → the app's real ProgramCanvas, flattener volume/mute, cut-centered transition windows → real audio crossfades, av-link); W1 meter bridge; W2 maintainPitch varispeed on the bridge (the retime 3-site schedule law + the byte-capped recency-LRU span cache, 11 pins W2a–W2k (W2k = the out-of-domain legacy-fallback pin, `nle-bridge.test.ts:899`; aligned with 02 §0's W2a–W2k — the two specs disagreed at the same engine pin; R33, engine seal-round register P3-2) — consumes WDC's typed LGPL WSOLA port; **maintainPitch = the D29.5(b) LANDED disposition**: the flattener threads `el.retime?.maintainPitch` → `source.maintainPitch` at scene-to-segments.ts:596/:623 (the segment type fields :207-214/:273); **N2b keyframed volume LANDED** (`37cdd28`, seam round S2 — the flattener folds `animations["volume"]` to gain-automation breakpoints (lane REPLACES the clip static, merge-blocking) + the adapter's dedicated automation gain node (exp-ramp exact for linear-in-dB, mid-entry/rate/hold, disposal at all 3 sites) + the native core path (`foldVolumeKeyframesToAutomation`, `ScheduleClipOptions.gainAutomation`); 18 bridge pins + 2 REAL offline-render sample-level parity pins; design doc `gaps/audit/design-n2b-gain-automation.md` v2 — audit-REJECTED once, 3 P0s folded); the RR1-A fade-clamp SPAN law (`clampAuthorFadeToSpan`, one shared law at BOTH native twins — the flattener and the player/export; `d421446` + the R23 review round `b8c6f88`) with the A1 merged-run tail-fade carry and the A2 out-of-domain retime legacy fallback; the S3 engine seam-audit wave (`eda6e2f` — 2 P1 + 4 P2 + 6 P3, 23 tests); the consumer-compat `toSorted` fix (`798104e` — the vendored tree compiles under CONSUMER lib targets); the cross-module combined suites (`tests/vitest/engine/timeline-edit-ops` / `video-sync` / `bridge-seams` / `planner` — the K2 seeds). **The R24 landings:** (1) **the W2.5 D29 effects sidecar** (`8a0b7fe` — `CompositionElementParams.effects` + `CompositionElementEffect` ({name, params} — the consumers' Inspector stack minus their id/enabled fields) + the PURE export `buildElementFilterString()` (composition-frame.ts:257): 'Gaussian Blur' → `blur(<radius>px)` (default 10 — the GPU registry's declared default per the F2-5 cross-module coherence pin, non-finite→10; `GAUSSIAN_BLUR_DEFAULT_RADIUS` :210 — the old default-4 previewed a blur the render never shipped), Vignette/Glow/Film Grain/Motion Blur/everything else = HONEST SKIP (the GPU effects registry — **44 registered effects: 43 ids in `effects/pipeline.ts` + `gpu-lut` @ `effects/lut.ts:321`** — renders them at fidelity; a fake Canvas2D approximation would preview something the render never ships), multiple compatible effects JOIN with spaces, empty/absent = identity ''; the painter applies the string per element INSIDE the existing save/restore blocks; Safari <18 graceful no-op; the scene GRADE stays consumer-side FINAL PASS by design (D29.5(c) below); the ops' `CompositionDrawOp.filter` field; +17 pins, 440→457); (2) **the R8-REV #5 blur-radius clamp** (`4b2dfd1` — a negative finite radius emitted `blur(-5px)` (invalid CSS) and the WHOLE `ctx.filter` silently no-oped; the radius now clamps at ≥0; +1 pin → **458**); (3) **the fleet-coherence re-pin** (`5036387` — the OT submodule a4e971d→c15a629, SUBMODULE-ONLY delta (git stat: `vendor/opencut-timeline | 2 +-`, zero engine source change), 458/458 against the new pin: the OT W11 wire-dispatch surface is importable at the engine's TEST SEAM (the vitest alias `opencut-timeline` → `vendor/opencut-timeline/src/lib/timeline/` — HeadlessTimelineApi attach + the dispatch recorder (ring 500 + coverage Set) + `WIRE_COMMAND_TYPES` + `applyBatch` `data.results` + keyframe `lockPreCheck`/TRACK_LOCKED) with ZERO engine mediation by design — the engine consumes the scene STRUCTURALLY only (type-only imports; grep-verified zero wire usage in src/ + tests/)). **The vendor pins (re-keyed R29):** OT @ `17a19f8` (re-pinned c15a629→…→`6e2b91a`→`55c81c0`→`39003d3`→`17a19f8` — re-keyed R34, the last two hops were missed: the R32 twin re-pin + the s18 lockstep; verified at engine `2a0ecf4`; the D-ARCH-6 `970948a` absorption LANDED at the engine's `e3f55bd` re-pin — the R27 queue row, engine PLAN:481, executed), WDC @ `94f6460` (re-pinned 494f6ff→…→`ec8fd5c`→`83b8850`→`94f6460` by the S-series wave `480a216` + the R29/R32 re-pins — re-keyed R34, verified at engine `2a0ecf4`); the W11 widenings at the current pin: `WIRE_COMMAND_TYPES` `headless/api.ts:268-300` + the tsc-lockstep asserts `:353-360` (the census re-declared 31 = 28 routed + 3 exceptions @ `970948a`; re-anchored R34 — the WIRE_COMMAND_TYPES/WIRE_UI_EXCEPTION_VERBS cites at the live `17a19f8` pin, was `:243-275`/`:277-297`). (R33 @ `2a0ecf4` — **the exhaustive-testing waves LANDED, the register P2-1 disposition DISCHARGED** (the engine seal-round's W0-W4 rollout, `gaps/audit/seal-round/design-exhaustive-testing.md` v2, folded with the register): W0 the `pull_request` CI trigger UNFILTERED (paths-ignore stays push-only); W1 the vitest count gate `scripts/gate-vitest-count.mjs` — the per-file pin table GENERATED from the runtime JSON report (never the static census: locked-track 12 static→49 runtime, nan-guard 31→81), fails on ANY drift in EITHER direction, fails CLOSED on malformed, + its standing self-tests `gate-vitest-count.test.mjs` (9 bun tests outside the counted include); W2 **THE PROPERTY TIER** `tests/vitest/property/` (4 test files + the shared `arbitraries.ts`; `fast-check@4.10.1` declared ^4 over the resident transitive 3.23.2; the fixed CI seed 20260921 + the `FC_SEED`/`FC_NUMRUNS_SCALE` knobs; **33 sites** — the timeline-undo-soak 11 [the S1 op-sequence soak: `fc.commands` ≤30 × 34 command classes over the 61-mutator `execute()` surface, the 10-law invariant battery per command — the undo/redo canon trio, exactly-one-entry, no-op suppression, the S3a/S3b throw-gate split (the R2-3 finally-commit law), locked STRUCTURAL canon, id uniqueness, structural validity; the mutator-coverage table names the 34 reached + the 27 residuals (probe-p114 keeps their deterministic trio)] + timeline-math 10 + persistence-roundtrip 6 [`canon()` = the timeline sub-object — the envelope is always wall-clock-stamped] + transform-resolver 6 — **caught 2 real engine bugs on its first runs: P-F1** (removeClip's unknown-id SILENT no-op — now throws like every id-addressed op) **+ P-F2** (the explicit-undefined patch class — a patch key with an undefined value bypassed N4+N3 and teleported a clip to from=0/dur=1 ON A LOCKED TRACK; `normalizeClipUpdates` now DROPS explicit-undefined keys, +3 nan-guard pins 78→81 runtime, + the `_clearTransformParent` corollary `28aa387`; the S7 window-containment correction — containment is NOT an engine invariant)]; W3 the parallel `coverage (report artifact)` CI job (`@vitest/coverage-v8@3.2.7` version-locked to vitest, v8, include `src/lib/nle/**` ONLY — the browser venue owns src/app; **report-artifact-first, NO thresholds until W5**; the precedence law in the yaml: coverage SUPPLEMENTS the count/census pins, never replaces); W4 `perf.yml` **THE FLEET'S FIRST SCHEDULED WORKFLOW** (cron `0 7 * * *` + dispatch fallback; PERF-1..3 + `tests/perf/` 10 envelope tests OUTSIDE the counted include — telemetry-mode first, `PERF_ASSERT=1` nightly-only) — **749→785** (749 + the 33 property sites + the 3 P-F2 pins), **29 vitest test files**, tsc 0, 785/785 + the gate + the self-tests live-verified this round; **the code anchor MOVES `74bef08`→`28aa387`** — the first src/ movement since R28 (the P-F1/P-F2 fixes, `28aa387` the last src-touching commit / `2a0ecf4` the docs-only HEAD; timeline.ts 9,051 LOC); the app's engine vendor moved `e3f55bd`→`574d8d3` at the app's own lockstep wave (`7664603`) — the W0-W4 landings are engine-repo-only beyond it (the BY-DESIGN post-re-pin pattern); **the r1 port map** (`gaps/audit/seal-round/design-r1-port.md` §5.3) lands the engine suite at **≈630-640 at r1 END** (749 − 106 carried to OT − 19 retired av-link/fence its + the grown fences/corpus; the band re-derives at each stage boundary — the 785-era property additions ride the same map).) (R33, engine seal-round register P2-1 + design-exhaustive-testing W0-W4)
- nle-test-app @ **`876f2b8`** — **256/256** (8-suite roof: GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + wire-coverage 13 + engineSeam 7 + waveformPeaks 9; census 42 = 36 zero-action + 5 carriers + 1 host @ OT mirror `55c81c0`; AR-2 wire-coverage gate GREEN against the live 31/28+3 registry; maintainPitch threaded at engineService.ts:510-513 + sceneBridge.ts:262/:447), tsc 0 — the consumption proof: EngineMount + ProgramCanvas render real frames through the N1 seam; real audio crossfades + av-link dispatch; W3 JKL transport-rate audio follow (both rate-change entry paths reschedule, one idempotence key — `3f351bc`) + RR1-B; the N2b app-side glue (scheduleAll threads the automation lane + the segmentSignature gate); the R8 "REAL wiring" round (W2.1 sidechain ducking, W2.2 effects-drop targets, W2.3 autosave/persistence (engine-free localStorage), W2.4 deliver export via mediabunny, W2.5 effects/color preview v1 — ProgramCanvas maps `el.effects` via `buildElementFilterString` + the per-scene GRADE final pass; the engine vendor re-pinned `8a0b7fe`→`4b2dfd1` in-flight) + the R8-b1 scene-switch rebuild fix + the R8-REV six-fix wave; the R9 W-A fleet re-pin + W-B adopt-wholesale files. Consumes engine @ `e3f55bd` (the R29 re-pin — re-converged), nle-ui @ `6754979` (the AW1-2 absorb landed with the freshness re-pin), OT lock-copy @ `55c81c0` (byte-exact minus `testing/`, live-verified; the s17 re-pin LANDED @ `876f2b8`), WDC @ `ec8fd5c`. **[R30 re-pin: `8f12cf9` — 377/377, the 12-file roof (GluedShell 124 + wire-coverage 26 + audioService 42 + deliverService 20 + keybindings-repeat 4 + waveformPeaks 9 + history-laws 15 + persistenceService 20 + engineService 31 + sceneBridge 41 + timeline-geometry 38 + engineSeam 7); the K3 corpus round — the four re-expression maps + the store/policy + geometry tranches + the C16 guards; consumers nle-ui @ `5779295` (701/701) + engine `e3f55bd` + OT lock `55c81c0` + WDC `ec8fd5c` unchanged] **[R31 re-pin: `f58147c` — 418/418, the 21-file roof (+9 NEW: dom-structural 8 + scrub-session 4 + clip-lifecycle 3 + trim-gesture 3 + edge-autoscroll 2 + zoom-anchor 2 + dnd-fallback 3 + mid-drag-yield 5 + e2e-crawl 6 (→11 at the round's K4b close — the +5 beyond the 36 listed; 377 + 36 + 5 = 418, per-file git-verified: the other 8 new files match their listed counts exactly) — the K3 DOM-structural tranche: all 13 RE-EXPRESS-PORT rows LANDED + the D26.5 four partial rulings as code + D51 the mid-drag keyboard yield; the vendor/nle-ui pin bumped → `dc3644b` (docs-only, 701/701 unchanged — the D29 DECISIONS first-party record + the C0 MiniShell row filing); engine `e3f55bd` + OT lock `55c81c0` + WDC `ec8fd5c` UNCHANGED at the R31 pin (git-verified; the engine-side M60 vendor re-pin OT/WDC→`39003d3`/`94f6460` is engine-repo-only, beyond the app's R31 pins) — the R31 bracket completed (R33, engine seal-round register P3-8)] **[R33 re-pin: `7664603` — the s18 lockstep re-pin over `ce7cfc3`/426 (R32; `f58147c`/418 R31, `8f12cf9`/377 R30, `876f2b8`/256 R29): 426/426 + the 22-file roof UNCHANGED; the vendor engine `e3f55bd`→`574d8d3` + BOTH OT mirrors `39003d3`→`17a19f8`; the census register re-declared **43 = 32 zero-action (6 byte + 26 mechanical) + 9 carriers + 2 hosts** @ the `17a19f8` re-pin (R33 re-key)]****

**GAP (the work — owner + phase in the D24 ladder vocabulary per `IMPLEMENTATION-PLAN.md`; acceptance in parentheses):**
- The projector parity corpus — the N1 composition-frame family's formal S4 suite: SceneTracks→ingestion translator + parity corpus — S-engine, crawl (the engine-CI K1/K2-class parity suite; re-verified still-open at `2a0ecf4` (R33 re-key) — no S4 suite in the repo, not in the engine HANDOFF queue) (acceptance: parity green on the corpus, engine CI).
- K2's engine-side combined pairs — OT-ops→engine-decode round-trips at the app's seam shapes (extends `tests/vitest/engine/timeline-edit-ops` / `video-sync`) — S-engine, K2 (crawl — the combined-family formalization, registered in spec 12/17's K2 census) (acceptance: the round-trip pairs green in engine CI + present in the K2 census).
- The union façade — the 78-union service slice, JSON-RPC re-typed internal per D12.2 — r1 (engine-additive, OT's op families graduate in-phase) (acceptance: dispatch-complete + typed NOT_IMPLEMENTED).
- N5 real media decode (registry + decode → VirtualMediaAsset) — r4 (engine), PENDING user re-affirmation of D6 (deprioritized per the engine PLAN's P1.9; G3 queue row) (acceptance: decode round-trip pins on a real-file corpus).
- The color instruments — scopes (waveform/vectorscope), the secondary qualifier, the power window as color-page instruments — S-engine authors ∥ crawl, consumed at r3 (NOT an r3 deliverable — S-engine builds, r3's grade-math consumes; zero scopes code today, re-verified at `2a0ecf4` (R33 re-key) — the repo carries only the param-metadata comment + the wire-schema strings `gpu-power-window`/`gpu-secondary-qualifier` (headless/api.ts:1529/:1535, the byte-identical FreeCut wire contract, NOT implementations); the P2 backlog still lists scopes (waveform/vectorscope) — the row stands) (acceptance: per-instrument engine milestones green as queued; r3's scopes-plot-engine-data + grade-math parity pins consume them).
- ShapeItem/Lottie/Subtitle/Controller surfacing + CPU transition renderers — r6 (engine, non-blocking) (acceptance: per-item milestones when queued).
- The CSS-filter preview widening — Vignette / Glow / LUT-preview v1 on the composition seam (the D29.5(d) disposition: only the Gaussian-Blur subset landed at `8a0b7fe`; the remaining mappings are the open widening) — S-engine, ∥ crawl (acceptance: each mapping either a pinned CSS-filter law or an explicit honest-skip at the seam, the same pin shape as the W2.5 family; the LUT-preview v1 subset matches the app's monitor-side ask).

**D29.5 DISPOSITIONS (the engine-repo record vs the app's D30 R10 asks — per ARCH-R24 D29.5; all re-verified at `2a0ecf4` (re-keyed R33)):**
- (a) The transform sidecars — **FUTURE-EXTENSION, not a current gap**: `bridge/composition-frame.ts` carries the documented slots ("v1: no spatial transforms in the opencut model — future transform sidecars extend the dest", :80; "op.dest is v1-inert data kept for the future transform extension", :106); owner S-engine when queued (not filed as an engine PLAN row — the future-extension CR re-file).
- (b) `maintainPitch` — **LANDED** (BASE above; scene-to-segments.ts:596/:623) — **RESOLVED end-to-end (R9-b, R27)**: `el.preservePitch` is ElementJSON law (absent≡true; nle-ui `6754979`; R30: `5779295` (R31: `dc3644b` — 701 UNCHANGED, docs-only: the D29 DECISIONS first-party record + the C0 MiniShell row filing)); the app threads it (engineService.ts:510-513, sceneBridge.ts:262/:447); the engine's `retime.maintainPitch` (scene-to-segments :596/:623) is the runtime projection.
- (c) The scene-grade promotion into composition/export — **DECLINED BY DESIGN**: the consumer-side final pass is the law (the engine's `8a0b7fe` design — hue-rotate/saturate are non-linear under per-element alpha compositing, per-element grade filters ≠ composited grade; the app host composes first, filter-draws once). Spec 04/08 carry the same decision as decision rows, not gaps.
- (d) The CSS-filter widening — **OPEN GAP** (the GAP row above; owner S-engine).

**ACCEPTANCE & TEST PLAN:** §8 (the intent list) + the unnumbered `## Testing` section at file end (Tier 1/2/3 + property — the executable contract); spec 17 §13A facet rows (§13A.4 wire-protocol conformance for the union façade; §13A.5 projector property tests for the parity corpus); **the property-tier disposition REGISTERED (R33, engine seal-round register P2-1 — DISCHARGE-SHAPED): the engine's CLASS-API property tier LANDED @ `2a0ecf4`** (the 4-file fast-check soak family over the native Timeline — the register's preferred disposition, executed by the engine's W2 wave; 33 sites, the 2 real catches P-F1/P-F2; `## Testing`'s R33 note the record) — it is NOT this spec's blueprint property surface (the `EngineCommand`/r1-ported world, §13A.5's projector + wire properties), which rides the r1 port per 17 §13A.8; the battery's posture checks (battery_r24 — BASE rows re-verified at their pins — the regression role).
...
**Reference:** OpenCut-classic `apps/web/src/core/managers/playback-manager.ts` (257 LOC). Verified (§7 Q2): OpenCut's clock is `performance.now()` + rAF; we override with FreeCut's `AudioContext.currentTime` clock (see `03-playback-engine.md`).
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — 63 files / 57,166 src/lib/nle LOC at `60232ea` (re-keyed R34, was 57,139 @ `2a0ecf4`; the R28 census read 57,085, same src/lib/nle scope; the full src/lib tree = 65 files / 57,184 — the scope label fixed and the mixed-scope 57,157 read corrected R33, engine seal-round register P3-3), **789 vitest / 29 test files** — the R33 exhaustive-testing waves: +the 4-file property tier + the 3 P-F2 pins; the count gate `gate-vitest-count.mjs` self-pins both drift directions + its standing self-tests, + 9 CI-wired probes + the parallel `coverage (report artifact)` job + the `perf.yml` nightly — +4 the R34 P-F4 pins (the nightly exploration venue's first catch, fixed; the gate re-pinned 785→789 same-commit; the nightly + the 5-job CI GREEN) — re-keyed R34, was 785 @ `2a0ecf4` (the R33 re-key (R33, engine seal-round register P2-1))) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — still true at HEAD). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`. (The scout-era snapshot read 37,958 LOC / 124 tests / single-tier tests; the table below is that point-in-time record — rows carry supersession notes where the repo has since moved.)

| Spec section | Engine file:line | Verified quote | Status | Note |
|---|---|---|---|---|
| §3.2 EditorCore singleton, 12 managers | `src/lib/nle/index.ts:7` | `import { initGpu, MediaRegistry, Timeline, Player, RealtimeAudioBridge } from '@/lib/nle';` | CORRECTIVE | No EditorCore/manager root — concrete classes are the public API; spec's `EditorCore.getInstance(deps)` + Decision 9 dispatcher win. AudioMixer class retired 2026-09-04 (m30 one-engine); realtime audio = RealtimeAudioBridge (exported `index.ts:45`; absent from the 455-name frozen list) → WDC SegmentStripAdapter/ChannelStrips — the row's substance remains true (R33, engine seal-round register P3-1) |
| §3.3 TimelineManager | `src/lib/nle/timeline/timeline.ts:1894` | `export class Timeline {` | CORRECTIVE | Class API is the engine's primary mutation surface; spec mandates thin wrappers over `engine.command.apply()` |
| §4.1/§4.2 Command + undo | `src/lib/nle/timeline/timeline.ts:7375` | `execute<T>(label: string, fn: () => T): T {` | SUPERSEDED | Scout-era: zero callers (engine P1.5). CLOSED since (P1.14, 2026-09-04): 59 ops wrapped via `execute()` — undo-by-default, one user op = one undo step; probe-p114 (318 checks) CI-wired; 61 `execute(` call sites in timeline.ts at `2a0ecf4` (still 61 — ⭐ re-verified R27, re-verified R33; the row's cite re-anchored :7321→:7375 — R33 re-key) |
...
| §3.5 EngineDeps DI | `src/lib/nle/core/types.ts:1030` | `export type Clip = VideoClip \| AudioClip \| ImageClip \| AdjustmentItem \| CompositionItem \| TextItem;` | CORRECTIVE | No EngineDeps injection; 6 of 8 item types now (Wave-4C P1.3 + text/adjustment) |
| §3.6 two entry points | `src/lib/nle/headless/api.ts:122` | `Apply a batch of JSON-RPC edit ops + $ref resolution` | CORRECTIVE | 19-op JSON-RPC vs spec 15's 78-type union |
| §3.3 RendererManager (10-bit) | `src/lib/nle/gpu/compositor.ts:1130` | `format: 'rgba8unorm',` | CORRECTIVE | All textures 8-bit; Decision 5 wins |
| §3.6 render entry WYSIWYG | `src/lib/nle/playback/player.ts:1580` | `if (clip.type !== 'video') continue;` | CORRECTIVE | Scout-era state. At HEAD the visual loop admits video + image (`clip.type !== 'video' && clip.type !== 'image'` — Wave 4C P1.3), text renders via the text layer, audio rides the audio path; the spec's all-types WYSIWYG demand stands |
...
> **Execution-status note (R33, engine seal-round register P2-1 — DISCHARGE-SHAPED):** the property tier is NO LONGER r1-only content — **the engine's class-API property tier LANDED @ `2a0ecf4`** (`tests/vitest/property/` — the 4-file family + the shared `arbitraries.ts`, `fast-check@4.10.1`, the fixed CI seed 20260921, 33 sites): the randomized undo soak over the native `Timeline`'s 61 `execute()`-wrapped mutators (34 command classes + 27 named residuals on probe-p114's deterministic trio; the 10-law invariant battery; 2 real bugs caught — P-F1/P-F2), timeline-math, persistence round-trips, transform-resolver — the register's preferred disposition, executed by the engine's W2 wave BEFORE r1. **What the catalogue below still is:** the BLUEPRINT for the r1-ported surface (`arbitraryCommand`/`arbitraryState` over the `EngineCommand` world, the 78-variant exhaustiveness switch, madge CI) — those ride the r1 port per 17 §13A.8; the landed soak deliberately targets the class API that exists TODAY (the D12 posture: the native Timeline stays the offline reference + the parity twin). `undo-is-involutive` below is carried by the soak's S1 (the canon-equal undo/redo trio); the r1 port map (design-r1-port §5.3) lands the engine suite at ≈630-640 at r1 END with the carried corpus re-keyed into OT.
