...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — 63 files / 57,139 src/lib/nle LOC at `2a0ecf4` (the R28 census read 57,085, same src/lib/nle scope; the full src/lib tree = 65 files / 57,157 — the scope label fixed and the mixed-scope 57,157 read corrected R33, engine seal-round register P3-3), **785 vitest / 29 test files** — the R33 exhaustive-testing waves: +the 4-file property tier + the 3 P-F2 pins; the count gate `gate-vitest-count.mjs` self-pins both drift directions + its standing self-tests, + 9 CI-wired probes + the parallel `coverage (report artifact)` job + the `perf.yml` nightly — the R33 re-key (R33, engine seal-round register P2-1)) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — still true at HEAD). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`. (The scout-era snapshot read 37,958 LOC / 124 tests / single-tier tests; the table below is that point-in-time record — rows carry supersession notes where the repo has since moved.)
