...
**D29.5 DISPOSITIONS (the engine-repo record vs the app's D30 R10 asks — per ARCH-R24 D29.5; all re-verified at `e3f55bd`):**
...
| §4.1/§4.2 Command + undo | `src/lib/nle/timeline/timeline.ts:7321` | `execute<T>(label: string, fn: () => T): T {` | SUPERSEDED | Scout-era: zero callers (engine P1.5). CLOSED since (P1.14, 2026-09-04): 59 ops wrapped via `execute()` — undo-by-default, one user op = one undo step; probe-p114 (318 checks) CI-wired; 61 `execute(` call sites in timeline.ts at `f9ac806` (still 61 — ⭐ re-verified R27) |
