...
- nle-engine @ `5036387` — 458/458 vitest, tsc 0 (13 vitest files: bridge-seams 113 + nle-bridge 96 + planner 52 + timeline-math 42 + transform-resolver 40 + load-validation 32 + video-sync 31 + undo-serialize 15 + persistence 13 + realtime-midi 8 + render-abort 9 + timeline-edit-ops 4 + api-surface 3; live-run venue = GHA CI) — this spec's core architecture contract landed: the frozen API surface + layer fence (A5/D9 — freeze opened at 453 names 2026-09-04; now **455** via two D9-legal additive names: `clampAuthorFadeToSpan` (RR1-A), `foldVolumeKeyframesToAutomation` (N2b) — **unmoved at `5036387`**: W2.5's `buildElementFilterString` is bridge-level (`bridge/composition-frame.ts`), NOT re-exported through the barrel `src/lib/nle/index.ts` (the barrel's only `bridge`-adjacent export is `audio/realtime-bridge`'s `RealtimeAudioBridge`; the bridge is app-level G-layer code per D8, deep-imported) — D9 intact; the layer fence = `scripts/lint-layering.mjs` + `gaps/audit/LAYER-SNAPSHOT.txt`, **52 pinned cross-layer edges, CI-wired**); the N1-N4 bridge seam family (composition-frame → the app's real ProgramCanvas, flattener volume/mute, cut-centered transition windows → real audio crossfades, av-link); W1 meter bridge; W2 maintainPitch varispeed on the bridge (the retime 3-site schedule law + the byte-capped recency-LRU span cache, 10 pins W2a-j — consumes WDC's typed LGPL WSOLA port; **maintainPitch = the D29.5(b) LANDED disposition**: the flattener threads `el.retime?.maintainPitch` → `source.maintainPitch` at scene-to-segments.ts:246/:611/:638 — the residual is the app's own toggle wiring, app-owned); **N2b keyframed volume LANDED** (`37cdd28`, seam round S2 — the flattener folds `animations["volume"]` to gain-automation breakpoints (lane REPLACES the clip static, merge-blocking) + the adapter's dedicated automation gain node (exp-ramp exact for linear-in-dB, mid-entry/rate/hold, disposal at all 3 sites) + the native core path (`foldVolumeKeyframesToAutomation`, `ScheduleClipOptions.gainAutomation`); 18 bridge pins + 2 REAL offline-render sample-level parity pins; design doc `gaps/audit/design-n2b-gain-automation.md` v2 — audit-REJECTED once, 3 P0s folded); the RR1-A fade-clamp SPAN law (`clampAuthorFadeToSpan`, one shared law at BOTH native twins — the flattener and the player/export; `d421446` + the R23 review round `b8c6f88`) with the A1 merged-run tail-fade carry and the A2 out-of-domain retime legacy fallback; the S3 engine seam-audit wave (`eda6e2f` — 2 P1 + 4 P2 + 6 P3, 23 tests); the consumer-compat `toSorted` fix (`798104e` — the vendored tree compiles under CONSUMER lib targets); the cross-module combined suites (`tests/vitest/engine/timeline-edit-ops` / `video-sync` / `bridge-seams` / `planner` — the K2 seeds). **The R24 landings:** (1) **the W2.5 D29 effects sidecar** (`8a0b7fe` — `CompositionElementParams.effects` + `CompositionElementEffect` ({name, params} — the consumers' Inspector stack minus their id/enabled fields) + the PURE export `buildElementFilterString()` (composition-frame.ts:223): 'Gaussian Blur' → `blur(<radius>px)` (default 4, non-finite→4), Vignette/Glow/Film Grain/Motion Blur/everything else = HONEST SKIP (the GPU effects registry — **44 registered effects: 43 ids in `effects/pipeline.ts` + `gpu-lut` @ `effects/lut.ts:321`** — renders them at fidelity; a fake Canvas2D approximation would preview something the render never ships), multiple compatible effects JOIN with spaces, empty/absent = identity ''; the painter applies the string per element INSIDE the existing save/restore blocks; Safari <18 graceful no-op; the scene GRADE stays consumer-side FINAL PASS by design (D29.5(c) below); the ops' `CompositionDrawOp.filter` field; +17 pins, 440→457); (2) **the R8-REV #5 blur-radius clamp** (`4b2dfd1` — a negative finite radius emitted `blur(-5px)` (invalid CSS) and the WHOLE `ctx.filter` silently no-oped; the radius now clamps at ≥0; +1 pin → **458**); (3) **the fleet-coherence re-pin** (`5036387` — the OT submodule a4e971d→c15a629, SUBMODULE-ONLY delta (git stat: `vendor/opencut-timeline | 2 +-`, zero engine source change), 458/458 against the new pin: the OT W11 wire-dispatch surface is importable at the engine's TEST SEAM (the vitest alias `opencut-timeline` → `vendor/opencut-timeline/src/lib/timeline/` — HeadlessTimelineApi attach + the dispatch recorder (ring 500 + coverage Set) + `WIRE_COMMAND_TYPES` + `applyBatch` `data.results` + keyframe `lockPreCheck`/TRACK_LOCKED) with ZERO engine mediation by design — the engine consumes the scene STRUCTURALLY only (type-only imports; grep-verified zero wire usage in src/ + tests/)). Vendors OT @ `c15a629` (re-pinned a4e971d→c15a629 by `5036387`), WDC @ `494f6ff` (re-pinned f446512→494f6ff by `5e5bbe4` — A6 barrel-consumption docs-only for consumers, all deep-import).
- nle-test-app @ `c885ece` — 174/174 (static census: GluedShell 70 + audioService 40 + sceneBridge 25 + persistence 17 + deliverService 15 + engineSeam 7), tsc 0 — the consumption proof: EngineMount + ProgramCanvas render real frames through the N1 seam; real audio crossfades + av-link dispatch; W3 JKL transport-rate audio follow (both rate-change entry paths reschedule, one idempotence key — `3f351bc`) + RR1-B; the N2b app-side glue (scheduleAll threads the automation lane + the segmentSignature gate); the R8 "REAL wiring" round (W2.1 sidechain ducking, W2.2 effects-drop targets, W2.3 autosave/persistence (engine-free localStorage), W2.4 deliver export via mediabunny, W2.5 effects/color preview v1 — ProgramCanvas maps `el.effects` via `buildElementFilterString` + the per-scene GRADE final pass; the engine vendor re-pinned `8a0b7fe`→`4b2dfd1` in-flight) + the R8-b1 scene-switch rebuild fix + the R8-REV six-fix wave; the R9 W-A fleet re-pin + W-B adopt-wholesale files. Consumes engine @ `5036387` (AT HEAD — the consumer re-pin wave CLOSED), nle-ui @ `fc4cc35`, OT lock-copy @ `c15a629` (byte-exact minus `testing/`, live-verified), WDC @ `85b81b0` (opportunistic, docs-only ahead of the engine's `494f6ff`).

**GAP (the work — owner + phase in the D24 ladder vocabulary per `IMPLEMENTATION-PLAN.md`; acceptance in parentheses):**
- The projector parity corpus — the N1 composition-frame family's formal S4 suite: SceneTracks→ingestion translator + parity corpus — S-engine, crawl (the engine-CI K1/K2-class parity suite; re-verified still-open at `5036387` — no S4 suite in the repo, not in the engine HANDOFF queue) (acceptance: parity green on the corpus, engine CI).
...
**D29.5 DISPOSITIONS (the engine-repo record vs the app's D30 R10 asks — per ARCH-R24 D29.5; all re-verified at `5036387`):**
- (a) The transform sidecars — **FUTURE-EXTENSION, not a current gap**: `bridge/composition-frame.ts` carries the documented slots ("v1: no spatial transforms in the opencut model — future transform sidecars extend the dest", :70-71; "op.dest is v1-inert data kept for the future transform extension", :93-94); owner S-engine when queued (not filed as an engine PLAN row — the future-extension CR re-file).
- (b) `maintainPitch` — **LANDED** (BASE above; scene-to-segments.ts:246/:611) — the residual is the app's own toggle wiring (owner S-app, w1-grammar work; the app's Inspector toggle is mock-local per its own zero-no-op audit).
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — 54,791 src/lib LOC, 458 vitest + 9 CI-wired engine probes @ `5036387`) is a clean-room FreeCut-port **in-between reference, NOT canon**. It de-risks implementation but inherits FreeCut patterns these specs correct (8-bit rgba8unorm, JSON-RPC + `$ref`, class-API mutation surface, procedural media, zero Web Workers — still true at HEAD). Where engine code conflicts with this spec, **the spec wins**. Full reconciliation: `19-code-references.md`. (The scout-era snapshot read 37,958 LOC / 124 tests / single-tier tests; the table below is that point-in-time record — rows carry supersession notes where the repo has since moved.)
...
| §4.1/§4.2 Command + undo | `src/lib/nle/timeline/timeline.ts:5225` | `execute<T>(label: string, fn: () => T): T {` | SUPERSEDED | Scout-era: zero callers (engine P1.5). CLOSED since (P1.14, 2026-09-04): 59 ops wrapped via `execute()` — undo-by-default, one user op = one undo step; probe-p114 (318 checks) CI-wired; 61 `execute(` call sites in timeline.ts at `5036387` (stable since `b8c6f88`) |
...
| §3.6 render entry WYSIWYG | `src/lib/nle/playback/player.ts:1038` | `if (clip.type !== 'video') continue;` | CORRECTIVE | Scout-era state. At HEAD the visual loop admits video + image (`clip.type !== 'video' && clip.type !== 'image'` — Wave 4C P1.3), text renders via the text layer, audio rides the audio path; the spec's all-types WYSIWYG demand stands |
