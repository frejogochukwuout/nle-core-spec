...
- **Runtime (new, this session):** shell-mini serves on **:3000** (vite base '/', strictPort) — the Z-container's Caddy :81 reverse-proxies localhost:3000, so the preview URL = the app. Dev server survives toolcall reaping via the committed double-fork launcher (`ui-mock/shell-mini/scripts/dev3000.py`); recycle resurrection via the PAT-free boot hook (`ui-mock/shell-mini/scripts/boot-restore.sh`, iso `/home/z/my-project/.zscripts/dev.sh`, restores from the newest /home/sync bundle). **GO-LIVE FIX:** the FC edge rewrites Host to ...fcapp.run and Vite's default allowedHosts 403'd every public request (localhost probes were a false pass) — `server.allowedHosts: ['.space-z.ai', '.fcapp.run']` added; public URL now verified end-to-end (title, a11y tree, playback TC advance). **STORYBOOK PUBLIC SURFACE (same session):** XTransformPort proven unable to carry multi-asset UIs (query doesn't propagate to sub-resources) → static storybook build mounted at `public/stories/` (committed) → `/stories/index.html` at the public URL, verified + VLM-confirmed; dev storybook daemonized via `scripts/sb6007.py` (:6007, localhost HMR). Full verified law-set: SKILL.md "preview-URL serving reference" Laws 1-5.
