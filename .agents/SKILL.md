...
## #106-118: the R24-mini mini-plus round (the feature-parity port into a law-register mock)

**106. The gate-over-fork ruling for a law-register artifact:** when a mock's test corpus is an acceptance list a downstream port consumes (the mini = the crawl's LAW-REGISTER), NEVER fork to add features — the fork orphans the register + the census + the review surface. Instead ONE view-state gate, default ON, **additive-by-construction**: every plus affordance MINTS new DOM with new testids; nothing existing is superseded, re-homed, or re-toggled. Then gate-OFF is the old surface BY CONSTRUCTION (net it per feature — the claim is tested, not asserted), and the legacy corpus is green under BOTH defaults with no setup seam.

**107. The additive law's text-collision trap:** getByText queries collide with added DOM even when testids don't (a facts-dl "Duration" vs a new field's label). Fix the NET with a selector pin (`getByText('Duration', { selector: 'dt' })`) — the net's law is unchanged; renaming the label to dodge a test hurts the UX.

**108. The REAL critical path in a whole-doc-snapshot store is the commit trio, not the model:** adding nested optional fields (effects/transitionOut) is trivial UNTIL you check (a) the draft clone is DEEP (a shallow {...c} shares nested refs into history — undo corruption: a nested edit writes through into every entry), (b) docChanged compares the FULL superset (deep for nested — or every plus-edit silently no-ops), (c) the preview paths never mutate nested in place. Write the no-alias net (the live doc's nested objects !== the past entry's) BEFORE the first nested action exists.

**109. The absent-is-default write law keeps docs minimal:** for optional fields whose absence IS the legacy semantic, writing the default value DELETES the field (volume=1, opacity=1, solo=false, sourceStart=0) — seeds stay byte-identical through edit cycles, resets are idempotent, no phantom history for no-semantic-change writes. Same law for shared state: a tracks array shared between draft and live doc MUST be mapped to new objects on flip (an in-place t.solo = true aliases — docChanged sees nothing).

**110. The field-primitive contract that survives review:** ONE commit per settle on Enter/blur (a 50ms debounce is droppable — fewer timers, same guarantee; register the drop); invalid input = red + inline message + focus retained + NOTHING dispatched; blur-invalid reverts the display; Escape reverts; double-click resets through the SAME commit path; **the settle rewrites the display to the committed SNAPPED value** (else the field disagrees with the doc indefinitely — the resync effect is focus-gated) and the snapped value re-clamps to the field's own bounds (an off-grid max can snap past it).

**111. The transition-on-left-clip model needs an invariant sanitizer:** if a seam transition rides the LEFT clip's tail, every placement-changing op (move/trim/delete/split/insert) can orphan it. ONE place inside commit() re-validates: no touching right neighbor → delete; duration past the new seam bound → re-clamp. Every render/editor path can then trust the doc — three doors (UI guard, zone guard, store guard) agree only if the store is one of them.

**112. The wedge/hit-area law for overlays over gesture surfaces:** a lane-level overlay (transition wedges, fades) must be a VISUAL (pointer-events:none) with only a small centered interactive button — a full-width interactive box over the seam steals the trim/drag hit zones of the frozen gesture surface underneath (the behavior nets can't see it; only review can).

**113. The tool-dispatch seam for a frozen gesture engine:** read the tool ONCE at gesture start, remap the effective kind (roll rides the trim handles, slip/slide the body); the select-family dispatch stays byte-identical (the freeze's diff gate). The previews for new tools follow the engine's own shape: from-snapshot idempotence (recompute each event, never accumulate), live-doc mutation, ONE seal entry at pointerup.

**114. Off-grid edges break grid-clean assumptions everywhere downstream:** if raw-pointer trims commit off-grid times by design, every later split/insert math that assumes 0.5-clean inputs can mint sub-MIN halves. Guard at the split (clamp the cut into [start+MIN, end−MIN]) AND at the straddle branches (drop degenerate remainders — the reference's own law); pass CONSISTENT values from the caller (a clamped cut with an un-clamped rightDur is still broken).

**115. The delegate-implement/review-verify split works for a well-specced wave:** a full-stack sub-agent with the design section + the law patterns + the net templates delivered a 942-line wave at zero regressions (491/491) — but the adversarial review found 3 real P2s it missed (the ported-guard omission, the gate-off zombie session, the dishonest still-marks). ALWAYS review sub-agent code with a fresh-context agent before sealing; fold the P1/P2s yourself.

**116. The count-re-pin discipline per wave (declared == actual at EVERY push):** land the wave, sweep the count claims ONCE, push. The battery's census-math + scrape checks caught 4 of the orchestrator's own arithmetic slips in one round (a +19-vs-+13 miscount, a 147-vs-141 units slip, a GAP-tests sum, a comparison literal left stale) — the automated checks are the truth, not the mental math. Sweep with word-boundary regex (\b441\b — "44100" sample rates lurk) + verify the battery's comparison literals (not just the message strings) changed.

**117. The touching-chain fixture trap:** back-to-back clips have ZERO move slack (the clamp law parks at the neighbor) — a gate-off "classic move still works" net written against a touching chain fails honestly. Test fixtures must model the surface the law needs (gapped docs for move slack; touching docs for seams; off-grid docs for MIN guards).

**118. The gate-off handoff for multi-surface modes:** if a feature spans surfaces (source mode = viewer + keyboard + entry points), turning the gate OFF must not strand the mode (dead advertised keys = the silent-dead-control anti-pattern). The clean handoff: the gate toggle exits the mode (view-state reset), and the keyboard branches gate on miniPlus so the hints never lie.

## R27 additions (the final-tightness round — 2026-09-14)

146. **The sibling-race cadence law.** When the sibling streams are mid-round (the mini R24-2 / the variants R25 both landed waves WHILE our fleet ran), expect a push race every ~30-60 min: NEVER rebase local, ALWAYS fetch+merge+push; the merge conflicts concentrate in the fleet-row/count paragraphs of 00/05/12/13/16/17 — resolve by keeping BOTH (our census text as base + their newer count figures folded in; their per-wave count re-keys are their own maintenance duty — we do not re-derive their numbers, we take theirs on merge). A resolution script must slice PAST the closing `>>>>>>>` marker line (the `text[e:]` bug left 9 marker lines in the tree — grep for `^>>>>>>>` before committing any script-resolved merge).
147. **The dispatch-saturation pattern (SKILL #121's big brother).** Six full waves of 5-7 agents ran clean; the STALLS concentrated in LONG-SINGLE-AGENT amendment waves (4 of 6 stalled after completing 40-90% of their files, silently — no report, no worklog). The mitigation that worked: (a) split the remaining scope into SMALLER agents (2-3 files each), (b) watch the tree (`git status -s`) not the tool's return — a stalled agent's edits are usually correct up to its death point, (c) verify the partial work yourself (grep the expected tokens) before re-dispatching the remainder — never redo the landed files, (d) the report file (amend-X.md) is the LAST thing a stalling agent writes — its absence ≠ failure.
148. **The battery-check calibration class.** Two of the 13 first-run battery fails were CHECK bugs, not corpus bugs: (a) a case mismatch ("WIDEN-via-projection" vs the register's "WIDEN-VIA-PROJECTION"); (b) a backtick-blind regex (`performInsertEdit :5630` never matches "`performInsertEdit` :5630" — markdown-quoted code spans put the backtick between the name and the line colon). Rule: when a battery check fails on content you BELIEVE landed, grep the exact bytes first — the corpus's quoting conventions (backticks, em-dashes, ≡ vs =) defeat naive substring matching; prefer `re.search` with optional backticks, or read the check's own slicing bounds (the B-6 matrix slice nearly excluded the table it checked).
149. **The estimate-free plan law (D40).** A phase ends when its exit gate is green — never on a calendar. When auditing a plan for executability: grep for the duration vocabulary FIRST (wk\b|week\b|solo|two-dev|month), then run the cold-executor test (pick 5 leaf rows at random; can a fresh agent start each from the doc alone? owner + entry-point-path + machine-checkable gate + explicit dependencies). The two highest-value structural facts: the parallel carve must count its own streams correctly (the 8-stream arithmetic bug), and every OPEN decision needs a phase home or it silently blocks a rung.
150. **The "no man's land" verdict pattern.** When the user asks "does X have more?" about a reference lineage: the answer lived in OUR OWN spec's reference quotes (FreeCut's trim family preserved in 06 §9's Q1-Q15 — /tmp/freecut is GONE, the quotes ARE the reference). Enumerate the leverage map BEFORE proposing greenfield work: 8 of 10 advanced modes had existing tested algorithms (the engine's class methods + the variants' gesture layer); only replace was truly greenfield. The build-vs-reuse table (mode × OT seam × FreeCut reference × engine method × UI surface × risk × order) is the deliverable that de-risks the port program.

## Round 25 (variants) — entries #146-#148 (the reaction + audit-fleet round)

**#146 — The claimed-but-never-landed class (recovery commits must VERIFY, not trust).** A wave agent that dies mid-flight can leave a tree whose commit-message-to-be CLAIMS items the diff doesn't contain (R25-F1 claimed the X1 fades fix; the agent died before editing Inspector.tsx; the recovery commit inherited the claim and pushed it as fact). A fresh-context auditor re-filed the exact finding as a P1 two batches later — the only reason it was caught. The law: when folding a dead agent's tree, open the diff and verify EVERY item the message will claim (grep the marker the fix should have left) before writing the commit; a recovery commit is an audit, not a relay. Symmetric law for auditors: "the fix never landed" is a finding class worth its own severity — claims are part of the surface under audit.

**#147 — Max-turns deaths leave ~95%-complete trees; the finish is orchestrator work.** Three of four fix-wave agents (F2/F3/F4) hit the 200-turn ceiling with the implementation + tests written and the suite green, dying between their final vitest run and the commit. The recovery is mechanical: run tsc + the full suite + build on the frozen tree, fix the agent's own failing pins (format typos, stale element picks, wrong structure assumptions — the F3 set: '0.0s' vs '0.0 s', followerless picks under a NEW adjacency law, a pin querying the wrong DOM level), and commit with the recovery provenance. The pins an agent writes for its OWN unfinished work are the map of what it didn't finish — read the failures as a TODO list. ALSO: dispatch-time context deadlines are cosmetic (4/6 waves "failed" dispatch while running to ~95%) — the SKILL #121 law held every time; poll the tree, not the tool result.

**#148 — The reaction-round fleet shape (and its scope law).** When a round's driver is reviewer feedback + "deep deep UX audit with 50+ sub-agent rounds": (a) the orchestrator reads the reference artifacts + pulls ALL threads BEFORE any dispatch (SKILL #130 held); (b) research agents FIRST for the reviewer's "research how X works in Resolve" asks — their verdicts become binding design-doc sections BEFORE implementation waves; (c) implementation waves one-area-each with commit→push→bundle per wave (#143 held across 11 waves + 3 sibling races, all absorbed by fetch-before-push, zero force-pushes); (d) the audit fleet in batches of 2-3 with TIGHT scopes (~60 tool calls, ≤10 findings) — the two full-scope auditors died at max-turns without returning, every tight-scoped one returned; (e) fix waves per batch (P1/P2 first, P3 family second); (f) the exit gate is TWO fresh verification agents (a fix-class probe pass + a console/story sweep) whose verdict line IS the round's bar — "iterate till ≤P3" is measured by the verification pass, not by the fix count. The VLM net sweeps in the background against the SERVED runtime (parallel-safe), rate-limits at ~1 call/s (429s are resumable — the append-style manifest retries), and its P1s are triaged against the KNOWN artifact classes (clipped story captions, isolated-component voids) before becoming findings.

## R28 additions (the seal round + the testability pass — 2026-09-16)

151. **The same-round test-law law.** A design-law fold that touches zero test-side files is a CONVICTION waiting to happen: R28's W4 amended 13 law-side files while 12/17 (the test specs) sat untouched — the coverage audit's line was "the corpus's test law is one full round behind its design law." The law: **every ruling wave ships its test-law wave in the SAME round** (matrix rows + facet rows + the invariants + the battery classes), and the plan carries a carve that makes the lag a machine-checked spec bug ("a stream row whose gates column cites no test law"). When the user says "make sure everything we build has a rigorous test plan," they mean: the test law is a FIRST-CLASS corpus artifact, not a follow-up round.

152. **The demonstrated-practice extraction pattern.** When asked to "check how the existing modules test themselves," the deliverable is not a summary — it's a PATTERN LIBRARY (named patterns, each with a canonical file:line cite and the class of feature it's FOR: the hand-derived-oracle anti-tautology, the table-loop, the no-commit trio, the drift fence, the M49C four-layer census gate, the completeness accumulator, the DOM-attribute rendering contract, the null-test ≥60 dB, the VLM two-tier net). The library then gets INSTANTIATED per ruled area (~530 acceptance pins from 24 patterns). The scouts must RUN the suites (static grep said 665 where the runner says 749 — dynamic `for (const row of rows) it(...)` registration hides 84 tests; suite-count batteries must use the runner, never grep).

153. **The carve-law enforcement lesson.** A law without a machine check convicts its own table: the plan's new carve law ("every track row's gates column cites a test law") landed while K4's own exit gate cited none — the law's first enforcement casualty was its own row, and nothing would have caught it because no battery class scraped the plan. Every new corpus law needs its enforcement leg DESIGNED IN (the plan-scrape class), or it's prose.

154. **The server-shutdown recovery pattern (SKILL #121/#147's fullest form).** Six of 42 R28 dispatches died at the deadline AFTER completing their edits — every one recovered by the tree-watch protocol (git status → diff verification → spot-checks → commit as-is → the orchestrator appends the dead agent's worklog record). The failure mode is now so reliable it's a phase of the workflow: expect N-1 reporting on N-agent waves; never re-dispatch before reading the tree; the worklog record for an unreporting agent is the ORCHESTRATOR's job (labeled as such).

155. **The same-commit anchor drift.** Parallel amendment agents editing the same corpus in one working tree drift each other's line anchors MID-ROUND: a review citing "15:5021" went stale by +14 lines because the prereq-sweep agent's insertions in the same file landed between the review's read and the fold's landing. The law: **every corpus cite is re-verified with grep at the moment of use** (the fold agents' first instruction), and battery checks verify target text live before encoding — never trust a line number that crossed a commit boundary, even within the same round.

156. **The ruled-with-default conversion (the user-gate retirement pattern).** When the user says "I can't answer that — figure out the process yourself," the answer is NOT to guess silently: it's the full pipeline (research packs → a drafted ruling with rationale → fresh-context adversarial review → the meta-lane ruling for the review-referred residues → the corpus fold), then CONVERT every "user-gated" mention in the corpus to "ruled (default) + reversal-registered." The reversal surface is registered (the six conversion sites), so a later user override is a one-line amendment, never a re-open. This retired every gate in the plan — the register's OPEN design table went to 0 rows, machine-checked.

## R29 additions (the execution round — the parallel-fleet re-base — 2026-09-16)

157. **The sibling-race absorption pattern.** A push rejected mid-session means a parallel session landed the same duty MINUTES earlier — the work is not lost and must not be duplicated: (1) fetch + read their landing's commit messages as the acceptance audit, (2) reset local to their tip (the disk of record — after proving your unpushed work is either subsumed or salvageable), (3) audit theirs against the FILED acceptance (this round: their pool test covered the mixed-kind split but the batches were single-member — the M49R pair's homogeneous half was missing), (4) land ONLY the gap-filling delta on top, push. The 40-minute overlap cost one absorbed commit + one completion pin, not a war.

158. **The submodule PAT law.** `git config url.insteadOf` does NOT apply to `git submodule update --init` clone URLs (the rewrite is consulted for plain fetches/ls-remote but the submodule clone path bypasses it — probe-verified). The working pattern: `git config submodule.<path>.url "https://x-access-token:${PAT}@github.com/<org>/<repo>.git"` per submodule (local `.git/config` only, never committed), then init. Symptom to recognize instantly: "could not read Username for 'https://github.com': No such device or address" while plain `git ls-remote` works.

159. **The dispatch-granularity law for huge-file work.** When parallel agents are assigned corpus files of 150-260KB with very long lines, split BY FILE, not by topic: the agents given 3+ heavy files timed out (twice), the single-file agents returned complete. The failure is TIME (reading + verifying long lines is slow), not context — so the fix is fewer files per agent, not shorter briefs. And when the timeout DOES happen, the saturation protocol applies unchanged: 2 of 5 "failed" agents had landed their full work in the tree; one had even written its worklog record — only the final report message died.

160. **The parallel-fleet re-base cadence.** When several repos move in one window and the battery has LIVE repo checks: fast-forward ALL local clones BEFORE the first battery run (the battery reads local HEADs — stale clones produce misleading partial-green), and classify each move's NATURE before re-keying the corpus (HEAD vs code-anchor vs docs-only: engine e3f55bd = chore-only over the unmoved anchor 74bef08; OT 3e18722 = docs-only over the code pin 970948a; nle-ui 6754979 = CODE — the count moved 690→691). The nature determines which corpus statements move (only the HEAD cites) and which stay (every code-anchor cite). The register may be PIN-FREE (the fleet rows moved out at R28) — verify with a survey before dispatching agents to it.

## Round 26 (variants) — entries #149-#154 (the full-issue verification sweep + the stale-serve incident round)

**#149 — The stale-serve incident class (threads-current, code-stale).** A self-healing DATA store layered over a non-synced CODE tree produces the most confusing failure state possible: the reviewer's threads (synced from the GH orphan branch) look alive and current while the served UI is rounds behind — every "this is NOT fixed" complaint gets VALIDATED by their own eyes against old code, and the orchestrator's "verified live" claims were true when made and false at review time. The chain of causes: an unclean recycle resurrected an R18b-era `.zscripts/dev.sh` (frozen in repo.tar at the last clean pre-stop) that predated the checkout guard + GitHub FF + code-sync entirely. The durable fixes: (a) `.zscripts/dev.sh` is a STUB delegating to `boot-restore.sh`; (b) boot-restore step 1b-2 SELF-HEALS the .zscripts copies from the repo's GitHub-versioned copy (the single source of truth) and re-execs the healed logic; (c) the session-start ritual now verifies the STAMP (`.code-sync-stamp` == repo HEAD) + the public story count — not just that the URL answers (SKILL #122's gate was checking a healthy daemon on a stale tree).

**#150 — The silent-resolution law (the reply-first protocol).** A thread resolved without a visible evidence reply is NOT resolved from the reviewer's seat — it reads as a silent closure, and "never responded either" is the exact complaint that follows. Two compounding causes found: (a) the R24/R25 batch-resolve scripts passed a `comment` field in the PATCH — this annotakit version SILENTLY IGNORES it (only `POST /threads/:id/comments` lands a reply; the PATCH takes status only); (b) the evidence lived in worklogs/commit messages the reviewer cannot see. The law: reply first (POST a comment with what landed + how to verify in 10 seconds), then set status. When reusing historical resolve scripts' note payloads, they are a gold mine — the notes were WRITTEN but never delivered; re-posting them via the working endpoint discharges years of debt in one wave.

**#151 — The display-layer escape-eating artifact (the false corruption).** The tool-output renderer can EAT bracket+letter sequences from displayed text: `const [mockT` displays as `const ockT`, `sourceRanges[hover.mediaId]` as `sourceRangesover.mediaId` (an ANSI-escape-interpretation class — `[m` and `[h` vanish). Every Read/grep/cat path showed the same mangled line while tsc/tests/build/the live app were ALL green. The law: a "corruption" finding that contradicts the gates gets a BYTE-LEVEL dump (`od -c` / `xxd`) before it becomes a finding — never debug the display layer from rendered text. (Corollary: `$?` after a `| head` pipe is head's exit code — capture the pipeline's real exit or you'll trust a bogus green.)

**#152 — The getAllByRole-on-zero trap.** `screen.getAllByRole('button')` THROWS when zero elements match; `queryAllByRole` returns `[]`. Tests asserting an honestly-EMPTY surface (a title-only toolbar, a DOM-absent toggle family) must use queryAll — the throw reads as "button missing" and sends the fixer chasing a working implementation. Symmetric law for new pins: a test that asserts absence-by-count is a queryAll test.

**#153 — The polish-vs-pinned-law conflict.** When an audit's polish suggestion contradicts a PINNED law, the law wins — even when the polish is genuinely nicer. R26's fade-out-on-disarm (a GC-audit P3) violated the pinned D-E2 absence law ("the display:none law: absence, never an opacity stub") — worse, it painted STALE geometry during ok:false refusals (the exact contract the pin protects). Landed, caught by the old pins failing, REVERTED with the law's rationale recorded at both the code site and the re-pinned test. The meta-rule for fix-wave agents: before landing any "polish" item, grep the pin corpus for the affected surface's laws — the pins are the contract, the audit's suggestion is not.

**#154 — The cap-dominance verification trap.** Verifying a range/clamp fix live requires a test case where the INPUT actually dominates — a 30s placement cap masked the sourceRange fix for a 62s source (the first live probe read dur=30 and looked like "not working" while the fix was correct; the deep-crop case to ~26s proved it). When a live verification "fails", enumerate the OTHER clamps in the path (caps, floors, min-widths, zoom-floors) before concluding the fix is broken — and re-probe with an input that isolates the changed term.

**#161 — The map-before-corpus law (the K3 pattern).** Re-expressing a mock's law corpus onto a real app is a TWO-STAGE process, never one: (1) the MAP (every law family → the app's real target surface, grep-verified, with an explicit disposition vocabulary: RE-EXPRESS / SUPERSEDED-with-citation / N-A-DIFFERENT-with-the-registered-deviation / DEFERRED-with-phase — the maps are durable work orders in the app repo's docs/); (2) the AUTHORING waves (each agent owns CONFLICT-FREE files — new test files preferred; extending shared files risks cross-agent clobber). The maps caught what no direct authoring could: ~50 N-A rows where porting the mock's law would have pinned a DEAD or WRONG law (the 0.5s floors that don't exist, the 12px snap that is 10px, the insert planner with no successor). Never author a re-expression without the map; never map without grepping the cited surface symbols.

**#162 — The jsdom drag-math trap family (the zero-rect world).** In jsdom, EVERY getBoundingClientRect is zero — this breaks gesture tests in four specific ways, each with its own fix: (a) the clickOffsetTime skew — the element's "grab offset" reads as the viewport origin, so small-delta drags compute nonsense absolute positions (FIX: stub the hit node's rect to the element's true content-space left edge — the same class as the scroller-rect stub); (b) the collapsed render window — only elements near time 0 render (el-1/el-2/el-6 in the boot fixture; FIX: pick a mover that renders); (c) the packed-fixture trap — a fully tail-adjacent track has NO free small same-track move, and the honest app REJECTS or DIVERTS TO A MINTED TRACK where the mock CLAMPED (FIX: find the one element alone on its track, or drag to the free tail); (d) the boot-time prop freeze — a store toggle + settle does NOT reliably reach a prop the component reads via getState() at render (FIX: pass the flag in the mount patch — the patch boots the store BEFORE the first render). Each trap was a "failing test the agent couldn't finish" — the diagnosis (probe the wireLog + the verb params, read the controller's resolution code) beats re-running the test.

**#163 — The selection-snapshot law (React-state lag IS the semantics).** A metaKey-mousedown that both selects-and-drags captures the drag group from the PRE-CLICK selection state (setState is queued; isSelected reads the committed render). The drag therefore carries the clicked element ALONE — the additive intent does not ride the gesture it opened. This is not a bug to fix but a law to pin: the two-gesture flow (cmd-CLICK to add, release, THEN plain-mousedown to drag) is the only way to drag a multi-selection. Test authors who don't know this write "the mixed group commits nothing" and are wrong — the probe shows a single-member commit with a minted track.

**#164 — The two-surface reject law (dispatch vs wire).** The same verb can have DIFFERENT failure semantics on its two surfaces: `core.moveElements` (the dispatch path) commits overlapping moves verbatim and clamps negatives to 0, while the WIRE (`validateWireMoves`) rejects them with CONFLICT/INVALID_PARAMS. A map or test that teaches "assert dispatch returns false" is WRONG — pin both surfaces as they actually behave. The general law: before asserting a reject, probe WHICH surface the code path actually validates on.

**#165 — The dead-entry defect class (withHistory no-pollution).** Any store verb whose lookup can MISS (an unknown fx id, a stale element id) must guard BEFORE the history wrapper routes it — otherwise the miss mints a DEAD undo entry (a snapshot of an unchanged doc; the user's ⌘Z does nothing). The class: setEffectParam/toggleEffect/removeEffect all had it (toggleEffect even on the ELEMENT miss). The pin: `past.length` UNCHANGED + the doc byte-identical on the miss + a positive control so the guard can't over-reach. Audit pattern: grep every `withHistory(...)` call for a lookup whose miss path still returns the doc.

## R31 additions (the DOM-structural tranche round — 2026-09-17)

166. **The two-listener enumeration law (any keyboard ruling counts its surfaces).** A glued app that mounts a view-layer keymap (document-capture) inside a shell that owns its own keymap (window listener) has TWO independent keyboard surfaces — a ruling that gates, yields, or rebinds keys must enumerate BOTH before claiming completeness. D51's drafted ruling claimed "the keymap gate IS the complete user-reachable lock" and the adversarial review probe-broke it in minutes: ⌘Z mid-drag rewound the doc under a live gesture through the SHELL listener, wire-invisible to the port's log. The fix pattern: land the gate in the surface you own, REGISTER the residual with the probe evidence, and file the twin as an explicit follow-up decision (the D52 candidate) — never silently defer and never overclaim.

167. **The rect-free gesture family (read what the controller consumes before stubbing).** The corpus's jsdom zero-rect trap family made rect-stubbing reflexive — but the trim gesture math turned out to consume NO rects at all: the ResizeController derives its delta purely from `clientX − session.startX` over `BASE_PPS × zoomLevel`. The discipline: when a gesture probe fails, first READ the controller's coordinate sources (grep the mousedown handler's reads), THEN stub only what it actually consumes. Blind rect-stubbing on a rect-free family adds fragile noise and can mask the real geometry. Symmetric law: probe the wireLog's verb params to learn what time-delta the controller computed — the params are the ground truth of which input is skewed.

168. **The unselected-side-at-boot law (the window-collapse corollary).** jsdom's zero viewport re-measures can UNMOUNT deep elements after any interaction (el-2 vanishes post-click). A selection-gated emission law ("unselected renders zero handles") must pin its unselected side at BOOT (the pre-interaction render, where the fixture's full window still shows), not post-interaction — the negative assert's subject may no longer be in the DOM, and the querySelector will throw for the wrong reason.

169. **The row-aware re-key scanner (layered appends outgrow fixed windows).** The corpus's layered-append convention produces fleet rows >3KB where the historical pin and the new R-layer sit hundreds of chars apart on the SAME row. A stale-pin scanner with a fixed ±260-char "new pin nearby" filter false-positives on exactly these rows. The fix: the new-pin filter must check the WHOLE logical row (find the enclosing newline bounds), not a character window — the scanner's false-positive class is itself a pin-able law (two residual "LIVE-stale" hits that were both 2-3KB rows carrying their R31 notes at the far end).

170. **The stale-premise filing check (cross-repo gap rows age in both directions).** When a spec row registers a cross-repo filing gap ("ledger ends at D28", "no C0 row in the package PLAN") for multiple rounds, VERIFY the target repo's live state before filing — parallel rounds may have half-closed it (R28-exec had landed the C0 row's chrome-family half and a D29 instruments flavor; the spec's stamps were two rounds stale). The filing then AMENDS the existing entries rather than duplicating, and the spec's stale stamps get their own re-key closure notes. Rule: a registered gap's verification stamp has a shelf life of exactly one round — re-verify before acting on it.

## R32 additions (the K4-exit + w1-entry + D52 round — 2026-09-19)

171. **The push-every-gate-pass law (the user-intervention incident).** A mid-flight agent death left Part 2 of the D52 implementation as 5 uncommitted files — hours of real work invisible on every remote, and the user (watching GitHub) reasonably concluded the process was dead and paused the session. The recovery was clean ONLY because the sandbox tree survived; had the sandbox been recycled, the work would have been lost with no remote copy. The law: **a green gate pass is the commit unit** — a test file going green, a suite pass, a docs landing, a vendor re-pin each get their own commit+push immediately; never batch a wave's commits to its end. And when the user reports "no updates": audit `git fetch && git log origin/main..HEAD` per repo BEFORE theorizing — this round the pushes HAD landed through 05:46 UTC; the gap was exactly the dead agent's uncommitted tree, not auth. Also: the local-only sync-bot repo (no remote) going quiet is NOT a push failure — check which repos actually carry remotes.

172. **The wire-invisible shell-rows law (two dispatch worlds, one wire log).** In the glued app the PORT keymap commits through the wire protocol (verbs land in `wireLog` — `logOf('timeline.split')` works), but the SHELL's useShortcuts rows call STORE actions that route through the router into the engine core DIRECTLY (`s.splitElement` → `r.dispatch({type:'split'})` → core, no wire entry — the D28 routed-world design). Pinning a shell row with a `logOf` assertion produces a structurally false test (expected 1, received 0 forever). The discriminator: assert ENGINE truth (element count, historyDepth, span values) for any router-dispatched verb; reserve `logOf` for the port keymap's wire verbs (the move/drag mouseup commit IS wired — know which surface you are pinning).

173. **The packed-seed collision law (zero-gap tracks force new-track landings).** The mock seed's main track abuts el-1..4 across [0,30) with zero gaps, so ANY real move of el-1 collides with a neighbor and the OT planner's `canApplyMoves→newTracksFallback` lawfully creates a NEW video track (`track-000001`, index 1) and lands the element there — `getTracks().main` drops from 4 to 3 mid-test. A same-track move test must either drag to a span that clears every other element (the R31 +32s shape worked only because [32,40.5) cleared el-4) or pin the new-track landing as the honest law. Symmetric trap: the playhead clamps to the scene duration (30s), so a post-move span beyond the duration ([32,40.5)) is UNREACHABLE by the playhead — trim/marker revival legs must target spans inside the playable window.

174. **The store-scene boot-staleness law (the routed world never mirrors back).** Once the router is attached, store actions dispatch to the engine and the STORE's scene data stays at its BOOT values forever (the engine is the SSOT; no back-mirror exists). Shell-row targeting therefore reads boot-stable spans: ⌘B finds the element whose STORE span contains the playhead (el-1's store span [0,8.5) even after the engine moved it to [32,40.5)); ] targets the STORE selection. Consequences for pin authorship: (a) mid-drag premises can ride the dragged element's BOOT span (the engine doc still holds the pre-move position until mouseup); (b) post-op revival legs must ride UNMOVED elements (store span = engine span) or seek into the moved element's post-move span for selection-based rows; (c) the split dispatch carries `atSec` explicitly (the store's playhead) while the trim dispatch carries NO time (the ENGINE's playhead) — two different playhead truths, know which one your row reads.

175. **The drag retargets the shell store's selection at MOUSEDOWN.** The port-world drag (ElementInteractionController) selects the dragged element in the PORT's selection module AND that retarget syncs into the nle-ui store's selection immediately (probe: mid-drag `store().selection` = ["el-1"], not just post-commit). Selection-based shell rows (] trim, , slip) therefore target the DRAGGED element mid-gesture — the mini's exact hazard shape — and keep targeting it after the mouseup (the W9 E2 journal's retarget). Pin authors cannot assume the boot selection [el-2] survives a drag.

176. **The runtime setPlayhead does sync the engine playhead (with the duration clamp).** The boot UiPatch seeds both playheads (store 12 = engine 12), and a runtime `store().setPlayhead(x)` follows through the bridge — BUT clamps to the scene duration (setPlayhead(35) lands at 30). The routed trim reads the ENGINE's playhead (tick truth) while the routed split/marker read the STORE's playhead (explicit atSec) — when a pin mixes both row families, assert the seek took effect on the truth the row actually reads.

177. **The marker one-per-tick dedupe (engine law, silent at the pin level).** `addMarker` at a tick that already carries a marker returns false — no bookmark, no history entry, no toast. The seed ships mk-1 at tick 0, so an `m` control at the default playhead 0 lands NOTHING (expected 5, received 4 with zero error signal). Marker controls in pins must seek a marker-free tick first; when a marker assert fails by exactly one, check the dedupe before the dispatch path.

## R33 additions (the intake fold round — 2026-09-21)

178. **The re-orient-first law (a "done" corpus can be invalidated between sessions).** Before ANY round: fetch all sibling repos, read the newest INCOMING FILING blocks in PLAN + the audits/ register, and run the current battery. R32's wrap said "next: the human rounds" — but between sessions the engine seal-round and the OT seal18 both LANDED and FILED (the 87-row register + the wire-reality notes), and the battery had silently gone 197/202 on pin-lag. The visible "nothing left but user reviews" state was an artifact of reading a stale HANDOFF against moved repos. The HANDOFF describes the PLAN at write-time; the repos describe the present.

179. **The intake-fold shape (absorbing a filed register — the reusable 5-wave pipeline).** (1) SCANNER FIRST: parse the register, extract every file:line cite, read the CURRENT corpus at each cite, emit an evidence pack (audits/r33-intake/SCAN-PACK.md + rows.json) — never apply rows against assumed line numbers. (2) File-disjoint sub-agent batches: each gets the row text verbatim + the scan-pack + the current pin world + the edit laws + the discharge law; disjoint files = safe parallelism with zero merge conflicts. (3) The orchestrator applies the P1s personally (materially-false-claim edits are too delicate to delegate). (4) The re-key scanner (the K1 layered convention) + the battery fork. (5) The adversarial gate as TWO fresh-context agents — one audits the corpus fidelity, one audits the battery/scanner quality; fold the amendments, re-gate ALL GREEN.

180. **The discharge law (a register's evidence can be stale in BOTH directions).** A filed row's premise may already be fixed at the current trees (P3-36: the engine's W0 pull_request fold landed AFTER the register was written — applying its "PRs bypass every gate" note would have introduced a NEW false claim), and its REVISION text may itself be overtaken (P2-19: "OT carries NO linkage field" was false by apply-time — seal18 landed Stage-0 between the audit and the fold; P2-21: the K4 exit sealed past the row's R31 shape). NEVER apply a row's literal sentence when the live tree contradicts it: verify the premise, shape the edit to the current truth, and record the discharge or mutation in the marker note. A register is a work order against a PINNED vintage, not against HEAD.

181. **The row-new-pin-filter laundering class (multi-repo mega-rows defeat row-scoped staleness filters).** A staleness scanner that exempts "any new pin in the same logical row" to protect layered fleet tables gets LAUNDERED by mega-rows: one 3KB §0 row that names five repos' pins can carry an old OT pin "protected" by an unrelated repo's new pin elsewhere in the same line (this round: 16:15 + 17:22's OT layers slipped past the filter because the row also carried the app's new pin). The fix: every CURRENT pin — including the UNCHANGED repos' pins (1e0c30b, 94f6460) — joins the new-pin set, AND the false-negative sweep is part of the adversarial review, not the scanner's own report. A scanner's "zero residual" claim is only as strong as its weakest pattern class.

182. **The submodule-init environment trap (restored sandboxes ship `-` submodule status).** A fresh/restored sandbox clones repos WITHOUT initializing submodules — `git submodule status` shows leading `-` and every battery LIVE check that READS a vendored file fails spuriously (this round: R30-3/R30-5 looked exactly like corpus regressions; they were environmental). Diagnose BEFORE re-keying any battery check: if a failing check reads a path under vendor/, check the submodule state first. Fix: `git config --global protocol.file.allow always` + point each submodule's URL at the LOCAL sibling repo (`git config submodule.<path>.url /home/z/my-project/<repo>`) + `git submodule update --init` — no network, no PAT, exact SHAs.

183. **The public-vs-impl cite rule (name the symbol you re-anchor to).** When line cites drift and two agents re-anchor "the same" function, they can verify DIFFERENT symbols that both contain the search string: `performOverwriteEdit(` the public method at :6039 vs `_performOverwriteEditImpl(` the private at :6055 — the substring check passes for both. A battery regex expecting one silently diverges from the corpus citing the other. Rule: re-anchor to the PUBLIC method (the API surface), record the impl's own cite as a range with its full name where the impl is the subject, and make the battery's expectation name the exact form.

184. **The dead-PAT mid-round protocol (the two-remote redundancy is the design, not a failure).** A GitHub PAT can die mid-round (this round: valid for the first two pushes, 401 from the third — "Bad credentials" at the API too). The protocol: do NOT stop work; commit locally (the tree is the buffer), push everything to the GitLab mirror (the WAF 403 is probabilistic — retry with backoff, it passes), refresh the /home/sync bundle, and record the pending re-push in the HANDOFF as a ranked item ("when the user supplies a fresh PAT: update URLs, fetch-first push, verify origin/main..HEAD empty"). Never force-push to "resolve" the divergence — the divergence is auth, not history.

## R34 additions (the seal round — 2026-09-22)

185. **The user-gated register discipline (the seal-round's core ask).** When the user says "gate only true business/product calls," build and maintain an explicit USER-GATED REGISTER (the implementation plan's head + the HANDOFF's own section): every item names its verdict options + where it's registered + what mechanism landed autonomously vs what verdict stays theirs. Everything NOT on the register is agent-actionable by definition — the register is the contract that lets you drive to completion without treating the user as the quality gate.

186. **The operational-plan law (a plan must DRIVE the tracks).** An implementation plan is "operationalized" when: every open item lives in exactly one track row with a gates column citing test law; the two biggest work orders (the next demo phase's leg inventory, the next port's stage rows) are carried VERBATIM in the plan; the ladder state is explicit (which rungs are landed records vs standing predicates vs future work); and the battery machine-checks its structure (the carve law, the taxonomy, the vocabulary). If a new session can pick up any track and execute from the plan alone, it's operational; if it needs the chat log, it isn't.

187. **The self-laundering re-key clause (the scanner-marker trap).** A staleness scanner that exempts "any clause labeled R(n) re-key" (to protect layered history) will ALSO exempt YOUR OWN mid-round re-key clauses from the END-of-round wrap re-key — a W2-era clause carrying stale pins survives labeled as current. The nets: (a) the adversarial reviewer's fresh-context read (it caught the P0 this round); (b) the r+1 pattern: a re-key clause must carry CURRENT pins regardless of its label — the label marks provenance, never freshness.

188. **The CI-run-truth law (a green workflow file ≠ a green CI).** The engine's W0-W4 waves landed a 5-job CI + a nightly workflow that had NEVER actually run green — every push since Sep 19 failed at "Materialize private submodules" (the CI_VENDOR_URL_PREFIX repo variable was never configured). The lesson: a NEW workflow's first pushes are UNVERIFIED until you read the run logs — "the workflow file exists + parses" is not "CI is green." Diagnose via the API (the jobs' failing step names), fix the credential plumbing, re-run, and only then record GREEN.

189. **The exploration-venue law (random seeds are the discovery instrument).** The nightly FC_SEED=random property pass caught P-F4 (the degenerate head-trim window inversion) on its FIRST run — after the fixed seed had passed the same path for a week. A new red nightly run is a FINDING (triage material — the venue working as designed), never a reason to pin the seed or soften the assertion. The division of labor: the random venue discovers; the deterministic suite + the regression pin hold the class once found.

190. **The wrap-rekey completeness law (the layered convention binds EVERY repo row, not just the one that moved most).** The wrap re-key appended the R34 layer to the ENGINE's canon rows but left the OT/app/nle-ui rows at their R33 layers (the reviewer's P1) — because the engine's movement felt like "the" story of the round. Rule: at the wrap, EVERY fleet row (00's bullets, 12 §0, 17 §0A, 18 §13) gets its current-round layer or an explicit "unmoved" note; the pin canon must be readable at ONE place without archaeology.

191. **The bool-tuple check-shape hazard (battery authoring).** A battery check whose lambda returns a bare bool crashes the check() helper ("cannot unpack non-iterable bool") — every check returns (ok, detail). The subtle variant: `lambda: (A and B, "detail")` is correct, but `lambda: (\n A and\n B), "detail")` parses as a bool-returning lambda plus a THIRD ARGUMENT to check() — syntactically valid, silently wrong. And boolean precedence: `A and B and C or D` binds as `(A and B and C) or D` — parenthesize alternatives. Run the battery after EVERY shape edit; a check that "passes" via an exception-path is a false green.

## R35 additions (the final seal + fleet re-home — 2026-09-22/23)

192. **The NLE-conventions gold-standard law (product calls resolve by RESEARCH, not opinion).** The owner's standing principle: "it is not about my review opinion but what DaVinci Resolve / Premiere Pro etc. do conventionally — that's your gold standard. When in doubt research; close open items that way, never by asking." The operational shape: a product/UX/workflow question that feels gated FIRST goes to a conventions-research pass (web research against the reference NLEs, sources cited, the artifact filed under audits/r35/+), and the verdict registers as the convention-conformant choice; TRUE flavor-splits (where the references genuinely differ) become app-setting/user-pref variants (prioritized: the primary flavor first, the toggles as follow-ups); an option NONE of the references offer is retired on the spot (this round: the unquantized playhead path — no NLE tweens the playhead; the only sub-frame flavors are audio-domain display variants). The standing artifact: the conventions catalog (`audits/r35/W1-b-nle-edit-conventions-catalog.md`) — extend it, never re-derive from scratch.

193. **The app-state-first law (state at app level ALWAYS; surfacing is a separate, reversible call).** The owner's general principle: "always safe to have it at app state level and then decide whether to surface or not — general rule that applies to all." The shape: a UI-state question (rows, states, modes) splits into (a) the STATE — which lands in app state unconditionally (agent-actionable, never gated, cheap to carry) and (b) the SURFACING — which is a settings-tier toggle decided independently (default surfaced unless a convention argument says otherwise). Never gate (a) on (b): the state is the safe substrate, the surface is the reversible presentation. This round's application: the programMonitor viewer-state rows (loading/error/empty at app-state level + surfaced by default + the surfacing toggle at the settings tier).

194. **The dispatch-deadline pattern (a timed-out dispatch ≠ a dead agent).** The Task tool's dispatch reports regularly hit "context deadline exceeded" while the agent WORKS anyway (~15 min of productive edits, then silence). The recovery held 6× in R35: NEVER re-dispatch on a timeout report alone — first watch the worktree (file mtimes, git status), wait out a quiet period, then audit the diff, run the gates yourself, commit the work, and reconstruct the worklog fragment with an honest "recovered mid-flight" credit. The worktree IS the record; the dispatch report is not.

195. **The worktree-parallelism law (never two implementation agents in one tree).** Two agents editing one working tree race each other's test runs (a mid-edit file breaks the other's suite) and collide on git index.lock. The pattern: `git worktree add /home/z/rNN/<repo>-<batch> -b <branch>` per implementation agent — init its submodules from the LOCAL siblings (law #182's pattern) + symlink node_modules; the agent commits its branch; the orchestrator merges (file-disjoint per the design's batch carve; the shared census register resolves in the merge with the checker as ground truth).

196. **The CI-credentials-first law (variables and secrets do not transfer with git).** A repo move that carries GitHub Actions recreates the workflow's repo-scoped VARIABLES + SECRETS on the NEW repo BEFORE the first CI-triggering push — else the first run goes red (the engine's CI_VENDOR_URL_PREFIX + the test-app's NLE_GH_PAT are the fleet's two live instances; the API path: variables via POST /actions/variables, secrets via the libsodium sealed box against the repo's public key). The corollary: read the FIRST run's logs after any move (law #188) — a green workflow file on a new home is unverified until the run reads green.

197. **The nested-IIFE vacuous-check class (the #191 extension — a check that can never fail).** The shape `check(name, lambda: ((lambda bad: (not bad, detail))([...]), "tag"))` returns `((verdict, detail), "tag")` — `ok` unpacks to a truthy TUPLE, `if not ok` can never fire, and the tag lands in `detail`. Six of seven such checks inherited from R33 hid FALSE verdicts for two rounds (the honest battery count was 6 lower than reported). The laws: NEVER nest the verdict lambda — flatten to a named function returning the tuple; after ANY battery shape edit, run a runtime probe that forces a failing case (can the check actually fail?); and the GATE-2-style fresh-context reviewer re-derives the count independently.
