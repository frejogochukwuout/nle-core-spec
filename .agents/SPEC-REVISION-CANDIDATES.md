...
| # | Mock invention / gap | Spec clause touched | Where / status |
|---|---|---|---|
| C45 | **7-mode source edit-function family** — insert/overwrite/append/placeOnTop/rippleOverwrite/replace/fitToFill mapped to OT: only `timeline.insert` exists (and it is reject-not-shift, not ripple); overwrite/replace/rippleOverwrite/fitToFill are command composites (split+delete+insert / +retime), placeOnTop is engine-only `insertElementOnNewTrack`, append is insert-at-totalDuration | 06 §5.9 (add an "edit functions" subsection enumerating the family as composites), 15 §13.15 (InsertCommand `ripple` param already listed as wire-need) | lib/insertPlan.ts (pure plan/apply split, R20-W2) + useUiStore.applyInsertPlan — mock-real, ONE undo entry per op; OT @4d56c17 verified read-only. **[R33, engine seal-round register P3-46 — the KEYBOARD-FACING HALF of this row is RETIRED, executing the retirement the R25/D34 amendment asserted (spec 16 §3.4A :263 + §0's R25 GAP row :32 — "the deviation-register entry retires"): the `,`/`.`/`⇧⌥.`/`E` + F9-F12 keymap rows are spec law, no longer a registered deviation; the 06 §5.9 composites half landed with the R25 restructure (§5.9 preamble + §5.9B-§5.9F); the 15 §13.15 wire rows ride r1 — the row's live remainder is the mock-real implementation status + the r1 port]** |
| C46 | **Source-transport insertion-mode cluster** — the 7 one-shot mode buttons live in the SOURCE-preview transport row (single-viewer adaptation of Resolve's under-source edit overlay / Premiere's source-monitor Insert/Overwrite); program monitor hosts no edit ops; NOT a radiogroup (no persistent mode — no NLE has one) | 18 §4.3 v1.2 (source-preview chrome contract) | components/shell/SourceEditBar.tsx (replaces the R19 program-monitor EditOverlay dock, removed per issue #63); kebab overflow <560px carries the APG menu keyboard law (R20-W6FIX P2-3). **[R33, engine seal-round register P3-46 — the KEYBOARD-FACING HALF of this row is RETIRED per spec 16 §3.4A (R25/D34): the C46 context gate (viewerMode === 'source' + a non-zero-duration source) is the ratified §6.2 step-3 panel context and the `,`/`.` rows are spec law, no longer a registered deviation; the 18 §4.3 chrome-contract half (the SourceEditBar surface) STAYS ACTIVE until 18 absorbs the cluster]** |
