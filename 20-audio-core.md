...
### 4.1 Layer S input — `StructuralAudioSource` (`src/lib/nle/scene-to-segments.ts:26`)
...
Two flatteners ship in-core: `opencutSceneToAudioSources` (TYPE-ONLY imports from the vendored opencut-timeline submodule — zero runtime dependency) and nle-engine's `AudioScene`-shaped producer. A spec-05-conformant host may add its own; the interface above is the contract.

### 4.2 Layer G — `MixerTrackSettings` / `MixerSceneSettings` (`src/lib/nle/mixer-track-model.ts`)
...
All pinned by `src/test/nle-audio-core-derisk.test.ts` (H0-H7, H1b) and `src/test/nle-bridge.test.ts` (C1, H8-H12):
...
2. **The bridge already IS core-owned code** (`src/lib/nle/` in web-daw-core — NLE semantics live ON the core so the engine's diff stays minimal and the bridge can use the harness). nle-engine consumes `SceneMixer` + `buildAudioSegments` for offline mixdown and `SegmentStripAdapter` + `materializeStrips` for realtime playback.
...
| Triangle conformance | F | T1 | C1/H8-H12 with opencut's real types+ops | all green (737 suite) |
...
| Retirement gate | F | T1/T2 | m23 ported expectations + null parity, then LOC asserts on AudioMixer deletion | gate green before deletion (§6.5) |
...
|---|---|
| `src/lib/nle/scene-to-segments.ts` | §4.1 S interface; §5.1-5.3 merge laws (Wave-5A semantics) |
| `src/lib/nle/mixer-track-model.ts` | §4.2 G layer; C1 round-trip; track-kind union |
| `src/lib/nle/segment-strip-adapter.ts` | playSegment + equal-power curves; strip-level pan law |
| `src/lib/nle/scene-mixer.ts` | scene materialization; canonical send/return; master |
...
| `src/test/nle-audio-core-derisk.test.ts` | H0-H7 acceptance suite |
| `src/test/nle-bridge.test.ts` | C1/H8-H12 triangle suite |
...
4. **`export/audio-mix.ts` end-state**: mux-adapter vs deletion — decided by how much of its WAV/codec probing survives as the mediabunny glue after M1.5 (C9's resolution note goes here at the seal round).
