...
| 8.8 | Mixer / Metadata panels (toolbar2 right) | Removed | Out of scope (8.4); project metadata is a Deliver-page section. **REGISTERED DIVERGENCE (R27):** the accepted BASE ships an Audio page (`Page = 'edit'|'color'|'audio'|'deliver'` — nle-ui useUiStore.ts:17) with the full mixer family (ChannelEditor/ChannelStrip/MixerDock — AppShell.tsx:308-330/:373), and the variants mock adds an FX page (5-page set); the register's mixer-family row owns the law (spec 20's M2 design round; MOCK-YIELDS-REGISTERED) — the removal ledger records the v1.0 chrome decision, not a ban on the BASE's landed surface |
