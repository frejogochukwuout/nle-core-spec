...
| **nle-ui (the productized package)** | `/home/z/my-project/nle-ui` (the private-source package; the app pins it via `vendor/nle-ui` @ `83ff8a8` — the AW1-2 absorb queued; the package HEAD is `32abd58`) | The full-shell chrome family productized: `AppShell` + the four prop slots (`timelineRegion`/`mediaDragSource`/`programMonitor`/`exportRequest`), Toolbar2/MediaPool/Viewer/Inspector/AppDock/StatusStrip/ToastRegion/CheatSheet, the mixer family (ChannelEditor — the F3 real-gain slider), Color/Deliver pages; **690 tests** (37 test files, 690/690 run live at the R27 re-pin; the R27 landings: R9-b `el.preservePitch` + FW-D `setElementFieldAll` + F4-6/F4-7 + AW1-2 the Inspector Gain ceiling [−60,+20]) + 74 Storybook stories, engine-free by law (the boundary script); the D29 seams (the saveState lifecycle, the DeliverPage `exportRequest` + the 5 type-only deliver shapes, the per-scene grade sidecar + `setGrade`, the retryJob blob-URL revoke); F3/F7 the real-gain laws + the kind-aware mute-all pin; the §10 `shell-*` testid family emitting (70 sites / 62 names, live-counted) |
| **opencut-timeline** | `src/components/timeline/` (41 files / ~9,200 LOC — **THE canonical React timeline UI tree, D25/D26**; hosts the W11 wire-dispatch layer — 28 routed + 3 exception verbs, `WIRE_COMMAND_TYPES` (api.ts:243-275) + the EXPORTED `WIRE_UI_EXCEPTIONS` (api.ts:328-335) + the M49C coverage gate) + `src/lib/timeline/` (core) + `src/app/view/` (the `/view` miniature editor shell: library panel + compositor preview + the timeline; deterministic fixtures; `window.__VIEW_TEST__` hooks; 452 in-page + 30 DragEvent/keyboard + 150 real-mouse suites — **632/632** @ `55c81c0` [code pin `970948a` — src+scripts diff empty to HEAD]) | The timeline-area internals this shell frames (spec 05 §16.5) + the gold-sample host for the mini theme (D25.4 — `/view` at mini theme once both grammar halves land) + the DOM-vocabulary source of the timeline units (`data-test=`, 68 sites — D26.5) |
