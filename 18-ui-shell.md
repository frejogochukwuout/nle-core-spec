...
| **opencut-timeline** | `src/components/timeline/` (41 files / ~9,200 LOC — **THE canonical React timeline UI tree, D25/D26**; hosts the W11 wire-dispatch layer — 28 routed + 3 exception verbs, `WIRE_COMMAND_TYPES` (api.ts:243-275) + the EXPORTED `WIRE_UI_EXCEPTIONS` (api.ts:328-335) + the M49C coverage gate) + `src/lib/timeline/` (core) + `src/app/view/` (the `/view` miniature editor shell: library panel + compositor preview + the timeline; deterministic fixtures; `window.__VIEW_TEST__` hooks; 452 in-page + 30 DragEvent/keyboard + 150 real-mouse suites — **632/632** @ `3e18722` [code pin `970948a` — src+scripts diff empty to HEAD; the seal18 W1+W2 docs-only]) [R31: HEAD `31f2764` — docs-only (the R31 queue filing), the code pin UNMOVED] [R32: HEAD `39003d3` — the code pin MOVED: the D25.3a mini-theme wave (theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins + the gold-sample captures); 649/649 = 632 classic + 17 M60] | The timeline-area internals this shell frames (spec 05 §16.5) + the gold-sample host for the mini theme (D25.4 — `/view` at mini theme once both grammar halves land) + the DOM-vocabulary source of the timeline units (`data-test=`, 68 sites — D26.5) |
