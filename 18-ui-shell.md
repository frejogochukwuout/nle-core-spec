...
| **nle-ui (the productized package)** | `/home/z/my-project/nle-ui` (the private-source package; the app pins it via `vendor/nle-ui` @ `6754979` — the AW1-2 absorb LANDED; the package HEAD is `6754979`) | The full-shell chrome family productized: `AppShell` + the four prop slots (`timelineRegion`/`mediaDragSource`/`programMonitor`/`exportRequest`), Toolbar2/MediaPool/Viewer/Inspector/AppDock/StatusStrip/ToastRegion/CheatSheet, the mixer family (ChannelEditor — the F3 real-gain slider), Color/Deliver pages; **691 tests** (37 test files, 691/691 run live at the R29 re-pin — 690/690 at R27; the R27 landings: R9-b `el.preservePitch` + FW-D `setElementFieldAll` + F4-6/F4-7 + AW1-2 the Inspector Gain ceiling [−60,+20]; the R29 landings: AW1-4 the mediaInsertAtPlayhead pool seam + ENG-1) + 74 Storybook stories, engine-free by law (the boundary script); the D29 seams (the saveState lifecycle, the DeliverPage `exportRequest` + the 5 type-only deliver shapes, the per-scene grade sidecar + `setGrade`, the retryJob blob-URL revoke); F3/F7 the real-gain laws + the kind-aware mute-all pin; the §10 `shell-*` testid family emitting (70 sites / 62 names, live-counted) |
