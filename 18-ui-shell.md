...
**BASE (accepted, pinned 2026-09-07; re-verified + re-pinned 2026-09-08 at the R24 fleet; re-verified 2026-09-09 at the R25 fleet — nle-ui `3026099` [674/674 + mini 355/355; 6 shell/keymap/docs commits, the timeline gesture surface byte-identical to R24], app `64fb0ab` [206/206, the D30 program landed], OT `fdb771c` [src ≡ `c15a629`], engine `3989506` [one docs-only commit], WDC `85b81b0` — the pins per ARCH-R25 §1; **re-pinned 2026-09-14 at the R27 fleet — nle-ui `32abd58` [690/690; R9-b preservePitch + FW-D setElementFieldAll + AW1-2 the Gain ceiling verified in-tree], app `c020b2a` [252/252, 8 suites], OT `55c81c0`/code `970948a` [632/632; the wire census 31 = 28 routed + 3 exceptions], engine `f9ac806` [748/748, docs-only HEAD over code anchor `50b91f5`], WDC `ec8fd5c` [777/777] — the pins per the R27 fleet audit; **re-pinned 2026-09-16 at the R29 fleet — nle-ui `6754979` [691/691; AW1-2 the Gain ceiling + AW1-4 the mediaInsertAtPlayhead pool seam + ENG-1], app `876f2b8` [256/256, 8 suites], OT `3e18722`/code `970948a` [632/632; docs-only seal18 W1+W2], engine `e3f55bd` [749/749, chore-only over code anchor `74bef08`], WDC `94f6460` [777/777, docs-only over `ec8fd5c`]; **re-pinned 2026-09-17 at the R30 fleet — nle-ui `5779295` [701/701; the C16 package twin + the K3 FIX-2 dead-entry class], app `8f12cf9` [377/377, 12 test files — the K3 corpus round], OT `3e18722`/code `970948a` + engine `e3f55bd` + WDC `94f6460` UNCHANGED]**):** *(R31 note: re-pinned 2026-09-17 at the R31 fleet — nle-ui `dc3644b` [701 UNCHANGED — docs-only: the D29 DECISIONS first-party record + the C0 MiniShell row filing], app `f58147c` [418/418, 21 test files — the DOM-structural tranche: all 13 RE-EXPRESS-PORT rows + the D26.5 rulings + D51 the mid-drag yield + the e2e-crawl suite begun], OT HEAD `31f2764`/code `970948a` [docs-only — the R31 queue filing: the carrier-reduction work order + the D25.3b structural-gap worklist + the data-test freeze ask] + engine `e3f55bd` + WDC `94f6460` UNCHANGED)* *(R32 note: re-pinned 2026-09-19 at the R32 fleet — nle-ui `1e0c30b` [743/743 — the C0 MiniShell composition LANDED 701→737 with 78 stories + the ConfirmProvider/MiniShell exports; the D52 twin 737→743: the optional TimelineRouter.isInteractionActive + the 9 doc-mutating row guards + the 6-pin family], app `ce7cfc3` [426/426, 22 test files — the K4 exit verdict + the move-drag LEG 2b + the doc-committing session registry + the D52 shell-twin family + the w1-entry deliverables: the human-rounds protocol, the annotakit review loop, the side-by-side fixtures, the VLM net 7/7 / 0 REAL-BUG], OT HEAD `39003d3` [the code pin MOVED off `970948a` — the D25.3a mini-theme wave: theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins; 649/649 = 632 classic + 17 M60] + engine `e3f55bd` + WDC `94f6460` UNCHANGED)* *(R33 note: the engine seal-round re-pin — nle-ui `1e0c30b` [743/743 — the R32 pin, UNCHANGED], app `7664603` [426/426 + the 22-file roof UNCHANGED — the lockstep re-pin: vendor engine→`574d8d3` + both OT mirrors→`17a19f8`; the census register re-declared 43 = 32 zero-action (6 byte + 26 mechanical) + 9 carriers + 2 hosts @ the `17a19f8` re-pin], OT HEAD `008e7f3`/code `344123a` [675/675 = 632 classic (incl. the 3-test M58R family) + 17 M60 + 11 M61 + 11 M63 + 4 M62 — 77 milestone entries, the report json the count authority; the seal18 lib-tree wave + the M58R flake fix], engine `2a0ecf4` [785/785 = 749 + 33 property sites + 3 P-F2 pins; 25→29 vitest test files; the code anchor `74bef08`→`28aa387`; the vendored OT `55c81c0`→`17a19f8` + the vendored WDC `83b8850`→`94f6460`], WDC `94f6460` [777/777] UNCHANGED)*
- The full-shell DESIGN (§3-§15): region geometry, panel inventory, gesture→command contracts, state rows, a11y floor — the design of record, validated in `ui-mock/shell-variants` (1,521+ tests — 1,542 at the R23 W-F wrap: the W-A FX surface + the W-B color wave + the W-C/D/E polish waves + the W-F deliver view; real color math, W4 real pipelines; the **VLM visual net** `scripts/vlm-capture.mjs`/`vlm-review.mjs`/`vlm-run.sh` — the walk/run design reference) and PRODUCTIZED as the `nle-ui` package (`32abd58` @ R27; consumer pin: the app's `vendor/nle-ui` @ `6754979` — the AW1-2 absorb LANDED, the vendor re-converged at the package HEAD (the R29 record; **R33 re-key: the app's vendor/nle-ui @ `1e0c30b` — the R32 D52 re-pin, UNCHANGED at R33; the package at 743/743 [the C0 MiniShell 701→737 + the D52 twin 737→743]; the 691-era figures above are the R29 layer**) — **691 tests** — 690/690 RUN LIVE at the R27 re-pin, 691 at the R29 re-pin, **701 at the R30 re-pin (the C16 twin + the K3 FIX-2)**, 37 test files, 74 stories; engine-free, boundary-script-gated; the R27 landings: **R9-b `el.preservePitch` + FW-D `setElementFieldAll` + F4-6/F4-7 the identity/kind-aware guards + AW1-2 the Inspector Gain ceiling [−60,+20]**; the 674/674 RUN LIVE at the R24 module card reading is the recorded baseline; **F3/F7 the real-gain laws** — the clip-gain slider (ChannelEditor) and the Inspector's Gain both write the true dB law over `el.volume` [display 20·log10, commit 10^(dB/20) clamped ≥ 0.001 — exactly §4.4's B2 display-side-dB law] + the kind-aware mute-all convergence pin; the §10 `shell-*` testid family emitting — 70 sites / 62 distinct names [55 static + 7 templated families] across 22 non-test component files, live-counted; **the four AppShell prop seams** — `timelineRegion`/`mediaDragSource`/`programMonitor`/`exportRequest` (each absent = the honest mock world verbatim); **the D29 seam waves (R8/D29 — W2.3/W2.4/W2.5 + R8-REV #1)**: **W2.3** — the store's `saveState` `'idle'|'dirty'|'saving'|'saved'` + `setSaveState` (view-state, plain set, NEVER a history entry; the `'idle'` default IS the mock-world discriminator — the package never writes non-idle) driving the ⌘S real-dirty gate (`dirty`/`saving` = unsaved; the idle-world fallback `past.length>0 || saveAttempt>0`), the beforeunload guard (arms on dirty/saving), and StatusStrip's dual-world machine (service world = the "Unsaved changes" chip + the real Saved stamp; idle keeps the 600ms theater + the simulateSaveFail drill verbatim); **W2.4** — the DeliverPage `exportRequest` engine-free contract (4 shape types — `DeliverSettings`/`NleRenderProgressLike`/`DeliverEffectiveSettings`/`DeliverExportResult` — + the request fn; 5 TYPE-ONLY barrel exports; present-world job rows queued→running[progress+stage]→done/cancelled[AbortError distinct]/failed + the warnings badge + Reveal[the blob URL] + Retry[the same settings in place]; absent = the honest mock world verbatim; fcpxml stays mock in BOTH worlds); **W2.5** — the per-scene color-grade sidecar `color: Record<sceneId, SceneGrade>` + `setGrade` (the neutral-seeded partial-merge law, a new record ref per write — view state, never in SceneJSON) + the ColorPage Contrast/Saturation sliders LIVE store-driven (Pivot + Qualifier-hue honest-LOCAL; brightness/hue contract-complete but slider-less v1; LUT/wheels stay mock); **R8-REV #1** — `retryJob` revokes the previous run's blob URL before clearing it (tens of MB per 1080p master per retry); + the OT S-round queue LANDED (P-widen — the router `patch` member carries `transitionOut` [+ `speed`, the D28/T-round widening]; P-lock-route — `toggleTrackCmd('locked')` routes via `toggleTrackLock`, the engine lock the SSOT; the package's own PLAN rows are landed-but-unticked filing debt).
- The mini DESIGN + law register (§16, `ui-mock/shell-mini` @ 495 tests — 333 at the R22 retirement, +10 review-loop nets, +12 R23 seal + fix-round nets, +2 R24 clip-box nets, +20 W0 + +19 W1 + +19 W2 + +13 W2-fix + +19 W3 mini-plus nets; the R23 seal artifacts: `docs/CORE-SEAMS.md` [the 26-seam whole-surface audit — §E S22-S26 the chrome/view seams, the 5-class store partition, the C0-C4 transport map, the audited-absent register] + `docs/LAW-NET-INVENTORY.md` [150 census units / 495 tests — 33 HOLDS / 412 authored; Part B the chrome-side corpus: App 60 / MediaPool 19 / timecode 8 / waveform 7 / otProject 12; the testid census 60 static (59 app-emitted) + 15 templated] + the OT-SEAMS seam map + `RH-skin-extraction.md`; annotakit live review surface).
- The app assembly skeleton: `AppShell` + the **four prop seams** (`timelineRegion`/`mediaDragSource`/`programMonitor`/`exportRequest` — the W2.4 deliver seam wired to the app's `deliverService.exportRequest`) + ProgramCanvas (engine N1 — the effects filter-string preview + the per-scene grade final pass) + the OT-tree `EngineMount` wired in `nle-test-app` @ `c020b2a` (**252 tests** — 8 suites at the R27 re-pin: GluedShell, audioService, sceneBridge, deliverService, persistenceService, wire-coverage, engineSeam, waveformPeaks; the R24 static census: GluedShell 70 + audioService 40 + sceneBridge 25 + persistence 17 + deliverService 15 + engineSeam 7; the GluedShell assembly; the W3 JKL transport seam — `setPlaybackRate` routed through the timelineRouter, ⇧J/⇧L yielding to the port's keymap; RR1-B; the R8 real-wiring waves — persistenceService [2s-debounce localStorage autosave keyed on state identity incl. the grade sidecar; boot hydrate; ⌘S/beforeunload], the W2.1 ducking DSP, the mediabunny deliver pipeline, the W2.5 effects/grade preview). The timeline port is a **CONVERGING MIRROR of OT's canonical `src/components/timeline/` tree under the census discipline (D26 — amends D25.2's mechanism; the D25.1 canon + gold sample stand)**: **(R29 register-equality census @ `55c81c0`: 42 port files = 41 mirrors + the `EngineMount.tsx` host — 36 zero-action (6 byte-exact + 30 mechanical-exact) + 5 carriers (TimelineView, use-timeline-actions, use-keybindings, TimelineContextMenu, use-playback-ticker); `use-wire-dispatch` a zero-action mirror since W-C; CI-enforced (census-check + the mutation gate 8/8 green); the D-ARCH-6 re-pin (the OT re-pin + the port's gesture-seam switch) LANDED @ `876f2b8` — the open half closed)** — **(the R25 re-read @ `64fb0ab`: the W-C/W-D adoptions LANDED — `use-wire-dispatch` ported; the census reached its declared irreducible set; the 32+7 figures are the R24 static census, kept as the recorded baseline)**; the `vendor/nle-timeline` mirror @ `55c81c0` byte-exact minus `testing/` (live-verified; the s17 tip over the code pin `970948a` — the D-ARCH-6 absorption LANDED); fork retirement = the D30-declared irreducible carrier set, tracked by the census — a carrier without a divergence rationale is a census violation. The honest-mock ledger (marked honest in both worlds, per the module cards): the ColorPage LUT select + 4 wheels (display-only); the fcpxml preset (toast, no row); Inspector Pan/Transform (mock-local; preserve-pitch MODEL-BACKED since R9-b — `Inspector.tsx:729-733` writes `el.preservePitch`, absent≡true); the ColorPage brightness/hue sliders (contract-complete, slider-less v1); the compact 8-effect mock library (click = honest toast); the StatusStrip idle theater + the hardcoded `OPFS · local` label; the fixture deliver rows j-0..j-3.
...
- **K3's three-way DOM-structural mapping (D26.5/D29.3 — REQUIRED — RATIFIED R31)** — timeline units → OT's `data-test=` (the frozen 68-site convention); chrome units → nle-ui's `shell-*` testid family (already emitting: 70 sites / 62 distinct names [55 static + 7 templated] across 22 non-test files, live-counted at `fc4cc35`); no-counterpart units (the track-head column, the minimized strip, the compact mode) → the D25.3b structural-gap worklist above. The mini's 59-static + 15-templated census gets the MAPPING ROW (mini-unit → target vocabulary + name → present/absent/structural-gap) (owner S-app; gates K3's DOM half together with D30's W-C — the ordering law per D26.5, NOT on a swap+testid layer; acceptance: the mapping row authored + the census-gap worklist filed in OT's queue + the `data-test` freeze ask). **RATIFIED (R31, @ app `f58147c`): the mapping table authored in the app repo (`nle-test-app/docs/k3-map-timeline.md` — the timeline surface's 43 units: 21 CLEAN / 4 PARTIAL / 18 GAP); the 4 PARTIAL rows RULED as code — (a) the resize-handle gains `data-element-id` (the composed-selector grammar `[data-test="resize-handle-{side}"][data-element-id="…"]`, the timeline-element flagship row's own form), (b) the drop-indicator stays marker+`data-track-index` scoped (display:none test-only by design — the DragLine pair is the visual), (c) `mini-btn-audiolane` → `label-visibility-${id}` ruled a REGISTERED DIVERGENCE (engine track visibility, not a lane-collapse eye), (d) the port's zoom-slider gains `aria-valuetext` (the §11.3 slider contract, the package twin's exact `px/s` form — the row-40 fix-first); the 18 GAP units FILED in OT's queue at R31 (`31f2764`, the era's HEAD — the D25.3b structural-gap worklist enumerating the five absent features [minimized strip 5 / binding-window 2 / transition layer 5 / filmstrip toggle 1 / mini-plus+solo 2] + the port tool radio 2 + the trivial zoom wrapper 1, PLUS the `data-test` freeze ask + the carrier-reduction work order — the acceptance trio discharged in one filing).**
- **The crawl app's chrome surfaces — the behavior half** — inspector/pool/topbar/toast/splitter/viewer+transport laws realized engine-wired (**K3**; the LAW-NET-INVENTORY corpus checked row-by-row — 322 authored + 33 projection pins; the DOM-structural gate = the three-way mapping row above + D30's W-C [the ordering law re-stated per D26.5]; the store/policy halves may author ∥ the bridge against the injection-point contracts; K3's routed-verb machine-check = the D30 W-F coverage-gate port — the accumulator ⊇ `WIRE_COMMAND_TYPES` − exceptions [31 = 28 routed + 3 exceptions — the census UNCHANGED at the R33 pins (R33 re-key @ OT `008e7f3`/code `344123a`); the registry EXPORTED since AR-1 — the gate consumes the live barrel], the shell-path surface excepted with its compensating pin family, D29.1b). The qc- restyle is the TOKEN half above (w1); the viewer FRAME visuals → **w1**. Verified un-started at `c885ece` (zero LAW-NET/e2e hits in the app tree).
- **The mini-parity gate** — the automated e2e (import → cut → play → export, zero mock paths; drives the 28 routed verbs through the real UI, D29.1c) → **K4**; the human side-by-side + the annotakit review-loop config for the app → **w1-entry** (the side-by-side is **app-vs-OT-runner** — two renderings of ONE tree per D25.4, the VLM net pointed at both; not app-vs-mock). Verified un-started at `c885ece`.
- **The cross-repo queue-protocol debt (D27's filing gaps)** — nle-ui's DECISIONS ledger ends at **D28** (D29 lives only as the app-side design doc `nle-test-app/docs/design-r8-d29.md`; D30 never filed package-side, despite HANDOFF's "D22–D29" pointer); the app's D30 W-D package-edit queue — R6 the `r`-key yield-set, Z5 the Inspector video-tab A/V-link toggle, Z7 the StatusStrip truth rows + storage prop, the cheat-sheet dual-world truth rows, the Viewer scrub-row `data-transport` attr, the ColorPage Brightness/Hue sliders — is designed + queued **app-side only** (`design-r9-d30.md:183-186,246-250`), none landed, none filed package-side (owner S-package + S-app, rides the D30 W-D/W-G waves; acceptance: the rows filed in nle-ui's `.agents/PLAN.md` + a DECISIONS D30 entry). The OT S-round queue rows that lived here are **LANDED → BASE** (P-widen + P-lock-route, above). **(R31 closure of the D29 half: the ledger's D29 entry gained its FIRST-PARTY record @ nle-ui `dc3644b` — the three seam waves (W2.3 saveState / W2.4 exportRequest / W2.5 the per-scene grade sidecar) with their store APIs, consumers, and acceptance pins, folded per the ledger's own D28-amendments convention + the spec 18 v1.6 BASE re-pin citation; the R28-exec round had landed the ARCH-R24 instruments flavor @ `6754979` — the R31 fold completes it. The D30 package-side filing remains OPEN — the W-D/W-G rows are still app-side only.)**
...
├──────────────────────────────────────────────────────────────────────┤
│ app-dock (42px): brand · page dock (Edit/Color/Audio/FX/Deliver —      │
│                       R28-D50's five-page law; Audio = focus mode) ·  │
│                       right cluster                                    │
└──────────────────────────────────────────────────────────────────────┘
```
...
|---|---|---|
| **Video** | video/comp element selected | Transform: position X/Y, scale %, rotation°, opacity %, flip H/V (→ transform-resolver fields, spec 07 §5.4); Speed: rate % + preserve-pitch toggle (→ retime family, spec 06 §5.12) |
| **Audio** | audio-bearing element selected | Levels group: Gain dB ([−60, +20], the D38.1 one-home domain), pan (−100..100), fade-in/fade-out seconds (→ spec 03 §9 / spec 09 §3.1 audio fields; the B2 paragraph below) |
...
Inspector edits are **commit-on-release** (slider drag) / **commit-on-enter** (numeric field) with live preview via the same preview-commit coalescing as timeline drags (spec 06 §4.6). Multi-selection shows the common-parameter subset with "mixed" indicators. Nothing selected → "Nothing to inspect" (mock's default text). The mock's `image` and `file` tabs are dropped (§8.6-8.7). (Round 15 amendment, B2 — **re-written R27 per the BASE + D38.1**: the audio tab's Levels group is **Gain dB** — the **[−60, +20] fleet-coherent domain** (opencut's `core/audio-params.ts` the ONE home; AW1-2 raised the authoring ceiling +4→+20) — authoring `el.volume` through the D28-A2 dual representation (display 20·log10, commit 10^(dB/20) clamped ≥ 0.001; 09 §3.1's linear 0..1 is the PERSISTED unit, the dB rail the authoring+display form — 09 §3.1A B2's cross-ref expects exactly this row) + pan −100..100 (ElementJSON-backed per 09 §3.1; the BASE's per-element pan authoring is still mock-local — nle-ui's strip-layer pan is the real pan, the register's C40 family) + fades. The R15 "'Gain dB' is withdrawn" clause is itself withdrawn — it was false on both ends: the BASE ships the rail (`Inspector.tsx:997-1008` — `min={-60} max={20}`, commit `10^(dB/20)` clamped ≥ 0.001) and 09 §3.1A B2 cross-cites this row as the display form. `preservePitch` is ElementJSON law — **absent ≡ true**; the engine's `retime.maintainPitch` is the runtime projection (R9-b/D38.2); its editor stays on the Video tab's Speed row — a retime concern — while the field backs both.)

**The inspector's non-element targets (Round 28 amendments, D48 + D49 — two rail swaps):**

- **The marker inspector (D48 — the ⌥M re-home; 16 §3.7's row opens THIS rail, not a dialog):** when the resolved target is a marker (⌥M at the playhead, or a ruler-band marker click), the inspector rail swaps to the marker inspector — the v2 field set: **Time / Duration [range-only — absent for point markers] / Name / Notes / Color**. `notes` is user content that round-trips OT's `Bookmark.note` (the D48/F1 disposition: `notes` owns the OT text home, `label` doc-side-synthesized); **`keyword` is REJECTED — no field, no control** (no OT home, no consumer). Edits commit as `updateMarker` (`Partial<Marker>` — 15 §4.3.51); the rail's Remove button is the **id-addressed** remove route (deletes range markers regardless of playhead — 16 §3.7's START-match law applies to ⇧M only).
- **The caption inspector (D49 — the captions model's track-scoped editor; reference: the variants' `CaptionInspector.tsx` + its 10 pins):** when the resolved target is a caption element (`type:'text'` on a `captions` track — 09 §3.1's fourth SceneTracksJSON family), the rail swaps to the caption inspector: the **text-body editor** (`ElementJSON.text` — the first-class body field, D49; styled params stay in `params`), In/Out timecodes, the live char count, the track's caption table (**# / Time In/Out / Caption / CPS**), and the track's `language` tag rendered display-only. **CPS is COMPUTED** — `round(chars / duration)` — never authored (the reference's decorative CPS values were rejected as data). Timeline edits ride the ordinary element verbs (`updateElements` patches; shape-ops apply — the mode matrix needs zero new rows).
...
- **Refresh law (R25-W6, registered forward)**: on any view/editor-mode/page transition the inspector re-resolves its target entity; an unresolvable target shows the empty state, never stale data (C44's empty-state → active-track fallback rides the same seam).
...
The region's internals belong to spec 05 (component hierarchy §4, zoom/scroll §5, virtualization §6, clip rendering §7, interactions §8, track headers §10). The shell fixes: the 160px header column (big TC readout in `#track-headers` mirrors the viewer TC; per-track M/S/lock buttons on ALL track kinds, plus visibility (V) on NON-AUDIO kinds only → `toggleTrackMute`/`toggleTrackSolo`/`toggleTrackLock`/`toggleTrackVisibility` commands (Round 15 amendment, A5: M/S/L render on every track kind — solo on video/text/overlay = monitor-solo semantics, 18's shape wins over 05 §10's "no S on video"; V renders only where 05 §10 permits it, non-audio — correcting this spec's unqualified v1.1 list in BOTH directions; the mock ships exactly this shape). **The per-track waveform flag is a RENDER GATE (R27 re-truth):** flag false → no waveform band (thumbnail + name stay; the variants' `Clip.tsx:1067-1083` per spec-05's re-read) + the `setAllTrackWaveforms` ONE-batch write (one undoable `withHistory` write converges every audio track — the N6 lock-all precedent, §4.5:207); the ViewOptionsPopover (APG menu: Compact-tracks checkbox — DOM-absent on the FX page; Clip style Filmstrip|Block radio; Audio-waveforms checkbox with honest aria-disabled + reason while compact) is the owning chrome (the register's view-state row). The scroll container keeps native scrollbars (the mock's custom 14px `#hscrollbar-row` is deferred, §8.11). The playhead line + head render per spec 05 §11. **(Round 28 amendment, D49 — the caption track-head):** a `captions` track's header renders the **CC badge** (the parchment-chip identity), the track's `language` tag (uppercase; "CC" when absent), and the **"N captions"** count — never a V control's filmstrip/waveform meta (caption elements are text chips, 05 §7.3); the lane is the 32px caption lane (05 §12.2), always the compact single-row header; add-above/below routes `addTrack {type:'caption', language?}` (15 §4.3.22 — 05 §12.1's TOP lane).

### 4.8 app-dock (`shell-dock`)

**(Round 28 amendment, D50/ARCH-R28 — the page-shape decision CLOSED: FIVE pages, not three, not seven.** The v1.7 three-page inventory below is re-keyed; the three-way divergence registration (this spec's 3 vs the BASE nle-ui's 4 vs the variants mock's 5 — D35.4's OPEN row) is RETIRED with the v1.8 bump: the ruling adopts the mock's five-page dock, matching the accepted BASE (nle-ui's 4-page union + FX), the design-of-record mock, and the corpus's own Resolve reference (page-per-workflow; the clone's 7-tab dock simplified: Media/Cut dropped, Fusion→FX, Fairlight→Audio-focus). The option-(b) rejection — three-pages-plus-deviations — would re-litigate the audio-only mixer ruling, leave K3 tests off-spec, and deadlock the D26.4 retirement on unflippable rows 7/8.**)**

Left: brand mark + app name. Center: page dock — **five pages** with the ⌘1-⌘5 page keys (spec 16 §3.8 — the five-page law landed R28; ⌘6-⌘9 reserved for future pages):

- **`Edit`** (⌘1; default; the whole shell above) — the rough-cut workspace.
- **`Color`** (⌘2) — grading workspace: spec 08's panels — wheels, curves, LUT, qualifier, power window, scopes — in a simplified single-column layout; enters a color-focus mode that swaps the inspector for the grading panel stack while the timeline stays live.
- **`Deliver`** (⌘3) — export: FCPXML via `exportFCPXML` command (spec 15 §4.3.74), optional cloud master via `exportMaster` (§4.3.75), render settings, progress list.
- **`Audio`** (⌘4) — **a focus mode, page and mode both**: `'audio'` IS a Page-union member (every page-scoped law keys on `page === 'audio'`; the per-page view-state memory keys five entries) while its ACTIVATION grammar is a focus mode — the dock tab is a TOGGLE (⌘4; re-click or Esc exits back to Edit; the R13 re-entry law never resets user state). Its surface is the mixer family (the ChannelEditor rail + Sound Library + the MixerDock — register row 20's home; **20 §7's M2-remainder keeps r2 ownership of the real mixer surface** — home vs surface is the split). The coherence anchor, verbatim from the BASE's own design note (`docs/DESIGN-audio-mode.md:11`): *"No Fairlight-style hard page split… ONE timeline, ONE clock, ONE command stream."*
- **`FX`** (⌘5) — **the fifth page per the one-flag law**: `fxMode`, two doors one engine (the FX tool implies it; `setPage('fx')` writes it; the fx-selection domain dies on page-exit). The SAME full Timeline mounts — only the compact strip never does (the density exemption keys on `page === 'fx'`, NOT `fxMode` — the Edit-page FX tool keeps the Edit page's compact entry; the ViewOptionsPopover's Compact control is DOM-absent on the FX page). The FxBrowser row grammar + the fx-selection domain + ruling 22 land with the §4.11/W6 absorption (the register's row-7 riders).

Right cluster: keyboard cheat-sheet (spec 16 §7.3's modal, opened via `?`), settings (deferred, §8.12). Dropped pages: **Media, Cut** (§8.2-8.4 — Fusion and Fairlight are TRANSFORMED, not dropped: Fusion→FX, Fairlight→Audio-focus, per the ledger's re-keyed rows). **The DECISION is closed; the IMPL half is registered:** the nle-ui FX-page absorption + the app-side page-key tests ride XMOCK-6's r5/K3 (D50/F6 — the spec row is LANDED; the BASE remains 4-page until that impl half).

**Deliver range band (Round 25 amendment, D35.4/ARCH-R25 — the R23 W-F surface gains its rows here, where the Deliver page's rows live; reference: `ui-mock/shell-variants`' `RangeBand.tsx` + the TimelineCompact.test pins incl. the band's clone):** the Deliver page's export range is authored by the range band — a 32px interactive in/out band mounted **BELOW the compact ruler's head row on the Deliver page — the 22px read-only ruler is UNCONDITIONAL in the compact head stack (ruler + band = the 54px deliver head; W4 `6b0bd91`)**, and the ruler itself gains read-only in/out bracket FLAGS (`shell-timeline-compact-flag-in/out` — the loop seam's brackets in miniature, pointer-events-none), with full-height 12px bracket handles (thin-glyph law: the hit target ≠ the visual size), a TC readout, and a dim-not-erase fill over the excluded span. Laws: **the loop-seam ruling** — the export range ≡ the timeline's in/out marks ≡ the loop region: ONE seam, THREE writers (the band, the ruler's bracket handles, mark-in/mark-out), and every readout follows every writer (the deliver settings' range block reads the same fields; `exportMaster {range}` — §5's row — is the seam's consumer; 15 §4.3.29 `setLoop` the wire form); **the drag law (ruling 21)** — LOCAL preview state during the drag, ONE commit on release, a no-op release writes nothing, pointercancel discards; **the mouse path (R24-W4) CLAMPS against the opposite live edge — the release commits exactly the preview**; **the ordering law (R14) governs the keyboard path verbatim** — writing an edge past the other DRAGS the far edge along, never inverts (the same law as §4.3's scrub-row inversion guard); **the domain clamp** — the range is clamped to [0, timeline duration] (an export range cannot exceed the timeline).
...
> The three tool rows above (ripple/slip/slide) close the table's own gap — the variants' `Clip.tsx:834-861` is the reference implementation; the router/15-union verb names finalize at the r1 ports (06 §5.4-5.7's families; D33.5's sequencing law).
...
**Double-click resets**: sliders, knobs, and NumberFields reset to their spec 09 default on double-click — implemented as the same reset command as the inspector's per-group reset (undoable `updateElements`, never a local-only re-render). **EXCEPTION (R25-A3, ruled D39 — the mock's W3 implements):** double-click on a color wheel's DISC resets the COLOR component only — the master/luma value survives; the per-wheel corner button is the full (color+master) reset (08 §4.3a's cross-ref).
...
| Clip left/right edge (hit zone) | `w-resize` / `e-resize` (directional — the single-edge push; R25-W2, `Clip.tsx:834`) |
| Trim handle hover | `ew-resize` |
| Roll tool on cut point | `ew-resize` (the ONLY bidirectional edge cursor — the two-way junction) |
...
| 8.2 | Media / Cut pages | Removed | Media pool is an Edit-page panel (§4.2); Cut page's dual-timeline is a finishing workflow — rough cut happens in Edit. (Unchanged by R28-D50 — both stay dropped under the five-page ruling.) |
| 8.3 | Fusion page | **Transformed → the FX page (R28-D50)** | The v1.0 ledger row read "Removed" (compositing beyond our effect set out of scope); the five-page ruling RE-KEYS it: Fusion's simplified analog IS the fifth page (⌘5, the one-flag law — §4.8's FX row; the FxBrowser row grammar rides the W6 absorption). The full Fusion compositing surface stays out of scope (master spec §1) — the page shell is in, the node graph is not. |
| 8.4 | Fairlight page | **Transformed → the Audio focus mode (R28-D50)** | The v1.0 ledger row read "Removed" (audio post / bus routing downstream-NLE territory); the five-page ruling RE-KEYS it: Fairlight's simplified analog is the Audio page — a focus mode (⌘4 toggle, §4.8's Audio row) hosting the mixer family (ChannelEditor rail + Sound Library + MixerDock; register row 20's home; 20 §7's M2-remainder keeps the real mixer surface at r2). Audio params keep their inspector Audio-tab homes on Edit. |
...
| 8.8 | Mixer / Metadata panels (toolbar2 right) | Removed | Out of scope (8.4); project metadata is a Deliver-page section. **REGISTERED DIVERGENCE (R27; CLOSED by R28-D50):** the divergence's three-way registration (BASE 4-page + Audio / variants 5-page + Audio + FX) is RETIRED — the five-page ruling (§4.8) adopts the mock's dock, Audio gains the mixer family as its focus-mode surface (20 §7's M2-remainder owns the real parametric surface at r2), and FX is the fifth page; the register's mixer-family row (row 20) and page rows (7/8) carry the closed dispositions. The removal ledger records the v1.0 chrome decision, not a ban on the BASE's landed surface — the nle-ui FX-page absorption rides XMOCK-6's r5/K3 impl half |
...
| `--playhead` | `#fa1024` (the mock's red — `davinci_resolve_ui_mock.html:535`; variants `tokens.css:59`) | the playhead + time indicators — **time ≠ state** (the dedicated-token law, `tokens.css:8`) |
...
- **Motion tokens + anti-motion rules**: `--motion-fast` 100ms (fades), `--motion-base` 200ms (panels/menus), `--motion-slow` 400ms (page swaps). **No motion while tracking** (drag/scrub in progress = overlays freeze, no transitions); **playhead never animates** (position is per-frame state, not a tween — **RATIFIED R35 as the industry convention**, W1-a's research: all three reference NLEs frame-quantize the playhead; the only sub-frame flavors are audio-domain display variants [Premiere's Show Audio Time Units; Resolve's align-audio-edits pref], registered as the deferred r2/audio-era toggle, never an unquantized playhead); loading skeleton pulse at 1 Hz; `prefers-reduced-motion: reduce` zeroes all three tokens + kills the pulse globally (§11).
...
- **CSS layering (R27, the W5a lesson)**: control-reset CSS lives in `@layer base` — unlayered author CSS outranks Tailwind's `@layer utilities` (the range-collapse P1 fix, `app.css:44-47`); never re-emit an unlayered `input[type=range]` rule.
...
| **shell-mini (first MVP, R19)** | `ui-mock/shell-mini/` (this repo) | The §16 MVP: compact shell + timeline whose ops track the OT seam map (`docs/OT-SEAMS.md`); deviations register in its README; 495 vitest / 12 files (sealed — the register pin; re-keyed R34, was 355) + the R23 seal docs (`CORE-SEAMS.md` 26 seams; `LAW-NET-INVENTORY.md` the corpus census) + live annotakit review surface |
| **nle-ui (the productized package)** | `/home/z/my-project/nle-ui` (the private-source package; the app pins it via `vendor/nle-ui` @ `6754979` — the AW1-2 absorb LANDED; the package HEAD is `6754979` (R30: `5779295`)) (R31: `dc3644b` — docs-only: the D29 DECISIONS first-party record + the C0 filing) | The full-shell chrome family productized: `AppShell` + the four prop slots (`timelineRegion`/`mediaDragSource`/`programMonitor`/`exportRequest`), Toolbar2/MediaPool/Viewer/Inspector/AppDock/StatusStrip/ToastRegion/CheatSheet, the mixer family (ChannelEditor — the F3 real-gain slider), Color/Deliver pages; **691 tests** (37 test files, 691/691 run live at the R29 re-pin — 690/690 at R27; 701/701 at R30 — the C16 twin + the K3 FIX-2; the R27 landings: R9-b `el.preservePitch` + FW-D `setElementFieldAll` + F4-6/F4-7 + AW1-2 the Inspector Gain ceiling [−60,+20]; the R29 landings: AW1-4 the mediaInsertAtPlayhead pool seam + ENG-1) + 74 Storybook stories, engine-free by law (the boundary script); the D29 seams (the saveState lifecycle, the DeliverPage `exportRequest` + the 5 type-only deliver shapes, the per-scene grade sidecar + `setGrade`, the retryJob blob-URL revoke); F3/F7 the real-gain laws + the kind-aware mute-all pin; the §10 `shell-*` testid family emitting (70 sites / 62 names, live-counted) — **R32 layer (recorded R34 — missed in the R32/R33 folds): `1e0c30b` 743/743 (41 test files, 78 stories — the C0 MiniShell composition 701→737 + the D52 isInteractionActive twin 737→743)** | **R35 layer (the GATE-1 P1 fold): `45d42fb` 754/754 (42 test files, 79 stories — the C-1a edit-styles batch: the `mediaInsert(mediaIds, mode)` seam widening + the 4 MediaPool insert rows + the 5 shortcutMap rows; the app re-pinned `cb04919`→`45d42fb`)** |
| **opencut-timeline** | `src/components/timeline/` (41 files / ~9,200 LOC — **THE canonical React timeline UI tree, D25/D26**; hosts the W11 wire-dispatch layer — 28 routed + 3 exception verbs, `WIRE_COMMAND_TYPES` (api.ts:268-300 — re-anchored R33) + the EXPORTED `WIRE_UI_EXCEPTIONS` (api.ts:353-360 — re-anchored R33) + the M49C coverage gate) + `src/lib/timeline/` (core) + `src/app/view/` (the `/view` miniature editor shell: library panel + compositor preview + the timeline; deterministic fixtures; `window.__VIEW_TEST__` hooks; 452 in-page + 30 DragEvent/keyboard + 150 real-mouse suites — **675/675** @ `008e7f3` (R33 re-key) [code pin `344123a` — 675 = 632 classic (incl. the 3-test M58R family) + 17 M60 + 11 M61 + 11 M63 + 4 M62; 77 milestone entries, the report json the count authority; src diff empty to HEAD — the seal18 wrap docs/artifacts + the M58R flake-fix script past the code pin]) [R31: HEAD `31f2764` — docs-only (the R31 queue filing), the code pin UNMOVED] [R32: HEAD `39003d3` — the code pin MOVED: the D25.3a mini-theme wave (theme.ts mode maps + the waveform mode resolution + the Tier-2 residual overrides + the M60 JS-paint pins + the gold-sample captures); 649/649 = 632 classic + 17 M60] | The timeline-area internals this shell frames (spec 05 §16.5) + the gold-sample host for the mini theme (D25.4 — `/view` at mini theme once both grammar halves land) + the DOM-vocabulary source of the timeline units (`data-test=`, 68 sites — D26.5) |
...
1. **Source preview** (mockup dual-viewer): the v2 question is the COMMAND-SURFACED source monitor — the mock-side A1 grammar (the poster transport + the source ranges + the 7 one-shot modes, §4.3) is landed law; the wire-surface question (which spec-15 commands, if any, a v2 source monitor grows) is the seal round's. §8.5's deferral stands for the dual-viewer chrome only.
...
3. **Color page layout**: single-column simplified stack vs Resolve's node-graph-lite — superseded R24-W2/R25-A2 by 08 §0's W-B row (the W2 composition + the console-row TAB strip own the layout question; 18:220's page row cites the divergence set); this spec's §4.8 one-liner is the pointer, not the law.
...
3. **Testing (§12 / spec 17 Tier 3)**: the mini carries 495 vitest tests
   ...
   the R24-2 trim-mode/preview/law-net waves; the law corpus census + the testid census live in
