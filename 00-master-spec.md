...
**Status:** v9.0 (Round 24 — the per-file fleet-audit round: **Decisions 26-29 (ARCH-R24, adversarially reviewed — 17 findings folded)** — D26 the **adopt-wholesale convergence** amending D25.2's mechanism (the app's `timeline-port/` is a census-governed converging mirror of OT's canonical tree — 39 mirrors + the host, 32 zero-action + 7 documented carriers; the census is the law: register-equality, CI-enforced app-side, battery-re-verified; the D25.2 props list converts to the carrier-reduction program), D27 the **repo topology** (KEEP nle-ui — the engine-free shell package, role locked; KEEP the app — the only full-wiring surface; both evolve in place; cloudcut-nle dispositioned as the DORMANT June-July 2026 predecessor, reference-only — D27.3), D28 the **core-module evolution** (OT / nle-engine / web-daw-core EVOLVE IN PLACE through crawl and walk; the fork-to-clean-base option rejected; the **seal-snapshot option REGISTERED**, not executed — trigger: r1-entry or a packaging need, whichever precedes r1-entry), D29 the **W11 instruments entering the ladder** (the M49C coverage-gate instrument adopted K2/K3/K4-wide; the wire census re-based to 24 routed + 6 documented exceptions; the data-test contract rows; the waveform-contract promotion to WDC's session queue)); **the fleet re-pinned to the R24 HEADs**: engine `5036387` 458; OT `ded43c4` 536 (**code pin `c15a629`** — the last three commits are docs/runner-artifacts only, src diff empty; consumers pin `c15a629`); WDC `85b81b0` 759; nle-ui `fc4cc35` 674; nle-test-app `c885ece` 174; mini 355 (sealed) / variants 1521+ in-repo (live; the sibling stream's round-wrap declares the exact figure); the consumer pins re-read live (engine→OT `c15a629` + WDC `494f6ff`, docs-only behind; app→engine `5036387` + OT lock-copy `c15a629`, byte-exact minus `testing/` + nle-ui `fc4cc35` + WDC `85b81b0`). Prior ledger: v8.0 R23 (D23 the plan/spec separation, D24 the verification ladder K1-K4/w1-w3/r1-r6, D25 the timeline-UI single-tree bridge — D25's mechanism clause superseded by D26; the rest stand); v7.0 R22 (D18-D22 — D19's C0-C4 ladder superseded by D24's mapping, content re-homed; D18/D20/D21/D22 stand); R9 D12-D14; R15 D15-D17.)
**Date:** 2026-09-08 (v1 seed 2026-08-22; v2 testability 2026-08-30; v3 Round 7 2026-09-02; v4 Round 8 2026-09-02; v5 Round 9 2026-09-02; v6 Round 15 2026-09-05; v7 Round 22 2026-09-07; v8 Round 23 2026-09-07; v9 Round 24 2026-09-08)
...
- **nle-engine** (`github.com/bearachprema/nle-engine`, private) — clean-room FreeCut port, **458/458 vitest (the R24 module card's static census, exact; the live-run venue is its GHA CI), tsc 0, real A/V export decode-verified, one-audio-engine end-to-end; the N1-N4 bridge seam family (incl. N2b keyframed volume @ `37cdd28`) + W1 meter bridge + the RR1-A fade-clamp SPAN round + the W2.5 effects sidecar (`CompositionElementParams.effects` + `buildElementFilterString` — the honest CSS-filter preview subset, D29) + the R8-REV blur-radius clamp** @ `5036387` (R24 — the fleet re-pin; consumes OT @ `c15a629` (the W11 round, re-pinned from `a4e971d`) + WDC @ `494f6ff` (docs-only behind `85b81b0`) as submodules); the RUNTIME-domain core (player/GPU/transitions/export/media/fonts/audio-bridge) per Decision 12
- **opencut-timeline** (`github.com/bearachprema/opencut-timeline`, private) — clean-room OpenCut-classic distill, **536/536 tests (386 in-page + 150 real-mouse across 59 milestone entries — live-run at the R24 module card; the report json is the count authority), the W11 complete-UI round landed (every op wired via the wire-dispatch seam, machine-checked by the M49C coverage gate — 24 routed wire verbs + 6 documented exceptions; the W11 engine widenings: `HeadlessTimelineApi` attach + the recorder + `WIRE_COMMAND_TYPES` + `applyBatch data.results` + keyframe `TRACK_LOCKED`) + the W10F/M44-M48 hardening campaign + the op-surface parity audit (11/11 NLE edit verbs vs opencut-classic @ HEAD)** @ `ded43c4` (**code pin `c15a629`** — the last three commits are docs/runner-artifacts only, src diff empty; R24 — the fleet re-pin; consumers pin `c15a629`); the EDITING-domain core (SceneTracks/ops/controllers) **+ the CANONICAL React timeline UI tree** (`src/components/timeline/`, 40 files / 8,604 LOC, M17-M49 + real-mouse + coverage-gate-verified — Decision 25) per Decision 12
- **web-daw-core** (`github.com/bearachprema/web-daw-core`, private) — the AUDIO-domain core extracted from web-daw: channel strips, 20+ DSP effects, PDC, aux/sidechain, WAM hosting, offline render, null-test harness, **759/759 tests (the W2 + whole-CR + S4-seal rounds landed; the drift-gate now fails-on-drift; re-verified live at the R24 module card)** @ `85b81b0` (R24 — the fleet re-pin, docs-only over `494f6ff`; consumed by engine @ `494f6ff` (docs-only behind) + app @ `85b81b0` (opportunistic)); three-layer track model — Decision 13 / spec 20
- **nle-ui** (`github.com/bearachprema/nle-ui`, private) — the PRODUCTIZED shell package (engine-free, boundary-script-enforced, **674/674 — live-run at the R24 module card; the D29 seam waves (the `saveState` lifecycle + DeliverPage `exportRequest` + the per-scene grade sidecar) + F3/F7 the real-gain laws + the kind-aware mute-all pin**) @ `fc4cc35` (consumed @ `fc4cc35` — the app is at the package HEAD; role LOCKED per Decision 27.1) + **nle-test-app** (`github.com/bearachprema/nle-test-app`, private) — **THE APP** (the only full-wiring surface; the assembly: AppShell + EngineMount + ProgramCanvas + the WDC audio host; **174/174 (the R24 module card's static census) — the R8-D29 REAL-wiring waves (autosave/persistence, sidechain ducking, deliver export via mediabunny, the effects/color preview, the R8-b1 scene-switch rebuild fix) + the R9-D30 W11-absorption round mid-flight (W-C..W-G pending) + the timeline-port as the census-governed converging mirror (the adopt-wholesale convergence, D26); consumes engine @ `5036387` + the OT lock-copy @ `c15a629` (byte-exact minus `testing/`, live-verified) + nle-ui @ `fc4cc35` + WDC @ `85b81b0`**) @ `c885ece` — both R24 (Decisions 18/21/25/26-28); the in-repo `ui-mock/shell-variants` @ 1521+ tests (live; 1,470 at the 2026-09-07 pin-era reading; the R23 W-A FX surface + W-B color wave + the VLM net), the design reference; `ui-mock/shell-mini` 355 (the R23 seal — SEALED), the crawl's law register
