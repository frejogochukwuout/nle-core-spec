...
**Status:** v7.0 (Round 22 — the finality round: **Decisions 18-22 (ARCH-R22, twice-reviewed)** — D18 the app-layer topology (keep the nle-ui package + nle-test-app consumer split; the app is named/promoted), D19 the crawl-walk-run plan (spec 14 rewritten: C0-C4 mini-parity crawl → W-phases walk → R-phases run), D20 the spec posture law (BASE/GAP/ACCEPTANCE triad, every spec), D21 the fleet re-baseline (seven repos, 2026-09-07 pins: engine `f68ab8c` 356; OT `05584d8` 459; WDC `fe05d85` 740; nle-ui `dba8d52` 640; nle-test-app `e662759` 83; mini 355 / variants 1334 in-repo), D22 the drag-law freeze (the R22 user directive: the verbatim R18k law is the whole drag law — see spec 18 §16.2). Prior ledger: R9 D12/D13/D14; R15 D15/D16/D17 (D16's "fifth repo to be created" is REALITY as nle-ui + nle-test-app — the topology ruling supersedes the naming, not the law); R10-R21 the mockup + app-layer workstreams (see PLAN).)
...
- **nle-engine** (`github.com/bearachprema/nle-engine`, private) — clean-room FreeCut port, **356/356 vitest, tsc 0, real A/V export decode-verified, one-audio-engine end-to-end; the N1-N4 bridge seam family + W1 meter bridge landed** @ `f68ab8c` (R22 — Decision 21; the R15 baseline `f526e67` is history); the RUNTIME-domain core (player/GPU/transitions/export/media/fonts/audio-bridge) per Decision 12
- **opencut-timeline** (`github.com/bearachprema/opencut-timeline`, private) — clean-room OpenCut-classic distill, **459/459 tests (329 in-page + 130 real-mouse), the S-round landed (transport policy/track lock/transitionOut/bookmarks; consumer migrated)** @ `05584d8` (R22 — Decision 21; the R15 baseline `0412e41` is history); the EDITING-domain core (SceneTracks/ops/controllers; the React view tree lives app-side) per Decision 12
- **web-daw-core** (`github.com/bearachprema/web-daw-core`, private) — the AUDIO-domain core extracted from web-daw: channel strips, 20+ DSP effects, PDC, aux/sidechain, WAM hosting, offline render, null-test harness, **740/740 tests, zero runtime deps, PURE core; W1 canonical meter taps + the first upstream-direction push landed** @ `fe05d85` (R22 — Decision 21, consumed @ `5570321`; the R15 baseline `374711c` is history; three-layer track model — Decision 13 / spec 20)
- **nle-ui** (`github.com/bearachprema/nle-ui`, private) — the PRODUCTIZED shell-variants package (engine-free, boundary-script-gated, **640 tests**) @ `dba8d52` (consumed @ `752991d`) + **nle-test-app** (`github.com/bearachprema/nle-test-app`, private) — **THE APP** (the assembly: AppShell + EngineMount + ProgramCanvas + the WDC audio host, **83/83**) @ `e662759` — both R22 (Decision 18/21; the in-repo `ui-mock/shell-variants` @ `d42693e` R15 state is history: 1,334 tests now, the design reference; `ui-mock/shell-mini` 355 tests, the crawl's law register)
...
The implementation posture (Decisions 14-15): **the four foundational repos are the code baseline — inherited and evolved in place, never rewritten.** This is **not** a from-scratch rebuild (that posture was superseded in Round 9 and finally closed in Round 15: the OSS-derivation strategy exists precisely to avoid trial-and-error, and the four distillations have already succeeded under test gates). The spec set is their **contract + gap + acceptance layer**: it defines what the code must satisfy (shapes, protocol, seams, invariants, NFR floors), what still differs (the corrective register + gap tables), and how every facet is programmatically proven (spec 17 §13A). **Round 15 added the assembly ruling (Decision 16); Round 22 records its realization (Decision 18): the app layer EXISTS as two repos — `nle-ui` (the engine-free chrome package) + `nle-test-app` (the consumer/assembly repo, promoted to THE APP) — composing the cores as pinned submodules/vendored mirrors with the mock grammars productized. The crawl-walk-run plan (Decision 19, spec 14) drives the rest.** The genuinely greenfield surfaces are the app's assembly layer and FCPXML export. Each downstream stream spec refines a single concern; the scout plan (`13`) defines how refinements are produced; the testing strategy (`12`/`17`) and implementation phases (`14`) define execution.
