...
**Status:** v8.0 (Round 23 — the user's feedback round: **Decisions 23-25 (ARCH-R23, adversarially reviewed)** — D23 the plan/spec separation (`IMPLEMENTATION-PLAN.md` at the repo root is THE plan; spec 14 retired to a redirect stub), D24 the verification ladder (crawl = the programmatic-verification layer K1-K4 / walk = the real app with humans w1-w3 / run = the full NLE r1-r6 — the labels are VERIFICATION DEPTH, not UI scope), D25 the timeline-UI single-tree bridge (OT's `src/components/timeline/` is the canonical React tree; the app's timeline-port fork retires; the mini grammar lands as token+structural halves on the attached tree; the gold sample = OT's /view runner); **the fleet re-pinned to the R23 HEADs**: engine `b8c6f88` 440; OT `222532c` 489; WDC `494f6ff` 759; nle-ui `85dcf57` 648; nle-test-app `70e99f0` 117; mini 355 / variants 1521+ in-repo (live; 1,470 at the pin-era reading — the sibling stream landed more post-re-pin); the consumer pins re-read live. Prior ledger: v7.0 R22 (D18-D22 — D19's C0-C4 ladder superseded by D24's mapping, content re-homed; D18/D20/D21/D22 stand); R9 D12-D14; R15 D15-D17.)
**Date:** 2026-09-07 (v1 seed 2026-08-22; v2 testability 2026-08-30; v3 Round 7 2026-09-02; v4 Round 8 2026-09-02; v5 Round 9 2026-09-02; v6 Round 15 2026-09-05; v7 Round 22 2026-09-07; v8 Round 23 2026-09-07)
...
- **nle-engine** (`github.com/bearachprema/nle-engine`, private) — clean-room FreeCut port, **440/440 vitest, tsc 0, real A/V export decode-verified, one-audio-engine end-to-end; the N1-N4 bridge seam family (incl. N2b keyframed volume @ `37cdd28`) + W1 meter bridge landed; the RR1-A fade-clamp SPAN round** @ `b8c6f88` (R23 — the fleet re-pin; consumes OT @ `a4e971d` + WDC @ `f446512` as submodules); the RUNTIME-domain core (player/GPU/transitions/export/media/fonts/audio-bridge) per Decision 12
- **opencut-timeline** (`github.com/bearachprema/opencut-timeline`, private) — clean-room OpenCut-classic distill, **489/489 tests (359 in-page + 130 real-mouse; the report json is the count authority), the S-round landed (transport policy/track lock/transitionOut/bookmarks/mute-batch) + the F1 whole-codebase hardening + M45/M46** @ `222532c` (R23 — the fleet re-pin); the EDITING-domain core (SceneTracks/ops/controllers) **+ the CANONICAL React timeline UI tree** (`src/components/timeline/`, 39 files / ~7,700 LOC, M17-M46-verified — Decision 25) per Decision 12
- **web-daw-core** (`github.com/bearachprema/web-daw-core`, private) — the AUDIO-domain core extracted from web-daw: channel strips, 20+ DSP effects, PDC, aux/sidechain, WAM hosting, offline render, null-test harness, **759/759 tests (the W2 + whole-CR rounds landed; the drift-gate now fails-on-drift)** @ `494f6ff` (R23 — the fleet re-pin; consumed @ `f446512`); three-layer track model — Decision 13 / spec 20
- **nle-ui** (`github.com/bearachprema/nle-ui`, private) — the PRODUCTIZED shell-variants package (engine-free, boundary-script-gated, **648 tests; F3/F7 the real-gain laws + the kind-aware mute-all pin**) @ `85dcf57` (consumed @ `85dcf57` — the app is at the package HEAD) + **nle-test-app** (`github.com/bearachprema/nle-test-app`, private) — **THE APP** (the assembly: AppShell + EngineMount + ProgramCanvas + the WDC audio host; **117/117 — the W3 JKL audio half + the RR1-B round; consumes engine @ `4ef0147` + the OT vendored mirror @ `a4e971d`**) @ `70e99f0` — both R23 (Decision 18/21/25); the in-repo `ui-mock/shell-variants` @ 1521+ tests (live; 1,470 at the 2026-09-07 pin-era reading; the R23 W-A FX surface + W-B color wave + the VLM net), the design reference; `ui-mock/shell-mini` 355 (the R23 seal), the crawl's law register
...
**Reasoning:** freecut's `AudioMixer` is a scalar mix — no reverb, sends, sidechain, live EQ, PDC, or mixdown export; web-daw-core has all of them, null-test-hardened (≥60 dB offline/realtime parity, 737/737 — count superseded at R15 (721/721 PURE core) and again at R22/R23 (759/759 @ `494f6ff` — §0's fleet rows); the bridge relocated to nle-engine per M1.6). This is a has-vs-has-not gap, which clears the "significantly better" bar. The user's risk concern (another two-core pairing) is answered structurally: unlike the timeline duplicate (two same-level semantics + a bidirectional bridge), the audio seam is a layered contract sharing one key, the NLE semantics already live core-side (the bridge implements nle-engine's Wave-5A split-merge laws), and the legacy mixer is scheduled for deletion at a measured parity gate — nothing bidirectional exists to maintain.
...
5. **The drag-law freeze (Decision 22) and the D25 single-tree law are architecture-freeze items** — re-opening either requires the user's explicit request or a reviewed ARCH amendment.
...
**Reading the diagram against the live fleet (D12 + D25):** the Timeline UI box is OT's **canonical React tree** (`src/components/timeline/` — 39 files / ~7,702 LOC, the single-tree law, Decision 25; the app vendors it, the `timeline-port/` fork retires); the Engine Core / Playback & Render boxes are nle-engine (the RUNTIME domain); the audio DSP/mixing path (SoundTouch varispeed, channel strips, offline render) lives in web-daw-core (the AUDIO domain, spec 20); the app composes them all (Decisions 16/18).
