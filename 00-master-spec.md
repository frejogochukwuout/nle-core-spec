...
**Status:** v6.0 (Rounds 1-8 see prior ledger. Round 9: three-domain architecture (D12/D13/D14). **Round 15: assembly + path rulings — evolve-in-place (Decision 15), the `nle-app` assembly architecture (Decision 16: fifth repo + pinned-submodule lockset + ENGINE-home projector + two-staircase bus/events), four-walls-one-roof verification (Decision 17)**; repos re-baselined (engine 274+265+318 @f526e67, OT 423/423 @0412e41, WDC 721/721 @374711c, ui-mock 596/596 @d42693e). Rounds 10-14: the ui-mock workstream (spec-18 shell validated — see PLAN). Implementation execution plan = ARCH-R15 §3.4 + spec 14 (rewritten this round).)
...
The implementation posture (Decisions 14-15): **the four foundational repos are the code baseline — inherited and evolved in place, never rewritten.** This is **not** a from-scratch rebuild (that posture was superseded in Round 9 and finally closed in Round 15: the OSS-derivation strategy exists precisely to avoid trial-and-error, and the four distillations have already succeeded under test gates). The spec set is their **contract + gap + acceptance layer**: it defines what the code must satisfy (shapes, protocol, seams, invariants, NFR floors), what still differs (the corrective register + gap tables), and how every facet is programmatically proven (spec 17 §13A). **Round 15 adds the assembly ruling (Decision 16): a fifth repo, `nle-app`, composes the three cores as pinned submodules with the mock's chrome ported in — the app is the unit of assembly, the repos are the units of evolution.** The genuinely greenfield surfaces are the app's assembly layer and FCPXML export. Each downstream stream spec refines a single concern; the scout plan (`13`) defines how refinements are produced; the testing strategy (`12`/`17`) and implementation phases (`14`) define execution.
