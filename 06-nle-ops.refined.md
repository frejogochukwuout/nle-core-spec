...
2. **All ops are commands** at the engine layer: wrapped in `TracksSnapshotCommand` (single-op) or `BatchCommand` (multi-op). Per SCOUT-01 finding (`01-core-engine.refined.md:1368-1467`), there is **no `id`/`label`/`coalesceKey`** on the Command type.
...
**Reality (per SCOUT-01, `01-core-engine.refined.md:1368-1467`):** OpenCut-classic's `Command` is a class with **no `id`, no `label`, no `coalesceKey`, no state arg**. `execute()` is parameterless and pulls state by calling `EditorCore.getInstance()` inside the body.
...
Adopted verbatim from OpenCut-classic (per SCOUT-09 refined spec `09-project-model.refined.md:830-883`):

```ts
// From apps/web/src/timeline/types.ts:19-80 (quoted in 09-project-model.refined.md:830-880)
...
- Track-level cleanup: `CommandManager.registerReactor` prunes empty tracks after every command (per `01-core-engine.refined.md:1541`).
