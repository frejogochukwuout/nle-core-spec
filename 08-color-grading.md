...
- **The R23 W-B color wave LANDED (mock-side reference surfaces, honest view-state markers):** ScopesDock (the four scope TABS — Luma WFM / RGB Parade / Vector / Histogram — in the timeline-area console row next to the MixerDock, ONE scope at full panel size, the §11.4 10fps throttle; the R22-W4c four-at-once law is REVERSED by DESIGN-R23 D-B1, registered in the mock's deviation ledger), ColorNodeGraph (the nodes-in-viewer surface, D-B2 — the AppShell swaps the viewer for the graph; while it owns the region the dock honestly draws the LAST published frame, ruling 14), StillsPanel (the DaVinci-style Gallery: still cards with the honest derived node-count chip, clip-level captions, Save Still → the store's `colorStills` view state; the .drx/PowerGrade render-round boundary + gap C59 registered), and the `timelineCompact` auto|on|off density toggles (D-B3). Surface tests (re-counted live @ the R24 reading — ColorPage/ScopesDock/GradedViewerCanvas each +1 with the mock's own R23 review-sweep rounds): ColorPage 33 / ScopesDock 20 / StillsPanel 14 / ColorInspector 10 / GradedViewerCanvas 15 / mockGrades 18.
