...
**Status:** v-next (Round 23 — the per-file audit fleet's re-verify: the §0 BASE re-pinned to the R23 HEADs (the variants at 1,542 with the W-B color wave's surfaces now in BASE as mock references; the engine `b8c6f88` 440/440 with the instruments' zero-code state + the advertised-on-wire trap re-verified), the GAP rows re-tagged per D24 (W-color→**r3**; the instruments stay S-engine ∥ crawl, **consumed at r3**) + the retired spec-14 §3.2/§4 color rows confirmed re-homed into §0; Round 22's §0 posture + the R22 re-baseline stand — the grade-math binding (r3) + the S-engine instruments are the forward work); Refined spec — sub-agent scout SCOUT-08 has verified all claims against FreeCut + OpenCut-classic source (the §15A engine refs re-verified @ `b8c6f88` — content holds, line numbers drifted; see the §15A re-verify note)
...
## 0. FORWARD INVENTORY (R22 posture — what needs to be done; the BASE is accepted, not re-explained. R23 fleet re-verified 2026-09-07 against the live HEADs; phase tags re-based per the D24 ladder)

**BASE (accepted, re-verified @ the R23 pins — engine `b8c6f88`, app `70e99f0`, nle-ui `85dcf57`, variants @ the fleet HEAD):**
- The spec-08-exact math, verified in the variants mock: `ui-mock/shell-variants` @ **1,542 tests** (live at the fleet's HEAD; the R22 pin's 1,334 is superseded — the R23 W-A FX + W-B color waves, W-C/D/E (`ba46cf3`), and W-F + the WB-REV/WD P3 fix round (`acb2cf9`, the last variants-code commit) landed since; 00-master's 1,470 is its own R23-pin-era figure, pre-W-C/D/E — the battery's central post-fleet re-baseline reconciles) — the `src/lib/color/` libraries (colorSpace sRGB LUTs, gradeMath with the EXACT §4.2 14-step op order, qualifierMath, scopesMath, gradedImage, index; **167 dedicated tests, re-counted live**: colorSpace 31 / gradeMath 56 / qualifierMath 28 / scopesMath 20 / gradedImage 26 / index 6). MOCK-side, pinned in-repo — design reference, not shipping code; the W4 math is **r3's parity reference**.
- **The R23 W-B color wave LANDED (mock-side reference surfaces, honest view-state markers):** ScopesDock (the four scope TABS — Luma WFM / RGB Parade / Vector / Histogram — in the timeline-area console row next to the MixerDock, ONE scope at full panel size, the §11.4 10fps throttle; the R22-W4c four-at-once law is REVERSED by DESIGN-R23 D-B1, registered in the mock's deviation ledger), ColorNodeGraph (the nodes-in-viewer surface, D-B2 — the AppShell swaps the viewer for the graph; while it owns the region the dock honestly draws the LAST published frame, ruling 14), StillsPanel (the DaVinci-style Gallery: still cards with the honest derived node-count chip, clip-level captions, Save Still → the store's `colorStills` view state; the .drx/PowerGrade render-round boundary + gap C59 registered), and the `timelineCompact` auto|on|off density toggles (D-B3). Surface tests: ColorPage 32 / ScopesDock 19 / StillsPanel 14 / ColorInspector 10 / GradedViewerCanvas 14 / mockGrades 18.
...
- nle-engine @ `b8c6f88` — **440/440 vitest, tsc 0** (was 356 @ `f68ab8c` at the R22 pin; N2b keyframed volume `37cdd28` + the RR1-A fade-clamp round landed inside the 440 — audio/fade waves, not color-relevant, but they are what moved the pin) — the **44-effect registry** (43 in `effects/pipeline.ts` + `gpu-lut` in `effects/lut.ts`) + the WebGPU compositor: the engine landing pad r3 binds into. **The instruments are still ZERO engine code** (re-verified: the only mentions are the FreeCut wire-schema strings `gpu-power-window` / `gpu-secondary-qualifier` in `headless/api.ts`'s `GPU_EFFECT_TYPES` — the wire ADVERTISES both, the 44-id registry implements NEITHER, the advertised-on-wire ≠ implemented trap; the P2 backlog still lists "scopes (waveform/vectorscope)"). Its color ops are still FreeCut-baseline 8-bit/gamma (`rgba8unorm` — 46 sites/10 files, re-verified; §15A — the binding is the correction), and the Wave-3E `gpu-lut` is the 8-bit FreeCut-baseline port (Uint8Array/base64 rgba8, no linearization — §16.C/§17.D's fixes land with the binding).
- The app `70e99f0` (117/117) carries NO color surface (verified — its surfaces are the GluedShell/ProgramCanvas/audio/timeline family); nle-ui `85dcf57` (648) carries the ColorPage static-mock chrome only (spec 18 §4.8's single-column stack: role="img" wheels, display-state params + LUT readout, the honest "render round (spec 08)" toasts) — the package placeholder, consumed at w1, carries no math.
...
- **The grade-math binding + parity (the W-color row, re-homed from the retired spec-14 §3.2/§4; D24: W-color→r3).** Bind the W4 mock math (spec-08-exact) to the ENGINE pipeline's real pixels. Owner app+engine; **r3 (was W-color)** (acceptance: grade-math parity pins — the mock's W4 math vs the engine's output on the SAME shared fixtures, max delta ≤ 1 LSB-equivalent; the 8-bit→10-bit correction is the binding's substance — the engine's `rgba8unorm` chain, the LUT data path's Uint16Array/rgba16uint port (§16.B/§16.C), and §17.D's LUT linearization fix land with it; the parity corpus = the shared fixtures + both sides' pins, green).
- **The engine-side instruments — scopes, secondary qualifier, power window (still zero engine code — re-verified @ `b8c6f88`; the retired spec-14 §4 consumption row, re-homed).** Engine; **S-engine ∥ crawl, 4-6 wk** (the ARCH-R22 reviewed register; acceptance: each instrument's engine milestone green; consumed + pinned at **r3 (was W-color)** — the mock's ScopesDock/scopeDraw/scopesMath are the reference PLOTTING implementation, but at r3 the scopes must plot ENGINE data, and the qualifier + window apply in the linear-light pipeline; close the wire trap — either implement the advertised `gpu-secondary-qualifier`/`gpu-power-window` ids or pull them from `GPU_EFFECT_TYPES`).
- **Timeline-grade vs clip-grade sequential law — REGISTERED; keep as pointer:** shell-variants R20-W4c / `docs/r20/color-layout.md` §3.6 — `[clipGrade] → [timelineGrade]`, each a full application in sequence, not a params merge (mock-verified live: the store's TIMELINE_GRADE_KEY + the frame bus's 'program' mode); the spec-side ruling is the C50 follow-up.
- Register: `IMPLEMENTATION-PLAN.md` §2 (r3's row: "the grade-math binding + parity; the S-engine instruments consumed" — the parity pins ≤ 1 LSB-equivalent; scopes plot engine data) + §3 (the S-engine/S-app workstreams); spec 14 is retired to the redirect stub — its color-domain rows live HERE.
