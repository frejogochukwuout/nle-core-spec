...
- The spec-08-exact math, verified in the variants mock: `ui-mock/shell-variants` @ **1,939 it-blocks / 68 test files REGISTERED** (the R28 WRAP re-key per REFERENCE-REGISTER.md — the battery's line-start method; the declared pair recorded alongside: **1,950 runner-count / 126 stories**, the collector-unit reconciliation — 1,939 line-start + 7 REPLAY-case fan-outs + 4 AA5 loop iterations = 1,950 exactly; the "1,521+" canon was retired at R27 and this row's point-in-time readings — the R23 fleet's 1,542, the wrap-fix rounds `6ec61df` 1,594 / `de3fd47` 1,597, the R22 pin's 1,334, 00-master's 1,470 — are history; spec 04:19 carries the twin re-key) (R33, engine seal-round register P3-17 — re-keyed BEFORE the r3 fixtures are cut) — the `src/lib/color/` libraries (colorSpace sRGB LUTs, gradeMath with the EXACT §4.2 14-step op order, qualifierMath, scopesMath, gradedImage, index; **172 dedicated tests, re-counted live R33** (the r3 parity-oracle sizing): colorSpace 31 / gradeMath 57 / qualifierMath 32 / scopesMath 20 / gradedImage 26 / index 6 — was 167 at the R24 reading). MOCK-side, pinned in-repo — design reference, not shipping code; the W4 math is **r3's parity reference**.
- **The R23 W-B color wave LANDED (mock-side reference surfaces, honest view-state markers; the composition re-truthed R24-W2 + R25-A2 registered forward):** ScopesDock (the four scope TABS — Parade/Waveform/Vectorscope/Histogram — as the ~160px pane UNDER the viewer, store-gated, ONE scope at a time, the §11.4 10fps throttle; the R22-W4c four-at-once law is REVERSED by DESIGN-R23 D-B1; the R24-W1 console-row placement supersedes the R23 note that sat it next to the MixerDock — **REGISTERED FORWARD: R25-A2 supersedes — the color console row becomes a thin TAB strip [Timeline | Nodes | Scopes], the under-viewer pane RETIRES, and the scopes react to the PLAYHEAD FRAME, not selection**), ColorNodeGraph (the node-graph surface — console-row slot [6]; region [2] is ALWAYS viewer-led since W2 `0196c22`, so the earlier "the AppShell swaps the viewer for the graph" viewer-swap text and ruling-14's stale-frame clause RETIRE with the W2 composition; the C56 parallel-topology toast is the mock's honest marker), StillsPanel (the DaVinci-style Gallery: still cards with the honest derived node-count chip, clip-level captions, Save Still → the store's `colorStills` view state; the .drx/PowerGrade render-round boundary + gap C59 — the MOCK's own deviation ledger; the spec register's C-ledger ends at C58 — registered there), and the track-density control is the ViewOptionsPopover's Compact-tracks checkbox (W1 — the `timelineCompact` auto|on|off toggles are DEAD since W1's popover). Surface tests (re-counted live R33 — engine seal-round register P3-17; they size the r3 parity oracle): ColorPage 50 / ScopesDock 22 / StillsPanel 19 / ColorInspector 16 / GradedViewerCanvas 16 / mockGrades 24; the `src/lib/color/` libraries carry **172 dedicated tests** (colorSpace 31 / gradeMath 57 / qualifierMath 32 / scopesMath 20 / gradedImage 26 / index 6) — the counts re-keyed R33 over the R27 reading's 43/22/19/10/15/21 + 170 (33/56/28/20/27/6) and the R24 reading's 33/20/14/10/15/18 + 167.
...
- nle-engine @ `5036387` — **458/458 vitest, tsc 0** (was 440 @ `b8c6f88` at the R23 pin; +18 = the W2.5 D29 effects-sidecar round `8a0b7fe` (+17, bridge-seams) + the R8-REV #5 blur-radius clamp `4b2dfd1` (+1) — the round's ONLY engine source deltas, both in `bridge/composition-frame.ts`, NOT the GPU color stack) — the **44-effect registry** (43 in `effects/pipeline.ts` + `gpu-lut` in `effects/lut.ts`, re-counted) + the WebGPU compositor: the engine landing pad r3 binds into. **The instruments are still ZERO engine code** (re-verified @ `5036387`: still zero progress — the only mentions are the param-metadata comment at `effects/pipeline.ts:12` + the FreeCut wire-schema strings `gpu-power-window` / `gpu-secondary-qualifier` in `headless/api.ts`'s `GPU_EFFECT_TYPES` (:1492 the const, :1529 `gpu-power-window`, :1535 `gpu-secondary-qualifier` — the pins re-keyed R33, engine seal-round register P3-16, was :1509/:1515; spec 04:22 carries the twin pins) — the wire ADVERTISES both, the 44-id registry implements NEITHER, the advertised-on-wire ≠ implemented trap — **the full census (R33, engine seal-round register P2-5): 54 advertised vs the 44-id registry = 11 advertised-but-unimplemented + `gpu-tint` implemented-but-unadvertised (wire-rejected), re-verified live @ `2a0ecf4`; the two instruments are the color-relevant subset of the 11 — spec 04:22 carries the twin mirror**; the P2 backlog still lists "scopes (waveform/vectorscope)"). Its color ops are still FreeCut-baseline 8-bit/gamma (`rgba8unorm` — 46 sites/10 files, re-verified, the count IDENTICAL to R23: the 440→458 growth touched none of them; §15A — the binding is the correction), and the Wave-3E `gpu-lut` is the 8-bit FreeCut-baseline port (Uint8Array/base64 rgba8, no linearization — §16.C/§17.D's fixes land with the binding). The W2.5 effects CSS-filter subset (`CompositionElementParams.effects` + `buildElementFilterString` — the honest Gaussian-Blur-only map) is spec 04's BASE row; the scene-GRADE half of the seam is DECLINED here by design (the D29.5c row below).
...
>
> **R33 re-base (engine seal-round register P3-16, @ `2a0ecf4`, 785/785 — read-verified line-by-line):** the R29-R33 waves (the bridge/color/grade seams + the milestone suites) moved the files again, so the R24 note's "UNMOVED" clause is superseded — every LIVE row below is re-anchored: `registerEffects({` :4559→**:4595**, `GPU_EFFECT_REGISTRY` :3003→**:3039**, `wheelTint` :1883→**:1919**, `luminance601` :1894→**:1930**, the `rgba8unorm` data-texture site :4812→**:4957** (the R23/R24 notes' own :4850 reading drifted with the same growth — the site is the `nle-effect-${effectType}-data` createTexture), `runEffectChain` :5086→**:5179**, player's `enabledEffects` :1656→**:2109**, the .cube parser's `rgba8:` comment `lut.ts:49`→**:48** (:49 is the `data: Uint8Array` line); `pipeline.ts:37` + `lut.ts:337` + the `pipeline.ts:12` param-metadata comment re-verified EXACT; the wire strings are now `headless/api.ts:1529/:1535` (§0 above, reconciled with spec 04:22); the 8-bit baseline is still IDENTICAL (46 `rgba8unorm` sites / 10 files, live-recounted); the ENGINE-GAP rows hold (still no scopes module in `src/lib/nle`; `effects/` = `pipeline.ts` + `lut.ts` only).

| Spec section | nle-engine file:line | verified quote | status | note |
|---|---|---|---|---|
| §3 effect inventory | `src/lib/nle/effects/pipeline.ts:4595` | `registerEffects({` | ALIGNED | Full tool inventory; registry counts 43→44 (Wave 3E) |
| §13.Q7 registry pattern | `src/lib/nle/effects/pipeline.ts:3039` | `export const GPU_EFFECT_REGISTRY = new Map<string, GpuEffectDefinition>();` | ALIGNED | Same Map + buckets + registerEffects walk |
| §4.1/§16.A wheels model | `src/lib/nle/effects/pipeline.ts:1919` | `fn wheelTint(color: vec3f, hue: f32, amount: f32, mask: f32) -> vec3f {` | ALIGNED | Corrected (hue, amount) + scalar lift/gamma/gain/offset — confirms §16.A |
| §4.2 scene-linear port | `src/lib/nle/effects/pipeline.ts:1930` | `let luma = luminance601(c);` | CORRECTIVE | 8-bit gamma sRGB + BT.601 luma; spec's 10-bit linear wins (Decision 5) |
...
| §7.2 .cube parser | `src/lib/nle/effects/lut.ts:48` | `rgba8: size*size*size*4 bytes, red fastest axis` | CORRECTIVE | 8-bit quantization; spec's Uint16Array port wins |
...
| §7.3 GPU upload | `src/lib/nle/effects/pipeline.ts:4957` | `format: 'rgba8unorm',` | CORRECTIVE | 8-bit data textures; spec overrides |
| §12.1 runEffectChain | `src/lib/nle/effects/pipeline.ts:5179` | `const finalTex = this.runEffectChain(` | ALIGNED | Single-encoder chain + params-keyed cache |
| §12.2 real-time cache | `src/lib/nle/playback/player.ts:2109` | `const enabledEffects = (clip.effects ?? []).filter((e) => e.enabled);` | ENGINE-GAP | Whole-frame re-render; no dirty-flag cache |
