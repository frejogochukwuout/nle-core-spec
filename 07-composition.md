...
**Status:** v-next (Round 24 — the per-file audit fleet's re-verify: the §0 BASE re-pinned to the R24 HEADs (engine `5036387` 458/458 — the W2.5 D29 effects sidecar (`8a0b7fe`) + the R8-REV blur clamp (`4b2dfd1`) promoted to BASE: the composition pipeline now carries the honest CSS-filter subset, the bridge pins 97→113→151 (re-keyed R33, engine seal-round register P3-26 — 151 `it`/33 `describe` re-verified live @ `2a0ecf4`); the app's composition consumers re-verified @ `c885ece` — ProgramCanvas's effects/grade rows + deliverService's real export painter); the phase tags stripped to the D24 set (the dual-vocabulary window closed at this fleet); Round 23's re-verify stands — the S-engine projector parity corpus, the N1 visual seam's v1 exclusions + the open effects-preview widening, and the K3/K4 composition-frame consumption legs are the forward work); Refined by sub-agent scout (SCOUT-07) — open questions answered with source code references
...
## 0. FORWARD INVENTORY (R22 posture — what needs to be done; the BASE is accepted, not re-explained. R24 fleet re-verified 2026-09-08 against the live HEADs; phase tags are D24-ONLY now — the dual-vocabulary window (the retired spec-14 terms in parentheses) closed at this fleet's end; the re-homed §4 composition rows live here as plain D24 rows)

**BASE (accepted, re-verified @ the R24 pins — engine `5036387`, OT `ded43c4` (code pin `c15a629`), app `c885ece`; the engine's vendor submodules re-pinned: OT `c15a629` (gitlink), WDC `494f6ff`):**
- nle-engine @ **`60232ea`** — **789/789 vitest, tsc 0** (63 src files, 29 test files — the count gate's runtime-derived pin, `scripts/gate-vitest-count.mjs`; re-keyed R33, engine seal-round register P3-26; pin history: 440 @ `b8c6f88` R23 → 458 @ `5036387` R24 → 749 @ `e3f55bd` R31 → 785 @ `2a0ecf4` R33 → 789 live (R34 — the 4 P-F4 pins, the nightly's first exploration catch, the gate re-pinned 785→789 same-commit); the live-run venue is GHA CI) — the composition surface: the **N1 composition-frame bridge family** (`bridge/composition-frame.ts` — `buildCompositionFrame` pure composition math + `paintCompositionFrame` thin Canvas2D painter; the cut-centered transition window `[cut − D·alignment, cut + D·(1−alignment))` with head/tail source handles; z-order main→overlays-reversed; jsdom-safe) and the **N3 transition windows** (`bridge/transition-inputs.ts` — the cut-centered handle-extension equal-power windows driving the app's REAL audio crossfades), pinned in `tests/vitest/engine/bridge-seams.test.ts` — the bridge file's census: **64 → 97 → 113 → 151 pins** (18 → 21 → 33 describes; live count @ the R27 pin `f9ac806`, re-verified UNCHANGED 151 `it` / 33 `describe` / 2,584 LOC @ `2a0ecf4` — R33, engine seal-round register P3-26; UNCHANGED again @ `60232ea` (R34 re-key); the R22-era 64 = N1 21 + N2 14 + N3 17 + N4 12; +33 to 97 @ R23 from the T-round `transitionInputsFromScene`, the R1-B1/B2/B3 rounds (the RR1-A fade-span clamp + the N2b native volume-keyframe lane), the R2/R3 rounds, and the S3 hardening wave; +16 to 113 @ R24 from the W2.5/R8-REV rounds; +38 to 151 @ R27 from the F2-5/F2-6/F2-7 hardening + the P1 opencut-laws consolidation + OV-05 — five commits (`c8a4d00`/`02b0976`/`1c04887`/`2b2de33`/`9850231`) touched the file since the R24 pin; the W2.5 describes now at :980 (the filter-string law) / :1045 (the op threading) / :1073 (the painter call-order), the OV-05 pins at :869/:886/:901, the F2-6 exception-bracket describe at :1136, the F2-7 probe describe at :2208). **Landed since the R22 pin:** **N2b keyframed volume** (`37cdd28` — the flattener folds `animations["volume"]` to gainAutomation breakpoints; consumed app-side `c8c875e`), the **RR1-A fade-clamp SPAN** (`scene-to-segments.ts` RR1-A-1 raw-carries + clamps at the push, bounded by the final span; `transition-inputs.ts` `clampFade` (:186 — fades clamp to each element's own span; the name `clampFadeToElementSpan` appears nowhere in the live history)), the **W1 meter bridge** (R22-era, re-verified: `bridge/realtime-engine.ts`'s real `onMeter` master push + `onBusMeter(idx)` aux 1..4), the toSorted consumer-compat (`transitions/planner.ts:378`), and **the Wave 4A texture-ownership fixes** — the port's P0.1 (the effects singleton `return this._pongTexture;` → caller-owned per-call outputs) and P0.2 (cross-clip-shared mask combine textures → per-call outputs) are GONE, **re-verified fixed @ `5036387`**; THIS spec's §7.3 per-layer-outputs/pass-discipline laws are now the port's own contract (§12.A.1's two PORT-FIXED rows). **Landed since the R23 pin — the W2.5 D29 effects sidecar** (`8a0b7fe` + the R8-REV #5 hardening `4b2dfd1`): `CompositionElementParams.effects` (:133 — enabled-only, id-dropped: `CompositionElementEffect` = `{name, params}` at :138; enabled/bypass are the CONSUMER's domain, filtered BEFORE the seam) + the `CompositionDrawOp.filter` field (:179) + **`buildElementFilterString`** (:257 — the honest CSS-filter subset: 'Gaussian Blur' → `blur(radius px)`, **default 10 — the GPU registry's declared default per the F2-5 cross-module coherence pin** (`GAUSSIAN_BLUR_DEFAULT_RADIUS` `composition-frame.ts:210`, consumed at `:270`; non-finite→10, negative clamps ≥ 0 per R8-REV #5 — invalid CSS would no-op the WHOLE `ctx.filter`; the pin reads the live declared default at `bridge-seams:985-1012`) — R33 correction (engine seal-round register P1-4): the spec's former "default 4, non-finite→4" was a live FALSE law; Vignette/Glow/Film Grain/Motion Blur = the honest skip, the 44-effect GPU registry owns their fidelity; join with spaces; identity ''; the painter assigns per element inside its save/restore bracket, Safari<18 graceful no-op). The scene GRADE's final pass is **consumer-side at the current tier BY DESIGN** (hue-rotate/saturate are non-linear under per-element alpha compositing — per-element grade filters ≠ composited grade; the compose-then-filter ORDER is the law per D29.5(c)) — **R33 amendment (engine seal-round register P2-13, the D45 fold): the `{grade}` seam-side param — `paintCompositionFrame(ctx, ops, media, { grade })` — is RATIFIED as D29.5c's registered evolution (08:24)**, landing engine-side at r3 with `buildGradeFilterString` as the seam's string law (the `buildElementFilterString` precedent, `bridge/composition-frame.ts:257`; the painter today is the 3-param form at `:483-487` — the r3 landing correctly not yet landed); the compose-then-filter ORDER is the permanent law; the consumer-side final pass is the INTERIM tier (monitor + export — the Z2 export-grade sentence, `deliverService.ts:308-338`); spec 04:24's twin carries the same ratified-as-evolution, r3-consumable flip (landed R33, register P1-2).
- opencut-timeline @ HEAD **`8f96ab2`** / **code pin `8f96ab2`** — **714/714, 83 milestone entries** (re-keyed R34, was `008e7f3`/`344123a`/675 @ 77 — the M64 fps-param wave + the M59/M59A/B/C/D CR carrier families; the report json at the pin the count authority; `src/lib/timeline/ops/` byte-unchanged — the S3 law cites below re-verified exact at the new pin. The R33 layer: the report json's own breakdown: **632 classic + 17 M60-theme + 11 M61 + 11 M63 + 3 M62 + 1 M62-gate** — M63 is ELEVEN per `download/timeline-test-report.json`, the seal18-FINAL commit message's "10 M63" a typo; re-keyed R33, engine seal-round register P3-27 — the R31 corpus re-key missed this row; history: the R24-era reading 536/536 @ `ded43c4` / code pin `c15a629`, docs-only src diff, the citations re-verified via the app's then-lock-copy; re-based R27 to the `970948a` world; the R29 re-key `55c81c0`→`3e18722` docs-only; the R31 re-key `3e18722`→`31f2764` docs-only — the register's own interim figure, the code pin `970948a` then unmoved; the R32-a M60 mini-theme wave `39003d3` MOVED the code pin (649/649 = 632 + 17); the seal18 lib-tree wave `17a19f8` — linkage.ts NEW [D42], edit-domains.ts NEW [D43], api.ts +254, timeline-core.ts +144, split.ts +17, types +12, all additive, the components tree byte-unchanged, the wire census UNCHANGED 31 = 28 routed + 3 exceptions; the M58R flake fix at HEAD — the engine's 09-15 OV-12 CI red root-caused + closed; `src/` byte-identical `17a19f8`→HEAD, the two commits past the app's mirror touch only `download/`+`scripts/`) — the composition consumers: **the placeholder compositor** (`src/lib/timeline/render/placeholder-compositor.ts`, 175 LOC — Canvas2D solid-color composition behind `setTracks()` + `renderFrame(t)`, pixel-verified by M12's `readCenterPixel` in `testing/milestones-view.ts`; the file's header names spec 04's renderer as the real painter behind the same contract — only the painter changes) and **the S3 transitionOut laws** — the element-anchored `ElementTransition` (`types/index.ts:145` — `type: "crossfade"` + optional `presentation`/`alignment`, structurally §6.1A's wire; the field itself at `:190`), the null=DELETE-key law (`ops/timeline-core.ts:3235` — `transitionOut: null` DELETES the key, `undefined` keeps/restores the prior value; ref chain `:2613` @ `222532c` → `:2638` → `:3115` @ the code pin `970948a` → `:3235` live @ `008e7f3`, re-anchored R33, engine seal-round register P3-27; `ops/` byte-unchanged at `8f96ab2` — the cite exact (R34 re-key)), and split's left-drop law (`ops/split.ts:147-150/:192-194` — the out-boundary belongs to the right half, both retainSide paths; ref chain `:161` → `:147-149/:181-183` @ the code pin `970948a` → live @ `008e7f3`, re-anchored R33, engine seal-round register P3-27; exact at `8f96ab2` too — R34 re-key); the 7-way element union (Audio|Video|Image|Text|Sticker|Graphic|Effect) (`types/index.ts:243-250` — the `TimelineElement` union; was `:231-238`, pushed down by D42's `linkedTo` field on `BaseTimelineElement`, re-anchored R33, engine seal-round register P3-27).
- nle-test-app @ `d28847c` — **426/426, tsc 0, the 22-file roof** (re-keyed R33, engine seal-round register P2-12; R34: the lockstep re-pin `7664603`→`d28847c` — vendor engine→`60232ea` + both OT mirrors→`05d88d9` + nle-ui→`cb04919` + WDC→`94f6460`, the count + the roof UNCHANGED, the census register re-declared 43 = 33 zero-action + 8 carriers + 2 hosts; census history: 117 @ `70e99f0` R23 → 174 @ `c885ece` R24 → 418/418 @ `f58147c` the R31 canon → 422 @ `b5fe3b3` → 426 @ `ce7cfc3` R32 → 426 re-pinned @ `7664603` live → 426 UNCHANGED @ `d28847c` live) — the consumption proof, BOTH render directions of the N1 seam now REAL: **ProgramCanvas** (`src/ProgramCanvas.tsx`, 242 LOC) renders REAL monitor frames (the live core scene + D2e sidecars + the hash-color virtual-media resolver; jsdom pins the composition math + the W2.5 preview contract via `data-ops`/`data-playhead`/`data-filters`/`data-grade`) — the R8 W2.5 consumer rows: `compositionParamsFromDoc` maps `el.effects` (enabled-only, id-dropped — the engine's stated consumer-domain law) into `CompositionElementParams.effects`, and the scene GRADE rides the **final pass** (compose on a scratch canvas, then ONE `ctx.filter`-graded drawImage — the compose-then-filter law; the grade sidecar itself is nle-ui's store seam `setGrade` @ `fc4cc35`); **deliverService** (`src/deliverService.ts`, **426 LOC live @ `d28847c` (R34 re-key, was `7664603` — the file byte-unchanged through the re-pin)** — the R8 W2.4 landing `278b95b`, grown by the R25+ waves; the 344-LOC figure was the R24 reading, re-keyed R33, engine seal-round register P2-12) — the REAL deliver export: `renderFrame` = the SAME painter (`buildCompositionFrame` + `paintCompositionFrame` on an OffscreenCanvas at SPEC dims) fed to the engine's `renderCompositionCore` (deep import `@/lib/nle/export/orchestrator`) with **mediabunny@1.50.8** mux (the app's install; the frame-PNG path paints the same way); the **Z2 export-grade landing (R25, D30 W-E; registered here R33, engine seal-round register P2-12)**: the export grades via the same compose-then-filter final pass ProgramCanvas uses — `deliverService.ts:308-338` (`buildGradeFilterString` consumed at `:314`, `renderCtx.filter = gradeFilter` at `:332`) — the monitor↔export divergence CLOSED (the consumer-side interim tier per D45, spec 08:24); the audio legs: `buildAudioSegments` (`bridge/scene-to-segments.ts:347` — re-anchored from `:369`, R33, engine seal-round register P2-12) consumed with mid-entry math (`audioService.ts:469/:515`) + the ducking-sidecar fold (`insertEffects` → the external-sidechain compressor mapping; `rewireSidechains` after every materializeStrips, over WDC's exported `collectSidechainWires` @ `85b81b0`); `foldSidechains` as named in the charge does NOT exist in any live repo — the verified surface is the fold/rewire pair above.
- MOCK-side design reference (pinned in-repo, NOT shipping code): `ui-mock/shell-variants` @ `e89cd65` — **1,939 it-blocks / 68 test files** (the R28 WRAP re-key per R16 — ARCH-R28 §4; the battery's line-start method; the declared pair recorded alongside: **1,950 runner-count / 126 stories**, the collector-unit reconciliation; the prior 1,521 and 00-master's 1470 are point-in-time figures — the battery's central post-fleet re-baseline reconciles) — **the R23 W-A FX surface** (`components/fx/FxBrowser.tsx` + `FxInspector.tsx`: the FX page's asset dock — 5 effect rows + ALL 27 `TRANSITION_PRESENTATIONS` (the mock's 27-name list mirrors the engine registry's 27 built-ins) + 6 fade presets; the timeline's interactive transition boxes + the fades grammar in `Clip.tsx`; the drag-to-clip payload contract frozen, spec 15 §5.4) — **the mock reference for the FX/transition UI surface**.

**GAP (the work — owner + phase per the D24 ladder / `IMPLEMENTATION-PLAN.md` §2; acceptance in parentheses):**
- The projector parity corpus — the N1 composition-frame family's formal S4 suite (SceneTracks→ingestion translator + corpus) — S-engine (∥ crawl) (acceptance: S4 parity suite green vs the engine Timeline oracle on the corpus). STILL OPEN @ the R34 pin — engine `60232ea` (789/789), re-checked R34 (the re-check held at R33 too, `2a0ecf4`/785): no S4 suite / parity corpus in the engine's tree or its PLAN queue (`rg "S4|parity corpus"` over `src` + `.agents/PLAN.md` → zero hits; N1 is CLOSED as a seam; the formal corpus is unauthored — the plan's S-engine row is its only other home); the R24-era re-key text ("@ `5036387` … the HANDOFF next-steps list is G1/G2/N5/P3") is RETIRED — the live HANDOFF shape @ `60232ea`: #0 the perf.yml parse fix RESOLVED + #1 the first perf-nightly dispatch DONE R34 (the exploration venue's first random run CAUGHT P-F4 — the degenerate head-trim window inversion — + closed same-round; the nightly + the CI GREEN), #2 the r1-port EXECUTION round (design v2; its D-r1-4 CONSUMES the D45 projector — Stage 0's inputs landed in OT), #3 the W5 coverage thresholds, #4 the NS design queue, #5 the window-containment question. **The NS-4 adjacency, RULED (registered R33, engine seal-round register P2-10): D45 (R28): NS-4 = the bridge adapter — the render-plan projector `project(scene) → TimelineData`, the visual twin of scene-to-segments, per 08:24 + 17 §13A.5/:2390; the corpus's target contract is that projector** (the engine's NS queue carries NS-4 at `PLAN.md:481` as the r1-port actionable core — the old "may be reshaped by that ruling" hedge dropped). [spec 04 §0 carries the twin — cross-referenced, not duplicated.]
- The N1 visual seam's v1 exclusions — the seam's own header registers the v1 floor: sticker/graphic elements NOT drawn (documented exclusion), text ops dest = full frame (no spatial transforms in the opencut model — future transform sidecars; the engine documents the same slots as its future-extension CR, owner S-engine per D29.5(a)), `speed` visual-only (consumers must mirror it into `retime` or A/V drifts), track-level `muted` delegated to the consumer strip — S-engine (∥ crawl, additive; the bridge family is the seed) (acceptance: the excluded element types draw; text ops honor transform sidecars; the resolver seam extended per `composition-frame.ts`'s documented exclusions). **The effects-preview widening is the open half of the same floor @ R24 (D29.5(d))**: only the Gaussian-Blur subset landed (`8a0b7fe`); Vignette/Glow/Film Grain/Motion Blur stay honest-skips, the LUT-preview v1 is unlanded — owner S-engine.
- The composition-frame consumption behaviors — **K3 (spec 03 §0 owns the viewer/transport split — link, not duplicate)**: the scrub/play/seek legs driving `buildCompositionFrame` over the real engine clock, the D2e sidecar law, the store/policy halves authored NOW ∥ the D26 census discipline against the injection-point contracts (acceptance: the app-behavior nets green over the N1 seam; ProgramCanvas + engineService are the host surfaces; the app's **22-file / 426-test corpus** is the current approximation — the K3 corpus round's four re-expression maps + the store/policy + geometry + DOM-structural tranches LANDED R30/R31, the session-registry + D52 families R32 (the former "174-test glue suite" figure was four generations stale — R33, engine seal-round register P1-7); the remaining halves per 17 §13A.6's facet rows are the forward work).
- The crawl-exit legs — **K4**: the automated e2e's play + export legs render composition frames end-to-end (import(virtual) → cut → play → export; acceptance: the K4 suite green, zero mock paths — the N1 family is the play leg's render path; deliverService's W2.4 painter is the export leg's vehicle — **BEGUN @ R31, SEALED @ R32** (the K4 exit verdict LANDED: the e2e-crawl suite's 12 legs @ the app pin — import → cut → drag → play → export → undo/redo → JKL → bookmarks → loop-region → multipage + the move-drag LEG 2b, the cut clause 4-of-4; zero mock paths — ONE venue-law canvas.toBlob stub; zero human input; the interpretation ruling adversarially ratified; the verdict doc at the app repo's `docs/decision-k4-exit-verdict.md`; w2's full-scope demo extends the suite — the forward work)).
...
**ACCEPTANCE & TEST PLAN:** §12 (the architect's 9-bullet intent list) + §17 Testing (the executable contract: Tier 1 purity/ordering, Tier 2 blend/transition pixel rows, Tier 3); spec 17 §13A facet rows (§13A.5 projector property tests — both identity laws + never-loss); battery_r32's posture checks (BASE rows re-verified at their pins — the regression role; re-keyed battery_r24→r32 R33, engine seal-round register P3-26 — the register's r31 figure was R31-vintage, r32 the live latest in `scripts/`).
...
Scene assembly stops at the **topmost fully-occluding item**: scan tracks visually TOP-first (descending `order`); the first fully-occluding item found (the topmost occluder — opacity 1, no alpha-reducing effect or mask) sets the cutoff, and every track strictly below it (lower `order`) is excluded from the render plan — those items are provably invisible behind the occluder. This is FreeCut's occlusion cutoff (verified in the clean-room port @ `5036387`: nle-engine `playback/scene-assembly.ts:1358` — "Occlusion cutoff — scan visually TOP-first (desc by order), find the / first fully-occluding item (the topmost occluder). Tracks with / order < cutoff are visually BELOW it and get skipped"; the render-task walk itself is top-to-bottom DESC at `scene-assembly.ts:1379`). *(R23 note: the R22-era engine text scanned bottom-up (asc by order) and stopped at the first fully-opaque item — the port's own "Wave-seal occlusion fix" (`scene-assembly.ts:1180-1188`) documented the track-order axis inversion (OUR higher `order` = visually on TOP; FreeCut's axis is inverted) and refined the scan to top-first so the MAXIMAL cull is taken; the amendment's effect — occluded layers excluded, occluder's own track renders — is unchanged, and the earlier bottom-up quote is historical.)*

Why it matters: (a) it bounds the worst-case layer count per frame (early-out instead of always compositing every track); (b) export correctness — occluded layers must not be exported as visible. Amendment to §4's contract: `buildFrameDescriptor(state, frame, options?: { disableOcclusion?: boolean })` — the exporter passes `disableOcclusion: true` when it needs the full stack (lane assignment), the interactive renderer uses the cutoff. *(R23 re-verify: the engine's `buildFrameRenderTasks` carries exactly this option — `scene-assembly.ts:1334/1340` — used for export/offscreen render and when active shape masks could punch a hole through the occluder, per `scene-assembly.ts:1165-1169`.)* Items excluded by the cutoff do not appear in `FrameDescriptor.items`.
...
The seed's single-tier `type` union (`'crossfade' | 'wipe' | 'slide' | ...`) conflated timeline structure with visual presentation — reproducing a v1-scaffold bug the reference implementations explicitly fixed (nle-engine `transitions/registry.ts:30-35` documents the split as its first design decision — ref refreshed from `:29-33` @ `b8c6f88`, re-verified exact @ `5036387`). The corrected model:
...
- `getMaxTransitionDurationForHandles(left, right, alignment)` — binary-search the largest D satisfying both handle constraints (port target: nle-engine `transitions/handle-utils.ts:318` — ref refreshed from `:302`)
...
`AddTransitionCommand`/`UpdateTransitionCommand` params follow (spec 15 §4.3.61-63; the engine's headless op carries the same `leftClipId`/`rightClipId`/`presentation` fields — nle-engine `headless/api.ts:620-621,851-860` — refs refreshed from `:825`). §6.2's overlap-region description is superseded: there is no overlap region — there is a centered window and consumed handles.

> **R23 note — the LIVE consumer wire is element-anchored, the pair-form is the resolved internal form.** The landed consumer seam (OT `c15a629` + the engine bridge + the app — re-verified @ the R24 pins, citations live-checked against the app's byte-exact lock-copy) carries the transition as the element's own **`transitionOut` field** (`opencut-timeline` `types/index.ts:145/190` — `ElementTransition { type: "crossfade", duration, presentation?, alignment? }`, on the 7-way element union), NOT a standalone pair entity: the RIGHT member is derived by pairing (**left = the last source whose END is at/before the cut** — the F1 audit fix, `transition-inputs.ts:235-246`: pairing by start shifted the pair one boundary right on any track with ≥3 elements, the canonical adjacent chain silently no-opped; right = its immediate follower in start order — the engine's `bridge/transition-inputs.ts` pairing law, CUT_PAIR_TOLERANCE_SEC defensive no-op on gapped boundaries). The pair-form above is the composition runtime's RESOLVED form — both engine seams (N3 audio + N1 visual) derive it from the **ENGINE FIELD** (audio: field only; visual: field BASE + sidecar OVERRIDE — `composition-frame.ts:334`: `transitionOut: params[el.id]?.transitionOut ?? elementTransitionOut(el)`, the OV-05 one-input-source convergence; hosts that stopped supplying the legacy bag still get transitions). **The OV-05 one-input-source law (R33, engine seal-round register P1-5): BASE = the engine field `composition-frame.ts:334`; OVERRIDE = the consumer sidecar; hygiene = `transition-inputs.ts:117-135` (`transitionInputsFromScene` takes NO sidecar parameter — the sidecar never enters the structural path); pins `bridge-seams :869/:886/:901`.** The former text here ("derive it from the element-anchored sidecar") was falsified by OV-05 + F1 — corrected R33. The S3 laws that govern the field (all LANDED; the OT law cites re-anchored R33, engine seal-round register P2-12): patch `transitionOut: null` is the ONLY key-delete path in the patch surface (`ops/timeline-core.ts:3235` — the F-S3-1 null=DELETE law block, `undefined` keeps the prior value; ref chain `:2613` @ `222532c` → `:2638` → `:3115` @ the code pin `970948a` → `:3235` live @ the OT checkout `008e7f3`); split DROPS it from the LEFT product (`ops/split.ts:147-150/:192-194` — the out-boundary belongs to the right half, both retainSide paths; was `:161`, `:147-149/:181-183` @ `970948a`); params keys are overwrite-only and the null-clear law does NOT extend to them (`types/index.ts:166-168`, unchanged live). The composition runtime consuming a pair-form input MUST re-derive both members' windows from the same cut-centered geometry or the A/V floors diverge (the N1/N3 shared-window law, `composition-frame.ts`'s "THE SEAM LAW").
...
14 files, 112 LOC total. Already documented in `01-core-engine.md` §10.4. Summary:
...
> The private **nle-engine** repo (github.com/bearachprema/nle-engine — **789/789 vitest (29 test files — the count gate's runtime-derived pin), tsc 0 @ `60232ea`**, 57,166 LOC under `src/lib/nle` (63 files — re-keyed R34, was 785/785 · 57,139 @ `2a0ecf4`; see the R23/R24 re-verify notes below — the count/LOC figures re-based R33, engine seal-round register P3-26, the chain 458/458 · 54,791 LOC (62 files) @ `5036387` R24 → 749/749 @ `e3f55bd` R31 → live; the SCOUT-07-era snapshot was 37,958 LOC / 124 tests) is a clean-room FreeCut-port — **the D12-RATIFIED render/export venue model's engine, and the offline reference + the r1 parity twin for its Timeline model** (the former "in-between reference, NOT canon" framing re-based R33, engine seal-round register P3-26: per Decision 12 — engine `DECISIONS.md` D12 `:521`, ratified engine-side 2026-09-13 — the engine's stack is the RATIFIED render/export venue model). It de-risks implementation but inherits FreeCut patterns this spec corrects (8-bit sRGB pipeline, JSON-RPC+$ref headless protocol, class-based API, procedural media, single-tier tests, zero workers). Where engine and spec conflict, **the spec wins**; deltas are documented, not adopted. Full reconciliation: `19-code-references.md`.
>
> *(R23 fleet re-verify @ `b8c6f88`: **440/440 vitest, tsc 0, ~54,700 LOC under `src/lib/nle` (62 files)** — the SCOUT-07 snapshot was 37,958 LOC / 124 tests; the scout-era line refs live in git history, the rows below are refreshed. Two rows flip status: the engine’s own Wave 4A round fixed the port’s P0.1/P0.2 — noted inline.)*
>
> *(R24 fleet re-verify @ `5036387`: **458/458 vitest (static census; the live-run venue is GHA CI), tsc 0, 54,791 LOC under `src/lib/nle` (62 files)** — every row's file:line re-verified UNCHANGED (the W2.5/R8-REV rounds touched only `bridge/composition-frame.ts` + `bridge-seams.test.ts`); the two Wave-4A PORT-FIXED rows re-verified still-fixed; ONE row added (the W2.5 effects/filter seam).)*
>
> *(R33 re-base, engine seal-round register P2-12: the R24 claim above — "every row's file:line re-verified UNCHANGED" — was the R24-vintage state; the post-R24 engine waves drifted five rows of this table + the §6.1A OT law cites + the §0 app row. The drifted cites re-anchored LIVE this round — engine @ `2a0ecf4` (player `:1326`/`:1580` — text dispatch `:1562`, adjustment skip `:1556`, nested compositions `:1534-1536`; mask-manager `:734`/`:682` + the per-call pool copy `:750-768`; `buildAudioSegments` `bridge/scene-to-segments.ts:347`) + the OT checkout @ `008e7f3` (timeline-core `:3235`, split `:147-150/:192-194` — §6.1A's chain). **D12 ratification note:** the "in-between reference, NOT canon" framing above predates D12 — the engine's stack is now the RATIFIED render/export venue model, and the engine Timeline model stays the offline reference + the r1 parity twin (engine `DECISIONS.md` D12 `:521`, ratified engine-side 2026-09-13; the queue row in the engine's PLAN); the count/phrase re-bases (the 458/62-file figures, the "NOT canon" wording) were the P3 pack's pending items — **discharged by P3-26 this round**: the preamble above re-based to 785/785 · 57,139 LOC (63 files) @ `2a0ecf4` + the D12 venue-model framing.)* > *(R34 re-key: the preamble above re-based to 789/789 · 57,166 LOC (63 files) @ `60232ea` — the 4 P-F4 nan-guard pins + the P-F4 src fix in `timeline.ts`; every §13F file:line below re-verified exact at the new pin — the round's src delta is `timeline.ts` + the pin file only, no §13F-cited file moved.)*
...
| §5.5 occlusion cutoff | `src/lib/nle/playback/scene-assembly.ts:1358` | `Occlusion cutoff — scan visually TOP-first (desc by order), find the` | ALIGNED | Engine ahead of pre-amendment spec; adopted §5.5 — the R22-era bottom-up scan was refined by the port’s Wave-seal occlusion fix (top-first, the topmost occluder = the maximal cull; see §5.5’s R23 note) |
| §5.2 layer ordering | `src/lib/nle/playback/scene-assembly.ts:1379` | `Build render tasks — top to bottom (desc by order).` | ALIGNED | DESC walk + player reversal (ref refreshed from `:1263`) |
| §4 FrameDescriptor purity | `src/lib/nle/playback/player.ts:1326` | `private _buildLayers(frame: number, _offscreen: boolean): {` | ENGINE-GAP | Imperative layer build; the FrameItem/QuadTransform descriptor remains SPEC-ONLY — but the N1 bridge family (§0 BASE) now supplies the pure serializable DRAW-OP list as the consumer seam (ref refreshed `:1321`→`:1326` R33, engine seal-round register P2-12) |
| §12.2 type-agnostic items | `src/lib/nle/playback/player.ts:1580` | `if (clip.type !== 'video' && clip.type !== 'image') continue;` | ALIGNED (R23 flip) | The Round-7 failure mode is FIXED engine-side: image clips render exactly like video (Wave 4C P1.3), text dispatched (`:1562`), adjustment clips skipped (`:1556`), nested compositions (`:1534-1536`); the video-only continue is `collectFrameVideoCandidates`' own contract (`scene-assembly.ts:1870`, unchanged) — refs refreshed `:1575`→`:1580`/`:1557`→`:1562`/`:1533`→`:1534-1536` R33, engine seal-round register P2-12 |
| §6.1A structural/presentation split | `src/lib/nle/core/types.ts:311` | `export type TransitionType = 'crossfade';` | ALIGNED | Split matches the Round-7 amendment (ref refreshed from `:260`) |
| §6.1A presentation registry | `src/lib/nle/transitions/registry.ts:2249` | `const BUILTIN_PRESENTATIONS: TransitionPresentationDefinition[] = [` | ALIGNED | 27 presentations re-verified @ `5036387` (counted) — ≥ spec §6.3's list |
...
| §6.1A handle clamping | `src/lib/nle/transitions/handle-utils.ts:318` | `export function getMaxTransitionDurationForHandles(` | ALIGNED | Amendment B adopts (ref refreshed from `:302`) |
| §6.3 transition pipeline | `src/lib/nle/transitions/pipeline.ts:90` | `export class TransitionPipeline {` | ALIGNED | Single-pass + bind-group caching (`cachedBindGroups` at `:96`; ref refreshed from `:89`) |
| §7.3 effect chain | `src/lib/nle/effects/pipeline.ts:5113` | `const tempTex = inputTex;` | ALIGNED | Identical ping-pong swap loop (ref refreshed from `:5041`) |
| §7.2 effect → preview resolution (W2.5, D29) | `src/lib/nle/bridge/composition-frame.ts:257` | `export function buildElementFilterString(` | ALIGNED (preview seam) | The honest CSS-filter subset on the CONSUMER seam (added @ R24): `CompositionElementParams.effects` (:133, enabled-only/id-dropped per `CompositionElementEffect` :138) → the op's `filter` field (:179) → the painter's save/restore `ctx.filter` (Safari<18 graceful); Gaussian Blur → `blur(radius px)` (**default 10 — the registry's declared default per the F2-5 pin, `GAUSSIAN_BLUR_DEFAULT_RADIUS` `composition-frame.ts:210`, the pin `bridge-seams:985-1012`; non-finite→10, negative ≥ 0 per R8-REV #5 `4b2dfd1`** — R33, register P1-4); every other name = the honest skip (:263) — the 44-effect GPU registry owns their fidelity; the scene GRADE stays consumer-side BY DESIGN (D29.5(c)); consumers: the app's ProgramCanvas + deliverService (§0 BASE) |
| §7.3 texture ownership | `src/lib/nle/effects/pipeline.ts:5148` + `:5204` | `applyToTexture(` … “Wave 4A contract change: the returned texture is a NEWLY-CREATED,” | **PORT-FIXED (Wave 4A)** | The singleton `return this._pongTexture;` P0.1 is GONE — `applyToTexture` + the legacy `apply` return caller-owned per-call outputs; the spec's per-layer-outputs law is now the port's own contract |
| §8.3 multi-mask chaining | `src/lib/nle/gpu/mask-manager.ts:734` | `this._combinePipeline.combine(inputTexture, maskTexture, outputTexture, {` | ALIGNED | Sequential chaining matches (ref chain `:477`→`:723`→`:734` R33, engine seal-round register P2-12) |
| §8.3 mask texture ownership | `src/lib/nle/gpu/mask-manager.ts:682` | `getMaskResult(masks: MaskInfo[], canvasW: number, canvasH: number): {` | **PORT-FIXED (Wave 4A)** | The shared-persistent-textures P0.2 is GONE — `getMaskResult` returns per-call combined outputs (caller releases to the pool, `:750-768`); invert travels exactly once (`invertNext` in the chain / the flag on the single-mask path) — refs refreshed `:671`→`:682`/`:739-757`→`:750-768` R33, engine seal-round register P2-12 |
| §4 BlendMode contract | `src/lib/nle/gpu/compositor.ts:33` | `export const BLEND_MODE_INDEX: Record<BlendMode, number> = {` | CORRECTIVE | 25 FreeCut modes, no `plus-lighter`; map `plus-lighter ≡ linear-dodge` when porting (ref refreshed from `:32`) |
| §10.1 texture pool | `src/lib/nle/gpu/texture-pool.ts:5` | `Textures are keyed by \`{width}x{height}x{format}\` and recycled via` | ALIGNED | LIVE since Wave 4A — the player + mask-manager acquire/release through it (no longer dead code); this spec's cache design remains the consumer |
| §5.4 transform resolution | `src/lib/nle/core/transform-parent.ts:878` | `1. resolveTransform             — base fit-to-canvas + explicit override` | ALIGNED | 5-step pipeline ported; the FX-4 move relocated the pure core to `core/transform-parent.ts` (`transform-resolver.ts` re-exports for source-compat); corner-pin WIRED since Wave P1.8 (`core/types.ts:704` — a first-class clip warp field wired to the compositor shader; the “unwired — engine P1.8” note is retired) |
