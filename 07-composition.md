...
- The projector parity corpus — the N1 composition-frame family's formal S4 suite (SceneTracks→ingestion translator + corpus) — S-engine (∥ crawl) (acceptance: S4 parity suite green vs the engine Timeline oracle on the corpus). STILL OPEN @ the R33 pin — engine `2a0ecf4` (785/785), re-checked R33: no S4 suite / parity corpus in the engine's tree or its PLAN queue (`rg "S4|parity corpus"` over `src` + `.agents/PLAN.md` → zero hits; N1 is CLOSED as a seam; the formal corpus is unauthored — the plan's S-engine row is its only other home); the R24-era re-key text ("@ `5036387` … the HANDOFF next-steps list is G1/G2/N5/P3") is RETIRED — the live HANDOFF shape @ `2a0ecf4`: #0/#1 the perf.yml trigger fix + the first perf-nightly dispatch, #2 the r1-port EXECUTION round (design v2; its D-r1-4 CONSUMES the D45 projector — Stage 0's inputs landed in OT), #3 the W5 coverage thresholds, #4 the NS design queue, #5 the window-containment question. **The NS-4 adjacency, RULED (registered R33, engine seal-round register P2-10): D45 (R28): NS-4 = the bridge adapter — the render-plan projector `project(scene) → TimelineData`, the visual twin of scene-to-segments, per 08:24 + 17 §13A.5/:2390; the corpus's target contract is that projector** (the engine's NS queue carries NS-4 at `PLAN.md:481` as the r1-port actionable core — the old "may be reshaped by that ruling" hedge dropped). [spec 04 §0 carries the twin — cross-referenced, not duplicated.]
