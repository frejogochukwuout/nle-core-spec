...
- nle-engine @ `5036387` — **458/458 vitest, tsc 0** (was 440 @ `b8c6f88` at the R23 pin; count static-census-verified, the live-run venue is GHA CI) — the composition surface: the **N1 composition-frame bridge family** (`bridge/composition-frame.ts` — `buildCompositionFrame` pure composition math + `paintCompositionFrame` thin Canvas2D painter; the cut-centered transition window `[cut − D·alignment, cut + D·(1−alignment))` with head/tail source handles; z-order main→overlays-reversed; jsdom-safe) and the **N3 transition windows** (`bridge/transition-inputs.ts` — the cut-centered handle-extension equal-power windows driving the app's REAL audio crossfades), pinned in `tests/vitest/engine/bridge-seams.test.ts` — the bridge file's census: **64 → 97 → 113 pins** (18 → 21 describes; live count @ the R24 pin; the R22-era 64 = N1 21 + N2 14 + N3 17 + N4 12; +33 to 97 @ R23 from the T-round `transitionInputsFromScene`, the R1-B1/B2/B3 rounds (the RR1-A fade-span clamp + the N2b native volume-keyframe lane), the R2/R3 rounds, and the S3 hardening wave; +16 to 113 @ R24 from the W2.5/R8-REV rounds — the only commits to touch the file since the R23 pin; the three new `W2.5` describes (:921 the filter-string law, :970 the op threading, :998 the painter call-order) carry 20 pins incl. the R8-REV #5 clamp). **Landed since the R22 pin:** **N2b keyframed volume** (`37cdd28` — the flattener folds `animations["volume"]` to gainAutomation breakpoints; consumed app-side `c8c875e`), the **RR1-A fade-clamp SPAN** (`scene-to-segments.ts` RR1-A-1 raw-carries + clamps at the push, bounded by the final span; `transition-inputs.ts` `clampFadeToElementSpan`), the **W1 meter bridge** (R22-era, re-verified: `bridge/realtime-engine.ts`'s real `onMeter` master push + `onBusMeter(idx)` aux 1..4), the toSorted consumer-compat (`transitions/planner.ts:378`), and **the Wave 4A texture-ownership fixes** — the port's P0.1 (the effects singleton `return this._pongTexture;` → caller-owned per-call outputs) and P0.2 (cross-clip-shared mask combine textures → per-call outputs) are GONE, **re-verified fixed @ `5036387`**; THIS spec's §7.3 per-layer-outputs/pass-discipline laws are now the port's own contract (§12.A.1's two PORT-FIXED rows). **Landed since the R23 pin — the W2.5 D29 effects sidecar** (`8a0b7fe` + the R8-REV #5 hardening `4b2dfd1`): `CompositionElementParams.effects` (:119 — enabled-only, id-dropped: `CompositionElementEffect` = `{name, params}` at :124; enabled/bypass are the CONSUMER's domain, filtered BEFORE the seam) + the `CompositionDrawOp.filter` field (:161) + **`buildElementFilterString`** (:223 — the honest CSS-filter subset: 'Gaussian Blur' → `blur(radius px)`, default 4, non-finite→4, negative clamps ≥ 0 (R8-REV #5 — invalid CSS would no-op the WHOLE `ctx.filter`); Vignette/Glow/Film Grain/Motion Blur = the honest skip, the 44-effect GPU registry owns their fidelity; join with spaces; identity ''; the painter assigns per element inside its save/restore bracket, Safari<18 graceful no-op). The scene GRADE stays a **consumer-side final pass BY DESIGN** (hue-rotate/saturate are non-linear under per-element alpha compositing — per-element grade filters ≠ composited grade; the DECLINE is law per D29.5(c), spec 04/08 carry the decision).
...
| §7.2 effect → preview resolution (W2.5, D29) | `src/lib/nle/bridge/composition-frame.ts:223` | `export function buildElementFilterString(` | ALIGNED (preview seam) | The honest CSS-filter subset on the CONSUMER seam (added @ R24): `CompositionElementParams.effects` (:119, enabled-only/id-dropped per `CompositionElementEffect` :124) → the op's `filter` field (:161) → the painter's save/restore `ctx.filter` (Safari<18 graceful); Gaussian Blur → `blur(radius px)` (default 4, non-finite→4, negative ≥ 0 per R8-REV #5 `4b2dfd1`); every other name = the honest skip (:229) — the 44-effect GPU registry owns their fidelity; the scene GRADE stays consumer-side BY DESIGN (D29.5(c)); consumers: the app's ProgramCanvas + deliverService (§0 BASE) |
