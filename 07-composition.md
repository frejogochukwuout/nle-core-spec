...
| §4 FrameDescriptor purity | `src/lib/nle/playback/player.ts:1321` | `private _buildLayers(frame: number, _offscreen: boolean): {` | ENGINE-GAP | Imperative layer build; the FrameItem/QuadTransform descriptor remains SPEC-ONLY — but the N1 bridge family (§0 BASE) now supplies the pure serializable DRAW-OP list as the consumer seam |
| §12.2 type-agnostic items | `src/lib/nle/playback/player.ts:1575` | `if (clip.type !== 'video' && clip.type !== 'image') continue;` | ALIGNED (R23 flip) | The Round-7 failure mode is FIXED engine-side: image clips render exactly like video (Wave 4C P1.3), text dispatched (`:1557`), nested compositions (`:1533`); the video-only continue is `collectFrameVideoCandidates`' own contract (`scene-assembly.ts:1870`) |
...
| §8.3 multi-mask chaining | `src/lib/nle/gpu/mask-manager.ts:723` | `this._combinePipeline.combine(inputTexture, maskTexture, outputTexture, {` | ALIGNED | Sequential chaining matches (ref refreshed from `:477`) |
| §8.3 mask texture ownership | `src/lib/nle/gpu/mask-manager.ts:671` | `getMaskResult(masks: MaskInfo[], canvasW: number, canvasH: number): {` | **PORT-FIXED (Wave 4A)** | The shared-persistent-textures P0.2 is GONE — `getMaskResult` returns per-call combined outputs (caller releases to the pool, `:739-757`); invert travels exactly once (`invertNext` in the chain / the flag on the single-mask path) |
