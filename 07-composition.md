...
- The crawl-exit legs — **K4**: the automated e2e's play + export legs render composition frames end-to-end (import(virtual) → cut → play → export; acceptance: the K4 suite green, zero mock paths — the N1 family is the play leg's render path; deliverService's W2.4 painter is the export leg's vehicle, no e2e exists app-side yet — grep-verified @ `c885ece`).
