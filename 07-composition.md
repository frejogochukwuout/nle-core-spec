...
**Status:** v-next (Round 23 — the per-file audit fleet's re-verify: the §0 BASE re-pinned to the R23 HEADs (engine `b8c6f88` 440/440; the N1 composition-frame family + the N3/N4/W1 seams re-verified live — the bridge pins grew 64→97; N2b/RR1-A/the toSorted compat/the Wave-4A P0 fixes all landed); the GAP rows re-tagged per D24 (C2-behavior→K3, C4→K4, R-engine-p2→r6) + the retired spec-14 §4 composition rows re-homed into §0; Round 22's §0 posture + re-baseline stand — the S-engine projector parity corpus, the N1 visual seam's v1 exclusions, and the K3/K4 composition-frame consumption legs are the forward work); Refined by sub-agent scout (SCOUT-07) — open questions answered with source code references
...
## 0. FORWARD INVENTORY (R22 posture — what needs to be done; the BASE is accepted, not re-explained. R23 fleet re-verified 2026-09-07 against the live HEADs; phase tags re-based per D24 — spec 14 is retired to the redirect stub, its §4 composition rows re-homed HERE)

**BASE (accepted, re-verified @ the R23 pins — engine `b8c6f88`, OT `222532c`, app `70e99f0`; the engine's vendor submodules re-read live: OT `a4e971d`, WDC `f446512`):**
- nle-engine @ `b8c6f88` — **440/440 vitest, tsc 0** (was 356 @ `f68ab8c` at the R22 pin) — the composition surface: the **N1 composition-frame bridge family** (`bridge/composition-frame.ts` — `buildCompositionFrame` pure composition math + `paintCompositionFrame` thin Canvas2D painter; the cut-centered transition window `[cut − D·alignment, cut + D·(1−alignment))` with head/tail source handles; z-order main→overlays-reversed; jsdom-safe) and the **N3 transition windows** (`bridge/transition-inputs.ts` — the cut-centered handle-extension equal-power windows driving the app's REAL audio crossfades), pinned in `tests/vitest/engine/bridge-seams.test.ts` — the bridge family grew **64 → 97 pins** @ the R23 pin (the R22-era 64 = N1 21 + N2 14 + N3 17 + N4 12; +33 from the T-round `transitionInputsFromScene`, the R1-B1/B2/B3 rounds (the RR1-A fade-span clamp + the N2b native volume-keyframe lane), the R2/R3 rounds, and the S3 hardening wave). **Landed since the R22 pin:** **N2b keyframed volume** (`37cdd28` — the flattener folds `animations["volume"]` to gainAutomation breakpoints; consumed app-side `c8c875e`), the **RR1-A fade-clamp SPAN** (`scene-to-segments.ts` RR1-A-1 raw-carries + clamps at the push, bounded by the final span; `transition-inputs.ts` `clampFadeToElementSpan`), the **W1 meter bridge** (R22-era, re-verified: `bridge/realtime-engine.ts`'s real `onMeter` master push + `onBusMeter(idx)` aux 1..4), the toSorted consumer-compat (`transitions/planner.ts:378`), and **the Wave 4A texture-ownership fixes** — the port's P0.1 (the effects singleton `return this._pongTexture;` → caller-owned per-call outputs) and P0.2 (cross-clip-shared mask combine textures → per-call outputs) are GONE; THIS spec's §7.3 per-layer-outputs/pass-discipline laws are now the port's own contract (§12.A.1's two PORT-FIXED rows).
- opencut-timeline @ `222532c` — 489/489 — the composition consumers: **the placeholder compositor** (`src/lib/timeline/render/placeholder-compositor.ts` — Canvas2D solid-color composition behind `setTracks()` + `renderFrame(t)`, pixel-verified by M12's `readCenterPixel` in `testing/milestones-view.ts`; the file's header names spec 04's renderer as the real painter behind the same contract — only the painter changes) and **the S3 transitionOut laws** — the element-anchored `ElementTransition` (`types/index.ts:145` — `type: "crossfade"` + optional `presentation`/`alignment`, structurally §6.1A's wire), the null=DELETE-key law (`ops/timeline-core.ts:2613` — the only key-delete path in the patch surface), and split's left-drop law (`ops/split.ts:161` — the out-boundary belongs to the right half); the 7-way element union (Audio|Video|Image|Text|Sticker|Graphic|Effect).
- nle-test-app @ `70e99f0` — 117/117, tsc 0 — the consumption proof: **ProgramCanvas** (`src/ProgramCanvas.tsx`, 154 LOC) renders REAL frames through the N1 seam (the live core scene + D2e sidecars + the hash-color virtual-media resolver; jsdom pins the composition math via the data attributes).
...
- The projector parity corpus — the N1 composition-frame family's formal S4 suite (SceneTracks→ingestion translator + corpus) — S-engine (∥ crawl) (acceptance: S4 parity suite green vs the engine Timeline oracle on the corpus). STILL OPEN @ `b8c6f88`: no S4 suite / parity corpus in the engine's tree or its PLAN queue (N1 is CLOSED as a seam; the formal corpus is unauthored — the plan's S-engine row is its only other home). [The retired spec-14 §4.2 row; spec 04 §0 carries the twin — cross-referenced, not duplicated.]
- The N1 visual seam's v1 exclusions — the seam's own header registers the v1 floor: sticker/graphic elements NOT drawn (documented exclusion), text ops dest = full frame (no spatial transforms in the opencut model — future transform sidecars), `speed` visual-only (consumers must mirror it into `retime` or A/V drifts), track-level `muted` delegated to the consumer strip — S-engine (∥ crawl, additive; the bridge family is the seed) (acceptance: the excluded element types draw; text ops honor transform sidecars; the resolver seam extended per `composition-frame.ts`'s documented exclusions).
- The C2 composition-frame consumption behaviors — **K3 (was C2-behavior; spec 03 §0 owns the viewer/transport split — link, not duplicate)**: the scrub/play/seek legs driving `buildCompositionFrame` over the real engine clock, the D2e sidecar law, the store/policy halves authored NOW ∥ the D25 bridge against the injection-point contracts (acceptance: the app-behavior nets green over the N1 seam; ProgramCanvas + engineService are the host surfaces).
- The C4 crawl-exit legs — **K4 (was C4)**: the automated e2e's play + export legs render composition frames end-to-end (import(virtual) → cut → play → export; acceptance: the K4 suite green, zero mock paths — the N1 family is the play leg's render path).
...
**ACCEPTANCE & TEST PLAN:** §12 (the architect's 9-bullet intent list) + §17 Testing (the executable contract: Tier 1 purity/ordering, Tier 2 blend/transition pixel rows, Tier 3); spec 17 §13A facet rows (§13A.5 projector property tests — both identity laws + never-loss); battery_r23's posture checks (BASE rows re-verified at their pins — the regression role).
...
| §6.1A presentation registry | `src/lib/nle/transitions/registry.ts:2249` | `const BUILTIN_PRESENTATIONS: TransitionPresentationDefinition[] = [` | ALIGNED | 27 presentations re-verified @ `b8c6f88` (counted) — ≥ spec §6.3's list |
