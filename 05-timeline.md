...
- In-repo law register: `ui-mock/shell-mini/docs/OT-SEAMS.md` — the 14-row op map + the R22 drag-machinery retirement (the verbatim R18k law: clamp between same-track neighbors, single-edge live-field magnet, last-preview seal; the **D22 freeze is standing law — untouched by R23/R24**; shell-mini 357 tests — the R23 seal 355 + the R24 audio-clip-box nets) + `CORE-SEAMS.md` (S1-S10, the timeline-domain seams — the K3 re-expression worklist; S2's geometry-law map) + `LAW-NET-INVENTORY.md` Part A (92 families / 251 tests — K3's acceptance list).
