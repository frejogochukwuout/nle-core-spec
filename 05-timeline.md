...
**Reference repos audited:** `/tmp/opencut-classic` (archived MIT), `/tmp/freecut` (MIT); **opencut-timeline** (github.com/bearachprema/opencut-timeline, landed Round 8) is the live executable code reference for this stream's algorithmic core **and — per D25 (R23) — the canonical React component tree** (the components landed W4/W5 as M17-M21 and are verified M17-M46 + real-mouse; the app-side `timeline-port/` fork is retired); see spec 19 §3.2 and §16.5 below.

---

## 0. FORWARD INVENTORY (the R22 posture, R23-re-based — what needs to be done; the BASE is accepted, not re-explained)

**BASE (accepted, pinned 2026-09-07):**
- opencut-timeline (OT) @ `222532c` — **489/489** (359 in-page + 130 real-mouse across 51 milestone suites, 14 of them real-mouse; `download/timeline-test-report.json` is the count authority), tsc 0. This spec's interaction contract is executable there: the ops layer (split/trim/move/ripple/delete), snapping/placement/view-math, the headless API; the S-round landed (S1 transport policy — playbackRate/loopRegion/anti-drift/atomic boundary pause; S2 track lock; S3 transitionOut with the null=DELETE key law + split left-drops; S4 bookmark color/note + D-S5 typed ElementParams), the T-round (D-T3 `setTracksLocked` + `track.setAllLocked` lock-all batch; D-T7 move-NOOP; M44), **the S3 seam round (M46: `setTracksMuted` — the mute-all batch op, the setTracksLocked D-T3 twin — + the `track.setAllMuted` wire verb)**, and **the F1 whole-codebase hardening** (M45: F1A-1 box-select rows shift with expanded keyframe lanes; F1A-2/3/4 insert-on-new-track trims + heterogeneous-batch rejection + createTracks id validation; F1B-1..8 the wire/interaction validation laws — no-overlap enforced on update-patches, the dead-button + non-left-mousedown gates, INVALID_PARAMS for missing/null params, setLoopRegion clear:false never clears, negative trims rejected, failed applyBatch preserves the redo stack, keyframe flick commits; F1C-1 setTracksLocked unlock-direction; F1C-2/3/4 the runner + echo asserts).
- **OT's `src/components/timeline/` is THE canonical React timeline UI tree (D25 — the single-tree law):** 39 files / ~7,700 LOC (TimelineView 1,221L / TimelineElementView 883L / TimelineTrack 184L + Ruler/Playhead/Toolbar/BookmarksRow/ContextMenu/DragLine/SnapIndicator/Tick/AudioWaveform/AudioVolumeLine + the 19-hook family — selection-journal/use-bookmark-drag/use-box-select included), pure React (zero `next/*` imports — the Next coupling lives only in `src/app/`), verified M17-M46 + real-mouse + pixel reads; the 10 controllers live core-side (`src/lib/timeline/controllers/`); **the gold sample = OT's `/view` runner page** (the miniature editor shell: library panel + compositor preview + timeline, deterministic fixtures, `window.__VIEW_TEST__` hooks). OT's `src/lib/timeline/view/` remains utils-only (scale/layout/zoom/pixel/ruler).
- nle-test-app @ `70e99f0` — **117/117**, tsc 0. Its `timeline-port/` fork (TimelineView 1,328L — diverged from OT's 1,221L by PORT-LOCAL injectable props only: the selection bridge pair `initialSelectedIds`/`onSelectionChange`/`selectionIds`, the zoom bridge `onZoomChange`, `confirmDelete`, `@vendor/timeline` import paths) **RETIRES per D25**: the props upstream into OT as optional injectable props, the app vendors the FULL tree (`vendor/nle-timeline/components/` + the `@/lib/timeline` alias — no import rewrites), the fork deletes file-by-file. The fork is ≥2 upstream waves behind (pre-F1): it silently misses OT's lock-track context-menu item, the P2-6 Y-geometry threading, the P2-8 memo-stable callbacks, the P2-9 Escape cancel-registration, and ElementView's `cancelRegistry`; it carries a port-only `EMPTY_CONTEXT_ITEMS` stability fix and a hardcoded `id="timeline-scroll"` (both upstream-able). The post-swap checklist: one new app pin per surfaced fork-missed delta.
- In-repo law register: `ui-mock/shell-mini/docs/OT-SEAMS.md` — the 14-row op map + the R22 drag-machinery retirement (the verbatim R18k law: clamp between same-track neighbors, single-edge live-field magnet, last-preview seal; the **D22 freeze is standing law — untouched by R23**; shell-mini 355 tests, sealed) + `CORE-SEAMS.md` (S1-S10, the timeline-domain seams — the K3 re-expression worklist; S2's geometry-law map) + `LAW-NET-INVENTORY.md` Part A (91 families / 249 tests — K3's acceptance list).
...
- Op-family variants slip/slide/roll/rateStretch + wave-2 retime/freezeFrame/rangeRemoval (**r1**, was W-ops; tests carried from nle-engine timeline.ts — acceptance: carried engine tests green).
- The composed ripple family (the mini's cutHead/cutTail/ripple-trim/quantize — 4 census units / 16 tests): **K3** composes app-side over `timeline.trim`/`rippleDelete` at crawl; **r1** graduates the OT interval-diff family (was the W-ops compose half — K4's demo requires it at crawl; acceptance: the LAW-NET Part A ripple rows authored app-side).
- C7 rename: **30** prefixed wire names (24 at the R15 charter; the family grew with the S1 transport trio + `track.setAllLocked` + `track.setAllMuted`) → bare spec-15 union, at **r1 END** (was W-ops END) with the one-business-day app migration + keymap sync sub-gate (acceptance: spec 15 §13.15 rows flip to ALIGNED).
- Error-code coarseness — spec 15 §6.3 amendment first, then OT follows (**r1**, was W-ops; acceptance: the ~24-code envelope). *Annotated R23: the F1B-4 boundary fix already classifies malformed JSON as INVALID_PARAMS (was INTERNAL_ERROR) — the classification half is COVERED by the OT BASE; the envelope's coarseness itself remains the r1 refinement.*
- Element-level `toggleElementMuted`/`toggleElementVisibility` **wire verbs** (spec 15 §4.1A row: engine ops exist — OT `timeline-core.ts` `toggleElementsMuted`/`toggleElementsVisibility`, W8-d/M40-pinned, routed today via `updateElements` patches; the dedicated wire pair is the r1 addition — was the A2/W-ops wire addition; acceptance: the two verbs on the wire + spec 15 §4.1A rows flip).
- Zoom-ladder config / ripple-toggle semantics exposure — the OT-side view-config surface (**the D25 bridge** — S-ot, the crawl window, was the C1 OT-side halves; S-ot worklist item 3; acceptance: OT exposes the config + the C1 law-subset rows hold-on-OT).
- Selection single-subject projection over multi-ref (**K3**, was C1, pre-registered — CORE-SEAMS S7; acceptance: app-level projection pinned).
- `onViewStateChange` on `TimelineViewProps` (**r1** additive, was W-ops additive; the D25 additive-optional-props discipline is the mechanism; acceptance: prop + view-state persistence tests).
- **The D25 bridge (S-ot/S-app joint, crawl — the single-tree migration):** (a) upstream the fork's PORT-LOCAL props as optional injectable props + `EMPTY_CONTEXT_ITEMS` as the OT bug-fix + the `timeline-scroll` anchor as a prop; (b) **the testid-emission convention** (a default-off `data-testid` prop family — OT emits ZERO testids today and K3's DOM-structural gate is the 59-static + 15-templated census); (c) the view-config surface (the zoom-ladder/ripple-toggle row above); (d) the **structural grammar half** at crawl-tail (the track-head column, minimized strip, compact/pill mode, tools-row deltas — additive OT components, classic defaults M-pinned) and the **token half at w1** (the mini theme mode on `globals.css` `:root` + `theme.ts`); (e) the app's vendor extension + fork retirement with the post-swap checklist (acceptance: the app vendors the full tree with zero port-local patches; fork deleted; one new app pin per surfaced delta; the `/view` gold sample at mini theme contingent on both grammar halves).

**ACCEPTANCE & TEST PLAN:** §15 below is this stream's battery; BASE acceptance = the cited suites at the cited pins (OT 489 — 359 in-page + 130 real-mouse across the 51 milestone suites; app 117) — the regression role. GAP acceptance is per-row above; facet rows live in spec 17 §13A. K3's DOM-structural acceptance (the testid census) is downstream of the D25 bridge's testid half (the ordering law).
...
Every file path below is absolute within its reference repo. **OC** = `/tmp/opencut-classic/apps/web/src/timeline/`; **OC-App** = `/tmp/opencut-classic/apps/web/src/app/editor/[project_id]/`; **OC-Actions** = `/tmp/opencut-classic/apps/web/src/actions/`; **FC** = `/tmp/freecut/src/features/timeline/`. **OT** = opencut-timeline (`github.com/bearachprema/opencut-timeline` @ the R23 pin `222532c`; the R8-era pin of this lineage was `d3b2163`; root `src/lib/timeline/` + `src/components/timeline/`) — the live clean-room OpenCut-classic timeline port, landed Round 8; it is the executable counterpart of OC's algorithmic core (types/placement/ripple/snapping/controllers/view-math) plus a spec-15-shaped headless API **plus the canonical React component tree (`src/components/timeline/` — 39 files / ~7,700 LOC, pure React, landed W4/W5, verified M17-M46 + real-mouse + pixel reads) and the `/view` runner page (the miniature editor shell + `window.__VIEW_TEST__` hooks — the gold sample per D25)**. Note: nle-engine has **no React timeline** (player harness only — see §16.4); it contributes the data model (timeline.ts) but zero UI code for this stream.
...
> opencut-timeline (`github.com/bearachprema/opencut-timeline` @ `0412e41` — Round 15 re-baseline [R15-era pin — superseded by the R23 pin `222532c`, 489/489 + the S/T/F1/S3-seam rounds + the D25 canonical-tree ruling; the LOC/test counts in this preamble are R15-era and kept for lineage], ~42k LOC total: ~9.7k engine core + ~3.2k controllers + ~7.4k React view (≈20.2k implementation) + ~21.6k test harness (in-page 12.4k + real-mouse scripts 6.5k + fixtures 2.0k), **423/423 tests (303 in-page + 120 real-mouse, re-run Round 15)** — the W8 classic-parity UI round + W9 full-repo review are landed and terminal, **zero open P1/P2** per its own HANDOFF, real-mouse + fuzz + review-regression suites included) is a clean-room OpenCut-classic timeline port **built with this spec set in hand** (its README cites Decision-2 types and the spec-15 wire protocol). Per Decision 12 it is the **EDITING-domain normative core**: the executable reference for this stream's entire scope — the OC tables in §16.1 describe the ORIGINAL; OT is the running, tested version of the same algorithms, and its **W8 classic-parity React components + controllers have landed and are terminal — and per D25 (R23) that tree (`src/components/timeline/`) is THE canonical React timeline UI tree, the app fork retired** (this spec's §4 component hierarchy and §14.4 controller contracts now have an executable implementation; the F1 hardening wave further pinned the controller/wire validation laws — F1A/F1B/F1C, M45/M46). Where OT code conflicts with this spec, **the spec wins** (the known deltas: prefixed headless command names (C7 — 30 types at the R23 pin; 24 at the R15 charter, the family grew with the S1 transport trio + `track.setAllLocked`/`track.setAllMuted`; worklist refreshed R15), 5-kind TrackType taxonomy, `TIMELINE_INDICATOR_LINE_WIDTH_PX = 2` vs OC's 3px — OC's verified read wins for the visual constant, pending an OT-side correction).
...
| Headless API | `headless/api.ts:41-150` | 30-type prefixed union (24 at the R15 charter; grown by the S1 transport trio, `track.setAllLocked`, `track.setAllMuted`); `CommandResult {ok, code…}`; `apply`/`applyBatch` atomic; the F1B boundary hardening (missing/null params → INVALID_PARAMS, no-overlap on span patches, clear:false never clears) | CORRECTIVE | C7 rename pass chartered (spec 19 §6; worklist refreshed R15 in spec 15 §13.15): `timeline.*`/`track.*` prefixes are NOT spec-15 shape |
