...
- opencut-timeline (OT) @ HEAD `ded43c4` (**code pin `c15a629`** — the last three commits are docs/runner-artifacts only; the src diff `c15a629..ded43c4` is empty) — **536/536** (386 in-page + 150 real-mouse, **59 milestone entries** — M1–M49 + M49C; live-run at the R24 module audit: 386 tests/41 entries + 150 tests/18 entries), tsc 0. This spec's interaction contract is executable there: the ops layer (split/trim/move/ripple/delete), snapping/placement/view-math, the headless API; the S-round landed (S1 transport policy — playbackRate/loopRegion/anti-drift/atomic boundary pause; S2 track lock; S3 transitionOut with the null=DELETE key law + split left-drops; S4 bookmark color/note + D-S5 typed ElementParams), the T-round (D-T3 `setTracksLocked` + `track.setAllLocked` lock-all batch; D-T7 move-NOOP; M44), **the S3 seam round (M46: `setTracksMuted` — the mute-all batch op, the setTracksLocked D-T3 twin — + the `track.setAllMuted` wire verb)**, and **the F1 whole-codebase hardening** (M45: F1A-1 box-select rows shift with expanded keyframe lanes; F1A-2/3/4 insert-on-new-track trims + heterogeneous-batch rejection + createTracks id validation; F1B-1..8 the wire/interaction validation laws — no-overlap enforced on update-patches, the dead-button + non-left-mousedown gates, INVALID_PARAMS for missing/null params, setLoopRegion clear:false never clears, negative trims rejected, failed applyBatch preserves the redo stack, keyframe flick commits; F1C-1 setTracksLocked unlock-direction; F1C-2/3/4 the runner + echo asserts), **the W10F M47 hardening** (`c5b5b24`: marquee expanded-lane row math, commit-before-notify, pair-scoped wire resolution, createTracks validation, non-array guards, stale-pair NOT_FOUND, playhead button family, marquee mask, element fps-null drop-clear, seek selection ordering) + M48 (`6bb01d3`), **the W11 complete-UI round (the landings this spec now rides)** — the **wire-dispatch seam** (DECISIONS #25: `useWireDispatch({core, fps})` → `HeadlessTimelineApi(fps, {core, onApply})`, ONE engine via attach; command verbs + gesture commits cross the wire, reads and view-state never route) with **24 UI-routed verbs + 6 documented exceptions** (routed: `timeline.{delete, duplicate, insert, move, moveBookmark, pause, play, redo, removeBookmark, rippleDelete, seek, setLoopRegion, setPlaybackRate, split, toggleBookmark, undo, updateElements}` + `track.{add, remove, setAllLocked, setAllMuted, toggleLock, toggleMute, toggleVisibility}`; exceptions: `timeline.selectElements` (selection is VIEW state), `timeline.{upsertKeyframe, removeKeyframe, retimeKeyframe}` (batch-gesture one-history-entry law), `timeline.advancePlayhead` (ticker is rAF-local), `timeline.trim` (uniform-delta; UI resize commits as updateElements patches)), **machine-checked by the M49C coverage gate** (`ca91223`: every verb minus the exception registry fires through the real UI, WIRE_COMMAND_TYPES read LIVE from the page, drift tsc-enforced both directions at the constant); the **engine widenings** (`56dcd8f`: the attach option (SA-5 fps/instance validation) + the dispatch recorder (`wireLog` ring cap 500 + unbounded `wireCoverage` Set) + `WIRE_COMMAND_TYPES` + `applyBatch` `data.results` + keyframe-verb `TRACK_LOCKED` via lockPreCheck); the transport/track-heads/gesture rounds (`b64ed5a` M49T, `56f8634` M49H, `fa0c478` M49R, `c9b11e6` M49G); and **the op-surface parity audit at HEAD** (`ded43c4`, `.agents/OP-COVERAGE.md`, vs opencut-classic @ `cf5e79e`): **11/11 NLE edit verbs** (insert, move, trim, split×3, delete, ripple-delete, duplicate, retime-math, snapping) each with engine op + wire verb + UI affordance + pins, with the documented extensions beyond classic (track lock + lock-all/mute-all, loop region, playback rate/JKL, ripple-delete first-class, save/load, the wire error surface).
- **OT's `src/components/timeline/` is THE canonical React timeline UI tree (D25.1 — the single-tree law, unchanged by D26):** **40 files / 8,604 LOC** (TimelineView **1,516L** / TimelineElementView 883L / TimelineTrack 184L + Ruler/Playhead/Toolbar/BookmarksRow/ContextMenu/DragLine/SnapIndicator/Tick/AudioWaveform/AudioVolumeLine + the **20-hook family** — selection-journal/use-bookmark-drag/use-box-select included, `use-wire-dispatch.ts` (90L) added W11-b), pure React (zero `next/*` imports — the Next coupling lives only in `src/app/`), verified M17-M49 + real-mouse + pixel reads — the tree now hosts the wire-dispatch UI layer (a structural fact the D25 row did not know; keyed R24); the 10 controllers live core-side (`src/lib/timeline/controllers/`); **the gold sample = OT's `/view` runner page** (the miniature editor shell: library panel + compositor preview + timeline, deterministic fixtures, `window.__VIEW_TEST__` hooks). OT's `src/lib/timeline/view/` remains utils-only (scale/layout/zoom/pixel/ruler).
- nle-test-app @ `c885ece` — **174/174** (static it/test census at the R24 module audit; the suite was not executed in that sandbox), tsc 0. Its `timeline-port/` is **the census-governed converging mirror of OT's component tree (D26 — the R23 "diverging fork, ≥2 waves behind" row is superseded)**: the port's 40 files = **39 mirrors of OT's 40** (`hooks/use-wire-dispatch.ts` absent — D30's W-C, in flight at this R24 baseline; **R25: W-C LANDED @ `64fb0ab`** — the port file exists, the census reached 35 zero-action + 5 carriers + host) + the app host `EngineMount.tsx` (306L, the integration layer); the 39 classify **7 byte-exact + 25 mechanical-exact (the `@/lib/timeline` ↔ `@vendor/timeline` import-specifier only) = 32 zero-action** + **7 documented carriers** (TimelineView, use-timeline-actions, use-keybindings, use-element-interaction, TimelineToolbar, TimelineContextMenu, use-playback-ticker — 5 pending W-C/W-D adoptions + 2 documented port-local keeps, each with its divergence rationale). The core stays the vendored lock-copy: `vendor/nle-timeline` @ `c15a629`, byte-exact minus `testing/` (14,912 LOC both sides, UPSTREAM.lock.json). D25.2's two-path vendor extension (`vendor/nle-timeline/components/` + the app-side `@/lib/timeline` alias) and the file-by-file fork deletion are **RETIRED as the prescribed mechanism** (never built; superseded by evidence): every upstream wave re-pins via 3-way merge (no-conflict files adopt-wholesale; carriers merge against documented zone maps with per-wave acceptance pins — the R8/R9 pattern); **fork retirement = the carrier count reaching the D30-declared irreducible carrier set** (the merge carriers OT's injection points cannot absorb + the 2 keeps + the host), tracked by the census, not by a file-deletion event. The R8/R9 waves already absorbed the R23 row's missed deltas (F1 / P2-6 / P2-8 / P2-9 / lock-menu — pinned app-side; the post-swap fork-missed-delta checklist is satisfied by construction).
- **The census register (D26.2 — the mirror-integrity instrument; this row is the spec-side register, REQUIRED):** the register declares the per-pin classification values — at `c15a629`: 40 port files = 39 mirrors (32 zero-action + 7 carriers) + the host; `use-wire-dispatch` pending W-C *(the R24-declared values; R25 re-read @ the app pin `64fb0ab`: W-C landed, the set is 35 zero-action + 5 carriers + host — the next register amendment carries the re-declared values)* — and lives app-side in a **named app doc** (`timeline-port/CENSUS.md` or the app's `.agents/` — NOT a point-in-time audit; verified absent R24 — the D27-F17 filing action, due before W-C). **Enforcement:** the census is recomputed app-side by a CI-gated script (the vendored pin vs the port tree, import-specifier-normalized — mechanically derivable) and **register-inequality fails the build**; the battery re-verifies at round cadence by the same recomputation (the counts are per-pin declared values, never hardcoded constants). **A carrier without a divergence rationale is a census violation; a census that drifts from the tree is a build failure.**
- In-repo law register: `ui-mock/shell-mini/docs/OT-SEAMS.md` — the 14-row op map + the R22 drag-machinery retirement (the verbatim R18k law: clamp between same-track neighbors, single-edge live-field magnet, last-preview seal; the **D22 freeze is standing law — untouched by R23/R24**; shell-mini 396 tests — the R23 seal 355 + the R24-mini nets (clip-box + mini-plus W0/W1)) + `CORE-SEAMS.md` (S1-S10, the timeline-domain seams — the K3 re-expression worklist; S2's geometry-law map) + `LAW-NET-INVENTORY.md` Part A (95 families / 264 tests — K3's acceptance list).
...
- C7 rename: the 30-name wire census at `c15a629` = **24 UI-routed + 6 documented exceptions** (the D29 re-base; lineage 24 at the R15 charter → 28 at the R22 pin → 30 flat at R23 → the routed/exceptions split; on any future verb landing the census re-declares mechanically per the tsc-lockstep law) → bare spec-15 union, at **r1 END** with the one-business-day app migration + keymap sync sub-gate (acceptance: spec 15 §13.15 rows flip to ALIGNED).
...
Every file path below is absolute within its reference repo. **OC** = `/tmp/opencut-classic/apps/web/src/timeline/`; **OC-App** = `/tmp/opencut-classic/apps/web/src/app/editor/[project_id]/`; **OC-Actions** = `/tmp/opencut-classic/apps/web/src/actions/`; **FC** = `/tmp/freecut/src/features/timeline/`. **OT** = opencut-timeline (`github.com/bearachprema/opencut-timeline` @ the R24 pin — HEAD `ded43c4` / **code pin `c15a629`**; the R23 pin was `222532c`; the R8-era pin of this lineage was `d3b2163`; root `src/lib/timeline/` + `src/components/timeline/`) — the live clean-room OpenCut-classic timeline port, landed Round 8; it is the executable counterpart of OC's algorithmic core (types/placement/ripple/snapping/controllers/view-math) plus a spec-15-shaped headless API **plus the canonical React component tree (`src/components/timeline/` — 40 files / 8,604 LOC, pure React, landed W4/W5, verified M17-M49 + real-mouse + pixel reads, hosting the W11 wire-dispatch layer) and the `/view` runner page (the miniature editor shell + `window.__VIEW_TEST__` hooks — the gold sample per D25; the app mirrors the tree census-governed per D26)**. Note: nle-engine has **no React timeline** (player harness only — see §16.4); it contributes the data model (timeline.ts) but zero UI code for this stream.
...
| Headless API | `headless/api.ts:41-150` (the W11 surface `:182-230`) | 30-type prefixed union — **24 UI-routed + 6 documented exceptions** (`WIRE_COMMAND_TYPES` `:182-213`, tsc-lockstep both directions; 24 at the R15 charter); `CommandResult {ok, code…}`; `apply`/`applyBatch` atomic; the F1B boundary hardening (missing/null params → INVALID_PARAMS, no-overlap on span patches, clear:false never clears); **the W11 widenings @ `c15a629` (BASE): the attach constructor option (SA-5 fps/instance validation), the dispatch recorder (`wireLog` ring cap 500 + unbounded `wireCoverage` Set + `wireReset`), `applyBatch` success `data.results`, keyframe-verb `TRACK_LOCKED` (lockPreCheck)** | CORRECTIVE | C7 rename pass chartered (spec 19 §6; worklist refreshed R15 in spec 15 §13.15): `timeline.*`/`track.*` prefixes are NOT spec-15 shape |
...
| replace (06 §5.9C) | NEW dedicated op `performReplaceEdit` (absent at the R25 engine pin — mode-replace G-R2); the transition law = `joinItems`' id-remap pattern `:7411-7427` (NOT `removeItems :3757`'s cascade-drop `:3775-3777`); the linked companion is SEVERED not deleted (`linked:false`); an unfillable source refuses `INVALID_PARAMS` | 1 (R25 — D31.4) | OT ops — single-op atomicity (D31A: the transition-remap + sever semantics are why replace is the one dedicated op of the four) |
| append (06 §5.9D) | NONE — placement-level composite, no engine op; OT's own primitives carry it: `insertElements` relative-offset batch (`timeline-core.ts:886-911` + the intra-batch guard `:922-929`, one `withUndo` history entry `:979-990`); append point = the target track's last-element end (per-track; `calculateTotalDuration` `ops/element-utils.ts:71-91` is the rejected whole-timeline alternative) | 1 (R25 — D31.5) | OT placement — the 6th `PlacementStrategy {type:'append'}` riding the already-endorsed r1 `insertBatch` composite; optional `timeline.append` wrapper (the rippleDelete pattern) |
| ripple-overwrite (06 §5.9E) | REFERENCE ONLY — no op to port: `rippleTrimItem :2839`'s private signed-shift machinery (push AND pull `:2891-2951`; `_propagateRemovedInterval :3475` / `_propagateInsertedGap :3534`) is the delta-law precedent, private/coupled in the engine | 1 (R25 — D31.6) | OT composite — `delete + move + insert` (the order law; one `applyBatch`, one history entry; the classic §12 diff produces the pull but never the push, so this is NOT "overwrite + the ripple flag") |
| fit-to-fill (06 §5.9F) | the pure speed law `calculateSpeed` (`core/timeline-math.ts:133`; the fixed-span derivation already inside `rateStretchItem :3404-3408`) + the 3-point family's optional target-span overload (`performOverwriteEdit :4860`); rate domain **[0.1, 5]** — out-of-domain REFUSES, never clamps | 1 (R25 — D31.7) | OT composite — insert + `updateElements{retime}` (both verbs landed + UI-routed; one `applyBatch`); a first-class verb is optional at r2 |
