...
- nle-test-app @ `876f2b8` — **377/377 (R30 re-key — the 12-file roof)** (the 8-suite roof: GluedShell 104 + audioService 42 + sceneBridge 41 + deliverService 20 + persistenceService 20 + wire-coverage 9 + engineSeam 7 + waveformPeaks 9), tsc 0. Its `timeline-port/` is **the census-governed converging mirror of OT's component tree (D26 — the R23 "diverging fork, ≥2 waves behind" row is superseded)**: the port's 42 files = **41 mirrors + host** (W-C LANDED; `use-wire-dispatch.ts` a zero-action mirror since W-C) + the app host `EngineMount.tsx` (the integration layer); the 41 classify **36 zero-action (6 byte-exact + 30 mechanical-exact — the `@/lib/timeline` ↔ `@vendor/timeline` import-specifier only)** + **5 carriers** (TimelineView, use-timeline-actions, use-keybindings, TimelineContextMenu, use-playback-ticker — use-element-interaction + TimelineToolbar converged out) + the host. The core stays the vendored lock-copy: `vendor/nle-timeline` @ `55c81c0` (byte-exact minus `testing/`, live-verified — the s17 re-pin LANDED @ `876f2b8`; was `6e2b91a`, the `970948a` re-pin the filed queue `c020b2a`; R30: the app @ `8f12cf9` — the K3 corpus round, the 12-file roof, the four re-expression maps). D25.2's two-path vendor extension (`vendor/nle-timeline/components/` + the app-side `@/lib/timeline` alias) and the file-by-file fork deletion are **RETIRED as the prescribed mechanism** (never built; superseded by evidence): every upstream wave re-pins via 3-way merge (no-conflict files adopt-wholesale; carriers merge against documented zone maps with per-wave acceptance pins — the R8/R9 pattern); **fork retirement = the carrier count reaching the D30-declared irreducible carrier set** (the merge carriers OT's injection points cannot absorb + the 2 keeps + the host), tracked by the census, not by a file-deletion event. The R8/R9 waves already absorbed the R23 row's missed deltas (F1 / P2-6 / P2-8 / P2-9 / lock-menu — pinned app-side; the post-swap fork-missed-delta checklist is satisfied by construction).
