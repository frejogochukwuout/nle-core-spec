...
- In-repo law register: `ui-mock/shell-mini/docs/OT-SEAMS.md` — the 14-row op map + the R22 drag-machinery retirement (the verbatim R18k law: clamp between same-track neighbors, single-edge live-field magnet, last-preview seal; the **D22 freeze is standing law — untouched by R23/R24**; shell-mini 495 in-flight (the R24-2 W6/W7 pending; the register re-keys at the WRAP) tests — the R23 seal 355 + the R24-mini nets (clip-box + mini-plus W0/W1/W2/W3 + fixes)) + `CORE-SEAMS.md` (S1-S10, the timeline-domain seams — the K3 re-expression worklist; S2's geometry-law map) + `LAW-NET-INVENTORY.md` Part A (104 families / 320 tests — K3's acceptance list).
...
| §11.2 in/out points | `src/lib/nle/timeline/timeline.ts:7850` | `setInPoint(frame: number): void {` | ALIGNED | In/out model present |
