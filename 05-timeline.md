...
**Status:** v-next (Round 24 — the R24 re-baseline + the D26 census ruling: OT @ HEAD `ded43c4` / **code pin `c15a629`** 536/536 is the accepted interaction-contract BASE **and its `src/components/timeline/` is THE canonical React timeline UI tree** — the app's `timeline-port/` is its **census-governed converging mirror** (D26: D25.2's two-path vendor-extension mechanism superseded; fork retirement = the D30-declared irreducible carrier set, tracked by the census); the gap is the r1 / carrier-reduction / K3 tails — `audits/ARCH-R24-timeline-strategy-and-topology.md`, D26/D29/D30. **R27:** the fleet re-pin + the census re-declaration — OT @ HEAD `55c81c0` / code pin `970948a` **632/632**, the wire census **31 = 28+3** (D-ARCH-6), the canonical tree **41 files / 9,176 LOC**, the app census **42 = 36+5+1 @ `6e2b91a`** (`c020b2a` 252/252, the 8-suite roof). Round 23: the R23 re-baseline + the D25 single-tree ruling (OT @ `222532c` 489/489 — the interaction-contract BASE; the app fork retiring via PORT-LOCAL props upstreaming — the mechanism superseded by D26, the canon unchanged). Round 22: the §0 forward inventory + the R22 re-baseline (OT @ `05584d8` 459/459 — the interaction-contract BASE at that pin); Round 23: the pins/counts re-verified, every spec-14 §4.1 row re-homed here, phase tags re-keyed per D24 — `audits/ARCH-R23-plan-and-bridge.md`, D23/D24/D25); Refined by sub-agent scout (SCOUT-05) — open questions answered with source code references. Round-8 amendments: §5.2 zoom reworded (implementable multiplier model), §8.3 drag contract notes + canonical move shape, §9 screen-space snap threshold, §14.5A magnetic zero-anchor (all absorbed from the opencut-timeline reference), §16.5 opencut-timeline code-reference table. **Round-15 amendments:** §11.1 markers per-scene + ruler-seeks (A2/N10), §11.2 InOutPoints superseded by setLoop halves (N12), §5.2 zoom-ladder mock registration (N13/C28), §16.5A projector clauses (ARCH-R15 §2.2) — per `.agents/SPEC-REVISION-CANDIDATES.md`. **Round-25 amendments:** §8A the Timeline Affordance Grammar (the visual-grammar register's STRUCTURAL half — ARCH-R25 D33; register proposal + evidence: `audits/fleet-r25/xcut-visual-grammar.md`; the theme half lands 18 §9, the cursor layering 18 §5A), §14.10's `trim-handles.tsx` era-qualification (P10 — the R15-era FreeCut census qualified; §8A's green ruling is the law), §16.5A's op-port table +4 r1 families (replace/append/ripple-overwrite/fit-to-fill — D31.4-D31.7/D31A), §0's GAP register re-keyed (the grammar-carrier row + the four families on the op-family row) — per `audits/ARCH-R25-edit-mode-completeness.md`
...
**Reference repos audited:** `/tmp/opencut-classic` (archived MIT), `/tmp/freecut` (MIT); **opencut-timeline** (github.com/bearachprema/opencut-timeline, landed Round 8) is the live executable code reference for this stream's algorithmic core **and — per D25/D26 — the canonical React component tree** (the components landed W4/W5 as M17-M21 and are verified M17-M49 + real-mouse; the app-side `timeline-port/` is its census-governed converging mirror); see spec 19 §3.2 and §16.5 below.
...
- opencut-timeline (OT) @ HEAD **`3e18722`** (**code pin `970948a`** — the trailing commits are docs/runner-artifacts only; the src diff is empty) — **632/632** (72 milestone entries — M1–M58 + the *R re-runs; **the total only** per the R27 counting convention (R4-ii) — the in-page/real-mouse split changed shape; the report json committed at `970948a` is the count authority), tsc 0. This spec's interaction contract is executable there: the ops layer (split/trim/move/ripple/delete), snapping/placement/view-math, the headless API; the S-round landed (S1 transport policy — playbackRate/loopRegion/anti-drift/atomic boundary pause; S2 track lock; S3 transitionOut with the null=DELETE key law + split left-drops; S4 bookmark color/note + D-S5 typed ElementParams), the T-round (D-T3 `setTracksLocked` + `track.setAllLocked` lock-all batch; D-T7 move-NOOP; M44), **the S3 seam round (M46: `setTracksMuted` — the mute-all batch op, the setTracksLocked D-T3 twin — + the `track.setAllMuted` wire verb)**, and **the F1 whole-codebase hardening** (M45: F1A-1 box-select rows shift with expanded keyframe lanes; F1A-2/3/4 insert-on-new-track trims + heterogeneous-batch rejection + createTracks id validation; F1B-1..8 the wire/interaction validation laws — no-overlap enforced on update-patches, the dead-button + non-left-mousedown gates, INVALID_PARAMS for missing/null params, setLoopRegion clear:false never clears, negative trims rejected, failed applyBatch preserves the redo stack, keyframe flick commits; F1C-1 setTracksLocked unlock-direction; F1C-2/3/4 the runner + echo asserts), **the W10F M47 hardening** (`c5b5b24`: marquee expanded-lane row math, commit-before-notify, pair-scoped wire resolution, createTracks validation, non-array guards, stale-pair NOT_FOUND, playhead button family, marquee mask, element fps-null drop-clear, seek selection ordering) + M48 (`6bb01d3`), **the W11 complete-UI round (the landings this spec now rides)** — the **wire-dispatch seam** (DECISIONS #25: `useWireDispatch({core, fps})` → `HeadlessTimelineApi(fps, {core, onApply})`, ONE engine via attach; command verbs + gesture commits cross the wire, reads and view-state never route) with **31 wire verbs = 28 UI-routed + 3 documented exceptions @ `970948a`** (routed: `timeline.{delete, duplicate, insert, insertBatch, move, moveBookmark, pause, play, redo, removeBookmark, removeKeyframes, retimeKeyframes, rippleDelete, seek, setLoopRegion, setPlaybackRate, split, toggleBookmark, undo, updateElements, upsertKeyframe}` (21) + `track.{add, remove, setAllLocked, setAllMuted, toggleLock, toggleMute, toggleVisibility}` (7); exceptions: `timeline.selectElements` (selection is VIEW state), `timeline.advancePlayhead` (ticker is rAF-local), `timeline.trim` (uniform-delta; UI resize commits as updateElements patches); lineage 24 → 28 → 30 → 24+6 → 25+5 → **31 = 28+3**; `upsertKeyframe` ROUTED (AR-2), `removeKeyframe`/`retimeKeyframe` RETIRED, `insertBatch`/`removeKeyframes`/`retimeKeyframes` LANDED one-history-entry-each — `api.ts:243-275`/`:328-335`), **machine-checked by the M49C coverage gate** (28/28 routed + 3 exceptions PASS @ `970948a`, origin-attributed — only origin:'ui' dispatches count, HA-4-2; the gate reads BOTH registries LIVE from the page — `WIRE_COMMAND_TYPES` + the EXPORTED `WIRE_UI_EXCEPTION_VERBS`, runner `:390-399`; drift tsc-enforced both directions at the constants); the **engine widenings** (`56dcd8f`: the attach option (SA-5 fps/instance validation) + the dispatch recorder (`wireLog` ring cap 500 + unbounded `wireCoverage` Set) + `WIRE_COMMAND_TYPES` + `applyBatch` `data.results` + keyframe-verb `TRACK_LOCKED` via lockPreCheck); the transport/track-heads/gesture rounds (`b64ed5a` M49T, `56f8634` M49H, `fa0c478` M49R, `c9b11e6` M49G); and **the op-surface parity audit at HEAD** (`ded43c4`, `.agents/OP-COVERAGE.md`, vs opencut-classic @ `cf5e79e`): **11/11 NLE edit verbs** (insert, move, trim, split×3, delete, ripple-delete, duplicate, retime-math, snapping) each with engine op + wire verb + UI affordance + pins, with the documented extensions beyond classic (track lock + lock-all/mute-all, loop region, playback rate/JKL, ripple-delete first-class, save/load, the wire error surface).
- **OT's `src/components/timeline/` is THE canonical React timeline UI tree (D25.1 — the single-tree law, unchanged by D26):** **41 files / 9,176 LOC** (TimelineView **1,569L** / TimelineElementView **950L** / TimelineTrack **193L** + Ruler/Playhead/Toolbar/BookmarksRow/ContextMenu/DragLine/SnapIndicator/Tick/AudioWaveform/AudioVolumeLine + the **22-file hook family** incl. `use-keyframe-authoring.ts` (**262L** — the AR-2 dblclick authoring seam, the first wire command routed from below TimelineView) — selection-journal/use-bookmark-drag/use-box-select included, `use-wire-dispatch.ts` (**95L**) added W11-b), pure React (zero `next/*` imports — the Next coupling lives only in `src/app/`), verified M17-M49 + real-mouse + pixel reads — the tree now hosts the wire-dispatch UI layer (a structural fact the D25 row did not know; keyed R24); the controllers dir = **11 files** core-side (8 `*-controller.ts` + drop-target + drag-source + event-types; `src/lib/timeline/controllers/`); **the gold sample = OT's `/view` runner page** (the miniature editor shell: library panel + compositor preview + timeline, deterministic fixtures, `window.__VIEW_TEST__` hooks). OT's `src/lib/timeline/view/` remains utils-only (scale/layout/zoom/pixel/ruler).
- nle-test-app @ `7664603` — **426/426 across 22 files, tsc 0** (R33, engine seal-round register P2-9 — the counts/roof re-key the re-pin flips left undone; the layered record: 252/9 R27 `c020b2a` → 256/13 R29 `876f2b8` → 377/26 12-file R30 `8f12cf9` → 418/418 21-file R31 `f58147c` per 16:15 → 422/422 22-file R32-W3-a `b5fe3b3` → 426/426 22-file R32-final `ce7cfc3`/`7664603`; wire-coverage 26). The stale R27 8-suite enumeration ("GluedShell 104 + … wire-coverage 9 …" — sums to 252) that rode under the row's own R30 headline is DROPPED — exactly the LIVE-stale class `scripts/rekey_r31.py`'s own STALE_PATTERNS flags (the R30 count figure, its app pin, the roof phrase), but 05 was never in the sweep's edit set. Its `timeline-port/` is **the census-governed converging mirror of OT's component tree (D26 — the R23 "diverging fork, ≥2 waves behind" row is superseded)**: the port's 43 files = **32 zero-action mirrors (6 byte-exact + 26 mechanical-exact — the `@/lib/timeline` ↔ `@vendor/timeline` import-specifier only) + 9 documented carriers + 2 app-hosts** — EngineMount the integration layer + `doc-committing-sessions.ts` the R32-W3-a session-registry host; the carriers: TimelineView, use-timeline-actions, use-keybindings, TimelineContextMenu, use-playback-ticker, TimelineElementView + TimelineToolbar (the D26.5 pair, OT-absorb pending), AudioVolumeLine + use-keyframe-drag (the R32-W3-a registry joins) — use-element-interaction converged out, `use-wire-dispatch.ts` a zero-action mirror since W-C; the census re-declared unchanged at the `17a19f8` re-pin (`docs/port-census.md`, the R32-W3-a registry round). The core stays the vendored lock-copy: `vendor/nle-timeline` @ `17a19f8` (the lib-tree mirror, byte-exact minus `testing/`, live-verified; the components-tree census reference `vendor/nle-timeline-ui` sits at the SAME pin — the wire surface and the UI surface are one pin — and the engine submodule `vendor/nle-engine` rides `574d8d3`; the layered pin record: `55c81c0` the s17 re-pin LANDED @ `876f2b8` [was `6e2b91a`; the `970948a` re-pin the filed queue `c020b2a`; R30: the app @ `8f12cf9` — the K3 corpus round, the 12-file roof, the four re-expression maps] → `39003d3` the D25.3a mini-theme re-pin → `17a19f8` the s18 lockstep re-pin @ `7664603` — the D42 linkage + D43 edit-domains + D44 ERROR_CODES lib-tree wave, the components tree byte-unchanged, the census classes unchanged). D25.2's two-path vendor extension (`vendor/nle-timeline/components/` + the app-side `@/lib/timeline` alias) and the file-by-file fork deletion are **RETIRED as the prescribed mechanism** (never built; superseded by evidence): every upstream wave re-pins via 3-way merge (no-conflict files adopt-wholesale; carriers merge against documented zone maps with per-wave acceptance pins — the R8/R9 pattern); **fork retirement = the carrier count reaching the D30-declared irreducible carrier set** (the merge carriers OT's injection points cannot absorb + the 2 keeps + the host), tracked by the census, not by a file-deletion event. The R8/R9 waves already absorbed the R23 row's missed deltas (F1 / P2-6 / P2-8 / P2-9 / lock-menu — pinned app-side; the post-swap fork-missed-delta checklist is satisfied by construction).
- **The census register (D26.2 — the mirror-integrity instrument; this row is the spec-side register, REQUIRED):** the register declares the per-pin classification values — at `55c81c0`: **42 port files = 36 zero-action (6 byte + 30 mechanical) + 5 carriers + 1 host** (the R27 re-declared values, re-read at the R29 mirror re-pin; W-C landed, `use-wire-dispatch` a zero-action mirror since) — and lives app-side in a **named app doc** (`docs/port-census.md` — the D27-F17 named doc EXISTS, CI-gated). **Enforcement:** the census is recomputed app-side by a CI-gated script (the vendored pin vs the port tree, import-specifier-normalized — mechanically derivable) and **register-inequality fails the build**; the battery re-verifies at round cadence by the same recomputation (the counts are per-pin declared values, never hardcoded constants). **A carrier without a divergence rationale is a census violation; a census that drifts from the tree is a build failure.**
- In-repo law register: `ui-mock/shell-mini/docs/OT-SEAMS.md` — the 14-row op map + the R22 drag-machinery retirement (the verbatim R18k law: clamp between same-track neighbors, single-edge live-field magnet, last-preview seal; the **D22 freeze is standing law — untouched by R23/R24**; shell-mini 495 SEALED across 12 files / 157 families → 163 census units (the register's own R30 note: "the standing counts are UNCHANGED — the mini's corpus is sealed"; the in-flight/W6-W7-pending framing retired: R33, engine seal-round register P3-23) — the R23 seal 355 + the R24-mini nets (clip-box + mini-plus W0/W1/W2/W3 + fixes)) + `CORE-SEAMS.md` (S1-S10, the timeline-domain seams — the K3 re-expression worklist; S2's geometry-law map) + `LAW-NET-INVENTORY.md` Part A (**105 families / 339 tests** — K3's acceptance list; re-keyed from the pre-W3 104/320 reading: R33, engine seal-round register P3-23).

**GAP (the work — owner + phase per the D24 ladder; this register IS the editing-domain register: spec 14 §4.1's rows were re-homed HERE at R23, the posture law):**
- Op-family variants slip/slide/roll/rateStretch + wave-2 retime/freezeFrame/rangeRemoval + the four R25 families (replace = the dedicated op; append / ripple-overwrite / fit-to-fill = composites per D31A's dedicated-vs-composite criterion — §16.5A's port table carries the engine-side notes) (**r1, RULED-IN — R28 D46** (ARCH-R28 §4: the r1-entry absent-family scope gate closed, all four families enter r1; the registered mid-flight deferral lever + reversal protocol live in the ruling — on a fired cut this row is one of the ~10-11 corpus-wide re-tag sites, with §16.5A's four wave cells); tests carried from nle-engine timeline.ts — acceptance: carried engine tests green).
- The §8A affordance-grammar carrier (the register's rows → OT's canonical `src/components/timeline/` tree, riding the wave-1 op ports — D33.5's sequencing law: the register landed BEFORE the ports so the grammar does not fork a fifth time; the variants' insert-preview family + the mini's testid census are the reference instruments) (**r1**; acceptance: per-row computed-style/geometry pins — dual-edge presence, per-mode dim participants, ghost width == the row's law — the xcut report's VG-6/VG-7).
- The composed ripple family (the mini's cutHead/cutTail/ripple-trim/quantize — 4 census units / 16 tests): **K3** composes app-side over `timeline.trim`/`rippleDelete` at crawl; **r1** graduates the OT interval-diff family (K4's demo requires it at crawl; acceptance: the LAW-NET Part A ripple rows authored app-side).
- C7 rename: the 31-verb wire census @ `970948a` = **28 routed + 3 exceptions** (lineage: 24 at the R15 charter → 28 at the R22 pin → 30 flat at R23 → the routed/exceptions split at `c15a629` → 25+5 at AR-2 → **31 = 28+3 at D-ARCH-6**; the D29-F8 clause fired — re-declared mechanically per the tsc-lockstep law) → bare spec-15 union, at **r1 END** with the one-business-day app migration + keymap sync sub-gate (acceptance: spec 15 §13.15 rows flip to ALIGNED).
- Error-code coarseness — spec 15 §6.3 amendment first, then OT follows (**r1**; acceptance: the **LANDED 25-code two-tier envelope** — 15 §6.3's class-tag registry: the 24 + `RATE_OUT_OF_DOMAIN`, every code tagged exactly one class (R28/D44); the r1 refinement is the OT follow). *Annotated R24: the classification half is COVERED and W11-widened by the OT BASE — F1B-4 (malformed JSON → INVALID_PARAMS), the keyframe-verb `TRACK_LOCKED` (lockPreCheck), and the UI's NOOP-benign error classification (DECISIONS #25 ruling 6); the envelope's coarseness itself remains the r1 refinement.*
- Element-level `toggleElementMuted`/`toggleElementVisibility` **wire verbs** (spec 15 §4.1A row: engine ops exist — OT `timeline-core.ts` `toggleElementsMuted`/`toggleElementsVisibility`, W8-d/M40-pinned, routed today via `updateElements` patches; the dedicated wire pair is the r1 addition; acceptance: the two verbs on the wire + spec 15 §4.1A rows flip).
- Zoom-ladder config / ripple-toggle semantics exposure — the OT-side view-config surface (**S-ot**, the crawl window — spec 14 §4.1's re-homed C1 OT-side halves; adjacent to, but not carried by, the carrier-reduction work order; acceptance: OT exposes the config + the C1 law-subset rows hold-on-OT. Re-verified absent at `c15a629` and unchanged at the s18 pin `17a19f8` (the anchor re-keyed `:237`→`:240`: R33, engine seal-round register P3-22): `rippleMode` is internal `useState` (TimelineView.tsx:240) threaded to the toolbar — no shell exposure; zoom is a continuous clamped value, the hook takes only `minZoom` — no ladder config).
- Selection single-subject projection over multi-ref (**K3**, pre-registered — CORE-SEAMS S7; acceptance: app-level projection pinned).
- `onViewStateChange` on `TimelineViewProps` (**r1** additive; the additive-optional-props discipline is the mechanism; re-verified absent at `c15a629` — it exists only as an internal `useTimelineZoom` option, not on the view; acceptance: prop + view-state persistence tests).
- **The carrier-reduction program (D26.3 — supersedes the R23 D25-bridge row; ONE S-ot work order, the crawl window; it gates NOTHING — K3's halves are gated by the coverage vocabulary (D29), not by the props):** the props list — the selection-bridge pair (`initialSelectedIds`/`onSelectionChange`/`selectionIds`), the zoom bridge (`onZoomChange`), `confirmDelete`, the `timeline-scroll` anchor, `EMPTY_CONTEXT_ITEMS` (the stability bug-fix), `cancelRegistry` injection — **verified ZERO upstreamed at `c15a629`** (TimelineViewProps = core/fps/snappingEnabled/initialZoom/isShiftHeld/dragSource/mediaAssets/mediaLookup/`wire`/onSaveScene/onLoadScene — no selection/zoom bridges, no `confirmDelete`, no scroll anchor, no `EMPTY_CONTEXT_ITEMS`; `cancelRegistry` minted internally at TimelineView.tsx:762-764 — the anchor re-keyed `:715-719`→`:762-764`, the props list re-verified unchanged at the s18 pin `17a19f8`: R33, engine seal-round register P3-22). The app instead rides **OT's own W11 injection points** (the `wire` prop — the page-owns-the-wire pattern, D30 R3 uses it verbatim via EngineMount — and `onSaveScene`/`onLoadScene`); each OT landing flips a carrier toward exact. D30's W-G R10(h)-(j) asks stay the app's SEPARATE list (cross-referenced, never duplicated). **The DOM half re-points per D26.5:** OT's frozen convention is `data-test=` (69 occurrences / **59 unique names** in `src/components/`, ZERO `data-testid` — the +1 since the freeze is a SECOND `timeline-bookmark` selector REFERENCE in `hooks/use-keybindings.ts`, not a DOM site; unique-names basis per the R27 counting convention, R4-iii, stated once in 19 §10) — the census gap is coverage-of-existing-vocabulary, not a missing vocabulary; K3's DOM-structural gate targets the three-way map (OT `data-test` / nle-ui's `shell-*` testid family / the no-counterpart structural units: the track-head column, the minimized strip, the compact mode), with the mini's 85-app-emitted-static + 30-templated-families census getting the mapping row (re-censused R33, engine seal-round register P3-25 — the 60-static [59 app-emitted + the story-only harness] / 15-templated basis was the PRE-MINIPLUS census; all 59 declared ids re-verified present, the presence gate green). The **structural grammar half** (crawl-tail: the track-head column, minimized strip, compact/pill mode, tools-row deltas — additive OT components, classic defaults M-pinned) and the **token half at w1** (the mini theme mode on `globals.css` `:root` + `theme.ts` — fixed dark only today) stand unchanged.

**ACCEPTANCE & TEST PLAN:** §15 below is this stream's battery; BASE acceptance = the cited suites at the cited pins (OT 536 — 386 in-page + 150 real-mouse across the 59 milestone entries; app 174) — the regression role. GAP acceptance is per-row above; facet rows live in spec 17 §13A. K3's DOM-structural acceptance (the three-way data-test census) gates on the D26.5 mapping row + D30's W-C (the re-pointed ordering law — not a swap+testid layer); K3's routed-verb completeness machine-check is the app's D30 W-F coverage-gate port (D29.1b).

---

## 1. Purpose

Define the timeline UI: how tracks and clips are rendered, how the user interacts with them (drag, trim, scrub), and how the UI stays fast with hundreds of clips. This stream is mostly UI; the underlying NLE op logic is in `06-nle-ops.md`. The DOM-timeline patterns below (component hierarchy, controllers, placement, snapping) have a live executable port — **opencut-timeline** (landed Round 8; its React component tree landed W4/W5 and, per D25, is THE canonical tree — see §0 and §16.5; spec 19 §3.2) — cite it (OpenCut-classic remains the citation of record for the ORIGINAL constants/patterns OT normalized — the §16.1/§16.2 tables), not the monorepo, when implementing.
...
**(R33, engine seal-round register P2-8 — the per-track waveform visibility law, missing since R27):** the rendered lane reads BOTH view flags — the per-track doc flag (`track.waveform`, written by the ViewOptionsPopover/TrackHeader per 18 §4.7's row) AND the page's view gate (`pageTimelineView.waveforms`) — and a false on either renders the **flat gradient lane** (the clip's audio gradient + name, NO strip — never an empty canvas; undefined/true keep the strip): `waveformOn = track.waveform !== false && pageWaveforms` (the page gate shell-variants `Clip.tsx:475-479`; the flag→render gate + flat-lane law `:1188-1193`). The popover's ON path converges the per-track doc flags through **`setAllTrackWaveforms` = ONE `withHistory` write** setting every audio track's flag to the target (a converged no-change call mints NOTHING — the canonical no-op law; `state/useUiStore.ts:916/:973`); the OFF path writes the page gate ALONE — converging the doc flags to false would leak the off into every other page (the doc is global; the per-page restore law). Owning chrome: 18 §4.7's ViewOptionsPopover row (the register's view-state row). [R27 P2-7]
...
> **Re-point (R33, engine seal-round register P2-7):** the four-tool listing above is the **pre-R15 seed shape** — the LIVE tool-mode enum is **18 §4.5's nine-tool radio** (`18:210`: Select (V), Razor/Blade (B), Roll (T), Ripple (R), Slip (Y), Slide (U), Rate-stretch, Hand (H), Zoom (Z) — rate-stretch toolbar-only), carried on the wire as 15 §4.3.45's `selectTool` enum (`'select' | 'razor' | 'ripple' | 'slip' | 'slide' | 'roll' | 'rate-stretch' | 'hand' | 'zoom'`) and keyed per 16 §3.2 (the T/Y/U rows at `16:180-182`). The seed block + behavior table below stay as the recorded seed contract; §8A's keying clause ("the tool-mode enum §8.1/§4") resolves to the nine-tool radio — as written it keyed itself to an enum that lacked its own trim tools (roll/ripple/slip/slide). [R27 P2-4]
...
| `H` | Hand tool |
| `Z` | Zoom tool |
| `T` | Roll (trim) tool — the rollover edit between adjacent clips (`selectTool {tool:'roll'}`) |
| `Y` | Slip tool (`selectTool {tool:'slip'}`) |
| `U` | Slide tool (`selectTool {tool:'slide'}`) |
...
*(R33, engine seal-round register P2-7: the `T`/`Y`/`U` rows above added — 16 §3.2's tool keys at `16:180-182`; §8.1's seed enum lacked its own tools.)*
...
**The reference figures — the pinned grammar (D33.2, the green ruling):** `ui-mock/trim_edit_modes.html` (4 views: roll/ripple/slip/slide) and `ui-mock/timeline_edit_modes (2).html` (6 views: the source-edit family). **The mocks' GREEN trim grammar is the pinned reference**, over the FreeCut-era amber census quoted at §14.10 (era-qualified there per ARCH-R25 P10 — its files are not in any current checkout; the amber palette is recorded as the historical FreeCut alternative). Where a mock renders a didactic composite (both outcomes at once), the row below carries the single implementable reading (D33.4).

**Standing laws (every row rides them):**

1. **Hit zone ≠ visible band.** The register states the gesture hit zone and the visible affordance separately (the four-grammar divergence the census found). Carriers today: 8px ±4px OUTSIDE the edge (shell-variants testids `clip-trim-l-${el.id}` / `clip-trim-r-${el.id}` — `Clip.tsx:1642`/`:1662`, the fourth line-pin drift; the citation re-keyed to the drift-proof testids: R33, engine seal-round register P2-6) / 20px (shell-mini `timeline.css:810-824`, 10px in pill mode) — the carrier sizes the hit zone ≥ the visible band and may widen it for WCAG 2.5.8 target enhancement (the mini's PR69 C11 precedent).
2. **The z-order stack:** hit zones z-4 < clips z-5 < edge bands + slot frames z-6 < white boxes z-8 < preview frames z-9 < active clip z-10 < in-box arrows z-11 < cursor glyphs z-12 < free arrows z-20.
3. **Preview == commit.** The gesture planner computes the overlay geometry and the commit from the ONE plan (the shell-variants' law for the insert family — `insertPlan.ts` outputs the ghost/arrows/overwrite spans; never independently drawn); adopted here as the register's implementation contract.
4. **Refusal renders no geometry.** An honest `INVALID_PARAMS`-class refusal draws no ghost and no arrows — the `not-allowed` cursor (18 §5A) is the refusal's only presentation.

**The register** (row form per D33.3 — `element | states | geometry | token refs | owning spec | reference impl`):

| element | states | geometry | token refs | owning spec | reference impl |
|---|---|---|---|---|---|
| `trim-edge-crisp` | live while a trim gesture is armed on the edge. **Roll = BOTH facing edges of the junction pair; ripple = the ONE active edge; roll renders no dim and no owner** (the affordance sits on the junction, not on a clip). Never rendered for slip/slide (they render `trim-glow-soft`) | visible band flush to the clip edge — 18px left / 21px right — carrying a 9px box-shadow halo; z-6, `pointer-events:none`; hit zone stated separately (standing law 1) | `--trim-edge-active` + its ramp | 05 §8A | `ui-mock/trim_edit_modes.html` view-roll `:469`/`:474`, view-ripple `:529`; CSS `:226-245` |
| `trim-glow-soft` | slip only; BOTH the in and out edges of the active clip (the dual-edge slip law — the single-edge variant is the FreeCut census's, not this register's) | 15px wide, `blur(3px)`; height `calc(100% − 32px)` — **the title bar is excluded** (the title's 26px start-pad clears the glow); z-6, `pointer-events:none` | `--trim-glow-soft` | 05 §8A | `ui-mock/trim_edit_modes.html` view-slip `:599-600`; CSS `:247-264` |
| `inactive-clip-dim` | preview-only, per-mode participant set: **ripple = the downstream set; slip = the neighbor(s); slide = BOTH neighbors; roll = none** (the no-owner law). The source-edit family carries its own variant: all inactive clips dim while the active source clip takes `source-active` | full-body image overlay (inset 0) over the clip body, z-3, + the dimmed title treatment; the source-edit mock's variant is a uniform body overlay on every non-active clip | `--clip-dim-overlay` / `--clip-dim-border` / `--clip-dim-title` | 05 §8A | `ui-mock/trim_edit_modes.html` view-ripple `:531`, view-slip `:581`, view-slide `:651`/`:658`, CSS `:212-221`; `ui-mock/timeline_edit_modes (2).html` CSS `:162-168` (the source-edit variant) |
| `clip-active-edit-border` | the gesture's ACTIVE clip only — slip and slide (never roll/ripple: the roll junction has no owner, ripple's dim already marks the downstream set) | 3px solid border, radius 9, z-10 (above boxes, glows, and dim) | `--clip-active-edit` | 05 §8A | `ui-mock/trim_edit_modes.html` view-slip `:596`, view-slide `:662`; CSS `:223-224` |
| `white-box-family` | **one class, two semantics** (the mock's own CSS comment): slip = the source-duration outline around the FROZEN slot; slide = per-neighbor shrink boxes. **D33.4's composite ruling** (the mock's twin-box view is didactic — one real Δ shrinks one side only): the CEDING neighbor's box shrinks toward the shared edge, the GAINING neighbor's grows; **one box live per gesture side** | 4px solid outline, radius 10 (slip) / 9 (slide), z-8, `pointer-events:none`. Geometry sources: slip = the source extent `[0, sourceDuration]` mapped at pxPerSec around the fixed slot (the reference draws a ≈2.8× slot outline around the frozen clip); slide = the neighbor-Δ regions abutting the active clip's edges, lower-anchored on the clip's bottom band | — (no token minted; plain foreground contrast) | 05 §8A | `ui-mock/trim_edit_modes.html` view-slip `:590`, view-slide `:668-669`; CSS `:268-276`; named geometry owner (REF-only): §18 census rows 19-20 — `tool-operation-overlay-utils.ts`'s `getSlipOperationBoundsVisual` |
| `in-point-preview-frame` | slip only; BRIGHT (the dim law belongs to the neighbor, not this frame); shows the slipped clip's OWN source at the new in-point (the reference paints the neighbor's art — an authoring artifact, corrected by this row per mode-slip's refinement) | inset within the white box's head region (reference: 148×123 at +4/+4 inside the 440×131 outline), z-9, `overflow:hidden` | — (bright by contrast with the dim law) | 05 §8A | `ui-mock/trim_edit_modes.html` view-slip `:592-594`; CSS `:278-283` |
| `cursor-glyph-ladder` | **the layered ruling:** CSS hover cursors remain the hover/hit law (18 §5A's 16-row table — `ew-resize` over edges and cut points); the SVG bracket-glyph family renders ONLY inside an active-gesture preview, never on hover. Per-mode glyph: roll = brackets + outward arrowheads; ripple = clip-rect + bracket + right arrow; slip = brackets + two arrows INSIDE (diverging); slide = arrows OUTSIDE the brackets and **sitting LOW over the title bar** — the slip-high/slide-low glyph posture is the visual encoding of §14.11's pointer-position slip/slide detection | glyph z-12, drop-shadowed; reference sizes 74-88px | — (glyph strokes ride plain foreground contrast) | 05 §8A (the glyph layer) + 18 §5A (the CSS layer + the layered ruling) | `ui-mock/trim_edit_modes.html` `:478-483` (roll), `:537-543` (ripple), `:604-609` (slip), `:672-677` (slide); CSS `:286-290` |
| `arrow-family` | the direction state machine: **two-way ⇔ roll; push-right ⇔ the plan displaces downstream (ripple-trim, insert, ripple-overwrite — Δ≠0; the `r20/insert-modes.md:125` law promoted: the right arrow is shown ONLY when downstream moves); diverging pair inside the outline ⇔ slip (headroom/tailroom indicators, D33.4 — NOT slip direction); outward pair inside the shrink boxes ⇔ slide; down-arrow ⇔ every source-edit mode, GHOST-CENTERED (never playhead-anchored). The pull ruling (D33.4): when Δ<0 (ripple pull) the arrow INVERTS to left-pointing** (the reference never draws the pull case — a stated inference) | free arrows z-20; in-box arrows z-11. Reference sizes: roll two-way 136×30 BELOW the clip band; ripple push 68×24 below; source-edit down 16×28, right 28×16 at the displaced span's leading edge | `--overlay-arrow` | 05 §8A | `ui-mock/trim_edit_modes.html` `:486-490` (roll), `:546-549` (ripple), `:612-619` (slip), `:680-687` (slide); `ui-mock/timeline_edit_modes (2).html` `:453-459` + view-insert `:270-275` / view-ripple `:327-332`; interim carrier: shell-variants testids `insert-preview-arrow-down` (`Timeline.tsx:1885`) / `insert-preview-arrow-right-${elId}` (`:1899` — the fourth line-pin drift; re-keyed to the drift-proof testids: R33, engine seal-round register P2-6) + `insertPlan.ts:498` (right-arrow-off for overwrite, test-pinned) |
| `ghost` | the incoming-clip preview, all six source-edit modes: `hover-armed (≥150ms dwell) → ok:true / ok:false (refusal — no geometry, standing law 4) → commit-cleared` | 2px dashed outline, radius 4; z-4 normally, **z-8 when overlaying covered clips** (overwrite/ripple-overwrite — the covered clips render under it). **The width laws (the asymmetric-ghost tell):** replace ghost width == the TARGET's width (the exact-length swap); ripple-overwrite ghost width == the SOURCE's width (the different-length replacement tell); insert/overwrite/append/fit-to-fill ghost == the placed span | `--ghost-outline` | 05 §8A | `ui-mock/timeline_edit_modes (2).html` view-insert `:265-266`, view-overwrite `:279`, view-replace `:305`, view-append `:316`, view-ripple `:325`; interim carrier: shell-variants testid `insert-preview-ghost` (`Timeline.tsx:1763` — the fourth line-pin drift; re-keyed to the drift-proof testid: R33, engine seal-round register P2-6. `insertPlan.ts:83`/`:306` still exact; preview==commit pinned `insertPlan.test.ts:514-576`) |
| `fit-slot` | fit-to-fill only; the selected I/O span (the target the source will be retimed into) | 2px dashed outline at the I/O span (reference 105×110), z-6; anatomy: a brightness-dimmed source frame (the slot image) + a 26px slot-title bar + the speed badge | `--ghost-outline` (the dashed frame) | 05 §8A | `ui-mock/timeline_edit_modes (2).html` view-fitfill `:698-708`; CSS `:343-376` |
| `speed-badge` | fit-to-fill; rendered on the fit-slot AND on the source pool clip — **the "1.7x" vs "1x" contrast IS the affordance** (slot badge = the computed rate; source badge = identity) | gauge glyph (arc + hand, 14×12) + label; chip radius 10, inset bottom 34px / left 9px. **Precision law: ONE decimal** ("1.7x") — the shell-variants' two-decimal `toFixed(2)` is the registered divergence this row corrects | `--badge-chip-{bg,border,text}` | 05 §8A | `ui-mock/timeline_edit_modes (2).html` `:700-716` (slot "1.7x" + source "1x"); CSS `:378-395`; interim carrier: shell-variants testid `insert-preview-speed-badge` (`Timeline.tsx:1801` — the fourth line-pin drift; re-keyed to the drift-proof testid: R33, engine seal-round register P2-6. The gauge `editModeIcons.tsx:141`) |
| `source-active` | the active source clip in all six source-edit modes (the pool/source side of the gesture — distinct from the timeline-side `clip-active-edit-border`) | 2px active border, radius 6, z-10, elevated drop shadow | `--source-active-border` | 05 §8A | `ui-mock/timeline_edit_modes (2).html` CSS `:207-218` (the active-clip treatment) |

**Close-out rulings:**

- **The composite rulings (D33.4) live in the rows:** slide's twin shrink boxes are didactic — one real Δ shrinks one side only (the ceding neighbor's box shrinks toward the shared edge, the gaining neighbor's grows; one box live per gesture side — `white-box-family`); slip's diverging arrows are headroom/tailroom range indicators, not slip direction (`arrow-family`); the pull case inverts the ripple arrow (`arrow-family`); and view-ripple `:531`'s dimmed neighbor + push arrow is the didactic push+pull composite — the mock's one static frame teaches both directions ("pushed down the timeline or pulled in"), where a real ripple-trim renders ONE outcome per gesture: the downstream set dims (`inactive-clip-dim`) and the arrow carries the sign of Δ — push-right for Δ>0, the left-inversion for Δ<0 (`arrow-family`) (R33, engine seal-round register P3-23).
- **The sequencing law (D33.5):** this register is law BEFORE the r1 wave-1 op ports (roll/slip/slide + the source-edit family) land UIs — the grammar otherwise forks a fifth time (after FreeCut, the variants, the mini, and OT's tree). The carrier is OT's canonical `src/components/timeline/` tree (D25.1) at the port wave, with per-row computed-style/geometry pins (the xcut report's VG-6/VG-7 acceptance: dual-edge presence, per-mode dim participants, ghost width == the row's law); §0's GAP register carries the carrier row. **No production repo implements this register today** — the census's finding; the variants' insert-preview subset (ghost/badge/arrows, preview==commit, test-pinned) is the only tested implementation and is cited per row as the interim carrier.
- **The tool radio (the grammar's shell half)** stays 18 §4.5's (fully stated there; 16 §3.2's tool keys) — this register owns only the in-canvas overlay family.

**The ghost preview-visibility sub-laws (R33, engine seal-round register P2-6 — the `ghost` row's riders, the shell-variants' preview-visibility law `Timeline.tsx:1233-1256` + the effect `:1261-1322`; this spec carried zero `scrollIntoView` hits before this block — the census's gap):**

1. **Auto-scroll, one rAF AFTER PAINT.** While an armed preview plan holds, the ghost span is scrolled into `#timeline-scroll`'s view by `scrollIntoView({inline:'nearest', block:'nearest'})` issued inside a `requestAnimationFrame` after paint (`previewGhostRef` `Timeline.tsx:1257`, the scroll at `:1299`).
2. **The straddler's SPLIT ghost joins the scroll target.** The insert straddler's split ghost (the dashed right half, potentially far past the main ghost) is always at/after the main ghost's end by construction, so a 'nearest' scroll after the main ghost only ever moves RIGHT — the union span becomes visible, never a back-slide (`previewSplitGhostRef` `:1259`, the scroll at `:1304`; the geometry home `insertPlan.ts:97`/`:449`; the union-wider-than-viewport edge sets `scrollLeft` ONCE with the insertion head pinned at the viewport's LEFT edge, `:1287-1297`).
3. **The 24px zoom floor.** A ghost narrower than `PREVIEW_MIN_SPAN_PX` (24 — `Timeline.tsx:527`) at the current pps bumps zoom through the bus (`targetPps = 24/dur`) so it renders ≥ 24px — **ONE bump per ARM** (arm key = `mediaId:mode`; the plan's identity re-mints on every playhead/loop/selection change, and a re-mint must never become a continuous zoom creep).
4. **The refusal scrolls to the playhead.** The ok:false refusal (standing law 4 — no geometry ever paints) scrolls to the PLAYHEAD instead, the exact playhead-follow-scroll law from the playing case, so the refusal is legible in place (the bar's tip/status carry the why; the scroll shows the where — `:1306-1315`).
...
**(R28 amendment — D48/F3, the scene-shrink law; ARCH-R28 §4):** when a scene shrinks below a range marker's end (`time + duration`, the range half of D48's `Marker {id, time, label?, color?, duration?, notes?}` family — 09 §3.1A), the marker is **KEPT and its display clamps** to the new scene tail — a shrink is never a delete trigger (the mock's keep-and-display-clamp reading: only marker writes clamp).
...
2. **`trim-handles.tsx`** (267 LOC) — renders left/right trim edge handles. Edge color scheme is mode-aware: `TRIM_COLORS` (white), `RIPPLE_COLORS` (amber), `ROLL_COLORS` (amber, different glow), `FREE_COLORS` (green when actively trimming with headroom), `CONSTRAINED_COLORS` (red when hitting a bound) (`:18-35`, `:58-68`) *(R25 era-qualification, ARCH-R25 P10: point-in-time R15-era FreeCut reference-census — `trim-handles.tsx` and the whole `components/timeline-item/` tree are not in any current checkout (verified live at R25; `audits/fleet-r25/xcut-visual-grammar.md` spot-verification a — the freecut extraction carries none of them), so the palette is unresolvable against source and was never law; it also contradicts the corpus's own reference mocks — the law is §8A's green ruling (D33.2: the mocks' green trim grammar is the pinned reference); the amber ripple/roll reading stays recorded here as the historical FreeCut alternative)*. Supports join-left / join-right hover indicators (`:51-52`).
...
Every file path below is absolute within its reference repo. **OC** = `/tmp/opencut-classic/apps/web/src/timeline/`; **OC-App** = `/tmp/opencut-classic/apps/web/src/app/editor/[project_id]/`; **OC-Actions** = `/tmp/opencut-classic/apps/web/src/actions/`; **FC** = `/tmp/freecut/src/features/timeline/`. **OT** = opencut-timeline (`github.com/bearachprema/opencut-timeline` @ the R29 pin — HEAD `3e18722` / **code pin `970948a`** (docs-only over the code pin); the R27 pin was `55c81c0`/`970948a`; the R24 pin was `ded43c4`/`c15a629`; the R23 pin was `222532c`; the R8-era pin of this lineage was `d3b2163`; root `src/lib/timeline/` + `src/components/timeline/`) — the live clean-room OpenCut-classic timeline port, landed Round 8; it is the executable counterpart of OC's algorithmic core (types/placement/ripple/snapping/controllers/view-math) plus a spec-15-shaped headless API **plus the canonical React component tree (`src/components/timeline/` — 41 files / 9,176 LOC, pure React, landed W4/W5, verified M17-M58 + real-mouse + pixel reads, hosting the W11 wire-dispatch layer) and the `/view` runner page (the miniature editor shell + `window.__VIEW_TEST__` hooks — the gold sample per D25; the app mirrors the tree census-governed per D26)**. Note: nle-engine has **no React timeline** (player harness only — see §16.4); it contributes the data model (timeline.ts) but zero UI code for this stream.
...
> nle-engine (github.com/bearachprema/nle-engine) has **no React timeline** — no components, no controllers, no virtualization; the timeline UI is no longer greenfield — it is OT's canonical `src/components/timeline/` tree (D25; opencut-timeline §16.5 is the timeline-side code reference — algorithms + controllers + the components tree; see spec 19 §3.2). The engine contributes the data model and ordering contracts below. Where engine code conflicts with this spec, **the spec wins**.

| Spec section | Engine file:line | Verified quote | Status | Note |
|---|---|---|---|---|
| Whole spec (timeline UI) | (repo census) | only `src/components/ui/*` + test harness page | ENGINE-GAP | No React timeline; the UI home is OT's canonical tree (D25) |
| §12.1 track ordering | `src/lib/nle/playback/scene-assembly.ts:215` | `The \`zIndex = (maxOrder - trackOrder) * 1000\` formula` | ALIGNED | Stable z-index from track order |
| §7.3 clip DOM state | `src/lib/nle/playback/scene-assembly.ts:160` | `export interface StableDomItemDescriptor {` | ALIGNED | Minimal per-item state for virtualized DOM |
| §7.3 CSS transforms | `src/lib/nle/playback/transform-resolver.ts:303` | `export function buildCssTransform(` | ALIGNED | React-agnostic CSS transform builder |
| §12.3 linked selection | `src/lib/nle/timeline/timeline.ts:2569` | `getLinkedClips(clipId: string): Clip[] {` | ALIGNED | Linked A/V query |
| §11.1 markers | `src/lib/nle/timeline/timeline.ts:7708` | `addMarker(` | ALIGNED | Marker CRUD model |
| §11.2 in/out points | `src/lib/nle/timeline/timeline.ts:7904/:7939` | `setInPoint(frame: number): void {` / `setOutPoint(frame: number): void {` | VENUE-MODEL | N12 struck the dedicated `InOutPoints` model (§11.2's amendment above) — the live law is the setLoop halves / OT `timeline.setLoopRegion`; the engine's freecut-ported pair (`setInPoint` `:7904` / `setOutPoint` `:7939`; zero `setLoop` in the 9,051-LOC file) is the venue-model divergence; spec wins at the wire boundary. Re-annotated from the unqualified "ALIGNED": R33, engine seal-round register P3-21 |
...
| op entry surface | `src/lib/nle/headless/api.ts:830` | `case 'split': {` | CORRECTIVE | 19-op JSON-RPC vs spec 15 dispatcher; spec wins |

### 16.5. Code References — opencut-timeline (the editing-domain core per Decision 12; contract still canon)

> opencut-timeline (`github.com/bearachprema/opencut-timeline` @ `0412e41` — Round 15 re-baseline [R15-era pin — superseded by the R29 pin HEAD `3e18722` / code pin `970948a` (the R27 HEAD `55c81c0`), 632/632 + the S/T/F1/S3-seam/W10F/W11/D-ARCH-6 rounds + the D25 canonical-tree and D26 census rulings; the R23 pin was `222532c`, 489/489; the LOC/test counts in this preamble are R15-era and kept for lineage], ~42k LOC total: ~9.7k engine core + ~3.2k controllers + ~7.4k React view (≈20.2k implementation) + ~21.6k test harness (in-page 12.4k + real-mouse scripts 6.5k + fixtures 2.0k), **423/423 tests (303 in-page + 120 real-mouse, re-run Round 15)** — the W8 classic-parity UI round + W9 full-repo review are landed and terminal, **zero open P1/P2** per its own HANDOFF, real-mouse + fuzz + review-regression suites included) is a clean-room OpenCut-classic timeline port **built with this spec set in hand** (its README cites Decision-2 types and the spec-15 wire protocol). Per Decision 12 it is the **EDITING-domain normative core**: the executable reference for this stream's entire scope — the OC tables in §16.1 describe the ORIGINAL; OT is the running, tested version of the same algorithms, and its **W8 classic-parity React components + controllers have landed and are terminal — and per D25/D26 that tree (`src/components/timeline/`) is THE canonical React timeline UI tree, the app's `timeline-port/` its census-governed converging mirror** (this spec's §4 component hierarchy and §14.4 controller contracts now have an executable implementation; the F1 hardening wave further pinned the controller/wire validation laws — F1A/F1B/F1C, M45/M46; the W11 round added the wire-dispatch seam, M49C). Where OT code conflicts with this spec, **the spec wins** (the known deltas: prefixed headless command names (C7 — the 31-verb census @ `970948a` = 28+3; the lineage tail; worklist refreshed R15), 5-kind TrackType taxonomy, `TIMELINE_INDICATOR_LINE_WIDTH_PX = 2` vs OC's 3px — OC's verified read wins for the visual constant, pending an OT-side correction).

| Spec section | OT file:line | Key exports / behavior | Status | Note |
|---|---|---|---|---|
| Data model / Decision 2 (spec 06 §4.7) | `types/index.ts:116` | `SceneTracks {overlay[], main singleton, audio[]}`; 5 track kinds `:43`; 7-way `TimelineElement` union `:155-162` | ALIGNED | The spec-06 §4.7 shape, executable; 5-kind taxonomy maps to our 3 at the wire boundary (Decision 11.5, carried by Decision 12) |
...
| §5.2 zoom slider | `view/zoom-utils.ts:89/:105` | `sliderToZoom`/`zoomToSlider` exponential mapping | ALIGNED | Anchors re-keyed R33 (engine seal-round register P3-22) |
...
| §8.3 drag threshold | `controllers/element-interaction-controller.ts:53/:176` | `TIMELINE_DRAG_THRESHOLD_PX = 5`, strict `>` | ALIGNED | M16 boundary test at exactly 5px |
...
| §8.3/8.4 interaction | `controllers/element-interaction-controller.ts:208` | `ElementInteractionController` (771 LOC): pending→dragging machine, drop-target, group-move commit | ALIGNED | Spec 05 §14.4's six-point pattern 1:1 (verified by scout R8-A §5.1) |
| §8.4 trim | `controllers/resize-controller.ts:81` | `ResizeController` (348 LOC): trim-drag machine, preview/commit | ALIGNED | Consumes `ops/group-resize.ts` |
| §8.6 playhead | `controllers/playhead-controller.ts:114` | `PlayheadController` (378 LOC): scrub machine | ALIGNED | Arrow-key frame-nudge NOT ported yet (W4/W5) — spec 16 §3.1 stays SPEC-ONLY |
| §5.2 zoom gesture | `controllers/zoom-controller.ts:58` | `ZoomController` (242 LOC): Ctrl-wheel exponential, anchor scroll-keep | ALIGNED | |
...
| §14.5 placement | `placement/index.ts:43-54/:419` | `PlacementStrategy` 5 variants; `resolveTrackPlacement`; reject-not-shift | ALIGNED | Faithful port of OC §14.5; `:381`→`:419` re-keyed R33 (engine seal-round register P3-22) |
...
| §14.6 snapping | `snapping/index.ts:26/:51/:81` | 6 `SnapPointType`s; `getTimelineSnapThresholdInTicks` (10px/pps); `resolveTimelineSnap` closest-wins | ALIGNED | Keyframe source replaced by an open seam in OT — we snap to keyframes via that seam; anchors re-keyed R33 (engine seal-round register P3-22) |
...
| Split (spec 06 §5.1) | `ops/split.ts:33/:44` | `SplitElementsParams` (retainSide); `splitElementsOnTracks` | ALIGNED | Snap-once source spans; `:28/:39`→`:33/:44` re-keyed R33 (engine seal-round register P3-22 — the D42 `linkage` import shifted the anchors +1 past the register's `:32/:43` at the s18 pin) |
...
| Core engine | `ops/timeline-core.ts:317` | `TimelineCore`: insert/move/trim/split/delete/rippleDelete/duplicate/updateElements + preview/commit + snapshot undo (3,360 LOC) | ALIGNED | Accidentally implements spec 15 §4.2's manager-method names 1:1 |
| Waveform | `render/waveform.ts:68/:152/:180` | `computeWaveformPeaks`/`drawWaveform`/`measureDrawnColumnHeight` | ALIGNED | Feeds §7.2; anchors re-keyed R33 (engine seal-round register P3-22) |
...
| Headless API | `headless/api.ts` (union `:56-212`; the W11 surface `:243-297`) | 31-type prefixed union — **28 UI-routed + 3 documented exceptions** (`WIRE_COMMAND_TYPES` `:243-275`, tsc-lockstep asserts `:277-297`; the registry `WIRE_UI_EXCEPTIONS` `:328-335` EXPORTED tsc-asserted — the "runner-local registry" premise dead since AR-1; gate-consumed LIVE); `CommandResult {ok, code…}`; `apply`/`applyBatch` atomic; the F1B boundary hardening (missing/null params → INVALID_PARAMS, no-overlap on span patches, clear:false never clears); **the W11 widenings @ `c15a629` (BASE): the attach constructor option (SA-5 fps/instance validation), the dispatch recorder (`wireLog` ring cap 500 + unbounded `wireCoverage` Set + `wireReset`), `applyBatch` success `data.results`, keyframe-verb `TRACK_LOCKED` (lockPreCheck)** | CORRECTIVE | C7 rename pass chartered (spec 19 §6; worklist refreshed R15 in spec 15 §13.15): `timeline.*`/`track.*` prefixes are NOT spec-15 shape |
...
1. **The projector is ENGINE-home.** `nle-engine/src/lib/nle/projector/` — a fenced, ADDITIVE engine module (D9's additive-change rule; same-commit freeze-list update) importing opencut-timeline's `SceneTracks` TYPE-ONLY, exactly per the existing precedent (`nle-engine/src/lib/nle/bridge/scene-to-segments.ts:67` — `import type { SceneTracks, … }`; the zero-runtime-dep premise is QUALIFIED per 19 §3.1's amended law: TYPE-ONLY ×4 across the bridge family — scene-to-segments `:67`, av-link `:49`, transition-inputs `:70`, composition-frame `:113`, all `import type` — **plus the 2 VALUE leaf deep-imports** in `bridge/opencut-laws.ts:60-61` (`core/audio-params` + `ops/retime` — leaves, never the barrel; the OV-01/OV-02 amendment; R33, engine seal-round register P3-22)). The app's `src/projector/` is a THIN call site. Rationale: the projector owns rate/timebase reconciliation, transition-window translation, keyframe normalization, composition mapping — that IS timeline semantics; app-home would create the third timeline-semantics home Decision 12 eliminated.
2. **Contract:** `projectScene(scene: SceneTracks, ctx) → engine ingestion`, feeding render + the audio path + export. ONE-WAY by law (D12): *editing state* never flows engine→OT (telemetry flows UP a separate seam — ARCH-R15 §2.3bis). **The landed bridge halves are the projector's consolidation base (R33, engine seal-round register P3-24):** D-T6 `bridge/transition-inputs.ts:88-117` — the descriptors derive from the ENGINE scene's `el.transitionOut` ("the SSOT home since the opencut S-round … ONE input source for the audio pipeline"; the type-only OT import at `:70`) — and OV-05 `bridge/composition-frame.ts:39-48` — `el.transitionOut` the BASE, the consumer's sidecar the OVERRIDE, "the second authoring home is closed" (the type-only import at `:113`).
...
|---|---|---|---|
| slip | `:4956` | 1 (A2) | OT ops + invariant system + carried tests |
| slide | `:5097` | 1 (A2) | OT ops |
| rollingTrim | `:3324` | 1 (A2) | OT ops |
| rateStretch | `:3840` | 1 (A2) | OT ops |
...
| append (06 §5.9D) | NONE — placement-level composite, no engine op; OT's own primitives carry it: `insertElements` relative-offset batch (`timeline-core.ts:906+` + the intra-batch guard ~`:947-963`, one `withUndo` history entry; the D-ARCH-6 shared `validateInsertElement` helper lives wire-side — `headless/api.ts:699`, consumed by both `insert` and the `insertBatch` case `:1376`); append point = the target track's last-element end (per-track; `calculateTotalDuration` `ops/element-utils.ts:71-91` is the rejected whole-timeline alternative) | 1 (R25 D31.5; RULED-IN at r1 — R28 D46) | OT placement — the 6th `PlacementStrategy {type:'append'}` riding the LANDED `insertBatch` wire verb (one-history-entry-each — §0's BASE row records it LANDED with `removeKeyframes`/`retimeKeyframes` at D-ARCH-6; the "r1" qualifier dropped: R33, engine seal-round register P3-23); optional `timeline.append` wrapper (the rippleDelete pattern) |
| ripple-overwrite (06 §5.9E) | REFERENCE ONLY — no op to port: `rippleTrimItem :3132`'s private signed-shift machinery (push AND pull; `_propagateRemovedInterval :4178` / `_propagateInsertedGap :4241` — renamed `…ToSyncLockedTracks`) is the delta-law precedent, private/coupled in the engine | 1 (R25 D31.6; RULED-IN at r1 — R28 D46) | OT composite — `delete + move + insert` (the order law; one `applyBatch`, one history entry; the classic §12 diff produces the pull but never the push, so this is NOT "overwrite + the ripple flag") |
| fit-to-fill (06 §5.9F) | the pure speed law `calculateSpeed` (`core/timeline-math.ts:133`; the fixed-span derivation already inside `rateStretchItem :3872`) + the 3-point family's optional target-span overload (`performOverwriteEdit :6039`); rate domain **[0.1, 5]** — out-of-domain REFUSES, never clamps; the shell-variants carrier's `RATE_MIN = 0.01` (`trimLaws.ts:32` — the [0.01, 5] clamp domain) is the registered divergence this row corrects, carried like the badge row (§8A `speed-badge`; R33, engine seal-round register P3-23) | 1 (R25 D31.7; RULED-IN at r1 — R28 D46) | OT composite — insert + `updateElements{retime}` (both verbs landed + UI-routed; one `applyBatch`); a first-class verb is optional at r2 |
| retime / freezeFrame / rangeRemoval (closeGap, joinItems) | NO method named `retime` exists in timeline.ts (re-derived: the family is `RetimeConfig` + the wire's `updateElements{retime}` + keyframe-store) / `freezeFrameAtPosition` `:8652` / `closeGapAtPosition` `:7463` + `joinItems` `:8817` | 2 (A2.5) | OT ops + model extensions if spec'd |

(Wave/phase ownership: ARCH-R15 §2.3 item 2 + §3.4's A2/A2.5 rows. The four R25 rows ride the same wave 1 (r1) per ARCH-R25 D31.4-D31.7 + D31A's dedicated-vs-composite criterion — replace = the dedicated op; append/ripple-overwrite/fit-to-fill = composites over landed verbs (06 §5.9's preamble carries the criterion); the per-family evidence lives in `audits/fleet-r25/mode-{replace,append,ripple-overwrite,fit-to-fill}.md`. **R28 re-tag (D46 — the r1-entry absent-family scope gate, ARCH-R28 §4): the four families are RULED-IN at r1** — the absent-family scope gate is closed (all four enter r1; the registered mid-flight deferral lever + reversal protocol live in the ruling), and the four wave cells above flip from the R25-scheduled readings to the ruled-in readings. Should the deferral lever ever fire a cut, the re-tag surface is **~10-11 sites corpus-wide**: 06 §0 + §5.9X, 15 §13.15, 16 §3.4A/F-block, the plan's Stage line + user-gate text + the E5 consumer line, the battery's r1-SCHEDULED count, the K2 registration note, + this file's two sites — the §0 GAP row (05:21) and these four wave cells.)

*(R23 audit note: §16.5A was accidentally present twice in this file — a byte-identical duplicate; the second copy is removed, content unchanged.)*
