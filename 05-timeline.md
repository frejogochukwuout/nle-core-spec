...
| replace (06 §5.9C) | NEW dedicated op `performReplaceEdit` (absent at the R25 engine pin — mode-replace G-R2); the transition law = `joinItems`' id-remap pattern `:8817` (NOT `removeItems :4507`'s cascade-drop); the linked companion is SEVERED not deleted (`linked:false`); an unfillable source refuses `INVALID_PARAMS` | 1 (R25 — D31.4) | OT ops — single-op atomicity (D31A: the transition-remap + sever semantics are why replace is the one dedicated op of the four) |
| append (06 §5.9D) | NONE — placement-level composite, no engine op; OT's own primitives carry it: `insertElements` relative-offset batch (`timeline-core.ts:887+` + the intra-batch guard ~`:929-944`, one `withUndo` history entry (the D-ARCH-6 shared `validateInsertElement` extraction)); append point = the target track's last-element end (per-track; `calculateTotalDuration` `ops/element-utils.ts:71-91` is the rejected whole-timeline alternative) | 1 (R25 — D31.5) | OT placement — the 6th `PlacementStrategy {type:'append'}` riding the already-endorsed r1 `insertBatch` composite; optional `timeline.append` wrapper (the rippleDelete pattern) |
| ripple-overwrite (06 §5.9E) | REFERENCE ONLY — no op to port: `rippleTrimItem :3132`'s private signed-shift machinery (push AND pull; `_propagateRemovedInterval :4178` / `_propagateInsertedGap :4241` — renamed `…ToSyncLockedTracks`) is the delta-law precedent, private/coupled in the engine | 1 (R25 — D31.6) | OT composite — `delete + move + insert` (the order law; one `applyBatch`, one history entry; the classic §12 diff produces the pull but never the push, so this is NOT "overwrite + the ripple flag") |
| fit-to-fill (06 §5.9F) | the pure speed law `calculateSpeed` (`core/timeline-math.ts:133`; the fixed-span derivation already inside `rateStretchItem :3840`) + the 3-point family's optional target-span overload (`performOverwriteEdit :6007`); rate domain **[0.1, 5]** — out-of-domain REFUSES, never clamps | 1 (R25 — D31.7) | OT composite — insert + `updateElements{retime}` (both verbs landed + UI-routed; one `applyBatch`); a first-class verb is optional at r2 |
