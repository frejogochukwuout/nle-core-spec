...
**The ten-mode matrix (Decision 30 — the edit-mode completeness census; the R25 fleet pins engine `3989506` (ONE docs-only commit past the R24 BASE pin `5036387` below — timeline.ts untouched, the §10.4 line numbers stand) / OT code pin `c15a629` (unchanged from the R24 BASE — the code pin is the census's contract; HEAD moved docs-only to `fdb771c`) / app `64fb0ab`; the spec column is post-R25-amendment, the code columns are the R25 census; *R27 re-anchor: engine `f9ac806` (docs-only head over code anchor `50b91f5`; timeline.ts 8,997 LOC — every engine line below re-anchored) / OT code pin `970948a` (HEAD `55c81c0`) / app `c020b2a`; **R28 re-anchor: engine `074a2f6` (code anchor `74bef08`; timeline.ts UNMOVED at 8,997) / OT code pin `970948a` (HEAD `55c81c0`, unchanged) / app `85cff80`; R29 re-anchor: engine `e3f55bd` (chore-only re-pin — the engine's own code UNMOVED at code anchor `74bef08`; timeline.ts at 8,997) / OT code pin `970948a` (HEAD `3e18722`, seal18 W1+W2 reviews/docs-only) — R31: HEAD `31f2764` (the R31 queue filing, docs-only) / app `876f2b8`***):** **R33 re-anchor: the engine line cites in this matrix + the §5.x/§10.5/Testing inline engine refs below re-anchored @ `2a0ecf4` (timeline.ts 9,051 LOC) — engine seal-round register P2-17.**

| Mode | 06 spec home | 15 wire | engine | OT ops+UI | app | reference |
|---|---|---|---|---|---|---|
| Roll | §5.5 COVERED | `RollCommand` §4.3.5 | `rollingTrimItems` :3356 UNWIRED | ABSENT | ABSENT | variants (LIVE, 5 pins) |
| Ripple trim | §5.2A COVERED (wire/keys only) | `TrimCommand{ripple}` | `rippleTrimItem` :3164 UNWIRED | ABSENT (ripple-delete only; the trim verb carries NO ripple param — the W11 exception) | ⌥[/⌥] composed-over-patches | variants (the only DaVinci-faithful mouse surface) |
| Slip | §5.6 COVERED | `SlipCommand` §4.3.6 | `slip` :4988 UNWIRED | ABSENT | `,`/`.` engine-true (the app-inline law, unpinned) | variants R15-T4 |
| Slide | §5.7 COVERED | `SlideCommand` §4.3.7 | `slideItem` :5129 UNWIRED | ABSENT | ABSENT | variants (gesture+bounds; the source-capped divergence registered) |
| Insert edit | §5.9 (the two-semantics preamble + the splice law, D31.1/31.2 — R25) | `insert` verb (placement-only) | `performInsertEdit` :5662 (splice+push; E1 FIXED R26 — the r1 precondition met) | placement-insert only (reject-not-shift) | pool-DnD placement + split (2 of 3 facets) | variants insertPlan (the most faithful, honestly-labeled) |
| Overwrite edit | §5.9B (D31.3 — R25) | none | `performOverwriteEdit` :6039 (the covered-clip law deep; E2 FIXED R26 — the fate-based companion law; E3 the silent abort open — rides r1) | ABSENT (the phantom strategy rows retired R25, P2/P6) | ABSENT | variants R20-W2 (the only full impl) |
...
**ACCEPTANCE & TEST PLAN:** §8 + the Testing section below are this spec's battery; BASE acceptance = the cited suites at the cited pins (OT 675 @ `008e7f3` — 77 milestone entries, the census 632 classic + 17 M60 + 11 M61 + 11 M63 + 3 M62 + 1 M62-gate [was 536 — 386 in-page + 150 real-mouse]; engine 785 @ `2a0ecf4` — 29 test files, the count gate's runtime-derived pin [was 458]; WDC 777 @ `94f6460` [docs-only over the `ec8fd5c` count; was 759]; app 426 @ `7664603` — the 22-file roof [was 174]) — the regression role (K1; re-keyed R33, engine seal-round register P3-28 — LIVE figures; the register's engine 749/25-test-file interim was R31-vintage). GAP acceptance is per-row above (r1 exit per `IMPLEMENTATION-PLAN.md` §2; the composed-ripple half's K3 gate); facet rows live in spec 17 §13A.
...
|---|---|---|---|
| Slip (§5.6) | the same clamped δ applies to anchor + synchronized companions (06:1325); a window-mismatched companion is EXCLUDED — desync-safe by exclusion (engine `getSynchronizedLinkedItems` :856, re-anchored R33 @ `2a0ecf4` — engine seal-round register P2-17) | `clampSlipDeltaToPreserveTransitions` (FreeCut-only — the engine port dropped it) | n/a (timeline unchanged) |
...
| Ripple trim (§5.2A) | companions trimmed the same δ with `from` restored; downstream shifts on EVERY pair track (+ transition-neighbors, engine :3164/:3187+, re-anchored R33 @ `2a0ecf4` — engine seal-round register P2-17) — the track-family reading, RULED (D42.1 — R28; the in-table precedent for the insert row's track-family sentence below) | transition-followers ride the ripple | sync-lock WINS: interval/gap propagation, not link-following (§6) |
| Insert edit (§5.9's splice family) | the pair splits at the SAME insert frame; each half re-pairs (the ≥2-splits relink law); the right half + companions + downstream shift by the inserted duration — the left half NEVER moves (Decision 31.2). Downstream is the TRACK-FAMILY reading (D42.1 — R28): on the target track AND on every companion track hosting a shifted companion — the companion-track gap treatment (engine Phase 2b, timeline.ts:5849-5900 @ `2a0ecf4`, landed `a1448ad`, pinned timeline-edit-ops.test.ts:239-240 — line cite re-anchored R33, engine seal-round register P2-17; sync-locked companion tracks take §6's inserted-gap propagation; locked tracks stay put) | a split inside a transition's consumed window aborts the edit (R1-B4; §5.9B's E3 error contract) | inserted-gap propagation (§6) — linked companions take the link treatment; §6 owns the companion track's REMAINING clips, never double-applied (D42.12 — R28) |
...
| Replace (§5.9C) | SEVERED — `syncLinked: false` by law; the companion keeps position + duration; the new clip carries no link (re-establishable via 16 §3.4's `toggleAVLink`) | transition endpoint REMAP (the `joinItems` pattern, timeline.ts:8950-8958 — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17) — never cascade-drop | n/a (downstream never moves) |
...
| Fit to fill (§5.9F) | the pair takes the SAME computed rate and the SAME target duration — one rate law both streams, ONE commit (the engine `rateStretchItem` :3872+ precedent — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17) | overwrite-style (§5.9B's covered-clip law) | n/a (no downstream movement) |
...
**The split-link ruling (Decision 32.4 — the conflict stated, then decided).** §5.1's quoted FreeCut `relinkSplitSegments` SEVERS the link on a single-side split (one linked split → both `linkedGroupId`s stay `undefined` — both halves lose the group); the engine relinks both halves (`relinkSplitSegments`, timeline.ts:773-795 — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17 — a single linked split preserves the original group id on BOTH halves). **Relink-both-halves is the law** — Resolve-faithful: a split clip's halves both stay linked to the companion; representable in 09's pairwise `linkedTo` as both halves carrying the link. The §5.1 quote is REFERENCE-CENSUS (the R15-era FreeCut reference), not law; the nle-ui/shell-variants R14 right-half-sever is a registered divergence (the D26 carrier-reduction program's reconciliation row, not an immediate change).
...
**The covered-clip law** (the engine's verified implementation `_performOverwriteEditImpl`, timeline.ts:6055-6363 (re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17), and the R20-W2 mock `insertPlan.ts:192-255` — the two executable witnesses):
...
- **The 3-point shape:** duration = `sourceEnd − sourceStart` (the source viewer's in/out marks, `max(1, …)`), placed at the overwrite frame, typed by the track kind (`_buildSourceEditClip` :6373 — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17; assumes sourceFps === timelineFps — callers pre-convert).
...
| ID | Gap | Owner | Phase | Acceptance |
|---|---|---|---|---|
| OW-1 | §5.9B's executable home: the covered-clip law exists only engine-side (class-only, UNWIRED off the 19-op INTERNAL surface per Decision 16) and in the R20-W2 mock; OT has no overwrite op/verb/UI (the phantom `placement:'overwrite'` rows were retired this round — P2/P6) | S-ot | r1 (the Decision-12.3 port wave; `performOverwriteEdit` :6039 @ `2a0ecf4` (cite re-anchored R33, engine seal-round register P2-17) is the algorithm source; **E2 FIXED R26 `8188d4e`** — the fate-based companion law) | the ported pure op over SceneTracks implements the four overlap cases + no-shift + remnant survival + one-entry undo; the carried engine tests + a four-case vitest family (fully-contained-removed / straddle-both-split×2 / head+tail-trim / no-displacement) green; the R1-B4 abort classified INVALID_PARAMS-class (E3); the §10.4 row flips ALIGNED |
...
- **Transitions REMAP:** transitions at the target's edges re-target onto the new clip (the `joinItems` pattern, timeline.ts:8950-8958 — `replacementByRemovedId` endpoint remap, self-transitions dropped) — NOT the cascade-drop of `removeItems` (:4570-4572) (both cites re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17). This is the single clearest reason replace is a DEDICATED op (Decision 31A).
- **The companion SEVERS:** `syncLinked: false` by law — the companion keeps position + duration; the new clip carries no link (re-establishable via 16 §3.4's `toggleAVLink`). The naive-composition trap: `removeItems`' default-true linked expansion would DELETE the companion (§5.0's fan-out row; engine :4539 — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17).
...
> nle-engine (github.com/bearachprema/nle-engine — Round 15 re-baseline @ `f526e67`; **R24 re-verify @ `5036387`: 458/458 vitest (W2.5 effects sidecar +17 / R8-REV blur-radius clamp +1; the OT submodule re-pinned a4e971d→`c15a629` — the W11 wire surface importable at the engine's test seam, zero engine mediation by design), tsc 0 — N2b keyframed volume LANDED @ `37cdd28` (the S2 seam round), RR1-A the fade-clamp SPAN law landed (pre-R23), the `timeline-edit-ops` cross tests live (the K2 seed family)**; **the LIVE suite @ `2a0ecf4`: 785/785 vitest, 29 test files — the count gate's runtime-derived pin (re-keyed R33, engine seal-round register P3-28)**) implements ~20 of this spec's op families as public methods on its `Timeline` class (timeline/timeline.ts, **9,051 LOC at `2a0ecf4` — re-anchored R33, engine seal-round register P2-14; the R24-era "7,502 LOC / ~108 public methods at `5036387`, re-verified unchanged" claim superseded — the public-method count re-based R33 by the register's P3-28 pin-lag row: 108 public-method signatures @ `2a0ecf4` (the two-space-indent name-line census, UNCHANGED `5036387`→`e3f55bd`→`2a0ecf4` — the R24 "~108" was NOT drift; the register's R31-vintage "≈96" does not reproduce under any method-line pattern [94 modifier lines / 89 `private`-keyword lines / 79 private-signature lines are the nearest neighbours], recorded-not-adopted)**; 6,972/102 at the R15 census) with the R15-era suite counts (**274/274 vitest + 265/265 browser milestone rows (31 milestones, real WebGPU) + 318 probe checks** — real A/V export decode-verified) — the strongest ALIGNED subsystem in the reference repo. The deltas are architectural (class-based mutating manager vs this spec's pure-op + command layer; flat N-track array vs `SceneTracks`; 19-op JSON-RPC wire surface — re-typed INTERNAL transport per Decision 16 — vs spec 15's 78-type union) plus three absent families (track-state toggles, snap, splitAndRemove — **still absent at `2a0ecf4`, re-verified R33, engine seal-round register P3-28: zero track-state mutators (no toggleTrack\*/setTrack\* surface), zero snap-assist op code (the only `snap` lexemes in timeline.ts are doc-mentions + the unrelated `snap-to-parent` transform behavior at :6484-:6568), zero `splitAndRemove`**). Where engine code conflicts, **the spec wins**; see `19-code-references.md`. Per **Decision 12.3**, the engine's FreeCut-side op families (roll/slip/slide/rateStretch/retime/insert-edit-3-point/sync-lock) are **port-scheduled INTO opencut-timeline's ops layer** as pure functions over SceneTracks at **r1** (acceptance: OT's suite at the r1 pin (675 @ `008e7f3` live — was 536 @ `c15a629`) + the ported engine tests; the engine's implementations remain its internal fallback until each port lands). The engine's timeline as a whole is re-specified as the runtime domain's **render-scheduling projection** (Decision 12.1) — its editing surface is no longer a second normative home.

| Spec 06 op (§) | nle-engine timeline.ts:line | verified signature | status |
|---|---|---|---|
| §5.1 Split | :2652 | `splitClip(` | ALIGNED |
| §5.1 Split-all | :2872 | `splitAllItemsAtFrame(frame: number, options: { linked?: boolean } = {}): number {` | ALIGNED |
| §5.2 Trim head | :2952 | `trimHead(` | ALIGNED |
| §5.2 Trim tail | :3058 | `trimTail(` | ALIGNED |
| §5.2/§5.4 Ripple trim | :3164 | `rippleTrimItem(` | ALIGNED |
| §5.5 Roll | :3356 | `rollingTrimItems(` | ALIGNED |
| §5.11 Rate stretch | :3872 / :7699 | `rateStretchItem(` / `rateStretchWithRipple(` | ALIGNED |
| §5.4 Ripple delete | :4318 / :4352 | `rippleDelete(clipId: string): void {` / `rippleDeleteItems(` | ALIGNED |
| §5.8 Delete | :4539 | `removeItems(clipIds: string[], options: { linked?: boolean } = {}): void {` | ALIGNED |
| §5.10 Duplicate | :4607 | `duplicateItems(` | ALIGNED |
| §5.3 Move | :4747 / :4855 | `moveClip(` / `moveItems(` | ALIGNED |
| §5.6 Slip | :4988 | `slip(clipId: string, deltaFrames: number, options: { linked?: boolean } = {}): void {` | ALIGNED |
| §5.7 Slide | :5129 | `slideItem(` | ALIGNED |
...
| §5.9 Insert-edit (the 3-point splice — Decisions 31.1/31.2) | :5662 | `performInsertEdit(` | PORT-SCHEDULED (r1, Decision 12.3 — the prior "ALIGNED" verdict retired R25, P9: it certified the method against a § teaching the OTHER insert semantics; the splice law is §5.9's insert-edit preamble; the E1 linked defect FIXED R26 `8188d4e` — the precondition MET; UNWIRED off the 19-op INTERNAL surface) |
| §5.9B Overwrite edit | :6039 | `performOverwriteEdit(` | PORT-SCHEDULED (r1 — §5.9B's covered-clip law, Decision 31.3; the E2 companion law FIXED R26 `8188d4e` + the E3 silent abort is the open fix row; UNWIRED) |
| §5.14 Range removal family | :8289 / :8565 / :8611 / :8641 | `removeRangesFromClip(` / `removeSilenceFromItems(` / `removeFillerWordsFromItems(` / `removeTranscriptRangesFromItems(` | ALIGNED |
| §5.13 Freeze frame | :8706 | `freezeFrameAtPosition(` | ALIGNED |
| Join (spec 16 composite) | :8871 | `joinItems(clipIds: string[]): string \| null {` | ALIGNED |
| Close gap | :7517 / :7628 | `closeGapAtPosition(` / `closeAllGapsOnTrack(` | ALIGNED |
| §6 Sync-lock | timeline.ts:4250 + sync-lock.ts:579 | `applySyncLockRipplePatch(` + `propagateRemovedIntervalsToSyncLockedTracks(` | ALIGNED |
| Undo/redo/snapshot | :7404 / :7449 | `undo(): boolean {` / `snapshot(label: string = 'snapshot'): TimelineSnapshot {` | ALIGNED (P0.6 keyframe-blind equality FIXED @ :1665 `snapshotsEqual` — now keyframe/composition/backgroundColor-aware, verified by m20 20.7/20.8) |
...
| §4.6 pure-Op + command wrap | timeline.ts:1992 (and 20+ sites) | `_commit({...this._data, …})` | CORRECTIVE (class-based mutating manager; spec's layer wins) |
...
| §5.5 Roll / §5.6 Slip / §5.7 Slide | — | (absent — OpenCut never had them; still absent at `c15a629`, R24-verified) | OT-GAP | **nle-engine is the executable reference** (§10.4 :3356/:4988/:5129 — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17) — Decision 11.3 |
| §5.11 Rate-stretch command | — | (absent; R24-verified @ `c15a629`) | OT-GAP | nle-engine :3872 |
| §5.13 Freeze frame | — | (absent; R24-verified @ `c15a629`) | OT-GAP | nle-engine :8706 |
| §5.14 Range removal | — | (absent; R24-verified @ `c15a629`) | OT-GAP | Spec-only; nle-engine family :8289-8661 |
...
- `slide-continuity-zero-for-non-chain` — when left+right neighbors don't
  ...
  trim, the clip still moves; the engine slides anyway, `slideItem` timeline.ts:5129 — re-anchored R33 @ `2a0ecf4`, engine seal-round register P2-17)
