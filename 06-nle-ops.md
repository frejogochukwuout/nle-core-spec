...
**The ten-mode matrix (Decision 30 — the edit-mode completeness census; the R25 fleet pins engine `3989506` (ONE docs-only commit past the R24 BASE pin `5036387` below — timeline.ts untouched, the §10.4 line numbers stand) / OT code pin `c15a629` (unchanged from the R24 BASE — the code pin is the census's contract; HEAD moved docs-only to `fdb771c`) / app `64fb0ab`; the spec column is post-R25-amendment, the code columns are the R25 census; *R27 re-anchor: engine `f9ac806` (docs-only head over code anchor `50b91f5`; timeline.ts 8,997 LOC — every engine line below re-anchored) / OT code pin `970948a` (HEAD `55c81c0`) / app `c020b2a`; **R28 re-anchor: engine `074a2f6` (code anchor `74bef08`; timeline.ts UNMOVED at 8,997) / OT code pin `970948a` (HEAD `55c81c0`, unchanged) / app `85cff80`; R29 re-anchor: engine `e3f55bd` (chore-only re-pin — the engine's own code UNMOVED at code anchor `74bef08`; timeline.ts at 8,997) / OT code pin `970948a` (HEAD `3e18722`, seal18 W1+W2 reviews/docs-only) — R31: HEAD `31f2764` (the R31 queue filing, docs-only) / app `876f2b8`***):**

| Mode | 06 spec home | 15 wire | engine | OT ops+UI | app | reference |
|---|---|---|---|---|---|---|
| Roll | §5.5 COVERED | `RollCommand` §4.3.5 | `rollingTrimItems` :3324 UNWIRED | ABSENT | ABSENT | variants (LIVE, 5 pins) |
| Ripple trim | §5.2A COVERED (wire/keys only) | `TrimCommand{ripple}` | `rippleTrimItem` :3132 UNWIRED | ABSENT (ripple-delete only; the trim verb carries NO ripple param — the W11 exception) | ⌥[/⌥] composed-over-patches | variants (the only DaVinci-faithful mouse surface) |
| Slip | §5.6 COVERED | `SlipCommand` §4.3.6 | `slip` :4956 UNWIRED | ABSENT | `,`/`.` engine-true (the app-inline law, unpinned) | variants R15-T4 |
| Slide | §5.7 COVERED | `SlideCommand` §4.3.7 | `slideItem` :5097 UNWIRED | ABSENT | ABSENT | variants (gesture+bounds; the source-capped divergence registered) |
| Insert edit | §5.9 (the two-semantics preamble + the splice law, D31.1/31.2 — R25) | `insert` verb (placement-only) | `performInsertEdit` :5630 (splice+push; E1 FIXED R26 — the r1 precondition met) | placement-insert only (reject-not-shift) | pool-DnD placement + split (2 of 3 facets) | variants insertPlan (the most faithful, honestly-labeled) |
| Overwrite edit | §5.9B (D31.3 — R25) | none | `performOverwriteEdit` :6007 (the covered-clip law deep; E2 FIXED R26 — the fate-based companion law; E3 the silent abort open — rides r1) | ABSENT (the phantom strategy rows retired R25, P2/P6) | ABSENT | variants R20-W2 (the only full impl) |
...
- nle-engine @ **`e3f55bd`** (code anchor `74bef08`) — **749/749** vitest, tsc 0 (the R29 re-pin: chore-only — the vendored OT `6e2b91a`→`55c81c0` (the D-ARCH-6 absorption of `970948a`, the engine's queued next DISCHARGED) + the vendored WDC→`83b8850`; the engine's own code UNCHANGED; the R28 re-pin was `074a2f6`, NOT docs-only — ONE code commit + the `e7cdede` OV-12 infra fix + docs + the net-zero mirror merge; timeline.ts UNMOVED at 8,997 LOC; the engine's OT submodule @ **`55c81c0`**; fence 52, probes 9/9; the RC round's OV/M2 waves landed — **timeline.ts grew to 8,997 LOC: the R24 "line numbers unchanged" note is superseded, the §10.4 line numbers re-anchored at `f9ac806`**). The algorithm sources for the r1 port families (timeline.ts 8,997 LOC: `slip` :4956 / `slideItem` :5097 / `rollingTrimItems` :3324 / `rateStretchItem` :3840 / `freezeFrameAtPosition` :8652 / `removeRangesFromClip` :8235). **N2b keyframed volume LANDED** (`37cdd28`, the S2 seam round — engine+app closed); RR1-A the fade-clamp SPAN law landed (pre-R23; the engine's post-R23 deltas are composition-frame only); the `timeline-edit-ops` cross tests live (the K2 seed family — engine-in-the-middle vendored OT ops × engine composition).
...
| Ripple trim (§5.2A) | companions trimmed the same δ with `from` restored; downstream shifts on EVERY pair track (+ transition-neighbors, engine :2921-2950) — the track-family reading, RULED (D42.1 — R28; the in-table precedent for the insert row's track-family sentence below) | transition-followers ride the ripple | sync-lock WINS: interval/gap propagation, not link-following (§6) |
| Insert edit (§5.9's splice family) | the pair splits at the SAME insert frame; each half re-pairs (the ≥2-splits relink law); the right half + companions + downstream shift by the inserted duration — the left half NEVER moves (Decision 31.2). Downstream is the TRACK-FAMILY reading (D42.1 — R28): on the target track AND on every companion track hosting a shifted companion — the companion-track gap treatment (engine Phase 2b, timeline.ts:5817-5868, landed `a1448ad`, pinned timeline-edit-ops.test.ts:239-240; sync-locked companion tracks take §6's inserted-gap propagation; locked tracks stay put) | a split inside a transition's consumed window aborts the edit (R1-B4; §5.9B's E3 error contract) | inserted-gap propagation (§6) — linked companions take the link treatment; §6 owns the companion track's REMAINING clips, never double-applied (D42.12 — R28) |
...
| ID | Gap | Owner | Phase | Acceptance |
|---|---|---|---|---|
| OW-1 | §5.9B's executable home: the covered-clip law exists only engine-side (class-only, UNWIRED off the 19-op INTERNAL surface per Decision 16) and in the R20-W2 mock; OT has no overwrite op/verb/UI (the phantom `placement:'overwrite'` rows were retired this round — P2/P6) | S-ot | r1 (the Decision-12.3 port wave; `performOverwriteEdit` :6007 @ `f9ac806` is the algorithm source; **E2 FIXED R26 `8188d4e`** — the fate-based companion law) | the ported pure op over SceneTracks implements the four overlap cases + no-shift + remnant survival + one-entry undo; the carried engine tests + a four-case vitest family (fully-contained-removed / straddle-both-split×2 / head+tail-trim / no-displacement) green; the R1-B4 abort classified INVALID_PARAMS-class (E3); the §10.4 row flips ALIGNED |
...
| §5.9 Insert-edit (the 3-point splice — Decisions 31.1/31.2) | :5630 | `performInsertEdit(` | PORT-SCHEDULED (r1, Decision 12.3 — the prior "ALIGNED" verdict retired R25, P9: it certified the method against a § teaching the OTHER insert semantics; the splice law is §5.9's insert-edit preamble; the E1 linked defect FIXED R26 `8188d4e` — the precondition MET; UNWIRED off the 19-op INTERNAL surface) |
| §5.9B Overwrite edit | :6007 | `performOverwriteEdit(` | PORT-SCHEDULED (r1 — §5.9B's covered-clip law, Decision 31.3; the E2 companion law FIXED R26 `8188d4e` + the E3 silent abort is the open fix row; UNWIRED) |
