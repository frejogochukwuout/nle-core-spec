...
**R25 (2026-09-09 — the edit-mode completeness round; `audits/ARCH-R25-edit-mode-completeness.md`, Decisions 30-32):** the ruling's 06-side amendments landed in this file — **§5.0 "Linked-companion propagation" (D32: the ten-mode fan-out table, the precedence sentence lock > sync-lock (§6) > link, the canonical param `syncLinked`, the split-link relink-both-halves ruling, the E1 engine defect row)**; **the §5.9 restructure (D31/31A: the two-semantics preamble — the placement surface places, the source-edit surface splices — the mid-clip split law, the dedicated-vs-composite criterion, and the four previously-absent families now spec-first as §5.9B overwrite edit / §5.9C replace / §5.9D append at end / §5.9E ripple overwrite / §5.9F fit to fill, r1-scheduled per D30.2 with their GAP rows)**; **the ten-mode matrix below (D30.1 — the completeness census, re-derived at every spec round's fleet pass like the C7 census; every code cell re-read live by the ten mode reports at the R25 pins)**; **the P-list fixes (P1: the ripple-insert-via-diff phantom re-keyed to the D31.6 composite law; P2/P6: the phantom `placement:'overwrite'` test rows re-keyed to §5.9B/§5.9E; P3: the unreachable `ripple-insert-makes-room` row replaced by the composite's trajectory row; P4/P5: the slide continuity/chain corrections; P9: the §10.4 insert row split into placement vs insert-edit with honest verdicts)**. The landed C7 census (24 routed + 6 exceptions at `c15a629`) is NOT touched — fired twice since, mechanically: 25+5 @ AR-2 (s16), **31 = 28+3 @ D-ARCH-6 (`970948a`)** — the new family verbs appear only as r1-scheduled GAP rows that re-declare mechanically per the tsc-lockstep/M49C law (D29.2) when they land.

**The ten-mode matrix (Decision 30 — the edit-mode completeness census; the R25 fleet pins engine `3989506` (ONE docs-only commit past the R24 BASE pin `5036387` below — timeline.ts untouched, the §10.4 line numbers stand) / OT code pin `c15a629` (unchanged from the R24 BASE — the code pin is the census's contract; HEAD moved docs-only to `fdb771c`) / app `64fb0ab`; the spec column is post-R25-amendment, the code columns are the R25 census; *R27 re-anchor: engine `f9ac806` (docs-only head over code anchor `50b91f5`; timeline.ts 8,997 LOC — every engine line below re-anchored) / OT code pin `970948a` (HEAD `55c81c0`) / app `c020b2a`; **R28 re-anchor: engine `074a2f6` (code anchor `74bef08`; timeline.ts UNMOVED at 8,997) / OT code pin `970948a` (HEAD `55c81c0`, unchanged) / app `85cff80`; R29 re-anchor: engine `e3f55bd` (chore-only re-pin — the engine's own code UNMOVED at code anchor `74bef08`; timeline.ts at 8,997) / OT code pin `970948a` (HEAD `3e18722`, seal18 W1+W2 reviews/docs-only) — R31: HEAD `31f2764` (the R31 queue filing, docs-only) / app `876f2b8`***):** **R33 re-anchor: the engine line cites in this matrix + the §5.x/§10.5/Testing inline engine refs below re-anchored @ `2a0ecf4` (timeline.ts 9,051 LOC) — engine seal-round register P2-17.** **R34 re-key: engine `60232ea` (timeline.ts 9,078 LOC — the P-F4 src fix inside `calculateTrimSourceUpdate`, the nightly exploration venue's first catch, +27 net): the matrix + the §5.x/§10.4/§10.5/Testing inline engine refs below re-anchored again — every cite past the fix shifted a uniform +27, verified LIVE at the tree (re-keyed R34, was `2a0ecf4`).**

| Mode | 06 spec home | 15 wire | engine | OT ops+UI | app | reference |
|---|---|---|---|---|---|---|
| Roll | §5.5 COVERED | `RollCommand` §4.3.5 | `rollingTrimItems` :3383 UNWIRED | ABSENT | ABSENT | variants (LIVE, 5 pins) |
| Ripple trim | §5.2A COVERED (wire/keys only) | `TrimCommand{ripple}` | `rippleTrimItem` :3191 UNWIRED | ABSENT (ripple-delete only; the trim verb carries NO ripple param — the W11 exception) | ⌥[/⌥] composed-over-patches | variants (the only DaVinci-faithful mouse surface) |
| Slip | §5.6 COVERED | `SlipCommand` §4.3.6 | `slip` :5015 UNWIRED | ABSENT | `,`/`.` engine-true (the app-inline law, unpinned) | variants R15-T4 |
| Slide | §5.7 COVERED | `SlideCommand` §4.3.7 | `slideItem` :5156 UNWIRED | ABSENT | ABSENT | variants (gesture+bounds; the source-capped divergence registered) |
| Insert edit | §5.9 (the two-semantics preamble + the splice law, D31.1/31.2 — R25) | `insert` verb (placement-only) | `performInsertEdit` :5689 (splice+push; E1 FIXED R26 — the r1 precondition met) | placement-insert only (reject-not-shift) | pool-DnD placement + split (2 of 3 facets) | variants insertPlan (the most faithful, honestly-labeled) |
| Overwrite edit | §5.9B (D31.3 — R25) | none | `performOverwriteEdit` :6066 (the covered-clip law deep; E2 FIXED R26 — the fate-based companion law; E3 the silent abort open — rides r1) | ABSENT (the phantom strategy rows retired R25, P2/P6) | ABSENT | variants R20-W2 (the only full impl) |
...
*Decision 30.2: the four absent families (replace / append / ripple-overwrite / fit-to-fill) are r1-scheduled spec-first families — owner + phase + acceptance per their §5.9C-F GAP rows; the landed C7 census (24 routed + 6 exceptions at `c15a629`) is NOT reopened — fired twice since, mechanically: 25+5 @ AR-2 (s16), **31 = 28+3 @ D-ARCH-6 (`970948a`)**. Decision 30.3 (the ladder gate; wording generalized D46.3 — R28): at K2 every matrix row is LANDED or carries a live scheduled row **at its registered phase**, with owner + acceptance — zero orphan rows (the completeness is over the REGISTER, not over landed-ness — a registered deferral to a later phase satisfies the gate: the re-pointed row keeps owner + acceptance, never orphans); at K3 the app column matches the LANDED set with pins.*

**BASE (accepted, pinned 2026-09-07):**
- opencut-timeline (OT) @ HEAD **`3e18722`** (**code pin `970948a`** — the trailing commits are the seal18 W1+W2 reviews/docs rounds; src diff empty) (R31: HEAD `31f2764` — 632/632 UNCHANGED, docs-only over the code pin `970948a` — the R31 queue filing) — the R31 queue filing — **632/632** (72 report entries — the total only, the split convention changed shape; `download/timeline-test-report.json` committed at `970948a` is the count authority), tsc 0. The ops engine of this spec: split/trim/move/ripple/delete/insert/duplicate + placement with the landed semantics — the S-round (S1 transport policy; S2 track lock; S3 transitionOut null=DELETE key law + split left-drops; S4 bookmark color/note; D-S5 typed ElementParams), the T-round (D-T3 `setTracksLocked` — the lock-all batch, one history entry + the `track.setAllLocked` wire verb, M44; D-T7 `moveElements` same-position NOOP — the `ok:true` true-return asymmetry, no batch abort), **the S3 seam round (M48: `setTracksMuted` the mute-all batch — the D-T3 twin, the F1C-1-normalized every-at-target predicate — + the `track.setAllMuted` wire verb)**, the F1 whole-codebase hardening (M45: F1B-1..8 the wire/interaction validation laws — no-overlap enforced on update span patches → CONFLICT, INVALID_PARAMS for missing/null/array params (was INTERNAL_ERROR — F1B-4), negative trims/starts rejected (F1B-6), `setLoopRegion clear:false` never clears (F1B-5), failed `applyBatch` preserves the pre-batch redo stack (F1B-7), the keyframe flick commits (F1B-8); F1A-2/3/4 the insert-batch laws — heterogeneous-batch rejection + the intra-batch overlap guard (§5.9's law, now pinned) + `createTracks` id validation; F1A-1 the marquee expanded-lane rows; F1C-1 the unlock-direction every-at-target NOOP law), **the W10F controller rounds** (M45 round-1 + M47 round-3 — the marquee expanded-lane row math; M46 the `applyBatch` redo-outcome matrix + the absent-field NOOP predicate), and **the W11 complete-UI round** (the wire-dispatch seam, DECISIONS #25: `useWireDispatch` over `HeadlessTimelineApi` with attach + the recorder (`wireLog` ring 500 + the `wireCoverage` Set) + `WIRE_COMMAND_TYPES` tsc-lockstep + `applyBatch` `data.results` + keyframe-verb `TRACK_LOCKED`; the W11-b dispatch migration — command verbs cross the wire — + the W11-f gesture commits — move/updateElements-patch/moveBookmark/insert cross the wire with TRACK_LOCKED error drivers; M49T/H/R/G + the **M49C coverage gate: 28/28 routed verbs machine-checked through the real UI (origin-attributed — only origin:'ui' dispatches count)**). The wire surface is **31 prefixed command names split 28 routed + 3** (`selectElements`, `advancePlayhead`, `trim`) — the C7 census lineage: **24 at the R15/M29 charter → 28 actual at the R22 pin → 30 at R23 → the routed/exceptions split at `c15a629` → 25+5 at AR-2 (s16) → 31 = 28+3 at D-ARCH-6 (`970948a`)** (D29.2; the singular keyframe verbs RETIRED, the batch verbs live). Per D25, OT's `src/components/timeline/` is THE canonical React timeline UI tree (spec 05 §0's row — the ops layer it binds is this spec's surface).
...
- nle-engine @ **`e3f55bd`** (code anchor `74bef08`) — **749/749** vitest, tsc 0 (the R29 re-pin: chore-only — the vendored OT `6e2b91a`→`55c81c0` (the D-ARCH-6 absorption of `970948a`, the engine's queued next DISCHARGED) + the vendored WDC→`83b8850`; the engine's own code UNCHANGED; the R28 re-pin was `074a2f6`, NOT docs-only — ONE code commit + the `e7cdede` OV-12 infra fix + docs + the net-zero mirror merge; timeline.ts UNMOVED at 8,997 LOC; the engine's OT submodule @ **`55c81c0`**; fence 52, probes 9/9; the RC round's OV/M2 waves landed — **timeline.ts grew to 9,051 LOC at `2a0ecf4` (the property tier + the P-F1/P-F2 fixes past the R29 code anchor): the R24 "line numbers unchanged" note is superseded, and the §10.4 line numbers are re-anchored R33 @ `2a0ecf4` — the prior "re-anchored at `f9ac806`" claim in this sentence was false as written (the §10.4 table carried the R24-era numbers until the R33 pass landed register P2-14; §0's ten-mode matrix stays the live mode anchor) (R33, engine seal-round register P2-15)**). The algorithm sources for the r1 port families (timeline.ts 9,078 LOC @ `60232ea` — re-keyed R34, was `2a0ecf4` at 9,051: `slip` :5015 / `slideItem` :5156 / `rollingTrimItems` :3383 / `rateStretchItem` :3899 / `freezeFrameAtPosition` :8733 / `removeRangesFromClip` :8316). **N2b keyframed volume LANDED** (`37cdd28`, the S2 seam round — engine+app closed); RR1-A the fade-clamp SPAN law landed (pre-R23; the engine's post-R23 deltas are composition-frame only); the `timeline-edit-ops` cross tests live (the K2 seed family — engine-in-the-middle vendored OT ops × engine composition).
- web-daw-core @ **`94f6460`** (docs-only over code anchor `ec8fd5c` — the ENG-2 S-series docs wrap) — **777/777** (the S-series upstream wave — the diagnostics/heartbeat law, the rebuilt dsp-effects-worklet, the rt-safety-gate heritage fix; **W2 SoundTouch varispeed LANDED** pre-R23: `src/lib/daw/soundtouch.ts` + `varispeed.ts` + the retime rate-domain hardening — **the retime audio half for r1's wave 2**, consumed via the engine, not re-ported).
...
- Error-envelope coarseness — spec 15 §6.3 amendment LANDED (R28/D44: the **25-code two-tier table** — the 24 + `RATE_OUT_OF_DOMAIN`, class-tagged per §6.3's registry); **r1**: OT follows. *Annotated R23: F1B-4's INVALID_PARAMS classification half is now COVERED by the OT BASE; the coarseness itself — incl. the T-round reviews' registered residuals (F1B-15 empty-moves→CONFLICT family trap; F1B-18 NOOP-class successes reporting NOT_FOUND) — remains the r1 refinement. Annotated R24: the coverage is richer than the R23 note — TRACK_LOCKED now also fires on the singular keyframe verbs (the W11 lockPreCheck, api.ts:340/:849/:904) and the UI classifies NOOP-benign (DECISIONS #25 ruling 6, the wire-error chip); the ~24-code fine-grained envelope itself remains open. Annotated R28 (D44.7 — the lock-gate riders, the N3 survey's 9 lockPreCheck additions): the count is complete ONLY under the ruled verb forms — ripple-trim rides `TrimCommand{ripple}` on **trim's existing gate** (06:19's param-on-verb form, no new gate); insert-edit/overwrite/append ride the **widened placement-target check** (the resolved-target widening — today's insert gate fires only on explicit `trackId`); ripple-overwrite/fit-to-fill **inherit the gate via `applyBatch`'s gated constituents**; the 8 dedicated new verbs (roll/slip/slide/rateStretch/retime/freezeFrame/rangeRemoval/replace) each gain their own gate; the 2 element toggles stay cosmetic-exempt (engine timeline.ts:2585-2601 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)). DEPENDENCY REGISTERED (D47.3): a dedicated-verb re-shape of ripple-trim or insert-edit re-opens the count (9 → 10-11).*
...
**ACCEPTANCE & TEST PLAN:** §8 + the Testing section below are this spec's battery; BASE acceptance = the cited suites at the cited pins (OT 714 @ `8f96ab2` — 83 milestone entries (the R33 census's 77 + the M64 fps param + the M59/59A-D carrier families) [was 675 @ `008e7f3`; was 536 — 386 in-page + 150 real-mouse]; engine 789 @ `60232ea` — 29 test files, the count gate's runtime-derived pin (785 + the 4 P-F4 nan-guard pins) [was 458]; WDC 777 @ `94f6460` [docs-only over the `ec8fd5c` count; was 759]; app 426 @ `d28847c` — the 22-file roof [was 174]) — the regression role (K1; re-keyed R34, was re-keyed R33 (engine seal-round register P3-28) — LIVE figures; the register's engine 749/25-test-file interim was R31-vintage). GAP acceptance is per-row above (r1 exit per `IMPLEMENTATION-PLAN.md` §2; the composed-ripple half's K3 gate); facet rows live in spec 17 §13A.
...
**The precedence (Decision 32.2 — stated once, referenced everywhere):** track LOCK > sync-lock (track-level, §6 — interval/gap propagation over whole tracks) > link (clip-level, this section — pairs one clip with one companion). The two mechanisms are never conflated: linking answers "which clips move with this clip"; sync-lock answers "which tracks ripple when this track ripples" (§5.14's precedent: sync-locked companions are excluded from link expansion and take §6's propagation instead, :2022-2034). **The enforcement point (D42.12 — R28, NEW):** D32.2's precedence gets ONE enforcement home in the OT model — the **uniform closure predicate at the Stage-4 port: locked ⇒ skip now** (lands with the pairwise field); **sync-lock ⇒ skip when §6 owns the track** (slots at the Stage-4 sync-lock port) — never per-op improvisation. The engine findings the predicate closes: P-A (the effective insert precedence lock > link > sync-lock for straddling companions — effect-equivalent for positions, divergent for the straddle treatment; the split treatment is the sync-preserving one, so it stays) and P-B (closeGapAtPosition :7526-7548 — a linked+sync-locked downstream companion link-moved then interval-mangled; registered as the engine P3, pre-r1, frozen venue — the port's uniform predicate is its closure by construction).
...
|---|---|---|---|
| Slip (§5.6) | the same clamped δ applies to anchor + synchronized companions (06:1325); a window-mismatched companion is EXCLUDED — desync-safe by exclusion (engine `getSynchronizedLinkedItems` :883, re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)) | `clampSlipDeltaToPreserveTransitions` (FreeCut-only — the engine port dropped it) | n/a (timeline unchanged) |
...
| Ripple trim (§5.2A) | companions trimmed the same δ with `from` restored; downstream shifts on EVERY pair track (+ transition-neighbors, engine :3191/:3214+, re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)) — the track-family reading, RULED (D42.1 — R28; the in-table precedent for the insert row's track-family sentence below) | transition-followers ride the ripple | sync-lock WINS: interval/gap propagation, not link-following (§6) |
| Insert edit (§5.9's splice family) | the pair splits at the SAME insert frame; each half re-pairs (the ≥2-splits relink law); the right half + companions + downstream shift by the inserted duration — the left half NEVER moves (Decision 31.2). Downstream is the TRACK-FAMILY reading (D42.1 — R28): on the target track AND on every companion track hosting a shifted companion — the companion-track gap treatment (engine Phase 2b, timeline.ts:5876-5927 @ `60232ea` (re-keyed R34, was `2a0ecf4`), landed `a1448ad`, pinned timeline-edit-ops.test.ts:239-240 — line cite re-anchored R34 (R33's anchor was seal-round P2-17); sync-locked companion tracks take §6's inserted-gap propagation; locked tracks stay put) | a split inside a transition's consumed window aborts the edit (R1-B4; §5.9B's E3 error contract) | inserted-gap propagation (§6) — linked companions take the link treatment; §6 owns the companion track's REMAINING clips, never double-applied (D42.12 — R28) |
...
| Replace (§5.9C) | SEVERED — `syncLinked: false` by law; the companion keeps position + duration; the new clip carries no link (re-establishable via 16 §3.4's `toggleAVLink`) | transition endpoint REMAP (the `joinItems` pattern, timeline.ts:8977-8985 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)) — never cascade-drop | n/a (downstream never moves) |
...
| Fit to fill (§5.9F) | the pair takes the SAME computed rate and the SAME target duration — one rate law both streams, ONE commit (the engine `rateStretchItem` :3899+ precedent — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)) | overwrite-style (§5.9B's covered-clip law) | n/a (no downstream movement) |
...
**The split-link ruling (Decision 32.4 — the conflict stated, then decided).** §5.1's quoted FreeCut `relinkSplitSegments` SEVERS the link on a single-side split (one linked split → both `linkedGroupId`s stay `undefined` — both halves lose the group); the engine relinks both halves (`relinkSplitSegments`, timeline.ts:800-822 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17) — a single linked split preserves the original group id on BOTH halves). **Relink-both-halves is the law** — Resolve-faithful: a split clip's halves both stay linked to the companion; representable in 09's pairwise `linkedTo` as both halves carrying the link. The §5.1 quote is REFERENCE-CENSUS (the R15-era FreeCut reference), not law; the nle-ui/shell-variants R14 right-half-sever is a registered divergence (the D26 carrier-reduction program's reconciliation row, not an immediate change).
...
**The covered-clip law** (the engine's verified implementation `_performOverwriteEditImpl`, timeline.ts:6082-6390 (re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)), and the R20-W2 mock `insertPlan.ts:192-255` — the two executable witnesses):
...
- **The 3-point shape:** duration = `sourceEnd − sourceStart` (the source viewer's in/out marks, `max(1, …)`), placed at the overwrite frame, typed by the track kind (`_buildSourceEditClip` :6400 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17); assumes sourceFps === timelineFps — callers pre-convert).

**The companion law (§5.0's fan-out row):** companions of REMOVED clips cascade (removed with them — the `removeItems` law); companions of surviving TRIMMED remnants TRIM TO MATCH — both tracks' remnants by the same amounts. **E2 FIXED (R26, engine `8188d4e`):** the filed asymmetry was actually BROADER — the old companion block was DEAD CODE (the removed ids were filtered out of `clips` before `findLinkedClips` looked them up, so companions NEVER cascaded — probed live on the fully-contained case). The fix implements the fate-based law: cascade groups remove companions whole; trim groups carve the region out of the companion (the by-side ≥2-splits relink re-pairs both tracks' survivors); the pins (straddle / head-trim / CASCADE / by-side re-pair / carve-abort / `linked:false` / the unlinked regression) live in `timeline-linked-source-edit.test.ts`. The mixed-fate runtime-group case is the E2-a residue — **RULED (D42.5 — R28)**: if any member of a derived group is fully removed, the group's pairwise links die with it (**the prune law**); a runtime group with both a fully-removed and a trimmed member (constructible: split one side, then overwrite) resolves **CASCADE** for companions — removal is the stronger fate; the prune then clears every dead pointer on survivors (survivors of other pairs keep theirs). The ≥2-splits re-pair is OP CODE at the port — split's `...element` spread carries the field to both halves by construction, but the fresh by-side pointers are minted by the split consumer, never by the spread alone.
...
| ID | Gap | Owner | Phase | Acceptance |
|---|---|---|---|---|
| OW-1 | §5.9B's executable home: the covered-clip law exists only engine-side (class-only, UNWIRED off the 19-op INTERNAL surface per Decision 16) and in the R20-W2 mock; OT has no overwrite op/verb/UI (the phantom `placement:'overwrite'` rows were retired this round — P2/P6) | S-ot | r1 (the Decision-12.3 port wave; `performOverwriteEdit` :6066 @ `60232ea` (cite re-anchored R34 — was `2a0ecf4` at R33, seal-round P2-17) is the algorithm source; **E2 FIXED R26 `8188d4e`** — the fate-based companion law) | the ported pure op over SceneTracks implements the four overlap cases + no-shift + remnant survival + one-entry undo; the carried engine tests + a four-case vitest family (fully-contained-removed / straddle-both-split×2 / head+tail-trim / no-displacement) green; the R1-B4 abort classified INVALID_PARAMS-class (E3); the §10.4 row flips ALIGNED |
| OW-2 (E2) | ~~the engine companion asymmetry: removed clips cascade to companions, trimmed survivors' companions do NOT~~ **FIXED (R26, engine `8188d4e` — the fate-based companion law; the filed cascade was actually DEAD CODE, companions never cascaded at all; verified live)** — §5.9B's E2 note + the 06 §5.0 overwrite row carry the landed law; the pinned engine fixture (an A/V overwrite straddle: the trimmed survivor's companion trims by the same amounts) rides `tests/vitest/engine/timeline-linked-source-edit.test.ts` (13 tests) | S-ot (the port home — the law carries into the ported op) | **DONE** pre-r1 | the fate-based companion law lands with the r1 port verbatim — the carried 13-test pin file is the acceptance |
...
| OW-4 (E2-a — D42.5, R28) | the pairwise-model prune law + the mixed-fate tie-break, un-pinned at the port: a derived group with a fully-removed member must CASCADE its companions (removal is the stronger fate — a mere unlink is a defective port) and the prune must clear every dead `linkedTo` pointer on survivors | S-ot | r1 (the pairwise field port — D42) | the mixed-fate runtime-group pin: the split-then-overwrite fixture (one side split, then an overwrite fully covering one member + trimming another) — the companion cascades WHOLE + zero dead pointers on survivors |
...
- **Transitions REMAP:** transitions at the target's edges re-target onto the new clip (the `joinItems` pattern, timeline.ts:8977-8985 — `replacementByRemovedId` endpoint remap, self-transitions dropped) — NOT the cascade-drop of `removeItems` (:4597-4599) (both cites re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)). This is the single clearest reason replace is a DEDICATED op (Decision 31A).
- **The companion SEVERS:** `syncLinked: false` by law — the companion keeps position + duration; the new clip carries no link (re-establishable via 16 §3.4's `toggleAVLink`). The naive-composition trap: `removeItems`' default-true linked expansion would DELETE the companion (§5.0's fan-out row; engine :4566 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)).
...
| RO-2 | the engine's signed-shift machinery is private: `rippleTrimItem`'s push/pull downstream shift (:3191/:3214+) + both sync-lock propagations (`applySyncLockRipplePatch` timeline.ts:4277 + sync-lock.ts:579) — cites re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17; sync-lock.ts unmoved by the P-F4 fix) — the algorithm source for the composite, exposed by no public surface | S-engine (the exposure filing) + S-ot (the port) | r1 | the ripple-overwrite composite (or `performRippleOverwriteEdit`) lands as pure OT-side functions with the carried engine tests; the §10.4/§10.5 rows flip |
...
### 10.4. nle-engine op-coverage (runtime-domain core per Decision 12 — re-baselined Round 15 @ `f526e67`; **line numbers re-anchored R33 @ `2a0ecf4` (engine seal-round register P2-14 — every table row's cite rebuilt from the current tree, timeline.ts 9,051 LOC)**: the R24 "re-verified @ `5036387` — UNCHANGED" note was false at every pin past R27 — timeline.ts grew 7,502→8,997 LOC through the R27/R28 waves and 8,997→9,051 through the property tier + P-F1/P-F2 fixes, and the table was never re-anchored (only the insert-edit/overwrite rows were ever current, at the 8,997-LOC era); the R23 pin `b8c6f88` 440/440 in turn superseded; refresh at r1; **R34 re-key: line numbers re-anchored @ `60232ea` (timeline.ts 9,078 LOC — the P-F4 src fix in `calculateTrimSourceUpdate`, the nightly's first catch, +27 net): every row's cite re-anchored LIVE — a uniform +27 past the fix (re-keyed R34, was `2a0ecf4`)**)

> nle-engine (github.com/bearachprema/nle-engine — Round 15 re-baseline @ `f526e67`; **R24 re-verify @ `5036387`: 458/458 vitest (W2.5 effects sidecar +17 / R8-REV blur-radius clamp +1; the OT submodule re-pinned a4e971d→`c15a629` — the W11 wire surface importable at the engine's test seam, zero engine mediation by design), tsc 0 — N2b keyframed volume LANDED @ `37cdd28` (the S2 seam round), RR1-A the fade-clamp SPAN law landed (pre-R23), the `timeline-edit-ops` cross tests live (the K2 seed family)**; **the LIVE suite @ `2a0ecf4`: 785/785 vitest, 29 test files — the count gate's runtime-derived pin (re-keyed R33, engine seal-round register P3-28)**; **R34: the LIVE suite @ `60232ea`: 789/789 vitest — 785 + the 4 P-F4 nan-guard pins, 29 test files, the count gate re-pinned same-commit (re-keyed R34)**) implements ~20 of this spec's op families as public methods on its `Timeline` class (timeline/timeline.ts, **9,051 LOC at `2a0ecf4` — re-anchored R33, engine seal-round register P2-14; the R24-era "7,502 LOC / ~108 public methods at `5036387`, re-verified unchanged" claim superseded — the public-method count re-based R33 by the register's P3-28 pin-lag row: 108 public-method signatures @ `2a0ecf4` (the two-space-indent name-line census, UNCHANGED `5036387`→`e3f55bd`→`2a0ecf4` — the R24 "~108" was NOT drift; the register's R31-vintage "≈96" does not reproduce under any method-line pattern [94 modifier lines / 89 `private`-keyword lines / 79 private-signature lines are the nearest neighbours], recorded-not-adopted)** — **R34: 9,078 LOC @ `60232ea` (the P-F4 src fix, +27 net; the 108 public-method census UNMOVED — the fix lives inside the module-level `calculateTrimSourceUpdate`, no signature touched; re-keyed R34)**; 6,972/102 at the R15 census) with the R15-era suite counts (**274/274 vitest + 265/265 browser milestone rows (31 milestones, real WebGPU) + 318 probe checks** — real A/V export decode-verified) — the strongest ALIGNED subsystem in the reference repo. The deltas are architectural (class-based mutating manager vs this spec's pure-op + command layer; flat N-track array vs `SceneTracks`; 19-op JSON-RPC wire surface — re-typed INTERNAL transport per Decision 16 — vs spec 15's 78-type union) plus three absent families (track-state toggles, snap, splitAndRemove — **still absent at `60232ea`, re-verified R34 (was `2a0ecf4` at R33, seal-round P3-28 — the P-F4 fix is the only src delta): zero track-state mutators (no toggleTrack\*/setTrack\* surface), zero snap-assist op code (the only `snap` lexemes in timeline.ts are doc-mentions + the unrelated `snap-to-parent` transform behavior at :6511-:6595), zero `splitAndRemove`**). Where engine code conflicts, **the spec wins**; see `19-code-references.md`. Per **Decision 12.3**, the engine's FreeCut-side op families (roll/slip/slide/rateStretch/retime/insert-edit-3-point/sync-lock) are **port-scheduled INTO opencut-timeline's ops layer** as pure functions over SceneTracks at **r1** (acceptance: OT's suite at the r1 pin (714 @ `8f96ab2` live, 83 entries — re-keyed R34, was 675 @ `008e7f3`, was 536 @ `c15a629`) + the ported engine tests; the engine's implementations remain its internal fallback until each port lands). The engine's timeline as a whole is re-specified as the runtime domain's **render-scheduling projection** (Decision 12.1) — its editing surface is no longer a second normative home.

| Spec 06 op (§) | nle-engine timeline.ts:line | verified signature | status |
|---|---|---|---|
| §5.1 Split | :2679 | `splitClip(` | ALIGNED |
| §5.1 Split-all | :2899 | `splitAllItemsAtFrame(frame: number, options: { linked?: boolean } = {}): number {` | ALIGNED |
| §5.2 Trim head | :2979 | `trimHead(` | ALIGNED |
| §5.2 Trim tail | :3085 | `trimTail(` | ALIGNED |
| §5.2/§5.4 Ripple trim | :3191 | `rippleTrimItem(` | ALIGNED |
| §5.5 Roll | :3383 | `rollingTrimItems(` | ALIGNED |
| §5.11 Rate stretch | :3899 / :7726 | `rateStretchItem(` / `rateStretchWithRipple(` | ALIGNED |
| §5.4 Ripple delete | :4345 / :4379 | `rippleDelete(clipId: string): void {` / `rippleDeleteItems(` | ALIGNED |
| §5.8 Delete | :4566 | `removeItems(clipIds: string[], options: { linked?: boolean } = {}): void {` | ALIGNED |
| §5.10 Duplicate | :4634 | `duplicateItems(` | ALIGNED |
| §5.3 Move | :4774 / :4882 | `moveClip(` / `moveItems(` | ALIGNED |
| §5.6 Slip | :5015 | `slip(clipId: string, deltaFrames: number, options: { linked?: boolean } = {}): void {` | ALIGNED |
| §5.7 Slide | :5156 | `slideItem(` | ALIGNED |
...
| §5.9 Insert-edit (the 3-point splice — Decisions 31.1/31.2) | :5689 | `performInsertEdit(` | PORT-SCHEDULED (r1, Decision 12.3 — the prior "ALIGNED" verdict retired R25, P9: it certified the method against a § teaching the OTHER insert semantics; the splice law is §5.9's insert-edit preamble; the E1 linked defect FIXED R26 `8188d4e` — the precondition MET; UNWIRED off the 19-op INTERNAL surface) |
| §5.9B Overwrite edit | :6066 | `performOverwriteEdit(` | PORT-SCHEDULED (r1 — §5.9B's covered-clip law, Decision 31.3; the E2 companion law FIXED R26 `8188d4e` + the E3 silent abort is the open fix row; UNWIRED) |
| §5.14 Range removal family | :8316 / :8592 / :8638 / :8668 | `removeRangesFromClip(` / `removeSilenceFromItems(` / `removeFillerWordsFromItems(` / `removeTranscriptRangesFromItems(` | ALIGNED |
| §5.13 Freeze frame | :8733 | `freezeFrameAtPosition(` | ALIGNED |
| Join (spec 16 composite) | :8898 | `joinItems(clipIds: string[]): string \| null {` | ALIGNED |
| Close gap | :7544 / :7655 | `closeGapAtPosition(` / `closeAllGapsOnTrack(` | ALIGNED |
| §6 Sync-lock | timeline.ts:4277 + sync-lock.ts:579 | `applySyncLockRipplePatch(` + `propagateRemovedIntervalsToSyncLockedTracks(` | ALIGNED |
| Undo/redo/snapshot | :7431 / :7476 | `undo(): boolean {` / `snapshot(label: string = 'snapshot'): TimelineSnapshot {` | ALIGNED (P0.6 keyframe-blind equality FIXED @ :1692 `snapshotsEqual` — now keyframe/composition/backgroundColor-aware, verified by m20 20.7/20.8) |
...
| §4.6 pure-Op + command wrap | timeline.ts:2019 (and 20+ sites) | `_commit({...this._data, …})` | CORRECTIVE (class-based mutating manager; spec's layer wins) |
| §4.7 SceneTracks model | core/types.ts (Clip union @ :1030) | flat `TimelineData`; `TrackKind = 'video' \| 'audio'` | CORRECTIVE (flat N-track; spec's main-singleton wins — the Decision-12 projector owns the one-way bridge; editing never reads back) |

§7 note (Round 7, updated Round 8, line refs refreshed R23): the snapshot stored by a drag-coalesced `TracksSnapshotCommand` must cover the full field set incl. keyframes — the engine's `snapshotsEqual` ignored them (keyframe-only edits stopped being undoable; engine P0.6). **Fixed in Wave 4A** (timeline.ts:1692-1715 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-14); at the R24 pin `5036387` this sat at :1521-1544, re-verified unchanged then; compares keyframes + compositions + backgroundColor via JSON-deep; m20 20.7 regression-tested). The counter-example stays in spec 19 §6 as documentation. Residual field-gap: `busAudioEq` exists in FreeCut's 17-field equality but not in the engine model — our spec 06 §7 / spec 15 §14.1 full-field bar covers it by construction.
...
| §5.5 Roll / §5.6 Slip / §5.7 Slide | — | (absent — OpenCut never had them; still absent at `c15a629`, R24-verified) | OT-GAP | **nle-engine is the executable reference** (§10.4 :3383/:5015/:5156 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17)) — Decision 11.3 |
| §5.11 Rate-stretch command | — | (absent; R24-verified @ `c15a629`) | OT-GAP | nle-engine :3899 |
| §5.13 Freeze frame | — | (absent; R24-verified @ `c15a629`) | OT-GAP | nle-engine :8733 |
| §5.14 Range removal | — | (absent; R24-verified @ `c15a629`) | OT-GAP | Spec-only; nle-engine family :8316-8688 |
| §6 Sync-lock | — | (absent; R24-verified @ `c15a629`) | OT-GAP | nle-engine sync-lock.ts (728 LOC) is the only implementation |
...
| Wire validation laws (F1B) | `headless/api.ts` (30 case arms; the CONFLICT/INVALID_PARAMS family, e.g. :468/:540/:563) | INVALID_PARAMS / CONFLICT / NOOP / TRACK_LOCKED classifications | ALIGNED (the envelope's classification half) | F1B-2/4/5/6 + F1A-3/4 + F1B-7 (M45); the W11 lockPreCheck extends TRACK_LOCKED to the keyframe verbs (api.ts:2338/:2438/:2536, re-anchored R33) — **13 commands total emit TRACK_LOCKED at `008e7f3` (the OT-seal18 recount: insertBatch :1417 + track.remove :2732 join the old 12)**; the coarseness refinement stays the r1 GAP row; the r1 lock-gate riders are named there per D44.7 — R28 (ripple-trim rides trim's existing gate; the placement family rides the widened target check; the composites inherit via `applyBatch`; the D47.3 dedicated-verb dependency registered) |
...
- `slide-continuity-zero-for-non-chain` — when left+right neighbors don't
  ...
  trim, the clip still moves; the engine slides anyway, `slideItem` timeline.ts:5156 — re-anchored R34 @ `60232ea` (was `2a0ecf4` — R33's seal-round P2-17))
