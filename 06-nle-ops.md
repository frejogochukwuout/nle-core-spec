...
- nle-engine @ **`074a2f6`** (code anchor `74bef08`) — **749/749** vitest, tsc 0 (the R28 re-pin: the range is NOT docs-only — ONE code commit + the `e7cdede` OV-12 infra fix + docs + the net-zero mirror merge; timeline.ts UNMOVED at 8,997 LOC; the engine's OT submodule @ **`6e2b91a`** — the D-ARCH-6 absorption of `970948a` is the engine's queued next; fence 52, probes 9/9; the RC round's OV/M2 waves landed — **timeline.ts grew to 8,997 LOC: the R24 "line numbers unchanged" note is superseded, the §10.4 line numbers re-anchored at `f9ac806`**). The algorithm sources for the r1 port families (timeline.ts 8,997 LOC: `slip` :4956 / `slideItem` :5097 / `rollingTrimItems` :3324 / `rateStretchItem` :3840 / `freezeFrameAtPosition` :8652 / `removeRangesFromClip` :8235). **N2b keyframed volume LANDED** (`37cdd28`, the S2 seam round — engine+app closed); RR1-A the fade-clamp SPAN law landed (pre-R23; the engine's post-R23 deltas are composition-frame only); the `timeline-edit-ops` cross tests live (the K2 seed family — engine-in-the-middle vendored OT ops × engine composition).
- web-daw-core @ **`83b8850`** (docs-only over code anchor `ec8fd5c`) — **777/777** (the S-series upstream wave — the diagnostics/heartbeat law, the rebuilt dsp-effects-worklet, the rt-safety-gate heritage fix; **W2 SoundTouch varispeed LANDED** pre-R23: `src/lib/daw/soundtouch.ts` + `varispeed.ts` + the retime rate-domain hardening — **the retime audio half for r1's wave 2**, consumed via the engine, not re-ported).
