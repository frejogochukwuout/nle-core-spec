...
*Decision 30.2: the four absent families (replace / append / ripple-overwrite / fit-to-fill) are r1-scheduled spec-first families — owner + phase + acceptance per their §5.9C-F GAP rows; the landed C7 census (24 routed + 6 exceptions at `c15a629`) is NOT reopened — fired twice since, mechanically: 25+5 @ AR-2 (s16), **31 = 28+3 @ D-ARCH-6 (`970948a`)**. Decision 30.3 (the ladder gate): at K2 every matrix row is LANDED or carries a live r1-scheduled row with owner + acceptance — zero orphan rows (the completeness is over the REGISTER, not over landed-ness); at K3 the app column matches the LANDED set with pins.*
